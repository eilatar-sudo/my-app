...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE rq_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_rq_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_rq_authorization_evidence (
    ...
    INSERT INTO agent_rq_profile (
      ...
      authorization_snapshot_hash, quorum_threshold,
      max_ballots_per_certificate, semantic_tags, procedure_ref, revocation_policy,
      created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident refresh quorum', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'rq-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3, 32,
      ARRAY['incident', 'quorum'], 'proc:quorum-incident',
      'STOP_BEFORE_BALLOT', 'principal:test', now()
    );

    INSERT INTO agent_rq_profile_quorum_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_observation_kinds, quorum_threshold, require_refresh,
      conflict_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'follow-current', 1,
      ARRAY['FOLLOW_CURRENT_REFRESH', 'SUPERSESSION_REFRESH'], 3, true,
      '{"narrow_only":true}'
    );

    SELECT approve_agent_rq_profile(
      101,
    ...
    INSERT INTO agent_rq_observation_catalog (
      account_id, observation_id, source_refresh_id, producer_session_id, participant_ref,
      observation_kind, status, fact_hash, attenuation_hash, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      'a0000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      'session:agent-b/purpose:incident-triage',
      'FOLLOW_CURRENT_REFRESH', 'SEALED', repeat('b', 64), repeat('c', 64), now(), now()
    );

    INSERT INTO agent_rq_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_nominate_units,
      budget_ballot_units, budget_vector_units, budget_seal_units,
      budget_refresh_units, consumed_nominate_units, consumed_ballot_units,
      consumed_vector_units, consumed_seal_units, consumed_refresh_units,
      budget_max_wall_time_ms, quorum_threshold, max_ballots_per_certificate,
      ...
      'Tally incident refresh quorum', 'request-1', 100, 40, 5, 8, 2,
    ...
    INSERT INTO agent_rq_nomination_receipt (
      account_id, receipt_id, session_id, observation_id, observation_kind,
      fact_hash, attenuation_hash, nomination_hash, nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'FOLLOW_CURRENT_REFRESH',
      repeat('b', 64), repeat('c', 64), repeat('1', 64), now()
    );

    INSERT INTO agent_rq_certificate (
      account_id, certificate_id, session_id, consumer_ref, purpose_hash,
      ballot_set_hash, winner_fact_hash, tally_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
    ...
    INSERT INTO agent_rq_certificate_ballot (
      account_id, ballot_id, certificate_id, session_id, observation_id, observation_kind,
      ballot_ordinal, status, fact_hash, attenuation_hash, sealed_revision,
      ...
      'FOLLOW_CURRENT_REFRESH', 0, 'SEALED',
      repeat('b', 64), repeat('c', 64), 1, now(), now()
    );

    UPDATE agent_rq_session
    SET status = 'ACTIVE',
        state_revision = 1,
    ...
    INSERT INTO agent_rq_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rq_certificate_ballot
        SET fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND ballot_id = 'a0000000-0000-0000-0000-000000000099';
      `),
    "mutation of certificate ballot identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_rq_refresh_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rq_refresh_intent
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_rq_session WHERE account_id = 101",
  );
