...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ske_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ske_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ske_authorization_evidence (
      ...
      '{"scopes":["provider.key.enroll"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_ske_profile (
      ...
      'Incident successor-key enrollment', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ske-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'successor-key-enrollment'],
      'proc:successor-key-enrollment-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_ske_profile_enroll_rule (
      ...
      require_trusted_key, enroll_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'enroll-case', 1,
      ARRAY['SEALED_REATTEST_CERTIFICATE', 'SEALED_CLOCK_CERTIFICATE',
        'SUCCESSOR_KEY_CLAIM', 'ROTATED_SIGNING_MATERIAL'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"history_never_rewritten":true}'
    );

    SELECT approve_agent_ske_profile(
      101,
    ...
    INSERT INTO agent_ske_key_catalog (
      ...
      donor_purpose_hash, hop_count, key_rotated_at,
      ...
      'SEALED_REATTEST_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
      ...
      'SUCCESSOR_KEY_CLAIM', 'RESTORED_WITHOUT_WINNER', 'MONOTONIC',
    ...
    INSERT INTO agent_ske_session (
      ...
      budget_invalidate_units, budget_enroll_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_enroll_units,
      ...
      'Enroll receipts without rewriting history',
      'request-1',
    ...
    INSERT INTO agent_ske_nomination_key (
      ...
      '50000000-0000-0000-0000-000000000002', 'SUCCESSOR_KEY_CLAIM',
      'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'ATTESTED',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('1', 64), now()
    );

    INSERT INTO agent_ske_evaluation_receipt (
    ...
    INSERT INTO agent_ske_enroll_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, key_set_hash, clock_set_hash, watermark_set_hash,
    ...
    INSERT INTO agent_ske_enroll_binding (
    ...
    INSERT INTO agent_ske_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ske_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_enroll_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ske_profile_enroll_rule
        SET require_trusted_key = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'enroll-case';
      `),
    "mutation of a sealed enroll rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ske_effect_intent (
          ...
          'provider-enroll-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ske_effect_intent (
      ...
      'provider-enroll-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ske_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ske_enroll_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SUCCESSOR_KEY_CLAIM', 2, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'ATTESTED',
          'SILENCE', 'ENROLL_KEY_TO_CLOCK', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ske_enroll_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SUCCESSOR_KEY_CLAIM', 3, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'ATTESTED',
          'TRUSTED_ENROLLED', 'ENROLL_KEY_TO_CLOCK', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ske_session WHERE account_id = 101",
  );
