...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE kcq_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_kcq_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_kcq_authorization_evidence (
      ...
      '{"scopes":["provider.key.quarantine"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_kcq_profile (
      ...
      max_bindings_per_certificate, max_nominated_receipts, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident key-compromise quarantine', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'kcq-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'key-compromise-quarantine'],
      'proc:key-compromise-quarantine-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_kcq_profile_quarantine_rule (
      ...
      require_trusted_receipt, quarantine_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'quarantine-case', 1,
      ARRAY['SEALED_ATTESTATION_RECEIPT', 'POST_RETIREMENT_RECEIPT',
        'COMPROMISED_KEY_RECEIPT'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"history_never_rewritten":true}'
    );

    SELECT approve_agent_kcq_profile(
      101,
    ...
    INSERT INTO agent_kcq_receipt_catalog (
      account_id, source_receipt_id, source_session_id, source_certificate_id,
      receipt_ref, source_receipt_kind, placement_kind, receipt_lifecycle,
      status, disputed_fact_hash, attenuation_hash, donor_purpose_hash,
      hop_count, retirement_watermark_at, receipt_signed_at, sealed_at,
      created_at
    ) VALUES (
      ...
      'POST_RETIREMENT_RECEIPT', 'HALTED', 'POST_WATERMARK', 'SEALED',
    ...
    INSERT INTO agent_kcq_session (
      ...
      budget_invalidate_units, budget_quarantine_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_quarantine_units,
      budget_max_wall_time_ms, evaluate_threshold,
      max_bindings_per_certificate, max_nominated_receipts, deadline_at,
      ...
      'Quarantine post-watermark receipts without rewriting history',
      'request-1',
    ...
    INSERT INTO agent_kcq_nomination_receipt (
      account_id, receipt_id, session_id, source_receipt_id,
      source_receipt_kind, placement_kind, receipt_lifecycle,
      disputed_fact_hash, attenuation_hash, donor_purpose_hash, hop_count,
      nomination_hash, nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000002', 'POST_RETIREMENT_RECEIPT',
      'RESTORED_WITHOUT_WINNER', 'POST_WATERMARK', repeat('b', 64),
      repeat('c', 64), repeat('9', 64), 1, repeat('1', 64), now()
    );

    INSERT INTO agent_kcq_evaluation_receipt (
      account_id, evaluation_id, session_id, receipt_set_hash,
      quarantine_set_hash, watermark_set_hash, attenuation_hash,
      evaluation_hash, evaluated_at
    ) VALUES (
      101, '70000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      repeat('2', 64), repeat('3', 64), repeat('4', 64), repeat('c', 64),
      repeat('5', 64), now()
    );

    INSERT INTO agent_kcq_quarantine_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, receipt_set_hash, quarantine_set_hash, watermark_set_hash,
      attenuation_hash, binding_watermark, sealed_revision, sealed_at,
      created_at
    ) VALUES (
      101, '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '70000000-0000-0000-0000-000000000001',
      'consumer:incident-triage', repeat('c', 64),
      repeat('2', 64), repeat('3', 64), repeat('4', 64), repeat('c', 64),
      1, 1, now(), now()
    );

    INSERT INTO agent_kcq_quarantine_binding (
      account_id, binding_id, certificate_id, session_id,
      source_receipt_id, source_receipt_kind, binding_ordinal, status,
      placement_kind, receipt_lifecycle, attested_receipt_kind,
      quarantine_kind, purpose_relation, disputed_fact_hash, attenuation_hash,
      requested_purpose_hash, donor_purpose_hash, hop_count,
      provider_receipt_hash, retirement_watermark_hash, sealed_revision,
      ...
      'POST_RETIREMENT_RECEIPT', 1, 'SEALED', 'RESTORED_WITHOUT_WINNER',
      'POST_WATERMARK', 'TRUSTED_POST_WATERMARK', 'QUARANTINE', 'NARROWS',
      repeat('b', 64), repeat('c', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('6', 64), repeat('7', 64), 1, now(), now()
    );

    UPDATE agent_kcq_session
    SET status = 'SEALED',
        state_revision = 1,
    ...
    INSERT INTO agent_kcq_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_kcq_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_quarantine_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_kcq_profile_quarantine_rule
        SET require_trusted_receipt = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'quarantine-case';
      `),
    "mutation of a sealed quarantine rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_kcq_effect_intent (
          ...
          'provider-quarantine-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_kcq_effect_intent (
      ...
      'provider-quarantine-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_kcq_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_kcq_quarantine_binding (
          ...
          source_receipt_id, source_receipt_kind, binding_ordinal, status,
          placement_kind, receipt_lifecycle, attested_receipt_kind,
          quarantine_kind, purpose_relation, disputed_fact_hash,
          attenuation_hash, requested_purpose_hash, donor_purpose_hash,
          hop_count, provider_receipt_hash, retirement_watermark_hash,
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'POST_RETIREMENT_RECEIPT', 2, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'POST_WATERMARK', 'SILENCE',
          'KEEP_PRE_WATERMARK', 'NARROWS', repeat('b', 64), repeat('c', 64),
          repeat('c', 64), repeat('9', 64), 1, repeat('0', 64), repeat('7', 64),
          1, now(), now()
        );
      `),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_kcq_quarantine_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'POST_RETIREMENT_RECEIPT', 3, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'POST_WATERMARK',
          'TRUSTED_POST_WATERMARK', 'QUARANTINE', 'AMPLIFIES',
          repeat('b', 64), repeat('c', 64), repeat('9', 64),
          repeat('9', 64), 1, repeat('6', 64), repeat('7', 64),
          1, now(), now()
        );
      `),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_kcq_session WHERE account_id = 101",
  );
