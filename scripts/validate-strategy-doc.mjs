...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE cwo_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_cwo_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cwo_authorization_evidence (
    ...
    INSERT INTO agent_cwo_profile (
      ...
      'Incident confirmation-window observability', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'cwo-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'confirmation-window-observability'],
      'proc:confirmation-window-observability-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_cwo_profile_observe_rule (
      ...
      expiring_threshold_ms, require_dual_control_refresh, observe_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'observe-case', 1,
      ARRAY['SEALED_TTL_CERTIFICATE', 'SEALED_CONFIRM_CERTIFICATE',
        'FIRST_ENROLLMENT_CLAIM', 'SUCCESSOR_KEY_CLAIM'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"countdown_never_confirms":true}'
    );

    SELECT approve_agent_cwo_profile(
      101,
    ...
    INSERT INTO agent_cwo_window_catalog (
    ...
    INSERT INTO agent_cwo_session (
      ...
      budget_invalidate_units, budget_observe_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_observe_units,
      ...
      'Observe remaining windows without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_cwo_nomination_window (
    ...
    INSERT INTO agent_cwo_evaluation_receipt (
    ...
    INSERT INTO agent_cwo_window_certificate (
    ...
    INSERT INTO agent_cwo_observe_binding (
      ...
      attested_key_kind, observe_kind, purpose_relation,
    ...
    INSERT INTO agent_cwo_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cwo_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_observe_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cwo_observe_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cwo_effect_intent (
          ...
          'provider-window-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cwo_effect_intent (
      ...
      'provider-window-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cwo_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cwo_observe_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'WINDOW_EXPIRING', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cwo_observe_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'WINDOW_EXPIRING', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_cwo_session WHERE account_id = 101",
  );
