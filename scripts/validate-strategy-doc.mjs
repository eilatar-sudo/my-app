...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE dmp_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_dmp_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_dmp_authorization_evidence (
      ...
      '{"scopes":["provider.key.promote"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_dmp_profile (
      ...
      authorization_snapshot_hash, promotion_window_ms, max_refresh_count,
      ...
      'Incident decision-memory-promotion', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'dmp-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'decision-memory-promotion'],
      'proc:decision-memory-promotion-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_dmp_profile_promotion_rule (
      ...
      promotion_window_ms, require_dual_control_promotion, promotion_instruction
    ) VALUES (
    ...
    SELECT approve_agent_dmp_profile(
      101,
    ...
    INSERT INTO agent_dmp_certificate_catalog (
    ...
    INSERT INTO agent_dmp_session (
      ...
      budget_invalidate_units, budget_promote_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_promote_units,
      ...
      'Resolve sealed governance certificates without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_dmp_nomination_window (
    ...
    INSERT INTO agent_dmp_evaluation_receipt (
    ...
    INSERT INTO agent_dmp_promotion_certificate (
    ...
    INSERT INTO agent_dmp_promotion_binding (
      ...
      attested_key_kind, promotion_kind, purpose_relation,
    ...
    INSERT INTO agent_dmp_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_dmp_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_promote_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_dmp_profile_promotion_rule
        SET require_dual_control_promotion = false
        WHERE account_id = 101
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_dmp_promotion_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_dmp_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_dmp_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_dmp_promotion_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'PROMOTION_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_dmp_promotion_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'PROMOTION_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_dmp_session WHERE account_id = 101",
  );
