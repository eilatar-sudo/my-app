...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ea_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ea_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ea_authorization_evidence (
      ...
      '{"scopes":["escalation.authority"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_ea_profile (
      ...
      authorization_snapshot_hash, authority_threshold,
      max_rulings_per_certificate, semantic_tags, procedure_ref, revocation_policy,
      created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident escalation authority', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ea-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3, 32,
      ARRAY['incident', 'escalation-authority'], 'proc:escalation-authority-incident',
      'STOP_BEFORE_RULING', 'principal:test', now()
    );

    INSERT INTO agent_ea_profile_escalation_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_case_kinds, authority_threshold, require_refresh,
      authority_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'escalate-case', 1,
      ARRAY['ESCALATION_CASE_FACT', 'SUPERSEDED_CASE_FACT'], 3, true,
      '{"narrow_only":true}'
    );

    SELECT approve_agent_ea_profile(
      101,
    ...
    INSERT INTO agent_ea_escalation_catalog (
      account_id, case_id, source_escalation_id, split_session_id, case_ref,
      case_kind, status, fact_hash, attenuation_hash, excluded_principal_ids,
      sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      'a0000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      'escalation:agent-b/purpose:incident-triage',
      'ESCALATION_CASE_FACT', 'SEALED', repeat('b', 64), repeat('c', 64),
      ARRAY['principal:first-tier-a', 'principal:first-tier-b'], now(), now()
    );

    INSERT INTO agent_ea_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_nominate_units,
      budget_ruling_units, budget_vector_units, budget_seal_units,
      budget_refresh_units, consumed_nominate_units, consumed_ruling_units,
      consumed_vector_units, consumed_seal_units, consumed_refresh_units,
      budget_max_wall_time_ms, authority_threshold, max_rulings_per_certificate,
      ...
      'Resolve incident escalation under higher authority', 'request-1', 100, 40, 5, 8, 2,
    ...
    INSERT INTO agent_ea_nomination_receipt (
      ...
      '50000000-0000-0000-0000-000000000001', 'ESCALATION_CASE_FACT',
      repeat('b', 64), repeat('c', 64), repeat('1', 64), now()
    );

    INSERT INTO agent_ea_certificate (
      account_id, certificate_id, session_id, consumer_ref, purpose_hash,
      authority_set_hash, resolved_fact_hash, ruling_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
    ...
    INSERT INTO agent_ea_certificate_ruling (
      account_id, ruling_id, certificate_id, session_id, case_id, case_kind,
      ruling_ordinal, status, fact_hash, attenuation_hash, sealed_revision,
      sealed_at, created_at
    ) VALUES (
      ...
      'ESCALATION_CASE_FACT', 0, 'SEALED',
      repeat('b', 64), repeat('c', 64), 1, now(), now()
    );

    UPDATE agent_ea_session
    SET status = 'ACTIVE',
        state_revision = 1,
    ...
    INSERT INTO agent_ea_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ea_profile_escalation_rule
        SET require_refresh = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'escalate-case';
      `),
    "mutation of a sealed escalation rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ea_certificate_ruling
        SET fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND ruling_id = 'a0000000-0000-0000-0000-000000000099';
      `),
    "mutation of certificate ruling identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ea_refresh_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ea_refresh_intent
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ea_ruling_cast (
          account_id, step_id, session_id, prior_case_id, case_id,
          authority_principal_id, ruling_ordinal, fact_hash, attenuation_hash,
          ruling_hash, cast_at
        ) VALUES (
          101, 'b0000000-0000-0000-0000-000000000001',
          '20000000-0000-0000-0000-000000000001',
          NULL, '50000000-0000-0000-0000-000000000001',
          'principal:first-tier-a', 1, repeat('b', 64), repeat('c', 64),
          repeat('6', 64), now()
        );
      `),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ea_session WHERE account_id = 101",
  );
