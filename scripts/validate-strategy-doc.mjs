...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE wca_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_wca_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_wca_authorization_evidence (
      ...
      '{"scopes":["provider.clock.attest"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_wca_profile (
      ...
      max_bindings_per_certificate, max_nominated_samples, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident watermark-clock attestation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'wca-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'watermark-clock-attestation'],
      'proc:watermark-clock-attestation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_wca_profile_clock_rule (
      ...
      require_trusted_sample, clock_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'clock-case', 1,
      ARRAY['SEALED_QUARANTINE_WATERMARK', 'PROVIDER_CLOCK_SAMPLE',
        'RETIREMENT_WATERMARK_CLAIM', 'COMPROMISE_WATERMARK_CLAIM'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"history_never_rewritten":true}'
    );

    SELECT approve_agent_wca_profile(
      101,
    ...
    INSERT INTO agent_wca_clock_catalog (
      account_id, source_clock_id, source_session_id, source_certificate_id,
      clock_ref, source_clock_kind, placement_kind, clock_lifecycle,
      status, disputed_fact_hash, attenuation_hash, donor_purpose_hash,
      hop_count, provider_reported_at, engine_observed_at,
      first_attested_sample_at, last_attested_sample_at, max_skew_ms,
      sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      '90000000-0000-0000-0000-000000000099',
      'clock:provider-a/purpose:incident-triage',
      'RETIREMENT_WATERMARK_CLAIM', 'HALTED', 'REGRESSED', 'SEALED',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '2 days', now() - interval '1 day',
      now() - interval '3 days', now() - interval '2 days', 5000,
      now() - interval '1 day', now()
    ), (
      101, '50000000-0000-0000-0000-000000000002',
      '80000000-0000-0000-0000-000000000002',
      '90000000-0000-0000-0000-000000000098',
      'clock:provider-b/purpose:incident-triage',
      'PROVIDER_CLOCK_SAMPLE', 'RESTORED_WITHOUT_WINNER', 'MONOTONIC',
      'SEALED', repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '12 hours', now() - interval '12 hours',
      now() - interval '3 days', now() - interval '1 day', 5000,
      now() - interval '12 hours', now()
    );

    INSERT INTO agent_wca_session (
      ...
      budget_invalidate_units, budget_clock_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_clock_units,
      budget_max_wall_time_ms, evaluate_threshold,
      max_bindings_per_certificate, max_nominated_samples, deadline_at,
      ...
      'Attest watermarks without rewriting history',
      'request-1',
    ...
    INSERT INTO agent_wca_nomination_receipt (
      account_id, receipt_id, session_id, source_clock_id,
      ...
      '50000000-0000-0000-0000-000000000002', 'PROVIDER_CLOCK_SAMPLE',
      'RESTORED_WITHOUT_WINNER', 'MONOTONIC',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('1', 64), now()
    );

    INSERT INTO agent_wca_evaluation_receipt (
      account_id, evaluation_id, session_id, sample_set_hash,
      clock_set_hash, watermark_set_hash, attenuation_hash,
    ...
    INSERT INTO agent_wca_clock_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, sample_set_hash, clock_set_hash, watermark_set_hash,
    ...
    INSERT INTO agent_wca_clock_binding (
      account_id, binding_id, certificate_id, session_id,
      source_clock_id, source_clock_kind, binding_ordinal, status,
      placement_kind, clock_lifecycle, attested_clock_kind,
      clock_kind, purpose_relation, disputed_fact_hash,
      attenuation_hash, requested_purpose_hash, donor_purpose_hash,
      hop_count, provider_sample_hash, retirement_watermark_hash,
      ...
      'PROVIDER_CLOCK_SAMPLE', 1, 'SEALED',
      'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'TRUSTED_MONOTONIC',
      'ATTEST_MONOTONIC', 'NARROWS', repeat('b', 64), repeat('c', 64),
      repeat('c', 64), repeat('9', 64), 1, repeat('6', 64), repeat('7', 64),
      1, now(), now()
    );

    UPDATE agent_wca_session
    SET status = 'SEALED',
        state_revision = 3,
    ...
    INSERT INTO agent_wca_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_wca_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_clock_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_wca_profile_clock_rule
        SET require_trusted_sample = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'clock-case';
      `),
    "mutation of a sealed clock rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_wca_effect_intent (
          ...
          'provider-clock-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_wca_effect_intent (
      ...
      'provider-clock-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_wca_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_wca_clock_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'PROVIDER_CLOCK_SAMPLE', 2, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'SILENCE',
          'ATTEST_MONOTONIC', 'NARROWS', repeat('b', 64), repeat('c', 64),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_wca_clock_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'PROVIDER_CLOCK_SAMPLE', 3, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC',
          'TRUSTED_MONOTONIC', 'ATTEST_MONOTONIC', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_wca_session WHERE account_id = 101",
  );
