...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE uae_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_uae_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_uae_authorization_evidence (
    ...
    INSERT INTO agent_uae_profile (
      ...
      authorization_snapshot_hash, escalation_window_ms, max_refresh_count,
      ...
      'Incident unanswered-ack escalation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'uae-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'unanswered-ack-escalation'],
      'proc:unanswered-ack-escalation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_uae_profile_escalation_rule (
      ...
      escalation_window_ms, require_dual_control_escalation, escalation_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'escalation-case', 1,
      ...
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"escalation_never_confirms":true}'
    );

    SELECT approve_agent_uae_profile(
      101,
    ...
    INSERT INTO agent_uae_ack_catalog (
    ...
    INSERT INTO agent_uae_session (
      ...
      budget_invalidate_units, budget_escalate_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_escalate_units,
      ...
      'Escalate unanswered acknowledgements without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_uae_nomination_window (
    ...
    INSERT INTO agent_uae_evaluation_receipt (
    ...
    INSERT INTO agent_uae_escalation_certificate (
    ...
    INSERT INTO agent_uae_escalation_binding (
      ...
      attested_key_kind, escalation_kind, purpose_relation,
    ...
    INSERT INTO agent_uae_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_uae_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_escalate_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_uae_profile_escalation_rule
        SET require_dual_control_escalation = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'escalation-case';
      `),
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_uae_escalation_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_uae_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_uae_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_uae_escalation_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'ESCALATION_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_uae_escalation_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'ESCALATION_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_uae_session WHERE account_id = 101",
  );
