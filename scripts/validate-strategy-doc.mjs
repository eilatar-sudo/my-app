...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE fec_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_fec_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_fec_authorization_evidence (
      ...
      '{"scopes":["provider.key.confirm"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_fec_profile (
      ...
      'Incident first-enrollment confirmation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'fec-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'first-enrollment-confirmation'],
      'proc:first-enrollment-confirmation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_fec_profile_confirm_rule (
      ...
      require_dual_control, confirm_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'confirm-case', 1,
      ARRAY['SEALED_ENROLL_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM',
        'SUCCESSOR_KEY_CLAIM', 'ROTATED_SIGNING_MATERIAL'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"dual_control":true}'
    );

    SELECT approve_agent_fec_profile(
      101,
    ...
    INSERT INTO agent_fec_key_catalog (
      ...
      'SEALED_ENROLL_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_fec_session (
      ...
      budget_invalidate_units, budget_confirm_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_confirm_units,
      ...
      'Confirm first-enrollment keys without auto-trust',
      'request-1',
    ...
    INSERT INTO agent_fec_nomination_key (
    ...
    INSERT INTO agent_fec_evaluation_receipt (
      account_id, evaluation_id, session_id, key_set_hash,
      clock_set_hash, confirmer_set_hash, attenuation_hash,
    ...
    INSERT INTO agent_fec_confirm_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, key_set_hash, clock_set_hash, confirmer_set_hash,
    ...
    INSERT INTO agent_fec_confirm_binding (
      ...
      attested_key_kind, confirm_kind, purpose_relation,
      disputed_fact_hash, attenuation_hash, requested_purpose_hash,
      donor_purpose_hash, hop_count, successor_key_hash,
      clock_attestation_hash, confirmation_count, distinct_confirmer_count,
      self_confirm, sealed_revision, sealed_at, created_at
    ) VALUES (
      ...
      'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
      'TRUSTED_ENROLLED', 'CONFIRM_FIRST_ENROLLMENT', 'NARROWS',
      repeat('b', 64), repeat('c', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('6', 64), repeat('7', 64), 2, 2, false, 1, now(), now()
    );

    UPDATE agent_fec_session
    SET status = 'SEALED',
        state_revision = 3,
    ...
    INSERT INTO agent_fec_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_fec_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_confirm_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_fec_profile_confirm_rule
        SET require_dual_control = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'confirm-case';
      `),
    "mutation of a sealed confirm rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_fec_effect_intent (
          ...
          'provider-confirm-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_fec_effect_intent (
      ...
      'provider-confirm-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_fec_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_fec_confirm_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'SILENCE', 'CONFIRM_FIRST_ENROLLMENT', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_fec_confirm_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'CONFIRM_FIRST_ENROLLMENT', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_fec_session WHERE account_id = 101",
  );
