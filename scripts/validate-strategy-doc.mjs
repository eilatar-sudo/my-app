...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE sas_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_sas_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_sas_authorization_evidence (
    ...
    INSERT INTO agent_sas_profile (
      ...
      authorization_snapshot_hash, suppression_window_ms, max_refresh_count,
      ...
      'Incident SLO ack suppression', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'sas-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'slo-ack-suppression'],
      'proc:slo-ack-suppression-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_sas_profile_ack_rule (
      ...
      suppression_window_ms, require_dual_control_ack, ack_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'ack-case', 1,
      ARRAY['SEALED_SLO_CERTIFICATE', 'SEALED_TTL_CERTIFICATE',
        'SEALED_CONFIRM_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"ack_never_confirms":true}'
    );

    SELECT approve_agent_sas_profile(
      101,
    ...
    INSERT INTO agent_sas_page_catalog (
    ...
    INSERT INTO agent_sas_session (
      ...
      budget_invalidate_units, budget_ack_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_ack_units,
      ...
      'Acknowledge SLO pages without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_sas_nomination_window (
    ...
    INSERT INTO agent_sas_evaluation_receipt (
    ...
    INSERT INTO agent_sas_ack_certificate (
    ...
    INSERT INTO agent_sas_ack_binding (
      ...
      attested_key_kind, ack_kind, purpose_relation,
    ...
    INSERT INTO agent_sas_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_sas_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_ack_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_sas_profile_ack_rule
        SET require_dual_control_ack = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'ack-case';
      `),
    "mutation of a sealed acknowledgement rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_sas_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_sas_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_sas_ack_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'ACK_SUPPRESSION_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_sas_ack_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'ACK_SUPPRESSION_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_sas_session WHERE account_id = 101",
  );
