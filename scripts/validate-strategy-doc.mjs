...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE cs_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_cs_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cs_authorization_evidence (
      ...
      '{"scopes":["citation.share"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_cs_profile (
      ...
      authorization_snapshot_hash, max_recipient_fan_out,
      max_recipients_per_citation, semantic_tags, procedure_ref, revocation_policy,
      created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident severity citation sharing', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'cs-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 4, 2,
      ARRAY['incident', 'severity'], 'proc:share-severity',
      'STOP_BEFORE_ACCEPT', 'principal:test', now()
    );

    INSERT INTO agent_cs_profile_recipient_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      surface_key_pattern, allowed_recipient_kinds, require_sync,
      attenuation_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'severity', 1,
      'incident:*:severity', ARRAY['AGENT_SESSION', 'WORKING_SET'], true,
      '{"narrow_only":true}'
    );

    SELECT approve_agent_cs_profile(
      101,
    ...
    INSERT INTO agent_cs_citation_catalog (
    ...
    INSERT INTO agent_cs_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_accept_units,
      budget_share_units, budget_vector_units, budget_sync_units,
      budget_notify_units, consumed_accept_units, consumed_share_units,
      consumed_vector_units, consumed_sync_units, consumed_notify_units,
      budget_max_wall_time_ms, max_recipient_fan_out, max_recipients_per_citation,
      ...
      'Share severity citation', 'request-1', 100, 20, 5, 8, 2, 0, 0, 0, 0, 0,
    ...
    INSERT INTO agent_cs_acceptance_receipt (
    ...
    INSERT INTO agent_cs_share_grant (
      account_id, share_grant_id, session_id, citation_id, receipt_id,
    ...
    INSERT INTO agent_cs_binding (
      account_id, binding_id, share_grant_id, session_id, citation_id,
      recipient_kind, recipient_ref, status, fact_hash, attenuation_hash,
      ...
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001',
      'AGENT_SESSION', 'session:agent-b/purpose:incident-triage', 'SEALED',
      repeat('b', 64), repeat('c', 64), 1, now(), now()
    );

    UPDATE agent_cs_session
    SET status = 'ACTIVE',
        state_revision = 1,
        consumed_accept_units = 1,
        consumed_share_units = 1,
        updated_at = now()
    WHERE account_id = 101
      AND session_id = '20000000-0000-0000-0000-000000000001';

    INSERT INTO agent_cs_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cs_profile_recipient_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cs_binding
        SET fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND binding_id = 'a0000000-0000-0000-0000-000000000001';
      `),
    "mutation of binding identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cs_notify_intent (
          account_id, notify_id, session_id, binding_id,
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_cs_session WHERE account_id = 101",
  );
