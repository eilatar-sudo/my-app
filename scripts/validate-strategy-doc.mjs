...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE urr_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_urr_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_urr_authorization_evidence (
    ...
    INSERT INTO agent_urr_profile (
      ...
      'Incident review-resolution', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'urr-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'review-resolution'],
      'proc:review-resolution-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_urr_profile_resolution_rule (
      ...
      ARRAY['SEALED_ACK_CERTIFICATE', 'SEALED_SLO_CERTIFICATE',
        ...
        'FIRST_ENROLLMENT_CLAIM', 'SEALED_ESCALATION_CERTIFICATE', 'SEALED_REVIEW_CERTIFICATE', 'SEALED_CONTAINMENT_CERTIFICATE'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"review_never_confirms":true}'
    );

    SELECT approve_agent_urr_profile(
      101,
    ...
    INSERT INTO agent_urr_review_catalog (
    ...
    INSERT INTO agent_urr_session (
      ...
      budget_invalidate_units, budget_resolve_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_resolve_units,
    ...
    INSERT INTO agent_urr_nomination_window (
    ...
    INSERT INTO agent_urr_evaluation_receipt (
    ...
    INSERT INTO agent_urr_resolution_certificate (
    ...
    INSERT INTO agent_urr_resolution_binding (
    ...
    INSERT INTO agent_urr_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_urr_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_resolve_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_urr_profile_resolution_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_urr_resolution_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_urr_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_urr_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_urr_resolution_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'RESOLVE_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_urr_resolution_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'RESOLVE_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_urr_session WHERE account_id = 101",
  );
