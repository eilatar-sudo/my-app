...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE rex_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_rex_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_rex_authorization_evidence (
    ...
    INSERT INTO agent_rex_profile (
      ...
      'Incident resolution-expiry-containment', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'rex-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'resolution-expiry-containment'],
      'proc:resolution-expiry-containment-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_rex_profile_containment_rule (
      ...
      ARRAY['SEALED_ACK_CERTIFICATE', 'SEALED_SLO_CERTIFICATE',
        'SEALED_TTL_CERTIFICATE', 'SEALED_CONFIRM_CERTIFICATE',
        'FIRST_ENROLLMENT_CLAIM', 'SEALED_ESCALATION_CERTIFICATE', 'SEALED_RESOLUTION_CERTIFICATE', 'SEALED_REVIEW_CERTIFICATE', 'SEALED_CONTAINMENT_CERTIFICATE'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"review_never_confirms":true}'
    );

    SELECT approve_agent_rex_profile(
      101,
    ...
    INSERT INTO agent_rex_resolution_catalog (
    ...
    INSERT INTO agent_rex_session (
      ...
      'Resolve unanswered reviews without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_rex_nomination_window (
    ...
    INSERT INTO agent_rex_evaluation_receipt (
    ...
    INSERT INTO agent_rex_containment_certificate (
    ...
    INSERT INTO agent_rex_containment_binding (
    ...
    INSERT INTO agent_rex_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rex_profile_containment_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_rex_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rex_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_rex_containment_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'CONTAIN_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_rex_containment_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'CONTAIN_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_rex_session WHERE account_id = 101",
  );
