...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE rer_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_rer_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_rer_authorization_evidence (
    ...
    INSERT INTO agent_rer_profile (
      ...
      'Incident review-expiry-resolution', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'rer-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'review-expiry-resolution'],
      'proc:review-expiry-resolution-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_rer_profile_resolution_rule (
    ...
    SELECT approve_agent_rer_profile(
      101,
    ...
    INSERT INTO agent_rer_review_catalog (
    ...
    INSERT INTO agent_rer_session (
      ...
      'Resolve expired reviews without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_rer_nomination_window (
    ...
    INSERT INTO agent_rer_evaluation_receipt (
    ...
    INSERT INTO agent_rer_resolution_certificate (
    ...
    INSERT INTO agent_rer_resolution_binding (
    ...
    INSERT INTO agent_rer_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rer_profile_resolution_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rer_resolution_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_rer_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_rer_effect_intent
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_rer_session WHERE account_id = 101",
  );
