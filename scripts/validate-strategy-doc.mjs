...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ucr_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ucr_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ucr_authorization_evidence (
    ...
    INSERT INTO agent_ucr_profile (
      ...
      'Incident unanswered-containment review', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ucr-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'unanswered-containment-review'],
      'proc:unanswered-containment-review-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_ucr_profile_review_rule (
      ...
      ARRAY['SEALED_ACK_CERTIFICATE', 'SEALED_SLO_CERTIFICATE',
        ...
        'FIRST_ENROLLMENT_CLAIM', 'SEALED_ESCALATION_CERTIFICATE', 'SEALED_CONTAINMENT_CERTIFICATE'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"review_never_confirms":true}'
    );

    SELECT approve_agent_ucr_profile(
      101,
    ...
    INSERT INTO agent_ucr_containment_catalog (
    ...
    INSERT INTO agent_ucr_session (
      ...
      'Review unanswered containments without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_ucr_nomination_window (
    ...
    INSERT INTO agent_ucr_evaluation_receipt (
    ...
    INSERT INTO agent_ucr_review_certificate (
    ...
    INSERT INTO agent_ucr_review_binding (
    ...
    INSERT INTO agent_ucr_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ucr_profile_review_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ucr_review_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ucr_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ucr_effect_intent
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ucr_session WHERE account_id = 101",
  );
