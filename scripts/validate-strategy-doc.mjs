...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ctt_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ctt_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ctt_authorization_evidence (
      ...
      '{"scopes":["provider.key.ttl"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_ctt_profile (
      ...
      authorization_snapshot_hash, default_ttl_ms, max_extend_count,
      ...
      'Incident confirmation-TTL retirement', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ctt-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 86400000, 4, 2, 32, 32,
      ARRAY['incident', 'confirmation-ttl-retirement'],
      'proc:confirmation-ttl-retirement-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_ctt_profile_ttl_rule (
      ...
      default_ttl_ms, require_dual_control_extend, ttl_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'ttl-case', 1,
      ARRAY['SEALED_CONFIRM_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM',
        'SUCCESSOR_KEY_CLAIM', 'ROTATED_SIGNING_MATERIAL'],
      2, 32, 86400000, true,
      '{"narrow_only":true,"no_winner":true,"dual_control_extend":true}'
    );

    SELECT approve_agent_ctt_profile(
      101,
    ...
    INSERT INTO agent_ctt_key_catalog (
      account_id, source_key_id, source_session_id, source_certificate_id,
      receipt_ref, source_key_kind, placement_kind, key_lifecycle,
      clock_lifecycle, status, disputed_fact_hash, attenuation_hash,
      donor_purpose_hash, hop_count, first_seen_at,
      ...
      'SEALED_CONFIRM_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_ctt_session (
      ...
      budget_invalidate_units, budget_retire_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_retire_units,
      ...
      'Retire unconfirmed keys without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_ctt_nomination_key (
      ...
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('1', 64), now() - interval '2 days', now() - interval '1 day',
      true
    );

    INSERT INTO agent_ctt_evaluation_receipt (
      account_id, evaluation_id, session_id, key_set_hash,
      clock_set_hash, ttl_set_hash, extender_set_hash, attenuation_hash,
      evaluation_hash, evaluated_at
    ) VALUES (
      101, '70000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      repeat('2', 64), repeat('3', 64), repeat('4', 64), repeat('8', 64),
      repeat('c', 64), repeat('5', 64), now()
    );

    INSERT INTO agent_ctt_ttl_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, key_set_hash, clock_set_hash, ttl_set_hash,
      extender_set_hash, attenuation_hash, binding_watermark, sealed_revision,
      sealed_at, created_at
    ) VALUES (
      ...
      repeat('8', 64), repeat('c', 64), 1, 1, now(), now()
    );

    INSERT INTO agent_ctt_retire_binding (
      ...
      attested_key_kind, retire_kind, purpose_relation,
      disputed_fact_hash, attenuation_hash, requested_purpose_hash,
      donor_purpose_hash, hop_count, successor_key_hash,
      clock_attestation_hash, ttl_set_hash, confirmation_count,
      distinct_confirmer_count, extend_count, distinct_extender_count,
      self_extend, ttl_expired, expires_at, sealed_revision, sealed_at,
      created_at
    ) VALUES (
      ...
      'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
      'AWAITING_CONFIRMATION', 'RETIRE_UNCONFIRMED', 'NARROWS',
      repeat('b', 64), repeat('c', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('6', 64), repeat('7', 64), repeat('4', 64), 0, 0, 0, 0,
      false, true, now() - interval '1 day', 1, now(), now()
    );

    UPDATE agent_ctt_session
    SET status = 'SEALED',
        state_revision = 3,
    ...
    INSERT INTO agent_ctt_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ctt_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_retire_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ctt_profile_ttl_rule
        SET require_dual_control_extend = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'ttl-case';
      `),
    "mutation of a sealed ttl rule",
    "P0001",
  );
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ctt_profile (
          ...
          'ctt-compiler-1', repeat('a', 64),
          '30000000-0000-0000-0000-000000000001', repeat('d', 64),
          86400000, 4, 2, 16, 16, ARRAY['unsafe'], 'STOP_BEFORE_EVALUATE',
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ctt_effect_intent (
          ...
          'provider-ttl-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ctt_effect_intent (
      ...
      'provider-ttl-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ctt_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ctt_retire_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'RETIRE_UNCONFIRMED', 'NARROWS',
          repeat('b', 64), repeat('c', 64),
          repeat('c', 64), repeat('9', 64), 1, repeat('0', 64), repeat('7', 64),
          repeat('4', 64), 0, 0, 0, 0, false, true,
          now() - interval '1 day', 1, now(), now()
        );
      `),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ctt_retire_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'RETIRE_UNCONFIRMED', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ctt_session WHERE account_id = 101",
  );
