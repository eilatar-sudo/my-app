...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE cp_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_cp_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cp_authorization_evidence (
      ...
      '{"scopes":["certificate.placement"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_cp_profile (
      ...
      authorization_snapshot_hash, compile_threshold,
      max_slots_per_packet, max_nominated_certificates, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident certificate placement', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'cp-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'certificate-placement'], 'proc:placement-incident',
      'STOP_BEFORE_COMPILE', 'principal:test', now()
    );

    INSERT INTO agent_cp_profile_placement_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_source_kinds, compile_threshold, max_slots_per_packet,
      require_notify, placement_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'placement-case', 1,
      ARRAY['SEALED_FREEZE_CERT', 'SEALED_BREACH_CERT', 'SEALED_THAW_CERT'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"halt_leak":true}'
    );

    SELECT approve_agent_cp_profile(
      101,
    ...
    INSERT INTO agent_cp_certificate_catalog (
      account_id, source_certificate_id, source_session_id, certificate_ref,
      source_certificate_kind, status, disputed_fact_hash, attenuation_hash,
      halt_scope_hash, certificate_sealed_at, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      'breach:agent-c/purpose:incident-triage',
      'SEALED_BREACH_CERT', 'SEALED', repeat('b', 64), repeat('c', 64),
      repeat('6', 64), now() - interval '2 days',
      now() - interval '2 days', now()
    ), (
      101, '50000000-0000-0000-0000-000000000002',
      '80000000-0000-0000-0000-000000000002',
      'thaw:agent-d/purpose:incident-triage',
      'SEALED_THAW_CERT', 'SEALED', repeat('b', 64), repeat('c', 64),
      repeat('6', 64), now() - interval '1 day',
      now() - interval '1 day', now()
    );

    INSERT INTO agent_cp_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_nominate_units,
      budget_compile_units, budget_seal_units, budget_vector_units,
      budget_invalidate_units, budget_notify_units, consumed_nominate_units,
      consumed_compile_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_notify_units, budget_max_wall_time_ms,
      compile_threshold, max_slots_per_packet, max_nominated_certificates,
      ...
      'Compile halt-aware incident working sets', 'request-1',
    ...
    INSERT INTO agent_cp_nomination_receipt (
      account_id, receipt_id, session_id, source_certificate_id,
      source_certificate_kind, disputed_fact_hash, attenuation_hash,
      halt_scope_hash, nomination_hash, nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'SEALED_BREACH_CERT',
      repeat('b', 64), repeat('c', 64), repeat('6', 64), repeat('1', 64), now()
    );

    INSERT INTO agent_cp_compile_receipt (
      account_id, compile_id, session_id, placement_set_hash,
      halt_scope_hash, disputed_fact_set_hash, compile_hash, compiled_at
    ) VALUES (
      101, 'b0000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      repeat('7', 64), repeat('6', 64), repeat('b', 64), repeat('3', 64), now()
    );

    INSERT INTO agent_cp_working_set (
      account_id, working_set_id, session_id, compile_id, consumer_ref,
      purpose_hash, halt_scope_hash, disputed_fact_set_hash,
      placement_set_hash, slot_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
      101, '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      'b0000000-0000-0000-0000-000000000001',
      'consumer:agent-d/purpose:incident-triage',
      repeat('2', 64), repeat('6', 64), repeat('b', 64),
      repeat('7', 64), 1, 1, now(), now()
    );

    INSERT INTO agent_cp_working_set_slot (
      account_id, slot_id, working_set_id, session_id, source_certificate_id,
      source_certificate_kind, slot_ordinal, status, placement_kind,
      disputed_fact_hash, attenuation_hash, sealed_revision, sealed_at,
      created_at
    ) VALUES (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cp_session (
          ...
          budget_invalidate_units, budget_notify_units, consumed_nominate_units,
          consumed_compile_units, consumed_seal_units, consumed_vector_units,
          consumed_invalidate_units, consumed_notify_units,
          budget_max_wall_time_ms, compile_threshold, max_slots_per_packet,
          max_nominated_certificates, deadline_at, started_by, principal_id,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cp_profile_placement_rule
        SET require_notify = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'placement-case';
      `),
    "mutation of a sealed placement rule",
    "P0001",
  );
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cp_profile (
          ...
          'cp-compiler-1', repeat('a', 64),
          '30000000-0000-0000-0000-000000000001', repeat('d', 64),
          2, 16, 16, ARRAY['unsafe'], 'STOP_BEFORE_COMPILE',
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cp_working_set_slot
        SET disputed_fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND slot_id = 'a0000000-0000-0000-0000-000000000099';
      `),
    "mutation of working-set slot identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cp_refresh_intent (
          account_id, refresh_id, session_id, working_set_id,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cp_refresh_intent
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cp_working_set_slot (
          account_id, slot_id, working_set_id, session_id,
          source_certificate_id, source_certificate_kind, slot_ordinal,
          status, placement_kind, disputed_fact_hash, attenuation_hash,
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000001',
          'SEALED_BREACH_CERT', 1, 'SEALED', 'RESTORED_WITHOUT_WINNER',
          repeat('b', 64), repeat('c', 64), 1, now(), now()
        );
      `),
    "halt-leak fence",
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cp_working_set_slot (
          ...
          'SEALED_THAW_CERT', 2, 'SEALED', 'HALTED',
          repeat('b', 64), repeat('c', 64), 1, now(), now()
        );
      `),
    "thaw-restore fence",
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_cp_session WHERE account_id = 101",
  );
