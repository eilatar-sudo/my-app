...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE uec_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_uec_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_uec_authorization_evidence (
    ...
    INSERT INTO agent_uec_profile (
      ...
      'Incident unanswered-escalation containment', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'uec-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'unanswered-escalation-containment'],
      'proc:unanswered-escalation-containment-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_uec_profile_containment_rule (
      ...
      ARRAY['SEALED_ACK_CERTIFICATE', 'SEALED_SLO_CERTIFICATE',
        ...
        'FIRST_ENROLLMENT_CLAIM', 'SEALED_ESCALATION_CERTIFICATE'],
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"containment_never_confirms":true}'
    );

    SELECT approve_agent_uec_profile(
      101,
    ...
    INSERT INTO agent_uec_escalation_catalog (
    ...
    INSERT INTO agent_uec_session (
      ...
      'Contain unanswered escalations without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_uec_nomination_window (
    ...
    INSERT INTO agent_uec_evaluation_receipt (
    ...
    INSERT INTO agent_uec_containment_certificate (
    ...
    INSERT INTO agent_uec_containment_binding (
    ...
    INSERT INTO agent_uec_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_uec_profile_containment_rule
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_uec_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_uec_effect_intent
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_uec_session WHERE account_id = 101",
  );
