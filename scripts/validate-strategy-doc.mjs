...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ets_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ets_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ets_authorization_evidence (
      ...
      '{"scopes":["envelope.tool.effect.saga"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_ets_profile (
      ...
      max_steps_per_certificate, max_nominated_decisions, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident envelope tool-effect saga', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ets-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'tool-effect-saga'], 'proc:tool-effect-saga-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_ets_profile_saga_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_source_kinds, evaluate_threshold, max_steps_per_certificate,
      require_effect_ack, saga_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'saga-case', 1,
      ARRAY['SEALED_TOOL_MUTATE_DECISION', 'SEALED_TOOL_READ_DECISION'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"halt_mutate":true}'
    );

    SELECT approve_agent_ets_profile(
      101,
    ...
    INSERT INTO agent_ets_decision_catalog (
      account_id, source_decision_id, source_session_id, decision_ref,
      source_decision_kind, placement_kind, gate_kind, status,
      disputed_fact_hash, attenuation_hash, donor_purpose_hash, hop_count,
      decision_sealed_at, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      'decision:agent-c/purpose:incident-triage',
      'SEALED_TOOL_READ_DECISION', 'HALTED', 'TOOL_READ', 'SEALED',
      ...
      'decision:agent-d/purpose:incident-triage',
      'SEALED_TOOL_MUTATE_DECISION', 'RESTORED_WITHOUT_WINNER', 'TOOL_MUTATE',
      'SEALED', repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '1 day', now() - interval '1 day', now()
    );

    INSERT INTO agent_ets_session (
      ...
      budget_invalidate_units, budget_effect_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_effect_units, budget_max_wall_time_ms,
      evaluate_threshold, max_steps_per_certificate, max_nominated_decisions,
      ...
      'Bind gated incident tool mutate to a saga', 'request-1',
    ...
    INSERT INTO agent_ets_nomination_receipt (
      account_id, receipt_id, session_id, source_decision_id,
      source_decision_kind, placement_kind, gate_kind, disputed_fact_hash,
      attenuation_hash, donor_purpose_hash, hop_count, nomination_hash,
      nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'SEALED_TOOL_READ_DECISION',
      'HALTED', 'TOOL_READ', repeat('b', 64), repeat('c', 64), repeat('9', 64),
      1, repeat('1', 64), now()
    );

    INSERT INTO agent_ets_evaluation_receipt (
      account_id, evaluation_id, session_id, decision_set_hash,
      step_set_hash, compensation_set_hash, attenuation_hash, evaluation_hash,
      evaluated_at
    ) VALUES (
    ...
    INSERT INTO agent_ets_saga_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, decision_set_hash, step_set_hash, compensation_set_hash,
      attenuation_hash, step_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
    ...
    INSERT INTO agent_ets_saga_step (
      account_id, step_id, certificate_id, session_id, source_decision_id,
      source_decision_kind, step_ordinal, status, placement_kind, gate_kind,
      step_kind, purpose_relation, disputed_fact_hash, attenuation_hash,
      requested_purpose_hash, donor_purpose_hash, hop_count, sealed_revision,
      ...
      'SEALED_TOOL_READ_DECISION', 0, 'SEALED', 'HALTED', 'TOOL_READ',
      'READ_ONLY', 'NARROWS', repeat('b', 64), repeat('c', 64), repeat('c', 64),
      repeat('9', 64), 1, 1, now(), now()
    );

    UPDATE agent_ets_session
    SET status = 'SEALED',
        state_revision = 1,
    ...
    INSERT INTO agent_ets_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ets_session (
          ...
          budget_invalidate_units, budget_effect_units, consumed_nominate_units,
          consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
          consumed_invalidate_units, consumed_effect_units,
          budget_max_wall_time_ms, evaluate_threshold,
          max_steps_per_certificate, max_nominated_decisions, deadline_at,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ets_profile_saga_rule
        SET require_effect_ack = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'saga-case';
      `),
    "mutation of a sealed saga rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ets_saga_step
        SET disputed_fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND step_id = 'a0000000-0000-0000-0000-000000000099';
      `),
    "mutation of saga step identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ets_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ets_effect_intent
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ets_saga_step (
          account_id, step_id, certificate_id, session_id,
          source_decision_id, source_decision_kind, step_ordinal, status,
          placement_kind, gate_kind, step_kind, purpose_relation,
          disputed_fact_hash, attenuation_hash, requested_purpose_hash,
          donor_purpose_hash, hop_count, sealed_revision, sealed_at, created_at
        ) VALUES (
          101, 'c0000000-0000-0000-0000-000000000001',
          '90000000-0000-0000-0000-000000000001',
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000001',
          'SEALED_TOOL_READ_DECISION', 1, 'SEALED', 'HALTED', 'TOOL_READ',
          'MUTATE', 'NARROWS', repeat('b', 64), repeat('c', 64), repeat('c', 64),
          repeat('9', 64), 1, 1, now(), now()
        );
      `),
    "halt-mutate fence",
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ets_saga_step (
          ...
          'SEALED_TOOL_MUTATE_DECISION', 2, 'SEALED', 'RESTORED_WITHOUT_WINNER',
          'TOOL_MUTATE', 'MUTATE', 'AMPLIFIES', repeat('b', 64), repeat('c', 64),
          repeat('9', 64), repeat('9', 64), 1, 1, now(), now()
        );
      `),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ets_session WHERE account_id = 101",
  );
