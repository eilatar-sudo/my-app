...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE cm_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_cm_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cm_authorization_evidence (
      ...
      '{"scopes":["citation.materialize"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_cm_profile (
      ...
      authorization_snapshot_hash, max_placement_fan_out,
      max_targets_per_citation, semantic_tags, procedure_ref, revocation_policy,
      created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident severity materialization', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'cm-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 4, 2,
      ARRAY['incident', 'severity'], 'proc:materialize-severity',
      'STOP_BEFORE_ACCEPT', 'principal:test', now()
    );

    INSERT INTO agent_cm_profile_target_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      surface_key_pattern, allowed_target_kinds, require_sync,
      attenuation_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'severity', 1,
      'incident:*:severity', ARRAY['BOARD_ITEM', 'WORKING_SET'], true,
      '{"narrow_only":true}'
    );

    SELECT approve_agent_cm_profile(
      101,
    ...
    INSERT INTO agent_cm_citation_catalog (
      account_id, citation_id, consumption_session_id, surface_key, status,
      fact_hash, ledger_entry_id, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001', 'incident:9:severity',
      'SEALED', repeat('b', 64),
      '70000000-0000-0000-0000-000000000001', now(), now()
    );

    INSERT INTO agent_cm_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_accept_units,
      budget_place_units, budget_vector_units, budget_sync_units,
      budget_writeback_units, consumed_accept_units, consumed_place_units,
      consumed_vector_units, consumed_sync_units, consumed_writeback_units,
      budget_max_wall_time_ms, max_placement_fan_out, max_targets_per_citation,
      ...
      'Materialize severity', 'request-1', 100, 20, 5, 8, 2, 0, 0, 0, 0, 0,
    ...
    INSERT INTO agent_cm_acceptance_receipt (
      account_id, receipt_id, session_id, citation_id, surface_key,
      fact_hash, attenuation_hash, acceptance_hash, accepted_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'incident:9:severity',
      repeat('b', 64), repeat('c', 64), repeat('1', 64), now()
    );

    INSERT INTO agent_cm_materialization (
      account_id, materialization_id, session_id, citation_id, receipt_id,
      fact_hash, attenuation_hash, sealed_revision, sealed_at, created_at
    ) VALUES (
    ...
    INSERT INTO agent_cm_placement (
      account_id, placement_id, materialization_id, session_id, citation_id,
      target_kind, target_ref, status, fact_hash, attenuation_hash,
      sealed_revision, sealed_at, created_at
    ) VALUES (
      101, 'a0000000-0000-0000-0000-000000000001',
      '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001',
      'BOARD_ITEM', 'board:9/item:42/col:severity', 'SEALED',
      repeat('b', 64), repeat('c', 64), 1, now(), now()
    );

    UPDATE agent_cm_session
    SET status = 'ACTIVE',
        state_revision = 1,
        consumed_accept_units = 1,
        consumed_place_units = 1,
        updated_at = now()
    WHERE account_id = 101
      AND session_id = '20000000-0000-0000-0000-000000000001';

    INSERT INTO agent_cm_profile_embedding (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cm_profile_target_rule
        SET require_sync = false
        WHERE account_id = 101
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cm_placement
        SET fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND placement_id = 'a0000000-0000-0000-0000-000000000001';
      `),
    "mutation of placement identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_cm_writeback_intent (
          account_id, writeback_id, session_id, placement_id,
          ...
          '20000000-0000-0000-0000-000000000001',
          'a0000000-0000-0000-0000-000000000001', 'UNKNOWN_EFFECT',
          'provider-key-1', 1, repeat('4', 64), now(), now()
        );
      `),
    "insertion of a non-prepared writeback intent",
    "P0001",
  );
  ...
  await database.exec(`
    ...
    INSERT INTO agent_cm_writeback_intent (
      ...
      '20000000-0000-0000-0000-000000000001',
      'a0000000-0000-0000-0000-000000000001', 'PREPARED',
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_cm_writeback_intent
        SET canonical_request_hash = repeat('7', 64)
        WHERE account_id = 101
          AND writeback_id = '60000000-0000-0000-0000-000000000002';
      `),
    "mutation of a prepared writeback identity",
    "P0001",
  );
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_cm_session WHERE account_id = 101",
  );
