...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE pcer_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_pcer_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_pcer_authorization_evidence (
      ...
      '{"scopes":["provider.key.review"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_pcer_profile (
      ...
      authorization_snapshot_hash, review_window_ms, max_refresh_count,
      ...
      'Incident promotion-containment-expiry-review', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'pcer-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'promotion-containment-expiry-review'],
      'proc:promotion-containment-expiry-review-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_pcer_profile_review_rule (
      ...
      review_window_ms, require_dual_control_review, review_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'review-case', 1,
      ARRAY['SEALED_PROMOTION_CONTAINMENT_CERTIFICATE', 'SEALED_ACK_CERTIFICATE',
      ...
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"containment_expiry_never_confirms":true}'
    );

    SELECT approve_agent_pcer_profile(
      101,
    ...
    INSERT INTO agent_pcer_certificate_catalog (
      ...
      'SEALED_PROMOTION_CONTAINMENT_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_pcer_session (
      ...
      budget_invalidate_units, budget_review_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_review_units,
      ...
      'Review expired promotion-containment certificates without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_pcer_nomination_window (
    ...
    INSERT INTO agent_pcer_evaluation_receipt (
    ...
    INSERT INTO agent_pcer_review_certificate (
    ...
    INSERT INTO agent_pcer_review_binding (
      ...
      attested_key_kind, review_kind, purpose_relation,
    ...
    INSERT INTO agent_pcer_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcer_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_review_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pcer_profile_review_rule
        SET require_dual_control_review = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'review-case';
      `),
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pcer_review_binding
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcer_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pcer_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcer_review_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'REVIEW_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcer_review_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'REVIEW_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_pcer_session WHERE account_id = 101",
  );
