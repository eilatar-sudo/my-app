...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE pra_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_pra_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_pra_authorization_evidence (
      ...
      '{"scopes":["provider.receipt.attestation"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_pra_profile (
      ...
      authorization_snapshot_hash, evaluate_threshold,
      max_bindings_per_certificate, max_nominated_effects, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident provider-receipt attestation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'pra-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'provider-receipt-attestation'],
      'proc:provider-receipt-attestation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_pra_profile_attestation_rule (
      ...
      require_trusted_receipt, attestation_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'attest-case', 1,
      ARRAY['SEALED_SAGA_MUTATE_EFFECT', 'SEALED_SAGA_COMPENSATE_EFFECT'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"silence_never_success":true}'
    );

    SELECT approve_agent_pra_profile(
      101,
    ...
    INSERT INTO agent_pra_effect_catalog (
      account_id, source_effect_id, source_session_id, source_certificate_id,
      effect_ref, source_effect_kind, placement_kind, step_kind, status,
      disputed_fact_hash, attenuation_hash, donor_purpose_hash, hop_count,
      effect_sealed_at, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      '90000000-0000-0000-0000-000000000099',
      'effect:agent-c/purpose:incident-triage',
      'SEALED_SAGA_COMPENSATE_EFFECT', 'HALTED', 'COMPENSATE', 'SEALED',
      ...
      'effect:agent-d/purpose:incident-triage',
      'SEALED_SAGA_MUTATE_EFFECT', 'RESTORED_WITHOUT_WINNER', 'MUTATE',
      'SEALED', repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '1 day', now() - interval '1 day', now()
    );

    INSERT INTO agent_pra_session (
      ...
      budget_invalidate_units, budget_attest_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_attest_units, budget_max_wall_time_ms,
      evaluate_threshold, max_bindings_per_certificate, max_nominated_effects,
      deadline_at, started_by, principal_id,
      ...
      'Bind unknown saga compensation to a trusted receipt', 'request-1',
    ...
    INSERT INTO agent_pra_nomination_receipt (
      account_id, receipt_id, session_id, source_effect_id,
      source_effect_kind, placement_kind, step_kind, disputed_fact_hash,
      attenuation_hash, donor_purpose_hash, hop_count, nomination_hash,
      nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'SEALED_SAGA_COMPENSATE_EFFECT',
      'HALTED', 'COMPENSATE', repeat('b', 64), repeat('c', 64), repeat('9', 64),
      1, repeat('1', 64), now()
    );

    INSERT INTO agent_pra_evaluation_receipt (
      account_id, evaluation_id, session_id, effect_set_hash,
      receipt_set_hash, compensation_set_hash, attenuation_hash,
      evaluation_hash, evaluated_at
    ) VALUES (
      101, 'b0000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      repeat('7', 64), repeat('6', 64), repeat('5', 64), repeat('c', 64),
      repeat('3', 64), now()
    );

    INSERT INTO agent_pra_attestation_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, effect_set_hash, receipt_set_hash, compensation_set_hash,
      attenuation_hash, binding_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
      101, '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      'b0000000-0000-0000-0000-000000000001',
      'consumer:agent-d/purpose:incident-triage',
      repeat('2', 64), repeat('7', 64), repeat('6', 64),
      repeat('5', 64), repeat('c', 64), 1, 1, now(), now()
    );

    INSERT INTO agent_pra_attestation_binding (
      account_id, binding_id, certificate_id, session_id, source_effect_id,
      source_effect_kind, binding_ordinal, status, placement_kind, step_kind,
      receipt_kind, attestation_kind, purpose_relation, disputed_fact_hash,
      attenuation_hash, requested_purpose_hash, donor_purpose_hash, hop_count,
      provider_receipt_hash, sealed_revision, sealed_at, created_at
    ) VALUES (
      101, 'a0000000-0000-0000-0000-000000000099',
      '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001',
      'SEALED_SAGA_COMPENSATE_EFFECT', 0, 'SEALED', 'HALTED', 'COMPENSATE',
      'TRUSTED_FAILURE', 'ATTEST_FAILURE', 'NARROWS', repeat('b', 64),
      repeat('c', 64), repeat('c', 64), repeat('9', 64), 1, repeat('4', 64),
      1, now(), now()
    );

    UPDATE agent_pra_session
    SET status = 'SEALED',
        state_revision = 1,
    ...
    INSERT INTO agent_pra_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pra_session (
          ...
          budget_invalidate_units, budget_attest_units, consumed_nominate_units,
          consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
          consumed_invalidate_units, consumed_attest_units,
          budget_max_wall_time_ms, evaluate_threshold,
          max_bindings_per_certificate, max_nominated_effects, deadline_at,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pra_profile_attestation_rule
        SET require_trusted_receipt = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'attest-case';
      `),
    "mutation of a sealed attestation rule",
    "P0001",
  );
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pra_profile (
          ...
          'pra-compiler-1', repeat('a', 64),
          '30000000-0000-0000-0000-000000000001', repeat('d', 64),
          2, 16, 16, ARRAY['unsafe'], 'STOP_BEFORE_EVALUATE',
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pra_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pra_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pra_attestation_binding (
          ...
          source_effect_id, source_effect_kind, binding_ordinal, status,
          placement_kind, step_kind, receipt_kind, attestation_kind,
          purpose_relation, disputed_fact_hash, attenuation_hash,
          requested_purpose_hash, donor_purpose_hash, hop_count,
          provider_receipt_hash, sealed_revision, sealed_at, created_at
        ) VALUES (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000001',
          'SEALED_SAGA_COMPENSATE_EFFECT', 1, 'SEALED', 'HALTED', 'COMPENSATE',
          'SILENCE', 'ATTEST_SUCCESS', 'NARROWS', repeat('b', 64),
          repeat('c', 64), repeat('c', 64), repeat('9', 64), 1, repeat('0', 64),
          1, now(), now()
        );
      `),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pra_attestation_binding (
          ...
          'SEALED_SAGA_MUTATE_EFFECT', 2, 'SEALED', 'RESTORED_WITHOUT_WINNER',
          'MUTATE', 'TRUSTED_SUCCESS', 'ATTEST_SUCCESS', 'AMPLIFIES',
          repeat('b', 64), repeat('c', 64), repeat('9', 64), repeat('9', 64),
          1, repeat('4', 64), 1, now(), now()
        );
      `),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_pra_session WHERE account_id = 101",
  );
