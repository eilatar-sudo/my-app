...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE ef_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_ef_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_ef_authorization_evidence (
      ...
      '{"scopes":["executive.freeze"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_ef_profile (
      ...
      authorization_snapshot_hash, freeze_threshold, release_threshold,
      max_holds_per_certificate, semantic_tags, procedure_ref, revocation_policy,
      created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident executive freeze', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'ef-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 2, 32,
      ARRAY['incident', 'executive-freeze'], 'proc:executive-freeze-incident',
      'STOP_BEFORE_FREEZE', 'principal:test', now()
    );

    INSERT INTO agent_ef_profile_freeze_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_case_kinds, freeze_threshold, release_threshold, require_notify,
      freeze_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'freeze-case', 1,
      ARRAY['FREEZE_CASE_FACT', 'AUTHORITY_DISAGREE_OUTCOME'], 2, 2, true,
      '{"narrow_only":true,"no_winner":true}'
    );

    SELECT approve_agent_ef_profile(
      101,
    ...
    INSERT INTO agent_ef_freeze_catalog (
      account_id, case_id, source_freeze_id, escalation_session_id, case_ref,
      case_kind, status, disputed_fact_hash, attenuation_hash,
      ...
      'FREEZE_CASE_FACT', 'SEALED', repeat('b', 64), repeat('c', 64),
      ARRAY['principal:first-tier-a', 'principal:authority-a'], now(), now()
    );

    INSERT INTO agent_ef_session (
      account_id, session_id, profile_id, profile_version, status,
      state_revision, purpose, idempotency_key, budget_nominate_units,
      budget_freeze_units, budget_release_units, budget_vector_units,
      budget_seal_units, budget_notify_units, consumed_nominate_units,
      consumed_freeze_units, consumed_release_units, consumed_vector_units,
      consumed_seal_units, consumed_notify_units, budget_max_wall_time_ms,
      freeze_threshold, release_threshold, max_holds_per_certificate,
      ...
      'Freeze disputed incident facts under executive dual control', 'request-1',
      100, 40, 40, 5, 8, 2, 0, 0, 0, 0, 0, 0, 60000, 2, 2, 32,
    ...
    INSERT INTO agent_ef_nomination_receipt (
      ...
      '50000000-0000-0000-0000-000000000001', 'FREEZE_CASE_FACT',
      repeat('b', 64), repeat('c', 64), repeat('1', 64), now()
    );

    INSERT INTO agent_ef_freeze_cast (
      account_id, step_id, session_id, prior_case_id, case_id,
      executive_principal_id, freeze_ordinal, halt_scope_hash, attenuation_hash,
      freeze_hash, cast_at
    ) VALUES (
      101, 'b0000000-0000-0000-0000-000000000099',
      '20000000-0000-0000-0000-000000000001',
      NULL, '50000000-0000-0000-0000-000000000001',
      'principal:exec-a', 1, repeat('6', 64), repeat('c', 64),
      repeat('7', 64), now()
    );

    INSERT INTO agent_ef_certificate (
      account_id, certificate_id, session_id, consumer_ref, purpose_hash,
      certificate_kind, executive_set_hash, disputed_fact_set_hash,
      ...
      repeat('2', 64), 'FREEZE', repeat('3', 64), repeat('b', 64),
      repeat('6', 64), 1, 1, now(), now()
    );

    INSERT INTO agent_ef_certificate_hold (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ef_session (
          ...
          0, 0, 0, 0, 0, 0, 60000, 2, 2, 32, now() + interval '1 minute',
          'principal:test', 'principal:test',
          '30000000-0000-0000-0000-000000000001',
          repeat('e', 64), 7, repeat('f', 64), now(), now()
        );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ef_profile_freeze_rule
        SET require_notify = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'freeze-case';
      `),
    "mutation of a sealed freeze rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ef_certificate_hold
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ef_refresh_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_ef_refresh_intent
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ef_freeze_cast (
          account_id, step_id, session_id, prior_case_id, case_id,
          executive_principal_id, freeze_ordinal, halt_scope_hash,
          attenuation_hash, freeze_hash, cast_at
        ) VALUES (
          101, 'b0000000-0000-0000-0000-000000000001',
          '20000000-0000-0000-0000-000000000001',
          NULL, '50000000-0000-0000-0000-000000000001',
          'principal:first-tier-a', 2, repeat('6', 64), repeat('c', 64),
          repeat('8', 64), now()
        );
      `),
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_ef_release_cast (
          account_id, step_id, session_id, case_id,
          executive_principal_id, release_ordinal, halt_scope_hash,
          attenuation_hash, release_hash, cast_at
        ) VALUES (
      ...
      `),
    "anti-self-release fence",
    "P0001",
  );

  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_ef_session WHERE account_id = 101",
  );
