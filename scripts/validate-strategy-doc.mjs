...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE epg_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_epg_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_epg_authorization_evidence (
      ...
      '{"scopes":["envelope.purpose.gate"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_epg_profile (
      ...
      max_decisions_per_certificate, max_nominated_slots, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident envelope purpose gate', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'epg-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'purpose-gate'], 'proc:purpose-gate-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_epg_profile_gate_rule (
      account_id, profile_id, profile_version, rule_id, ordinal,
      allowed_source_kinds, evaluate_threshold, max_decisions_per_certificate,
      require_notify, gate_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'gate-case', 1,
      ARRAY['SEALED_WORKING_SET_SLOT', 'SEALED_CITATION_GRANT'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"halt_read":true}'
    );

    SELECT approve_agent_epg_profile(
      101,
    ...
    INSERT INTO agent_epg_slot_catalog (
      account_id, source_slot_id, source_session_id, slot_ref,
      source_slot_kind, placement_kind, status, disputed_fact_hash,
      attenuation_hash, donor_purpose_hash, hop_count, slot_sealed_at,
      sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      'slot:agent-c/purpose:incident-triage',
      'SEALED_WORKING_SET_SLOT', 'HALTED', 'SEALED', repeat('b', 64),
      repeat('c', 64), repeat('9', 64), 1,
      now() - interval '2 days', now() - interval '2 days', now()
    ), (
      101, '50000000-0000-0000-0000-000000000002',
      '80000000-0000-0000-0000-000000000002',
      'slot:agent-d/purpose:incident-triage',
      'SEALED_WORKING_SET_SLOT', 'RESTORED_WITHOUT_WINNER', 'SEALED',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '1 day', now() - interval '1 day', now()
    );

    INSERT INTO agent_epg_session (
      ...
      evaluate_threshold, max_decisions_per_certificate, max_nominated_slots,
      ...
      'Gate hop-attenuated incident reads and tools', 'request-1',
    ...
    INSERT INTO agent_epg_nomination_receipt (
      account_id, receipt_id, session_id, source_slot_id,
      source_slot_kind, placement_kind, disputed_fact_hash, attenuation_hash,
      donor_purpose_hash, hop_count, nomination_hash, nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000001', 'SEALED_WORKING_SET_SLOT',
      'HALTED', repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('1', 64), now()
    );

    INSERT INTO agent_epg_evaluation_receipt (
      account_id, evaluation_id, session_id, slot_set_hash,
      gate_set_hash, attenuation_hash, evaluation_hash, evaluated_at
    ) VALUES (
      101, 'b0000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      repeat('7', 64), repeat('6', 64), repeat('c', 64), repeat('3', 64), now()
    );

    INSERT INTO agent_epg_gate_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, slot_set_hash, gate_set_hash, attenuation_hash,
      decision_watermark, sealed_revision, sealed_at, created_at
    ) VALUES (
    ...
    INSERT INTO agent_epg_gate_decision (
      account_id, decision_id, certificate_id, session_id, source_slot_id,
      source_slot_kind, decision_ordinal, status, placement_kind, gate_kind,
      ...
      'SEALED_WORKING_SET_SLOT', 0, 'SEALED', 'HALTED', 'READ_REDACTED',
      'NARROWS', repeat('b', 64), repeat('c', 64), repeat('c', 64),
      repeat('9', 64), 1, 1, now(), now()
    );

    UPDATE agent_epg_session
    SET status = 'SEALED',
        state_revision = 1,
    ...
    INSERT INTO agent_epg_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_epg_session (
          ...
          max_decisions_per_certificate, max_nominated_slots, deadline_at,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_epg_profile_gate_rule
        SET require_notify = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'gate-case';
      `),
    "mutation of a sealed gate rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_epg_gate_decision
        SET disputed_fact_hash = repeat('7', 64)
        WHERE account_id = 101
          AND decision_id = 'a0000000-0000-0000-0000-000000000099';
      `),
    "mutation of gate decision identity",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_epg_refresh_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_epg_refresh_intent
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_epg_gate_decision (
          account_id, decision_id, certificate_id, session_id,
          source_slot_id, source_slot_kind, decision_ordinal, status,
          placement_kind, gate_kind, purpose_relation, disputed_fact_hash,
          attenuation_hash, requested_purpose_hash, donor_purpose_hash,
          hop_count, sealed_revision, sealed_at, created_at
        ) VALUES (
          101, 'c0000000-0000-0000-0000-000000000001',
          '90000000-0000-0000-0000-000000000001',
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000001',
          'SEALED_WORKING_SET_SLOT', 1, 'SEALED', 'HALTED', 'READ_VISIBLE',
          'NARROWS', repeat('b', 64), repeat('c', 64), repeat('c', 64),
          repeat('9', 64), 1, 1, now(), now()
        );
      `),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_epg_gate_decision (
          ...
          'SEALED_WORKING_SET_SLOT', 2, 'SEALED', 'RESTORED_WITHOUT_WINNER',
          'READ_VISIBLE', 'AMPLIFIES', repeat('b', 64), repeat('c', 64),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_epg_session WHERE account_id = 101",
  );
