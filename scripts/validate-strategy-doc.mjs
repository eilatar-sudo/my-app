...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE pcrer_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_pcrer_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_pcrer_authorization_evidence (
      ...
      '{"scopes":["provider.key.resolution"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_pcrer_profile (
      ...
      authorization_snapshot_hash, resolution_window_ms, max_refresh_count,
      ...
      'Incident promotion-containment-review-expiry-resolution', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'pcrer-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'promotion-containment-review-expiry-resolution'],
      'proc:promotion-containment-review-expiry-resolution-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_pcrer_profile_resolution_rule (
      ...
      resolution_window_ms, require_dual_control_resolution, resolution_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'resolution-case', 1,
      ARRAY['SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE',
      ...
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"review_expiry_never_confirms":true}'
    );

    SELECT approve_agent_pcrer_profile(
      101,
    ...
    INSERT INTO agent_pcrer_certificate_catalog (
      ...
      'SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_pcrer_session (
      ...
      budget_invalidate_units, budget_resolution_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_resolution_units,
      ...
      'Resolve expired promotion-containment-review certificates without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_pcrer_nomination_window (
    ...
    INSERT INTO agent_pcrer_evaluation_receipt (
    ...
    INSERT INTO agent_pcrer_resolution_certificate (
    ...
    INSERT INTO agent_pcrer_resolution_binding (
      ...
      attested_key_kind, resolution_kind, purpose_relation,
    ...
    INSERT INTO agent_pcrer_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcrer_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_resolution_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pcrer_profile_resolution_rule
        SET require_dual_control_resolution = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'resolution-case';
      `),
    "mutation of a sealed review rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcrer_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pcrer_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcrer_resolution_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'RESOLUTION_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pcrer_resolution_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'RESOLUTION_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_pcrer_session WHERE account_id = 101",
  );
