...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE prec_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_prec_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_prec_authorization_evidence (
      ...
      '{"scopes":["provider.key.containment"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_prec_profile (
      ...
      authorization_snapshot_hash, containment_window_ms, max_refresh_count,
      ...
      'Incident promotion-resolution-expiry-containment', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'prec-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'promotion-resolution-expiry-containment'],
      'proc:promotion-resolution-expiry-containment-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_prec_profile_containment_rule (
      ...
      containment_window_ms, require_dual_control_containment, containment_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'containment-case', 1,
      ARRAY['SEALED_PROMOTION_CONTAINMENT_REVIEW_RESOLUTION_CERTIFICATE',
        'SEALED_PROMOTION_CONTAINMENT_CERTIFICATE', 'SEALED_ACK_CERTIFICATE',
      ...
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"resolution_expiry_never_confirms":true}'
    );

    SELECT approve_agent_prec_profile(
      101,
    ...
    INSERT INTO agent_prec_certificate_catalog (
      ...
      'SEALED_PROMOTION_CONTAINMENT_REVIEW_RESOLUTION_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_prec_session (
      ...
      budget_invalidate_units, budget_containment_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_containment_units,
      ...
      'Contain expired promotion-resolution certificates without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_prec_nomination_window (
    ...
    INSERT INTO agent_prec_evaluation_receipt (
    ...
    INSERT INTO agent_prec_containment_certificate (
    ...
    INSERT INTO agent_prec_containment_binding (
      ...
      attested_key_kind, containment_kind, purpose_relation,
    ...
    INSERT INTO agent_prec_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_prec_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_containment_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_prec_profile_containment_rule
        SET require_dual_control_containment = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'containment-case';
      `),
    "mutation of a sealed containment rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_prec_effect_intent (
          ...
          'provider-ack-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_prec_effect_intent (
      ...
      'provider-ack-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_prec_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_prec_containment_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'TRUSTED_ENROLLED', 'CONTAINMENT_ACTIVE', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_prec_containment_binding (
          ...
          'RESTORED_WITHOUT_WINNER', 'FIRST_ENROLLMENT', 'ATTESTED',
          'AWAITING_CONFIRMATION', 'CONTAINMENT_ACTIVE', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_prec_session WHERE account_id = 101",
  );
