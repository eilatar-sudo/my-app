...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE pec_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_pec_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_pec_authorization_evidence (
      ...
      '{"scopes":["provider.key.contain"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_pec_profile (
      ...
      'Incident promotion-expiry-containment', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'pec-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 3600000, 4, 2, 32, 32,
      ARRAY['incident', 'promotion-expiry-containment'],
      'proc:promotion-expiry-containment-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_pec_profile_containment_rule (
      ...
      101, '10000000-0000-0000-0000-000000000001', 1, 'promotion-case', 1,
      ARRAY['SEALED_PROMOTION_CERTIFICATE', 'SEALED_ACK_CERTIFICATE',
      ...
      2, 32, 3600000, true,
      '{"narrow_only":true,"no_winner":true,"promotion_expiry_never_confirms":true}'
    );

    SELECT approve_agent_pec_profile(
      101,
    ...
    INSERT INTO agent_pec_certificate_catalog (
      ...
      'SEALED_PROMOTION_CERTIFICATE', 'HALTED', 'REGRESSED', 'UNATTESTED',
      'SEALED',
    ...
    INSERT INTO agent_pec_session (
      ...
      budget_invalidate_units, budget_contain_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_contain_units,
      ...
      'Contain expired promotion certificates without inventing trust',
      'request-1',
    ...
    INSERT INTO agent_pec_nomination_window (
    ...
    INSERT INTO agent_pec_evaluation_receipt (
    ...
    INSERT INTO agent_pec_containment_certificate (
    ...
    INSERT INTO agent_pec_containment_binding (
    ...
    INSERT INTO agent_pec_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pec_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_contain_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pec_profile_containment_rule
        SET require_dual_control_containment = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'promotion-case';
      `),
    "mutation of a sealed promotion rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pec_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pec_effect_intent
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_pec_session WHERE account_id = 101",
  );
