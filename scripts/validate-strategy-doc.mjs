...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE scr_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_scr_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_scr_authorization_evidence (
      ...
      '{"scopes":["provider.clock.reattest"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_scr_profile (
      ...
      'Incident successor-clock re-attestation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'scr-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'successor-clock-reattestation'],
      'proc:successor-clock-reattestation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_scr_profile_reattest_rule (
      ...
      require_trusted_successor, reattest_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'reattest-case', 1,
      ARRAY['SEALED_QUARANTINE_RECEIPT', 'SEALED_CLOCK_CERTIFICATE',
        'SUCCESSOR_CLOCK_SAMPLE', 'RETIRED_KEY_CLAIM'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"history_never_rewritten":true}'
    );

    SELECT approve_agent_scr_profile(
      101,
    ...
    INSERT INTO agent_scr_receipt_catalog (
      account_id, source_receipt_id, source_session_id, source_certificate_id,
      receipt_ref, source_receipt_kind, placement_kind, successor_lifecycle,
      receipt_lifecycle, status, disputed_fact_hash, attenuation_hash,
      donor_purpose_hash, hop_count, retired_signed_at,
      ...
      'SEALED_QUARANTINE_RECEIPT', 'HALTED', 'REGRESSED', 'POST_WATERMARK',
      'SEALED',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '2 days', now() - interval '1 day',
      now() - interval '3 days', 5000,
      ...
      'SUCCESSOR_CLOCK_SAMPLE', 'RESTORED_WITHOUT_WINNER', 'MONOTONIC',
      'PRE_WATERMARK', 'SEALED', repeat('b', 64), repeat('c', 64),
      repeat('9', 64), 1,
    ...
    INSERT INTO agent_scr_session (
      ...
      budget_invalidate_units, budget_reattest_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_reattest_units,
      ...
      'Rebind receipts without rewriting history',
      'request-1',
    ...
    INSERT INTO agent_scr_nomination_receipt (
      account_id, receipt_id, session_id, source_receipt_id,
      source_receipt_kind, placement_kind, successor_lifecycle,
      receipt_lifecycle, disputed_fact_hash, attenuation_hash,
      donor_purpose_hash, hop_count, nomination_hash, nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000002', 'SUCCESSOR_CLOCK_SAMPLE',
      'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'PRE_WATERMARK',
      repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('1', 64), now()
    );

    INSERT INTO agent_scr_evaluation_receipt (
      account_id, evaluation_id, session_id, receipt_set_hash,
      successor_set_hash, watermark_set_hash, attenuation_hash,
    ...
    INSERT INTO agent_scr_reattest_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, receipt_set_hash, successor_set_hash, watermark_set_hash,
    ...
    INSERT INTO agent_scr_reattest_binding (
      account_id, binding_id, certificate_id, session_id,
      source_receipt_id, source_receipt_kind, binding_ordinal, status,
      placement_kind, successor_lifecycle, receipt_lifecycle,
      attested_successor_kind, reattest_kind, purpose_relation,
      disputed_fact_hash, attenuation_hash, requested_purpose_hash,
      donor_purpose_hash, hop_count, successor_clock_hash,
      retirement_watermark_hash, sealed_revision, sealed_at, created_at
    ) VALUES (
      101, 'a0000000-0000-0000-0000-000000000099',
      '90000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000002',
      'SUCCESSOR_CLOCK_SAMPLE', 1, 'SEALED',
      'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'PRE_WATERMARK',
      'TRUSTED_MONOTONIC', 'REBIND_TO_SUCCESSOR', 'NARROWS',
      repeat('b', 64), repeat('c', 64), repeat('c', 64), repeat('9', 64), 1,
      repeat('6', 64), repeat('7', 64), 1, now(), now()
    );

    UPDATE agent_scr_session
    SET status = 'SEALED',
        state_revision = 3,
    ...
    INSERT INTO agent_scr_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_scr_session (
          ...
          state_revision, purpose, idempotency_key, budget_nominate_units,
          budget_evaluate_units, budget_seal_units, budget_vector_units,
          budget_invalidate_units, budget_reattest_units,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_scr_profile_reattest_rule
        SET require_trusted_successor = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'reattest-case';
      `),
    "mutation of a sealed reattest rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_scr_effect_intent (
          ...
          'provider-reattest-1', 1, repeat('4', 64), now(), now()
        );
      `),
  ...
  await database.exec(`
    ...
    INSERT INTO agent_scr_effect_intent (
      ...
      'provider-reattest-2', 0, repeat('5', 64), now(), now()
    );
  `);
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_scr_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_scr_reattest_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SUCCESSOR_CLOCK_SAMPLE', 2, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'PRE_WATERMARK',
          'SILENCE', 'REBIND_TO_SUCCESSOR', 'NARROWS',
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_scr_reattest_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SUCCESSOR_CLOCK_SAMPLE', 3, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'MONOTONIC', 'PRE_WATERMARK',
          'TRUSTED_MONOTONIC', 'REBIND_TO_SUCCESSOR', 'AMPLIFIES',
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_scr_session WHERE account_id = 101",
  );
