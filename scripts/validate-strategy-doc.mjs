...
const ddlBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TYPE pkr_session_status"),
);
const vectorBlock = sqlBlocks.find((block) =>
  block.includes("CREATE TABLE agent_pkr_profile_embedding"),
);
...
try {
  ...
  await database.exec(`
    ...
    INSERT INTO agent_pkr_authorization_evidence (
      ...
      '{"scopes":["provider.key.rotation"]}', 'kms://evidence/fixture',
      repeat('8', 64), 'worm://evidence/fixture', now()
    );

    INSERT INTO agent_pkr_profile (
      ...
      max_bindings_per_certificate, max_nominated_keys, semantic_tags,
      procedure_ref, revocation_policy, created_by, created_at
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1,
      'Incident provider-key rotation', 'DRAFT',
      repeat('a', 64), 'jcs-v1', 'pkr-compiler-1',
      NULL, '30000000-0000-0000-0000-000000000001',
      repeat('d', 64), 2, 32, 32,
      ARRAY['incident', 'provider-key-rotation'],
      'proc:provider-key-rotation-incident',
      'STOP_BEFORE_EVALUATE', 'principal:test', now()
    );

    INSERT INTO agent_pkr_profile_rotation_rule (
      ...
      require_trusted_receipt, rotation_instruction
    ) VALUES (
      101, '10000000-0000-0000-0000-000000000001', 1, 'rotate-case', 1,
      ARRAY['SEALED_ATTESTATION_SIGNING_KEY', 'COMPROMISED_KEY'],
      2, 32, true,
      '{"narrow_only":true,"no_winner":true,"history_never_rewritten":true}'
    );

    SELECT approve_agent_pkr_profile(
      101,
    ...
    INSERT INTO agent_pkr_key_catalog (
      account_id, source_key_id, source_session_id, source_certificate_id,
      key_ref, source_key_kind, placement_kind, key_lifecycle, status,
      disputed_fact_hash, attenuation_hash, donor_purpose_hash, hop_count,
      key_sealed_at, sealed_at, created_at
    ) VALUES (
      101, '50000000-0000-0000-0000-000000000001',
      '80000000-0000-0000-0000-000000000001',
      '90000000-0000-0000-0000-000000000099',
      'key:provider-a/purpose:incident-triage',
      'SEALED_ATTESTATION_SIGNING_KEY', 'HALTED', 'ACTIVE', 'SEALED',
      ...
      'key:provider-b/purpose:incident-triage',
      'SEALED_ATTESTATION_SIGNING_KEY', 'RESTORED_WITHOUT_WINNER', 'ACTIVE',
      'SEALED', repeat('b', 64), repeat('c', 64), repeat('9', 64), 1,
      now() - interval '1 day', now() - interval '1 day', now()
    );

    INSERT INTO agent_pkr_session (
      ...
      budget_invalidate_units, budget_rotate_units, consumed_nominate_units,
      consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
      consumed_invalidate_units, consumed_rotate_units, budget_max_wall_time_ms,
      evaluate_threshold, max_bindings_per_certificate, max_nominated_keys,
      ...
      'Retire a signing key without rewriting attested history', 'request-1',
    ...
    INSERT INTO agent_pkr_nomination_receipt (
      account_id, receipt_id, session_id, source_key_id,
      source_key_kind, placement_kind, key_lifecycle, disputed_fact_hash,
      attenuation_hash, donor_purpose_hash, hop_count, nomination_hash,
      nominated_at
    ) VALUES (
      101, '40000000-0000-0000-0000-000000000001',
      '20000000-0000-0000-0000-000000000001',
      '50000000-0000-0000-0000-000000000002', 'SEALED_ATTESTATION_SIGNING_KEY',
      'RESTORED_WITHOUT_WINNER', 'ACTIVE', repeat('b', 64), repeat('c', 64),
      repeat('9', 64), 1, repeat('1', 64), now()
    );

    INSERT INTO agent_pkr_evaluation_receipt (
      account_id, evaluation_id, session_id, key_set_hash,
      successor_set_hash, retirement_set_hash, attenuation_hash,
    ...
    INSERT INTO agent_pkr_rotation_certificate (
      account_id, certificate_id, session_id, evaluation_id, consumer_ref,
      purpose_hash, key_set_hash, successor_set_hash, retirement_set_hash,
    ...
    INSERT INTO agent_pkr_rotation_binding (
      account_id, binding_id, certificate_id, session_id,
      source_key_id, source_key_kind, binding_ordinal, status,
      placement_kind, key_lifecycle, receipt_kind, rotation_kind,
      purpose_relation, disputed_fact_hash, attenuation_hash,
      requested_purpose_hash, donor_purpose_hash, hop_count,
      provider_key_hash, successor_key_hash, sealed_revision, sealed_at,
      created_at
    ) VALUES (
      ...
      'SEALED_ATTESTATION_SIGNING_KEY', 1, 'SEALED', 'RESTORED_WITHOUT_WINNER',
      'ACTIVE', 'TRUSTED_RETIRE', 'RETIRE', 'NARROWS', repeat('b', 64),
      repeat('c', 64), repeat('c', 64), repeat('9', 64), 1, repeat('6', 64),
      repeat('7', 64), 1, now(), now()
    );

    UPDATE agent_pkr_session
    SET status = 'SEALED',
        state_revision = 1,
    ...
    INSERT INTO agent_pkr_profile_embedding (
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pkr_session (
          ...
          budget_invalidate_units, budget_rotate_units, consumed_nominate_units,
          consumed_evaluate_units, consumed_seal_units, consumed_vector_units,
          consumed_invalidate_units, consumed_rotate_units,
          budget_max_wall_time_ms, evaluate_threshold,
          max_bindings_per_certificate, max_nominated_keys, deadline_at,
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pkr_profile_rotation_rule
        SET require_trusted_receipt = false
        WHERE account_id = 101
          AND profile_id = '10000000-0000-0000-0000-000000000001'
          AND profile_version = 1
          AND rule_id = 'rotate-case';
      `),
    "mutation of a sealed rotation rule",
    "P0001",
  );
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pkr_effect_intent (
  ...
  await expectDatabaseRejection(
    () =>
      database.exec(`
        ...
        UPDATE agent_pkr_effect_intent
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pkr_rotation_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SEALED_ATTESTATION_SIGNING_KEY', 2, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'ACTIVE', 'SILENCE', 'ROTATE_SUCCESS',
          'NARROWS', repeat('b', 64), repeat('c', 64), repeat('c', 64),
          repeat('9', 64), 1, repeat('0', 64), repeat('7', 64),
          1, now(), now()
        );
      `),
    ...
    () =>
      database.exec(`
        ...
        INSERT INTO agent_pkr_rotation_binding (
          ...
          '20000000-0000-0000-0000-000000000001',
          '50000000-0000-0000-0000-000000000002',
          'SEALED_ATTESTATION_SIGNING_KEY', 3, 'SEALED',
          'RESTORED_WITHOUT_WINNER', 'ACTIVE', 'TRUSTED_RETIRE', 'RETIRE',
          'AMPLIFIES', repeat('b', 64), repeat('c', 64), repeat('9', 64),
  ...
  const tenantCount = await database.query(
    "SELECT count(*)::int AS count FROM agent_pkr_session WHERE account_id = 101",
  );
