...
- [mondayDB Agentic Database Vision](docs/agentic-database-vision.md) defines
  ...
  schemas, GraphQL Open API surfaces, pgvector/HNSW compatibility,
  multi-tenant guardrails, performance checks, and agent perception metadata.

## Enterprise invariants

- Every query, memory record, budget entry, and audit event is scoped by
  `account_id`.
...
- Agent-originated work must declare budgets for rows, vector candidates,
  recursion depth, tool preparations, and timeout.
