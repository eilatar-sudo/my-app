# mondayDB Agentic Unanswered-Containment Review Plane
...
A sealed unanswered-escalation containment certificate can
freeze a first-enrollment key after dual-control
`CONTAIN_UNANSWERED_ESCALATION`. It does not decide **how an
unanswered, expired, or single-control containment pages a
steward for review** — without scanning every containment
binding, rewriting unanswered-containment silence into
`TRUSTED_ENROLLED`, or treating freeze silence as confirmation.

Without an unanswered-containment review plane, operators
and agents either:

- scan every pending containment, escalation, acknowledgement,
  page, and vote ledger looking for "who still has not
  dual-control reviewed the unanswered freeze" (neighbor-
  harmful on boards with 1M+ rows), or
- treat unanswered containment silence as retroactive engine
  truth, so freeze silence is rewritten as confirmation,
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a
  silent steward review auto-promotes `FIRST_ENROLLMENT` to
  `TRUSTED_ENROLLED`, a halt-scoped body is "unlocked" by
  later review, and hop-attenuated purpose is amplified
  back to the donor.

The product trade-off is **review fluency versus
review isolation**:

- Treating every unanswered containment, freeze timeout, or
  review timer as implicit confirmation maximizes agent
  fluency and reduces re-planning cost, but creates history-
  rewrite invention, review-as-confirmation, unanswered-
  containment-as-trust, silence-success, first-enrollment
  auto-trust, unauditable review storms, and recursive
  containment-ledger walks against neighbors.
- Binding a sealed review certificate under an approved
  review profile, containment-catalog point lookups,
  history-rewrite fences, silence-success fences, first-
  enrollment auto-trust fences, review-as-confirmation
  fences, unanswered-containment-as-trust fences, halt-review
  fences, purpose-amplification fences, successor-leak
  fences, prior-containment-authority-exclusion fences, ledger-scan
  fences, and steward budgets adds one bounded evaluate
  transaction and short-lived review storage.
- Semantic similarity may discover review profiles, but
  it must never decide whether a sealed containment,
  escalation, acknowledgement, SLO, window, TTL, or confirm
  certificate may be nominated, a review snapshot refreshed,
  a certificate sealed, or a first-enrollment key trusted.

The recommended model keeps the data plane deterministic:

1. An approved review profile defines allowed source
   kinds, review-window duration, dual-control
   review-steward policy, prior-containment-authority exclusion,
   and notify policy. Evaluation **never** invents a winning
   fact hash and **never** rewrites an unanswered containment
   into `TRUSTED_ENROLLED`.
2. A review session opens under purpose, budget, and
   authorization fences, and only nominates sealed
   containment certificates, sealed escalation certificates,
   sealed acknowledgement certificates, sealed SLO
   certificates, sealed window certificates, sealed TTL
   certificates, sealed confirm certificates, or first-
   enrollment claims by point lookup from the Unanswered-
   Escalation Containment, Unanswered-Ack Escalation, SLO
   Ack Suppression, Remaining-Window SLO, Confirmation-
   Window Observability, Confirmation-TTL Retirement, and
   First-Enrollment Confirmation planes.
3. mondayDB evaluates a review whose kind is a pure
   ...
   now_attested, ttl_expired, containment_count,
   distinct_containment_authority_count, prior_containment_authority_set_hash,
   review_steward_count, review_window_ms,
   review_kind)`. Silence cannot become historical
   success. An unanswered containment seals
   `REJECT_REVIEW_AS_CONFIRMATION` or
   `REVIEW_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `REVIEW_UNANSWERED_CONTAINMENT` pages a
   steward without inventing confirmation.
   `HOLD_REVIEW_WINDOW` holds further review for a
   bounded window without restoring a winner. Halted slots
   cannot become review-restored winners. Original
   containment authorities are excluded from the review-
   steward pair.
...
6. Unscoped key-catalog, clock-catalog, confirm-ledger, vote-
   ...
   ledger, containment-ledger, working-set, grant-graph, or
   board scans are
   **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor
protection: recursive "re-review every unanswered
containment forever" or "treat unanswered containment silence
...
For any unanswered-containment review evaluation,
mondayDB can answer:

- Which profile, principal, and session authorized the
  nomination, review refresh, evaluate, seal,
...
- Which nominated review certificates, placement kinds,
  ...
  prior-containment-authority exclusions, and review kinds were
  bound?
- Is the review certificate still current, invalidated,
  reviewed, hold-windowed, review-expired, or awaiting
...
The Unanswered-Containment Review Plane owns:

1. Immutable approved review profiles as procedural
   memory of "how unanswered containments page a steward without amplifying purpose, leaking halted
   facts, rewriting attested history, scanning vote, page,
   ack, escalation, or containment ledgers, or inventing
   `TRUSTED_ENROLLED` from unanswered-containment silence."
2. Tenant-scoped review sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed containment
   certificates, sealed escalation certificates, sealed
   acknowledgement certificates, sealed SLO certificates,
   sealed window certificates, sealed TTL certificates,
   confirm certificates, and first-enrollment claims by
   point lookup — never key-catalog, vote-ledger,
...
- **Unanswered-Escalation Containment:** supplies sealed
  ...
  single-control freezes this plane may review. This plane
...
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may page a steward only through a
  sealed review certificate.
...
- **Executive Freeze / Thaw SLA:** halt/restore context that
  still forbids review-as-restore against a halted body.
...
- Letting an LLM decide that an unanswered containment "feels
...
- Inventing a winning fact hash when a containment goes
  unanswered after a restored slot.
- Promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because
  a containment was unanswered, reviewed, held, or expired.
- Scanning vote, TTL, page, ack, escalation, or containment ledgers to
  decide who may review.
- Letting the original dual-control containment authorities also serve as
  the review-steward pair.
...
- allowed source kinds (`SEALED_CONTAINMENT_CERTIFICATE`,
...
- review-window buckets (`review_window_ms`), max
  ...
  steward principals for `REVIEW_UNANSWERED_CONTAINMENT`,
...
source-window point lookup against the containment catalog.
Refresh unanswered-containment review is a point lookup
...
authority exclusion, and purpose relation. It never walks
catalogs.
Seal binds window-set, clock-set, remaining-set, and
attenuation hashes. The certificate must not store
`resolved_fact_hash`. An unanswered containment cannot
...
6. Window nomination uses point lookup by
   ...
   escalation-ledger, containment-ledger, or board scans.
...
10. Silence and unknown windows cannot evaluate to
    `REVIEW_UNANSWERED_CONTAINMENT` or any kind that implies
    `TRUSTED_ENROLLED` (history-rewrite and silence-success
    fences).
11. Halted, extended-halt, and omitted windows cannot
    evaluate to a review kind that restores a winner
    (halt-review fence).
...
14. An unanswered containment never writes
    `attested_key_kind = TRUSTED_ENROLLED`
    (review-as-confirmation and unanswered-containment-
    as-trust fences).
15. `REVIEW_UNANSWERED_CONTAINMENT` requires two distinct
    review-steward principals who are not the
    nominating principal and are not members of the prior-
    containment-authority set.
16. Review certificates bind window set, clock set,
...
17. Plans that require unscoped board, session, working-set,
    ...
    vote-ledger, TTL-ledger, page-ledger, ack-ledger,
    escalation-ledger, containment-ledger, or citation-ledger scans are
    **FULL SCAN REJECTED**.
...
Nominate performs a point lookup on the containment catalog
and writes an immutable nomination receipt with
`remaining_ttl_ms` copied from the sealed source
certificate. Refresh unanswered-containment review is a
point lookup against that same source — never a vote-ledger,
page-ledger, ack-ledger, escalation-ledger, or containment-ledger walk. Evaluate
...
type TrustedNextAction =
  | "NOMINATE_SEALED_CONTAINMENT_CERTIFICATE"
  | "REFRESH_UNANSWERED_CONTAINMENT_REVIEW"
  | "EVALUATE_UNANSWERED_CONTAINMENT_REVIEW"
  | "SEAL_REVIEW_CERTIFICATE"
  | "INVALIDATE_UNANSWERED_CONTAINMENT_REVIEW"
  | "PREPARE_REVIEW_EFFECT"
  | "RESOLVE_REVIEW_UNCERTAINTY"
  | "CLOSE_SESSION";

type UnansweredContainmentReviewBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
...
type ReviewKind =
  ...
  | "REVIEW_ACTIVE"
  | "REVIEW_EXPIRED"
  | "REVIEW_UNANSWERED_CONTAINMENT"
  | "HOLD_REVIEW_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_REVIEW_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface UnansweredContainmentReviewBudget {
...
interface UnansweredContainmentReviewProfile {
...
interface UnansweredContainmentReviewSession {
  readonly accountId: AccountId;
  ...
  readonly budget: UnansweredContainmentReviewBudget;
  readonly consumed: Omit<
    UnansweredContainmentReviewBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface UnansweredContainmentReviewSnapshot {
  readonly accountId: AccountId;
...
interface UnansweredContainmentReviewEvaluationReceipt {
  readonly accountId: AccountId;
...
interface UnansweredContainmentReviewBinding {
  readonly accountId: AccountId;
...
interface UnansweredContainmentReviewCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface UnansweredContainmentReviewEffectObservation {
...
interface AgentUnansweredContainmentReviewPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  ...
  readonly reviewUnansweredContainmentBindingCount: number;
  readonly holdReviewWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectReviewAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly UnansweredContainmentReviewEffectObservation[];
  ...
  readonly blockedReasons: readonly UnansweredContainmentReviewBlockedReason[];
...
```ts
interface CreateUnansweredContainmentReviewSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface NominateSealedContainmentCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
...
interface RefreshUnansweredContainmentReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateUnansweredContainmentReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateUnansweredContainmentReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ResolveReviewUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface CloseUnansweredContainmentReviewSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type UnansweredContainmentReviewDecision =
  | { readonly decision: "ALLOWED"; readonly session: UnansweredContainmentReviewSession;
      readonly certificate?: UnansweredContainmentReviewCertificate;
      readonly member?: UnansweredContainmentReviewBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: UnansweredContainmentReviewSnapshot;
      readonly evaluation?: UnansweredContainmentReviewEvaluationReceipt;
      readonly perception: AgentUnansweredContainmentReviewPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: UnansweredContainmentReviewBlockedReason;
...
```sql
CREATE TYPE ucr_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE ucr_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_source_kind AS ENUM (
...
CREATE TYPE ucr_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_review_kind AS ENUM (
  'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED',
  'REVIEW_UNANSWERED_CONTAINMENT', 'HOLD_REVIEW_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_REVIEW_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE ucr_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_ucr_profile_authority NOLOGIN;

CREATE TABLE agent_ucr_authorization_evidence (
...
CREATE TABLE agent_ucr_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status ucr_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_ucr_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_ucr_profile_review_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_ucr_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_ucr_containment_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind ucr_source_kind NOT NULL,
  placement_kind ucr_placement_kind NOT NULL,
  key_lifecycle ucr_key_lifecycle NOT NULL,
  clock_lifecycle ucr_clock_lifecycle NOT NULL,
  status ucr_catalog_status NOT NULL,
...
CREATE TABLE agent_ucr_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status ucr_session_status NOT NULL,
...
CREATE TABLE agent_ucr_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_ucr_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_ucr_containment_catalog (account_id, source_window_id)
);

CREATE TABLE agent_ucr_review_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_review_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_ucr_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_ucr_review_binding (
  account_id BIGINT NOT NULL,
  ...
  status ucr_binding_status NOT NULL,
  placement_kind ucr_placement_kind NOT NULL,
  key_lifecycle ucr_key_lifecycle NOT NULL,
  clock_lifecycle ucr_clock_lifecycle NOT NULL,
  attested_key_kind ucr_attested_key_kind NOT NULL,
  review_kind ucr_review_kind NOT NULL,
  purpose_relation ucr_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_ucr_review_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_ucr_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_ucr_containment_catalog (account_id, source_window_id)
...
CREATE TABLE agent_ucr_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status ucr_effect_status NOT NULL,
...
CREATE TABLE agent_ucr_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_ucr_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_ucr_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status ucr_session_status NOT NULL,
...
CREATE TABLE agent_ucr_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_ucr_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.ucr_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.ucr_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_ucr_profile_protect
BEFORE INSERT OR UPDATE ON agent_ucr_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_ucr_profile();

CREATE FUNCTION protect_agent_ucr_profile_review_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status ucr_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_ucr_profile
...
CREATE TRIGGER agent_ucr_profile_review_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_ucr_profile_review_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_ucr_profile_review_rule();

CREATE FUNCTION protect_agent_ucr_review_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_ucr_review_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_ucr_review_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_ucr_review_snapshot();

CREATE FUNCTION protect_agent_ucr_review_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED',
       'REVIEW_UNANSWERED_CONTAINMENT', 'REJECT_REVIEW_AS_CONFIRMATION'
     )
  ...
  IF NEW.review_kind = 'REJECT_REVIEW_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'review-as-confirmation fence blocks TRUSTED_ENROLLED from an unanswered containment';
  END IF;

  IF NEW.review_kind = 'REVIEW_UNANSWERED_CONTAINMENT'
  ...
  IF NEW.review_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from unanswered-containment review';
  END IF;
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED', 'REVIEW_UNANSWERED_CONTAINMENT'
     )
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_UNANSWERED_CONTAINMENT'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-review fence blocks containment that would restore a halted key';
  END IF;
...
CREATE TRIGGER agent_ucr_review_binding_protect
BEFORE INSERT OR UPDATE ON agent_ucr_review_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_ucr_review_binding();

CREATE FUNCTION protect_agent_ucr_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_ucr_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_ucr_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_ucr_effect_intent();

CREATE FUNCTION approve_agent_ucr_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status ucr_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_ucr_profile_review_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_ucr_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_ucr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_ucr_profile_authority;
ALTER FUNCTION revoke_agent_ucr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_ucr_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_ucr_profile_authority;
GRANT SELECT ON
  agent_ucr_profile,
  agent_ucr_profile_review_rule
TO agent_ucr_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_ucr_profile TO agent_ucr_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_ucr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_ucr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_ucr_profile FROM PUBLIC;

CREATE INDEX agent_ucr_session_work_idx ON agent_ucr_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_ucr_session_profile_idx ON agent_ucr_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_ucr_binding_certificate_idx ON agent_ucr_review_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_ucr_binding_window_idx ON agent_ucr_review_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_ucr_catalog_ref_idx ON agent_ucr_containment_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_ucr_catalog_kind_idx ON agent_ucr_containment_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_ucr_evaluation_session_idx ON agent_ucr_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_ucr_certificate_session_idx ON agent_ucr_review_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_ucr_snapshot_session_idx ON agent_ucr_review_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_ucr_effect_work_idx ON agent_ucr_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_ucr_audit_time_idx ON agent_ucr_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_ucr_perception_status_idx ON agent_ucr_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_ucr_command_expiry_idx ON agent_ucr_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_ucr_invalidation_certificate_idx ON agent_ucr_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_ucr_authorization_evidence',
    'agent_ucr_profile',
    'agent_ucr_profile_review_rule',
    'agent_ucr_containment_catalog',
    'agent_ucr_session',
    'agent_ucr_nomination_window',
    'agent_ucr_review_snapshot',
    'agent_ucr_evaluation_receipt',
    'agent_ucr_review_certificate',
    'agent_ucr_review_binding',
    'agent_ucr_invalidation',
    'agent_ucr_effect_intent',
    'agent_ucr_budget_ledger',
    'agent_ucr_terminal_record',
    'agent_ucr_command_result',
    'agent_ucr_audit_head',
    'agent_ucr_audit_event',
    'agent_ucr_audit_anchor',
    'agent_ucr_perception_snapshot',
    'agent_ucr_projection_checkpoint'
  ]
  LOOP
...
enum AgentUcrClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentUcrAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentUcrUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentUcrInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentUcrSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentUcrSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentUcrBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentUcrNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentUcrSourceKind!
  placementKind: AgentUcrPlacementKind!
  keyLifecycle: AgentUcrKeyLifecycle!
  clockLifecycle: AgentUcrClockLifecycle!
...
type AgentUcrUnansweredContainmentReviewSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentUcrEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentUcrReviewCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentUcrBinding {
  ...
  sourceWindowKind: AgentUcrSourceKind!
  bindingOrdinal: Int!
  status: AgentUcrBindingStatus!
  placementKind: AgentUcrPlacementKind!
  keyLifecycle: AgentUcrKeyLifecycle!
  clockLifecycle: AgentUcrClockLifecycle!
  attestedKeyKind: AgentUcrAttestedKeyKind!
  reviewKind: AgentUcrReviewKind!
  purposeRelation: AgentUcrPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentUcrEffectObservation {
  effectId: ID!
  status: AgentUcrEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentUcrPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentUcrSessionStatus!
  ...
  reviewUnansweredContainmentBindingCount: Int!
  holdReviewWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectReviewAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentUcrEffectObservation!]!
  remainingBudget: AgentUcrBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentUcrNextAction!]!
  blockedReasons: [AgentUcrBlockedReason!]!
  cardHash: SHA256!
}

type AgentUcrMutationResult {
  decision: String!
  session: AgentUcrSession
  certificate: AgentUcrReviewCertificate
  member: AgentUcrBinding
  receipt: AgentUcrNominationReceipt
  snapshot: AgentUcrUnansweredContainmentReviewSnapshot
  evaluation: AgentUcrEvaluationReceipt
  perception: AgentUcrPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateUnansweredContainmentReviewSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentUcrBudgetInput!
}

input NominateSealedContainmentCertificateInput {
  accountId: ID!
  sessionId: ID!
...
input RefreshUnansweredContainmentReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateUnansweredContainmentReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateUnansweredContainmentReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentUcrInvalidationReason!
  idempotencyKey: String!
}
...
input ResolveReviewUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentUcrUncertaintyResolution!
  idempotencyKey: String!
}

input AgentUcrProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentUcrSession(accountId: ID!, sessionId: ID!): AgentUcrSession
  agentUcrReviewCertificate(accountId: ID!, certificateId: ID!): AgentUcrReviewCertificate
  agentUcrPerceptionCard(accountId: ID!, sessionId: ID!): AgentUcrPerceptionCard
  agentUcrNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentUcrNominationReceipt
  agentUcrSearchProfiles(input: AgentUcrProfileSearchInput!): [AgentUcrProfile!]!
}

type Mutation {
  createUnansweredContainmentReviewSession(
    input: CreateUnansweredContainmentReviewSessionInput!
  ): AgentUcrMutationResult!
  nominateSealedContainmentCertificate(
    input: NominateSealedContainmentCertificateInput!
  ): AgentUcrMutationResult!
  refreshUnansweredContainmentReview(
    input: RefreshUnansweredContainmentReviewInput!
  ): AgentUcrMutationResult!
  evaluateUnansweredContainmentReview(
    input: EvaluateUnansweredContainmentReviewInput!
  ): AgentUcrMutationResult!
  sealReviewCertificate(input: SealReviewCertificateInput!): AgentUcrMutationResult!
  invalidateUnansweredContainmentReview(
    input: InvalidateUnansweredContainmentReviewInput!
  ): AgentUcrMutationResult!
  prepareReviewEffect(input: PrepareReviewEffectInput!): AgentUcrMutationResult!
  resolveReviewUncertainty(
    input: ResolveReviewUncertaintyInput!
  ): AgentUcrMutationResult!
  closeUnansweredContainmentReviewSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentUcrMutationResult!
  approveUnansweredContainmentReviewProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealReviewCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `REVIEW_AS_CONFIRMATION_DENIED` when an unanswered containment is treated as confirmation, and with
  `LEDGER_SCAN_DENIED` when remaining time would be computed by walking
  a vote, TTL, page, ack, escalation, or containment ledger.
...
certificates become envelope-scoped steward reviews without
inventing a winner and without rewriting an unanswered
containment as dual-control success or `TRUSTED_ENROLLED`.
...
enrollment unanswered containment?"). Embeddings are
...
review. Vector `topK` is budgeted and clamped.

```sql
CREATE TABLE agent_ucr_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
sealed / review-current / review-active /
review-expired / review-unanswered-containment /
hold-review-window / hold-unknown /
reject-review-as-confirmation / unknown / skipped /
...
`REVIEW_CURRENT` as a first-seen unanswered containment
...
control trust, `REVIEW_UNANSWERED_CONTAINMENT` as a dual-
control steward page that does not confirm the key,
`HOLD_REVIEW_WINDOW` as a trusted hold that refuses
silent confirmation, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_REVIEW_AS_CONFIRMATION` as a trusted
negative for an unanswered containment that would have
rewritten silence as success, and `SKIP` as a sealed
refusal — never as a key that "must have been trusted
because nobody reviewed the unanswered containment."
...
review-current, review-active, or reject-
review-as-confirmation outcomes.
...
- Planner rejects unscoped key-catalog, clock-catalog,
  ...
  vote-ledger, TTL-ledger, page-ledger, ack-ledger,
  escalation-ledger, containment-ledger, or board scans as **FULL SCAN
  REJECTED**.
...
- Evaluation never auto-restores neighbor-visible board
  ...
  acknowledgements, unanswered containments, or unbound
  windows.
...
- Finding remaining unanswered containments to contain by
...
- unanswered-containment review refresh: unique
...
page-ledger/ack-ledger/escalation-ledger/containment-ledger/citation scan is
...
- Historical silence invented as `TRUSTED_ENROLLED` from an
  unanswered containment: history-rewrite, silence-success,
  and unanswered-containment-as-trust fences.
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED`
  by a containment: first-enrollment-auto-trust and
  review-as-confirmation fences.
- Remaining time or review targets computed by walking
  vote, TTL, page, ack, escalation, or containment ledgers: ledger-scan
...
- Original containment authorities serving as the review-steward
  pair: prior-containment-authority-exclusion fence.
...
Compile profiles and validate containment-catalog nomination
without durable certificates.

### Phase 2: reject-review-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_REVIEW_AS_CONFIRMATION` or `SKIP`.
Containment-active stays closed.

### Phase 3: expire-only review and halt-review fences

Enable budgeted `REVIEW_EXPIRED` from
`RESTORED_WITHOUT_WINNER` windows with `ttl_expired = true`
and `AWAITING_CONFIRMATION` only. An unanswered containment
never writes `TRUSTED_ENROLLED`.

### Phase 4: review-steward pages and notify uncertainty
...
budgets, including review-steward pages only for
...
- Nominate requires sealed containment-catalog material point
  lookup and hash match.
- Refresh unanswered-containment review is a point
...
- Seal is rejected when an unanswered containment would
...
Adopt the Unanswered-Containment Review Plane as the
deterministic steward review path for unanswered,
expired, or single-control containments bound by the
