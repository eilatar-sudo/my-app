# mondayDB Agentic Promotion-Containment-Expiry Review Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-09-07.v1`

## 1. Why this plane, before how

A sealed Promotion-Expiry Containment certificate can
dual-control `CONTAIN_EXPIRED_PROMOTION` or seal
`CONTAINMENT_EXPIRED` when the containment window closes
without two distinct containment stewards. It does not decide
**how a `CONTAINMENT_EXPIRED` first-enrollment key is
paged to successor stewards** — without scanning every
window, TTL, confirm, SLO, acknowledgement, escalation,
containment, review, resolution, or promotion ledger,
rewriting review silence into `TRUSTED_ENROLLED`, treating
containment expiry as confirmation, or restoring a halted
fact as a winner.
...
`PROMOTION_EXPIRED`. None of those planes accept
`CONTAINMENT_EXPIRED` as a dual-control steward page that
cannot rewrite silence into `TRUSTED_ENROLLED`.

Without a promotion-containment-expiry review plane,
...
- scan every pending window, TTL, confirm, SLO,
  ...
  sealed promotion-containment is still missing a
  containment-expiry review" (neighbor-harmful on boards
  with 1M+ rows), or
- treat review silence as retroactive engine truth, so
  a missing steward page is rewritten as confirmation,
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a
  silent review auto-promotes `FIRST_ENROLLMENT` to
  `TRUSTED_ENROLLED`, a halt-scoped body is "unlocked" by
  later placement, and hop-attenuated purpose is amplified
  back to the donor.

The product trade-off is **review fluency versus
review isolation**:

- Treating every sealed promotion-containment certificate as
  an implicit steward page maximizes agent fluency and
  reduces re-planning cost, but creates history-rewrite
  invention, containment-expiry-as-confirmation,
  expired-containment-as-trust, silence-success, first-
  enrollment auto-trust, unauditable review storms, and
  recursive containment-ledger walks against neighbors.
- Binding a sealed review certificate under an approved
  review profile, certificate-catalog point lookups,
  history-rewrite fences, silence-success fences,
  first-enrollment auto-trust fences,
  containment-expiry-as-confirmation fences,
  expired-containment-as-trust fences, halt-review fences,
  purpose-amplification fences, successor-leak fences,
  prior-containment-steward-exclusion fences, ledger-scan
  fences, and steward budgets adds one bounded evaluate
  transaction and short-lived review storage.
- Semantic similarity may discover review profiles, but it
  must never decide whether a sealed promotion-containment,
  promotion, review, containment, resolution, escalation,
  acknowledgement, SLO, window, TTL, or confirm certificate
  may be nominated, a review snapshot refreshed, a
  certificate sealed, or a first-enrollment key trusted.

The recommended model keeps the data plane deterministic:

1. An approved review profile defines allowed source kinds,
   review-window duration, dual-control review-steward
   policy, prior-containment-steward exclusion, and notify
   policy. Evaluation **never** invents a winning fact hash
   and **never** rewrites a sealed promotion-containment
   certificate into `TRUSTED_ENROLLED`.
2. A review session opens under purpose, budget, and
   ...
   from the Promotion-Expiry Containment, Decision-Memory
   Promotion, Review-Expiry Resolution, Containment-Expiry
   Review, Resolution-Expiry Containment, Review-Resolution,
   Unanswered-Containment Review, Unanswered-Escalation
   Containment, Unanswered-Ack Escalation, SLO Ack
   Suppression, Remaining-Window SLO, Confirmation-Window
   Observability, Confirmation-TTL Retirement, and
   First-Enrollment Confirmation planes.
3. mondayDB evaluates a review whose kind is a pure function
   ...
   ttl_expired, containment_count,
   distinct_containment_steward_count,
   prior_containment_steward_set_hash, review_steward_count,
   review_window_ms, review_kind)`. Silence cannot become
   historical success. A sealed certificate seals
   `REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION` or
   `REVIEW_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `REVIEW_EXPIRED_PROMOTION_CONTAINMENT` pages
   stewards for a first-enrollment key after
   `CONTAINMENT_EXPIRED` without inventing confirmation.
   `HOLD_REVIEW_WINDOW` holds further review for a bounded
   window without restoring a winner. Halted slots cannot
   become review-restored winners. Original containment
   stewards are excluded from the review-steward pair.
4. Sealing a review certificate binds
   `consumer_ref + purpose_hash + window_set_hash +
   clock_set_hash + remaining_set_hash + attenuation_hash`.
   The certificate **must not** emit a `reviewed_fact_hash`.
5. Upstream invalidation marks certificates stale; notify
   ...
   trusted refresh or review vote arrives.
...
protection: recursive "re-review every sealed
promotion-containment forever" or "treat review silence
as confirmation until it looks like success" loops are
rejectable before they scan boards with 1M+ rows. Perception
is restored by sealed review certificates, not by magic
review orchestration inside the engine.

### Product outcome

For any promotion-containment-expiry-review evaluation,
mondayDB can answer:

- Which profile, principal, and session authorized the
  nomination, review refresh, evaluate, seal, invalidate,
  or notify dispatch?
- Which nominated sealed certificates, placement kinds,
  hop counts, attenuation hashes, remaining-set hashes,
  prior-containment-steward exclusions, and review kinds
  were bound?
- Is the review certificate still current, invalidated,
  resolved, hold-windowed, review-expired, or awaiting
  notify acknowledgement?
- Did async notify or first-enrollment sync become
  `UNKNOWN_EFFECT`?
- Can the review history be replayed without invoking an
  LLM?

## 2. Scope and ownership

The Promotion-Containment-Expiry Review Plane owns:

1. Immutable approved review profiles as procedural
   memory of "how CONTAINMENT_EXPIRED first-enrollment keys are dual-control paged
   ...
   `TRUSTED_ENROLLED` from review silence."
2. Tenant-scoped review sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed promotion-containment
   certificates, sealed promotion certificates, sealed
   ...
   resolution-ledger, promotion-ledger, containment-ledger,
   working-set, or board scans.
4. Deterministic review snapshots, evaluation receipts,
   sealed review certificates, and immutable
   review bindings that never invent a winner.
...
- **Promotion-Expiry Containment:** supplies sealed
  ...
  page. This plane never invents a new containment by
...
- **Decision-Memory Promotion:** supplies sealed
  ...
  outcomes this plane may bind as review evidence. This
  plane never invents a new promotion by walking promotion
  bindings.
- **Review-Expiry Resolution:** supplies sealed
  ...
  outcomes this plane may bind as review evidence.
...
- **Containment-Expiry Review:** supplies sealed
  ...
  unanswered, or single-control outcomes this plane may
  contain. This plane never invents a new review by
...
- **Unanswered-Ack Escalation:** supplies sealed escalation
  ...
  escalations this plane may bind as review evidence. This
...
- **SLO Ack Suppression:** supplies sealed acknowledgement
  ...
  may bind as review evidence. This plane never invents
...
- **Remaining-Window SLO:** supplies sealed SLO certificates
  ...
  as review evidence. This plane never invents a new
...
- **Confirmation-Window Observability:** supplies sealed
  ...
  plane may bind as review evidence. This plane never
...
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may review a sealed governance
...
- **Certificate Placement / Envelope Purpose Gate / Decision
  Memory:** may consume sealed review certificates as
  dual-control freezes and perception
  evidence, not raw pager webhooks. This plane never
  compiles a working set or reuses a historical model
  answer; it only seals the review binding those
...
- **Executive Freeze / Thaw SLA:** halt/restore context that
  ...
  decides promotion-expiry containment.
- **Emergency Containment:** the coarse stop/drain path used
  when a contained key evaluates to `SKIP` or
  `HOLD_REVIEW_WINDOW`; this plane is purpose-scoped
  steward review isolation, not workspace-wide
  containment.
- **Query Governor / Budgets:** reserves nominate, evaluate,
  vector, seal, invalidate, and review units.

### Non-goals

- Letting an LLM decide that a sealed governance certificate "feels"
  confirmable enough" or "feels containment-ready enough."
- Auto-amplifying a hop-narrowed purpose back to the donor
  purpose.
- Reconstructing authoritative review certificates from
...
- Containing `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because
  a certificate was unanswered, contained, held, or expired.
- Scanning vote, TTL, page, ack, escalation, review,
  resolution, or containment ledgers to decide who may review.
- Letting the original dual-control promotion stewards
  also serve as the review-steward pair.

## 3. Product contract

### 3.1 Review profile contract

A profile version is immutable after approval. It defines:

- allowed source kinds (`SEALED_PROMOTION_CONTAINMENT_CERTIFICATE`,
...
- review-window buckets (`review_window_ms`), max
  refresh count, evaluate threshold (distinct review-
  steward principals for `REVIEW_EXPIRED_PROMOTION_CONTAINMENT`,
...
- review policy (`HISTORY_NEVER_REWRITTEN`,
  ...
  `CONTAINMENT_EXPIRY_NEVER_CONFIRMS`, `EXPIRED_CONTAINMENT_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_REVIEW_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_CONTAINMENT_STEWARD_NEVER_REVIEWS`);
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed review kinds (`REVIEW_CURRENT`,
  `REVIEW_ACTIVE`, `REVIEW_EXPIRED`,
  `REVIEW_EXPIRED_PROMOTION_CONTAINMENT`, `HOLD_REVIEW_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION`,
  `SKIP`) and notify policy after seal, invalidation, or
  upstream key or clock change;
- optional procedural refs for "how to review expired sealed
  promotion-containment certificates after CONTAINMENT_EXPIRED without inventing a winner."
...
Refresh promotion-containment-expiry-review is a point lookup
...
`reviewed_fact_hash`. A sealed certificate cannot
...
10. Silence and unknown windows cannot evaluate to
    `REVIEW_EXPIRED_PROMOTION_CONTAINMENT` or any kind that implies
...
14. A promoted certificate never writes
    ...
    (containment-expiry-as-confirmation and expired-promotion-
    as-trust fences).
15. `REVIEW_EXPIRED_PROMOTION_CONTAINMENT` requires two distinct
...
`review_window_ms > 0`, and fences the status
transition.
...
certificate. Refresh promotion-containment-expiry-review is a
...
type TrustedNextAction =
  | "NOMINATE_SEALED_PROMOTION_CONTAINMENT_CERTIFICATE"
  | "REFRESH_REVIEW"
  | "EVALUATE_REVIEW"
  | "SEAL_REVIEW_CERTIFICATE"
  | "INVALIDATE_REVIEW"
  | "PREPARE_REVIEW_EFFECT"
  | "REVIEW_EFFECT_UNCERTAINTY"
  | "CLOSE_SESSION";

type PromotionContainmentExpiryReviewBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "CONTAINMENT_EXPIRY_AS_CONFIRMATION_DENIED"
  | "EXPIRED_CONTAINMENT_AS_TRUST_DENIED"
  | "PRIOR_CONTAINMENT_STEWARD_EXCLUSION_DENIED"
  | "LEDGER_SCAN_DENIED"
  | "HALT_REVIEW_DENIED"
...
type ReviewKind =
  ...
  | "REVIEW_ACTIVE"
  | "REVIEW_EXPIRED"
  | "REVIEW_EXPIRED_PROMOTION_CONTAINMENT"
  | "HOLD_REVIEW_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface PromotionContainmentExpiryReviewBudget {
...
interface PromotionContainmentExpiryReviewProfile {
...
interface PromotionContainmentExpiryReviewSession {
  readonly accountId: AccountId;
  ...
  readonly budget: PromotionContainmentExpiryReviewBudget;
  readonly consumed: Omit<
    PromotionContainmentExpiryReviewBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface PromotionContainmentExpiryReviewSnapshot {
  readonly accountId: AccountId;
...
interface PromotionContainmentExpiryReviewEvaluationReceipt {
  readonly accountId: AccountId;
...
interface PromotionContainmentExpiryReviewBinding {
  readonly accountId: AccountId;
...
interface PromotionContainmentExpiryReviewCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface PromotionContainmentExpiryReviewEffectObservation {
...
interface AgentPromotionContainmentExpiryReviewPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly reviewCurrentBindingCount: number;
  readonly reviewActiveBindingCount: number;
  readonly reviewExpiredBindingCount: number;
  readonly reviewExpiredPromotionContainmentBindingCount: number;
  readonly holdReviewWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectContainmentExpiryAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly PromotionContainmentExpiryReviewEffectObservation[];
  ...
  readonly blockedReasons: readonly PromotionContainmentExpiryReviewBlockedReason[];
...
```ts
interface CreatePromotionContainmentExpiryReviewSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshPromotionContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluatePromotionContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface SealReviewCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly evaluationId: EvaluationId;
...
interface InvalidatePromotionContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface PrepareReviewEffectInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ReviewEffectUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface ClosePromotionContainmentExpiryReviewSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type PromotionContainmentExpiryReviewDecision =
  | { readonly decision: "ALLOWED"; readonly session: PromotionContainmentExpiryReviewSession;
      readonly certificate?: PromotionContainmentExpiryReviewCertificate;
      readonly member?: PromotionContainmentExpiryReviewBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: PromotionContainmentExpiryReviewSnapshot;
      readonly evaluation?: PromotionContainmentExpiryReviewEvaluationReceipt;
      readonly perception: AgentPromotionContainmentExpiryReviewPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: PromotionContainmentExpiryReviewBlockedReason;
...
```sql
CREATE TYPE pcer_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE pcer_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_source_kind AS ENUM (
...
CREATE TYPE pcer_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_review_kind AS ENUM (
  'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED',
  'REVIEW_EXPIRED_PROMOTION_CONTAINMENT', 'HOLD_REVIEW_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcer_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_pcer_profile_authority NOLOGIN;

CREATE TABLE agent_pcer_authorization_evidence (
...
CREATE TABLE agent_pcer_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status pcer_profile_status NOT NULL,
  ...
  review_window_ms BIGINT NOT NULL
    CHECK (review_window_ms BETWEEN 1000 AND 2592000000),
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_pcer_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_pcer_profile_review_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  review_window_ms BIGINT NOT NULL
    CHECK (review_window_ms BETWEEN 1000 AND 2592000000),
  require_dual_control_review BOOLEAN NOT NULL,
  review_instruction JSONB NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_pcer_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_pcer_certificate_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind pcer_source_kind NOT NULL,
  placement_kind pcer_placement_kind NOT NULL,
  key_lifecycle pcer_key_lifecycle NOT NULL,
  clock_lifecycle pcer_clock_lifecycle NOT NULL,
  status pcer_catalog_status NOT NULL,
...
CREATE TABLE agent_pcer_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status pcer_session_status NOT NULL,
  ...
  budget_review_units BIGINT NOT NULL CHECK (budget_review_units >= 0),
  ...
  consumed_review_units BIGINT NOT NULL
    CHECK (consumed_review_units >= 0),
  ...
  CHECK (consumed_review_units <= budget_review_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_pcer_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pcer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pcer_certificate_catalog (account_id, source_window_id)
);

CREATE TABLE agent_pcer_review_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_review_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_pcer_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_pcer_review_binding (
  account_id BIGINT NOT NULL,
  ...
  status pcer_binding_status NOT NULL,
  placement_kind pcer_placement_kind NOT NULL,
  key_lifecycle pcer_key_lifecycle NOT NULL,
  clock_lifecycle pcer_clock_lifecycle NOT NULL,
  attested_key_kind pcer_attested_key_kind NOT NULL,
  review_kind pcer_review_kind NOT NULL,
  purpose_relation pcer_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_pcer_review_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pcer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pcer_certificate_catalog (account_id, source_window_id)
...
CREATE TABLE agent_pcer_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status pcer_effect_status NOT NULL,
...
CREATE TABLE agent_pcer_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_pcer_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcer_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status pcer_session_status NOT NULL,
...
CREATE TABLE agent_pcer_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_pcer_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM 'DRAFT'
     AND (
       ...
       OR NEW.review_window_ms IS DISTINCT FROM OLD.review_window_ms
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.pcer_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.pcer_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_pcer_profile_protect
BEFORE INSERT OR UPDATE ON agent_pcer_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcer_profile();

CREATE FUNCTION protect_agent_pcer_profile_review_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status pcer_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_pcer_profile
  ...
  IF profile_status IS DISTINCT FROM 'DRAFT' THEN
    RAISE EXCEPTION 'sealed profile review rules are immutable';
  END IF;
  RETURN COALESCE(NEW, OLD);
END
$protect_rule$;

CREATE TRIGGER agent_pcer_profile_review_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_pcer_profile_review_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcer_profile_review_rule();

CREATE FUNCTION protect_agent_pcer_review_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'review snapshots are append-only';
  END IF;
  RETURN NEW;
END
$protect_snapshot$;

CREATE TRIGGER agent_pcer_review_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_pcer_review_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcer_review_snapshot();

CREATE FUNCTION protect_agent_pcer_review_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.source_window_id IS DISTINCT FROM OLD.source_window_id
      ...
      RAISE EXCEPTION 'review binding identity is immutable';
    END IF;
    RETURN NEW;
  END IF;

  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED',
       'REVIEW_EXPIRED_PROMOTION_CONTAINMENT', 'REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION'
     )
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'history-rewrite fence blocks review from rewriting confirmed trust';
  END IF;

  IF NEW.review_kind = 'REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'containment-expiry-as-confirmation fence blocks TRUSTED_ENROLLED from a sealed governance certificate';
  END IF;

  IF NEW.review_kind = 'REVIEW_EXPIRED_PROMOTION_CONTAINMENT'
     AND NEW.attested_key_kind IN ('SILENCE', 'UNKNOWN_EFFECT') THEN
    RAISE EXCEPTION 'silence-success fence blocks review of silent or unknown successor';
  END IF;
  ...
  IF NEW.review_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from promotion-containment-expiry-review';
  END IF;

  IF NEW.review_kind = 'REVIEW_EXPIRED_PROMOTION_CONTAINMENT'
     AND NEW.clock_lifecycle = 'UNATTESTED' THEN
    RAISE EXCEPTION 'unattested-clock fence blocks review against an unattested clock';
  END IF;

  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED', 'REVIEW_EXPIRED_PROMOTION_CONTAINMENT'
     )
     AND NEW.attested_key_kind = 'INVENTED_HISTORY' THEN
    RAISE EXCEPTION 'invented-history fence blocks review for invented successor history';
  END IF;

  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED_PROMOTION_CONTAINMENT'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-review fence blocks review that would restore a halted key';
  END IF;
  ...
  IF NEW.hop_count > 0
     AND NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED_PROMOTION_CONTAINMENT'
     )
     AND NEW.requested_purpose_hash IS NOT DISTINCT FROM NEW.donor_purpose_hash THEN
    RAISE EXCEPTION 'hop-leak fence blocks donor-purpose review after attenuation hops';
  END IF;
...
CREATE TRIGGER agent_pcer_review_binding_protect
BEFORE INSERT OR UPDATE ON agent_pcer_review_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcer_review_binding();

CREATE FUNCTION protect_agent_pcer_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_pcer_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_pcer_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcer_effect_intent();

CREATE FUNCTION approve_agent_pcer_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status pcer_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_pcer_profile_review_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;

  IF rule_count < 1 THEN
    RAISE EXCEPTION 'profile requires at least one review rule';
  END IF;

  PERFORM set_config(
    'app.pcer_profile_approval',
    concat(
...
CREATE FUNCTION revoke_agent_pcer_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_pcer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pcer_profile_authority;
ALTER FUNCTION revoke_agent_pcer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pcer_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_pcer_profile_authority;
GRANT SELECT ON
  agent_pcer_profile,
  agent_pcer_profile_review_rule
TO agent_pcer_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pcer_profile TO agent_pcer_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_pcer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_pcer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pcer_profile FROM PUBLIC;

CREATE INDEX agent_pcer_session_work_idx ON agent_pcer_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_pcer_session_profile_idx ON agent_pcer_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_pcer_binding_certificate_idx ON agent_pcer_review_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pcer_binding_window_idx ON agent_pcer_review_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pcer_catalog_ref_idx ON agent_pcer_certificate_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pcer_catalog_kind_idx ON agent_pcer_certificate_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pcer_evaluation_session_idx ON agent_pcer_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_pcer_certificate_session_idx ON agent_pcer_review_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_pcer_snapshot_session_idx ON agent_pcer_review_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_pcer_effect_work_idx ON agent_pcer_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_pcer_audit_time_idx ON agent_pcer_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_pcer_perception_status_idx ON agent_pcer_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_pcer_command_expiry_idx ON agent_pcer_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_pcer_invalidation_certificate_idx ON agent_pcer_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_pcer_authorization_evidence',
    'agent_pcer_profile',
    'agent_pcer_profile_review_rule',
    'agent_pcer_certificate_catalog',
    'agent_pcer_session',
    'agent_pcer_nomination_window',
    'agent_pcer_review_snapshot',
    'agent_pcer_evaluation_receipt',
    'agent_pcer_review_certificate',
    'agent_pcer_review_binding',
    'agent_pcer_invalidation',
    'agent_pcer_effect_intent',
    'agent_pcer_budget_ledger',
    'agent_pcer_terminal_record',
    'agent_pcer_command_result',
    'agent_pcer_audit_head',
    'agent_pcer_audit_event',
    'agent_pcer_audit_anchor',
    'agent_pcer_perception_snapshot',
    'agent_pcer_projection_checkpoint'
  ]
  LOOP
...
enum AgentPcerClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentPcerAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentPcerUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentPcerInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentPcerSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentPcerSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentPcerBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentPcerNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentPcerSourceKind!
  placementKind: AgentPcerPlacementKind!
  keyLifecycle: AgentPcerKeyLifecycle!
  clockLifecycle: AgentPcerClockLifecycle!
...
type AgentPcerPromotionContainmentExpiryReviewSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentPcerEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentPcerReviewCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentPcerBinding {
  ...
  sourceWindowKind: AgentPcerSourceKind!
  bindingOrdinal: Int!
  status: AgentPcerBindingStatus!
  placementKind: AgentPcerPlacementKind!
  keyLifecycle: AgentPcerKeyLifecycle!
  clockLifecycle: AgentPcerClockLifecycle!
  attestedKeyKind: AgentPcerAttestedKeyKind!
  reviewKind: AgentPcerReviewKind!
  purposeRelation: AgentPcerPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentPcerEffectObservation {
  effectId: ID!
  status: AgentPcerEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentPcerPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentPcerSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  reviewCurrentBindingCount: Int!
  reviewActiveBindingCount: Int!
  reviewExpiredBindingCount: Int!
  reviewExpiredPromotionContainmentBindingCount: Int!
  holdReviewWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectContainmentExpiryAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentPcerEffectObservation!]!
  remainingBudget: AgentPcerBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentPcerNextAction!]!
  blockedReasons: [AgentPcerBlockedReason!]!
  cardHash: SHA256!
}

type AgentPcerMutationResult {
  decision: String!
  session: AgentPcerSession
  certificate: AgentPcerReviewCertificate
  member: AgentPcerBinding
  receipt: AgentPcerNominationReceipt
  snapshot: AgentPcerPromotionContainmentExpiryReviewSnapshot
  evaluation: AgentPcerEvaluationReceipt
  perception: AgentPcerPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreatePromotionContainmentExpiryReviewSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentPcerBudgetInput!
}
...
input RefreshPromotionContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluatePromotionContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input SealReviewCertificateInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  evaluationId: ID!
...
input InvalidatePromotionContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentPcerInvalidationReason!
  idempotencyKey: String!
}

input PrepareReviewEffectInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
...
input ReviewEffectUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  promotion: AgentPcerUncertaintyResolution!
  idempotencyKey: String!
}

input AgentPcerProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentPcerSession(accountId: ID!, sessionId: ID!): AgentPcerSession
  agentPcerReviewCertificate(accountId: ID!, certificateId: ID!): AgentPcerReviewCertificate
  agentPcerPerceptionCard(accountId: ID!, sessionId: ID!): AgentPcerPerceptionCard
  agentPcerNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentPcerNominationReceipt
  agentPcerSearchProfiles(input: AgentPcerProfileSearchInput!): [AgentPcerProfile!]!
}

type Mutation {
  createPromotionContainmentExpiryReviewSession(
    input: CreatePromotionContainmentExpiryReviewSessionInput!
  ): AgentPcerMutationResult!
  nominateSealedCertificate(
    input: NominateSealedCertificateInput!
  ): AgentPcerMutationResult!
  refreshPromotionContainmentExpiryReview(
    input: RefreshPromotionContainmentExpiryReviewInput!
  ): AgentPcerMutationResult!
  evaluatePromotionContainmentExpiryReview(
    input: EvaluatePromotionContainmentExpiryReviewInput!
  ): AgentPcerMutationResult!
  sealReviewCertificate(input: SealReviewCertificateInput!): AgentPcerMutationResult!
  invalidatePromotionContainmentExpiryReview(
    input: InvalidatePromotionContainmentExpiryReviewInput!
  ): AgentPcerMutationResult!
  prepareReviewEffect(input: PrepareReviewEffectInput!): AgentPcerMutationResult!
  reviewEffectUncertainty(
    input: ReviewEffectUncertaintyInput!
  ): AgentPcerMutationResult!
  closePromotionContainmentExpiryReviewSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentPcerMutationResult!
  approvePromotionContainmentExpiryReviewProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- Perception cards never embed raw private keys, tool payloads, or redacted
  fact bodies.
- `sealReviewCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  remaining TTL would invent `TRUSTED_ENROLLED`, with
  `CONTAINMENT_EXPIRY_AS_CONFIRMATION_DENIED` when a sealed certificate is treated as promotion confirmation, and with
...
Approved review profiles are procedural memory:
...
review profile fits incident hop-attenuated first-
...
Semantic retrieval may return review profiles only. It
never authorizes nominate, refresh, evaluate, seal, or
resolve. Vector `topK` is budgeted and clamped.

```sql
CREATE TABLE agent_pcer_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
reject-containment-expiry-as-confirmation / unknown / skipped /
...
`REVIEW_CURRENT` as a first-seen sealed certificate
whose promotion window is still open and still cannot
invent a winner, `REVIEW_ACTIVE` as a bounded steward
page whose remaining time is below the profile
threshold without confirmation, `REVIEW_EXPIRED` as a
sealed certificate that the window closed without dual-
control trust, `REVIEW_EXPIRED_PROMOTION_CONTAINMENT` as a dual-
control sealed-certificate promotion that does not confirm the key,
`HOLD_REVIEW_WINDOW` as a trusted hold that refuses
silent confirmation, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION` as a trusted
...
containment-expiry-as-confirmation outcomes.
...
- promotion-containment-expiry-review refresh: unique
...
- Historical silence invented as `TRUSTED_ENROLLED` from a
  ...
  and expired-containment-as-trust fences.
...
- Original containment authorities serving as the review-steward
  pair: prior-containment-steward-exclusion fence.
...
### Phase 2: reject-containment-expiry-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_CONTAINMENT_EXPIRY_AS_CONFIRMATION` or `SKIP`.
Containment-active stays closed.

### Phase 3: expire-only containment and halt-review fences

Enable budgeted `REVIEW_EXPIRED` from
...
### Phase 4: promotion-expired-containment resolutions and notify uncertainty

Enable `REVIEW_CURRENT` / `REVIEW_ACTIVE` under
...
budgets, including promotion-expired-containment resolutions only for
...
- Refresh promotion-containment-expiry-review is a point
...
- Evaluate binds window set, remaining set, and attenuation
  under budget and prior-containment-steward exclusion.
- Seal is rejected when a sealed certificate would
  ...
  hash.
- Review-certificate seal binds immutable bindings
...
- Audit replay reconstructs sealed review certificates
  after process restart.

## 21. Product decision

Adopt the Promotion-Containment-Expiry Review Plane as the
deterministic steward-review path for first-enrollment
keys whose sealed Promotion-Expiry Containment certificates
expired without dual-control freeze, bound by the
Promotion-Expiry Containment, Decision-Memory Promotion,
Review-Expiry Resolution, Containment-Expiry Review,
Resolution-Expiry Containment, Review-Resolution,
Unanswered-Containment Review, Unanswered-Escalation
Containment, Unanswered-Ack Escalation, SLO Ack Suppression,
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-promotion-containment expiry gap without
   history rewrite, silence-success, first-enrollment auto-
   trust, containment-expiry-as-confirmation, expired-
   containment-as-trust, ledger scans, halt leak, purpose
   amplification, invented winners, prior-containment-
