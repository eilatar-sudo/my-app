# mondayDB Agentic Promotion-Resolution-Expiry Containment Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-09-09.v1`

## 1. Why this plane, before how

A sealed Promotion-Containment Review-Expiry Resolution
certificate can dual-control
`RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW` or seal
`RESOLUTION_EXPIRED` when the resolution window closes
without two distinct resolution stewards. It does not
decide **how a `RESOLUTION_EXPIRED` first-enrollment key is
dual-control frozen** — without scanning every window, TTL,
confirm, SLO, acknowledgement, escalation, containment,
review, resolution, or promotion ledger, rewriting
containment silence into `TRUSTED_ENROLLED`, treating
resolution expiry as confirmation, or restoring a halted
fact as a winner.
...
working sets. Decision-Memory Promotion already seals
placement-aware promotion receipts. Promotion-Expiry
Containment already freezes first-enrollment keys after
`PROMOTION_EXPIRED`. Promotion-Containment-Expiry Review
already pages successor stewards after
`CONTAINMENT_EXPIRED`. Promotion-Containment Review-Expiry
Resolution already dual-control resolves first-enrollment
keys after `REVIEW_EXPIRED`. None of those planes accept
`RESOLUTION_EXPIRED` as a dual-control freeze that cannot
rewrite silence into `TRUSTED_ENROLLED`.

Without a promotion-resolution-expiry containment plane,
operators and agents either:
...
- scan every pending window, TTL, confirm, SLO,
  ...
  sealed promotion-containment-review resolution is still
  missing a resolution-expiry freeze" (neighbor-harmful on
  boards with 1M+ rows), or
- treat containment silence as retroactive engine truth, so
  a missing steward vote is rewritten as confirmation,
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a
  silent freeze auto-promotes `FIRST_ENROLLMENT` to
  `TRUSTED_ENROLLED`, a halt-scoped body is "unlocked" by
  later placement, and hop-attenuated purpose is amplified
  back to the donor.

The product trade-off is **containment fluency versus
containment isolation**:

- Treating every sealed promotion-containment-review
  resolution certificate as an implicit freeze maximizes
  agent fluency and reduces re-planning cost, but creates
  history-rewrite invention, resolution-expiry-as-confirmation,
  expired-resolution-as-trust, silence-success, first-
  enrollment auto-trust, unauditable containment storms, and
  recursive resolution-ledger walks against neighbors.
- Binding a sealed containment certificate under an
  approved containment profile, certificate-catalog point
  lookups, history-rewrite fences, silence-success fences,
  first-enrollment auto-trust fences,
  resolution-expiry-as-confirmation fences,
  expired-resolution-as-trust fences, halt-contain fences,
  purpose-amplification fences, successor-leak fences,
  prior-resolution-steward-exclusion fences, ledger-scan
  fences, and steward budgets adds one bounded evaluate
  transaction and short-lived containment storage.
- Semantic similarity may discover containment profiles,
  but it must never decide whether a sealed
  promotion-containment-review-resolution, promotion-
  containment-review, promotion-containment, promotion,
  review, containment, resolution, escalation,
  acknowledgement, SLO, window, TTL, or confirm certificate
  may be nominated, a containment snapshot refreshed, a
  certificate sealed, or a first-enrollment key trusted.

The recommended model keeps the data plane deterministic:

1. An approved containment profile defines allowed source
   kinds, containment-window duration, dual-control
   containment-steward policy, prior-resolution-steward
   exclusion, and notify policy. Evaluation **never**
   invents a winning fact hash and **never** rewrites a
   sealed promotion-containment-review-resolution
   certificate into `TRUSTED_ENROLLED`.
2. A containment session opens under purpose, budget, and
   authorization fences, and only nominates sealed
   promotion-containment-review-resolution certificates,
   sealed promotion-containment-review certificates, sealed
   promotion-containment certificates, sealed promotion
   certificates, sealed resolution certificates, sealed
   review certificates, sealed containment certificates,
   sealed escalation certificates, sealed acknowledgement
   certificates, sealed SLO certificates, sealed window
   certificates, sealed TTL certificates, sealed confirm
   certificates, or first-enrollment claims by point lookup
   from the Promotion-Containment Review-Expiry Resolution,
   Promotion-Containment-Expiry Review, Promotion-Expiry
   Containment, Decision-Memory Promotion, Review-Expiry
   Resolution, Containment-Expiry Review, Resolution-Expiry
   Containment, Review-Resolution, Unanswered-Containment
   Review, Unanswered-Escalation Containment, Unanswered-Ack
...
3. mondayDB evaluates a containment whose kind is a pure
   ...
   distinct_containment_steward_count,
   prior_resolution_steward_set_hash,
   containment_steward_count, containment_window_ms,
   containment_kind)`. Silence cannot become historical
   success. A sealed certificate seals
   `REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION` or
   `CONTAINMENT_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `CONTAIN_EXPIRED_PROMOTION_RESOLUTION`
   freezes a first-enrollment key after `RESOLUTION_EXPIRED`
   without inventing confirmation. `HOLD_CONTAINMENT_WINDOW`
   holds further containment for a bounded window without
   restoring a winner. Halted slots cannot become
   containment-restored winners. Original resolution
   stewards are excluded from the containment-steward pair.
4. Sealing a containment certificate binds
...
5. Upstream invalidation marks certificates stale; notify
   ...
   trusted refresh or containment vote arrives.
6. Unscoped key-catalog, clock-catalog, confirm-ledger, vote-
   ...
   resolution-ledger, promotion-ledger, working-set,
   grant-graph, or board scans are **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor
protection: recursive "re-contain every sealed
promotion-resolution forever" or "treat resolution-expiry
silence as confirmation until it looks like success" loops
are rejectable before they scan boards with 1M+ rows.
Perception is restored by sealed containment certificates,
not by magic containment orchestration inside the engine.

### Product outcome

For any promotion-resolution-expiry-containment
evaluation, mondayDB can answer:

- Which profile, principal, and session authorized the
  nomination, containment refresh, evaluate, seal,
...
- Which nominated sealed certificates, placement kinds,
  ...
  prior-resolution-steward exclusions, and containment
  kinds were bound?
- Is the containment certificate still current,
  invalidated, contained, hold-windowed, containment-
  expired, or awaiting notify acknowledgement?
- Did async notify or first-enrollment sync become
  `UNKNOWN_EFFECT`?
- Can the containment history be replayed without invoking
  an LLM?

## 2. Scope and ownership

The Promotion-Resolution-Expiry Containment Plane owns:

1. Immutable approved containment profiles as procedural
   memory of "how RESOLUTION_EXPIRED first-enrollment keys
   are dual-control frozen for a first-enrollment key
   without amplifying purpose, leaking halted facts,
   rewriting attested history, scanning vote, page, ack,
   escalation, review, resolution, or containment ledgers,
   or inventing `TRUSTED_ENROLLED` from resolution-expiry
   silence."
2. Tenant-scoped containment sessions with purpose and
   budget fences.
3. Deterministic nomination of sealed promotion-containment-
   review-resolution certificates, sealed promotion-
   containment-review certificates, sealed promotion-
   containment certificates, sealed promotion certificates,
   sealed resolution certificates, sealed review
   certificates, sealed containment certificates, sealed
   escalation certificates, sealed acknowledgement
   certificates, sealed SLO certificates, sealed window
   certificates, sealed TTL certificates, confirm
   certificates, and first-enrollment claims by point
   lookup — never key-catalog, vote-ledger, TTL-ledger,
   page-ledger, ack-ledger, escalation-ledger, review-
   ledger, resolution-ledger, promotion-ledger,
   containment-ledger, working-set, or board scans.
4. Deterministic containment snapshots, evaluation
   receipts, sealed containment certificates, and immutable
   containment bindings that never invent a winner.
...
- **Promotion-Containment Review-Expiry Resolution:**
  supplies sealed promotion-containment-review-resolution
  certificates whose
  `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW`,
  `RESOLUTION_EXPIRED`, unanswered, or single-control
  outcomes this plane may freeze. This plane never invents
  a new resolution by walking resolution bindings.
- **Promotion-Containment-Expiry Review:** supplies sealed
  promotion-containment-review certificates whose
  `REVIEW_EXPIRED_PROMOTION_CONTAINMENT`, `REVIEW_EXPIRED`,
  unanswered, or single-control outcomes this plane may
  bind as containment evidence. This plane never invents a
  new review by walking review bindings.
- **Promotion-Expiry Containment:** supplies sealed
  promotion-containment certificates whose
  `CONTAIN_EXPIRED_PROMOTION`, `CONTAINMENT_EXPIRED`,
  unanswered, or single-control outcomes this plane may
  bind. This plane never invents a new containment by
  walking containment bindings.
- **Decision-Memory Promotion:** supplies sealed
  promotion certificates whose `PROMOTE_SEALED_CERTIFICATE`,
  `PROMOTION_EXPIRED`, unanswered, or single-control
  outcomes this plane may bind as containment evidence.
  This plane never invents a new promotion by walking
  promotion bindings.
...
- **Containment-Expiry Review:** supplies sealed
  ...
  single-control outcomes this plane may bind. This plane
  never invents a new review by walking review bindings.
- **Resolution-Expiry Containment:** supplies sealed
  ...
  containment evidence. This plane never invents a new
  containment by walking review bindings.
...
- **Unanswered-Containment Review:** supplies sealed
  review certificates whose unanswered, expired, or
  single-control steward votes this plane may bind as
  containment evidence. This plane never invents a new
...
- **Unanswered-Ack Escalation:** supplies sealed escalation
  ...
  escalations this plane may bind as containment evidence.
  This plane never invents a new escalation by walking
  escalation bindings.
- **SLO Ack Suppression:** supplies sealed acknowledgement
  certificates whose unanswered acknowledgements this plane
  may bind as containment evidence. This plane never
  invents a new acknowledgement by walking ack bindings.
- **Remaining-Window SLO:** supplies sealed SLO certificates
  and page bindings whose operator pages this plane may bind
  as containment evidence. This plane never invents a new
...
- **Confirmation-Window Observability:** supplies sealed
  ...
  plane may bind as containment evidence. This plane never
  computes remaining TTL by walking containment bindings.
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may contain a sealed governance
...
- **First-Enrollment Confirmation:** supplies sealed confirm
  ...
  contain. This plane never dual-control confirms a key into
  `TRUSTED_ENROLLED`.
- **Successor-Key Enrollment / Successor-Clock Re-Attestation /
  Provider-Key Rotation:** supply sealed enroll, clock, and
  rotation claims that this plane may bind a containment
...
- **Certificate Placement / Envelope Purpose Gate / Decision
  Memory:** may consume sealed containment certificates as
  dual-control freezes and perception evidence, not raw
  pager webhooks. This plane never compiles a working set
  or reuses a historical model answer; it only seals the
  containment binding those planes may later consume.
- **Executive Freeze / Thaw SLA:** halt/restore context that
  still forbids containment-as-restore against a halted body.
  ...
  decides higher-authority paging; promotion-containment
  review-expiry resolution decides first-enrollment
  resolution; this plane decides promotion-resolution-expiry
  containment.
- **Emergency Containment:** the coarse stop/drain path used
  when a contained key evaluates to `SKIP` or
  `HOLD_CONTAINMENT_WINDOW`; this plane is purpose-scoped
  steward freeze isolation, not workspace-wide
  containment.
- **Query Governor / Budgets:** reserves nominate, evaluate,
  vector, seal, invalidate, and contain units.

### Non-goals

- Letting an LLM decide that a sealed governance certificate
  "feels confirmable enough" or "feels containment-ready
  enough."
- Auto-amplifying a hop-narrowed purpose back to the donor
  purpose.
- Reconstructing authoritative containment certificates from
  columnar or vector projections.
- Cross-account containment or global nearest-neighbor
  authorization.
...
- Promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because
  a certificate was unanswered, contained, held, or expired.
- Scanning vote, TTL, page, ack, escalation, review,
  resolution, or containment ledgers to decide who may
  contain.
- Letting the original dual-control resolution stewards
  also serve as the containment-steward pair.

## 3. Product contract

### 3.1 Containment profile contract

A profile version is immutable after approval. It defines:

- allowed source kinds (
  `SEALED_PROMOTION_CONTAINMENT_REVIEW_RESOLUTION_CERTIFICATE`,
  `SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE`,
  `SEALED_PROMOTION_CONTAINMENT_CERTIFICATE`,
  `SEALED_PROMOTION_CERTIFICATE`,
...
- review-window buckets (`containment_window_ms`), max
  refresh count, evaluate threshold (distinct review-
  steward principals for `CONTAIN_EXPIRED_PROMOTION_RESOLUTION`,
...
- review policy (`HISTORY_NEVER_REWRITTEN`,
  ...
  `RESOLUTION_EXPIRY_NEVER_CONFIRMS`, `EXPIRED_RESOLUTION_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_CONTAINMENT_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_RESOLUTION_STEWARD_NEVER_CONTAINS`);
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed containment kinds (`CONTAINMENT_CURRENT`,
  `CONTAINMENT_ACTIVE`, `CONTAINMENT_EXPIRED`,
  `CONTAIN_EXPIRED_PROMOTION_RESOLUTION`, `HOLD_CONTAINMENT_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION`,
  `SKIP`) and notify policy after seal, invalidation, or
  upstream key or clock change;
- optional procedural refs for "how to freeze expired sealed
  promotion-resolution certificates after RESOLUTION_EXPIRED without inventing a winner."
...
Refresh promotion-resolution-expiry-containment is a point lookup
...
review snapshots, clock lifecycles, retire kinds,
dual-control review-steward counts, prior-resolution-
...
`resolved_fact_hash`. A sealed certificate cannot
...
REJECTED** planner rules and review-unit budgets.
...
3. Sealed profile definitions and review rules are
   immutable.
4. Binding identity
...
9. Semantic retrieval discovers profiles only; it never
   authorizes nominate/refresh/evaluate/seal/resolve.
10. Silence and unknown windows cannot evaluate to
    `CONTAIN_EXPIRED_PROMOTION_RESOLUTION` or any kind that implies
...
14. A resolved certificate never writes
    `attested_key_kind = TRUSTED_ENROLLED`
    (resolution-expiry-as-confirmation and expired-promotion-
    as-trust fences).
15. `CONTAIN_EXPIRED_PROMOTION_RESOLUTION` requires two distinct
...
certificate. Refresh promotion-resolution-expiry-containment is a
...
hashes, clock lifecycles, prior-resolution-steward exclusion, and
...
review rule. Approval is authority-fenced.
...
Budgets decrement in the ledger. Review snapshots are
append-only.
...
type TrustedNextAction =
  | "NOMINATE_SEALED_PROMOTION_RESOLUTION_CERTIFICATE"
  | "REFRESH_CONTAINMENT"
  | "EVALUATE_CONTAINMENT"
  | "SEAL_CONTAINMENT_CERTIFICATE"
  | "INVALIDATE_CONTAINMENT"
  | "PREPARE_CONTAINMENT_EFFECT"
  | "CONTAINMENT_EFFECT_UNCERTAINTY"
  | "CLOSE_SESSION";

type PromotionResolutionExpiryContainmentBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "RESOLUTION_EXPIRY_AS_CONFIRMATION_DENIED"
  | "EXPIRED_RESOLUTION_AS_TRUST_DENIED"
...
type ContainmentKind =
  ...
  | "CONTAINMENT_ACTIVE"
  | "CONTAINMENT_EXPIRED"
  | "CONTAIN_EXPIRED_PROMOTION_RESOLUTION"
  | "HOLD_CONTAINMENT_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface PromotionResolutionExpiryContainmentBudget {
  ...
  readonly containmentUnits: number;
...
interface PromotionResolutionExpiryContainmentProfile {
  ...
  readonly containmentWindowMs: number;
...
interface PromotionResolutionExpiryContainmentSession {
  readonly accountId: AccountId;
  ...
  readonly budget: PromotionResolutionExpiryContainmentBudget;
  readonly consumed: Omit<
    PromotionResolutionExpiryContainmentBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface PromotionResolutionExpiryContainmentSnapshot {
  readonly accountId: AccountId;
...
interface PromotionResolutionExpiryContainmentEvaluationReceipt {
  readonly accountId: AccountId;
...
interface PromotionResolutionExpiryContainmentBinding {
  readonly accountId: AccountId;
  ...
  readonly containmentKind: ContainmentKind;
...
interface PromotionResolutionExpiryContainmentCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface PromotionResolutionExpiryContainmentEffectObservation {
...
interface AgentPromotionResolutionExpiryContainmentPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly containmentCurrentBindingCount: number;
  readonly containmentActiveBindingCount: number;
  readonly containmentExpiredBindingCount: number;
  readonly containExpiredPromotionResolutionBindingCount: number;
  readonly holdContainmentWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectResolutionExpiryAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly PromotionResolutionExpiryContainmentEffectObservation[];
  ...
  readonly blockedReasons: readonly PromotionResolutionExpiryContainmentBlockedReason[];
...
```ts
interface CreatePromotionResolutionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshPromotionResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluatePromotionResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface SealContainmentCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly evaluationId: EvaluationId;
...
interface InvalidatePromotionResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface PrepareContainmentEffectInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ContainmentEffectUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface ClosePromotionResolutionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type PromotionResolutionExpiryContainmentDecision =
  | { readonly decision: "ALLOWED"; readonly session: PromotionResolutionExpiryContainmentSession;
      readonly certificate?: PromotionResolutionExpiryContainmentCertificate;
      readonly member?: PromotionResolutionExpiryContainmentBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: PromotionResolutionExpiryContainmentSnapshot;
      readonly evaluation?: PromotionResolutionExpiryContainmentEvaluationReceipt;
      readonly perception: AgentPromotionResolutionExpiryContainmentPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: PromotionResolutionExpiryContainmentBlockedReason;
...
```sql
CREATE TYPE prec_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE prec_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_source_kind AS ENUM (
  'SEALED_PROMOTION_CONTAINMENT_REVIEW_RESOLUTION_CERTIFICATE',
  'SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE',
  'SEALED_PROMOTION_CONTAINMENT_CERTIFICATE', 'SEALED_PROMOTION_CERTIFICATE',
  'SEALED_TTL_CERTIFICATE',
  'SEALED_CONFIRM_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM',
  'SEALED_SLO_CERTIFICATE', 'SEALED_ACK_CERTIFICATE',
  'SEALED_ESCALATION_CERTIFICATE', 'SEALED_RESOLUTION_CERTIFICATE',
  'SEALED_REVIEW_CERTIFICATE', 'SEALED_CONTAINMENT_CERTIFICATE'
);
CREATE TYPE prec_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE prec_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_containment_kind AS ENUM (
  'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED',
  'CONTAIN_EXPIRED_PROMOTION_RESOLUTION', 'HOLD_CONTAINMENT_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE prec_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_prec_profile_authority NOLOGIN;

CREATE TABLE agent_prec_authorization_evidence (
...
CREATE TABLE agent_prec_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status prec_profile_status NOT NULL,
  ...
  containment_window_ms BIGINT NOT NULL
    CHECK (containment_window_ms BETWEEN 1000 AND 2592000000),
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_prec_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_prec_profile_containment_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  containment_window_ms BIGINT NOT NULL
    CHECK (containment_window_ms BETWEEN 1000 AND 2592000000),
  require_dual_control_containment BOOLEAN NOT NULL,
  containment_instruction JSONB NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_prec_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_prec_certificate_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind prec_source_kind NOT NULL,
  placement_kind prec_placement_kind NOT NULL,
  key_lifecycle prec_key_lifecycle NOT NULL,
  clock_lifecycle prec_clock_lifecycle NOT NULL,
  status prec_catalog_status NOT NULL,
...
CREATE TABLE agent_prec_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status prec_session_status NOT NULL,
  ...
  budget_containment_units BIGINT NOT NULL CHECK (budget_containment_units >= 0),
  ...
  consumed_containment_units BIGINT NOT NULL
    CHECK (consumed_containment_units >= 0),
  ...
  CHECK (consumed_containment_units <= budget_containment_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_prec_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_prec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_prec_certificate_catalog (account_id, source_window_id)
);

CREATE TABLE agent_prec_containment_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_containment_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_prec_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_prec_containment_binding (
  account_id BIGINT NOT NULL,
  ...
  status prec_binding_status NOT NULL,
  placement_kind prec_placement_kind NOT NULL,
  key_lifecycle prec_key_lifecycle NOT NULL,
  clock_lifecycle prec_clock_lifecycle NOT NULL,
  attested_key_kind prec_attested_key_kind NOT NULL,
  containment_kind prec_containment_kind NOT NULL,
  purpose_relation prec_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_prec_containment_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_prec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_prec_certificate_catalog (account_id, source_window_id)
...
CREATE TABLE agent_prec_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status prec_effect_status NOT NULL,
...
CREATE TABLE agent_prec_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  entry_sequence BIGINT NOT NULL CHECK (entry_sequence >= 0),
  unit_type TEXT NOT NULL CHECK (
    unit_type IN (
      'NOMINATE', 'EVALUATE', 'SEAL', 'VECTOR', 'INVALIDATE', 'CONTAIN'
    )
  ),
...
CREATE TABLE agent_prec_terminal_record (
  account_id BIGINT NOT NULL,
  ...
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, containment_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_prec_session (account_id, session_id)
);

CREATE TABLE agent_prec_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_prec_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status prec_session_status NOT NULL,
...
CREATE TABLE agent_prec_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_prec_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM 'DRAFT'
     AND (
       ...
       OR NEW.containment_window_ms IS DISTINCT FROM OLD.containment_window_ms
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.prec_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.prec_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_prec_profile_protect
BEFORE INSERT OR UPDATE ON agent_prec_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_prec_profile();

CREATE FUNCTION protect_agent_prec_profile_containment_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status prec_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_prec_profile
...
CREATE TRIGGER agent_prec_profile_containment_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_prec_profile_containment_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_prec_profile_containment_rule();

CREATE FUNCTION protect_agent_prec_containment_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_prec_containment_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_prec_containment_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_prec_containment_snapshot();

CREATE FUNCTION protect_agent_prec_containment_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.source_window_id IS DISTINCT FROM OLD.source_window_id
       ...
       OR NEW.containment_kind IS DISTINCT FROM OLD.containment_kind
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED',
       'CONTAIN_EXPIRED_PROMOTION_RESOLUTION', 'REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION'
     )
  ...
  IF NEW.containment_kind = 'REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'resolution-expiry-as-confirmation fence blocks TRUSTED_ENROLLED from a sealed review-expiry certificate';
  END IF;

  IF NEW.containment_kind = 'CONTAIN_EXPIRED_PROMOTION_RESOLUTION'
  ...
  IF NEW.containment_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from promotion-resolution-expiry-containment';
  END IF;
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED', 'CONTAIN_EXPIRED_PROMOTION_RESOLUTION'
     )
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAIN_EXPIRED_PROMOTION_RESOLUTION'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-contain fence blocks review that would restore a halted key';
  END IF;
...
CREATE TRIGGER agent_prec_containment_binding_protect
BEFORE INSERT OR UPDATE ON agent_prec_containment_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_prec_containment_binding();

CREATE FUNCTION protect_agent_prec_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_prec_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_prec_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_prec_effect_intent();

CREATE FUNCTION approve_agent_prec_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status prec_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_prec_profile_containment_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_prec_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_prec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_prec_profile_authority;
ALTER FUNCTION revoke_agent_prec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_prec_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_prec_profile_authority;
GRANT SELECT ON
  agent_prec_profile,
  agent_prec_profile_containment_rule
TO agent_prec_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_prec_profile TO agent_prec_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_prec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_prec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_prec_profile FROM PUBLIC;

CREATE INDEX agent_prec_session_work_idx ON agent_prec_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_prec_session_profile_idx ON agent_prec_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_prec_binding_certificate_idx ON agent_prec_containment_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_prec_binding_window_idx ON agent_prec_containment_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_prec_catalog_ref_idx ON agent_prec_certificate_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_prec_catalog_kind_idx ON agent_prec_certificate_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_prec_evaluation_session_idx ON agent_prec_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_prec_certificate_session_idx ON agent_prec_containment_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_prec_snapshot_session_idx ON agent_prec_containment_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_prec_effect_work_idx ON agent_prec_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_prec_audit_time_idx ON agent_prec_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_prec_perception_status_idx ON agent_prec_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_prec_command_expiry_idx ON agent_prec_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_prec_invalidation_certificate_idx ON agent_prec_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_prec_authorization_evidence',
    'agent_prec_profile',
    'agent_prec_profile_containment_rule',
    'agent_prec_certificate_catalog',
    'agent_prec_session',
    'agent_prec_nomination_window',
    'agent_prec_containment_snapshot',
    'agent_prec_evaluation_receipt',
    'agent_prec_containment_certificate',
    'agent_prec_containment_binding',
    'agent_prec_invalidation',
    'agent_prec_effect_intent',
    'agent_prec_budget_ledger',
    'agent_prec_terminal_record',
    'agent_prec_command_result',
    'agent_prec_audit_head',
    'agent_prec_audit_event',
    'agent_prec_audit_anchor',
    'agent_prec_perception_snapshot',
    'agent_prec_projection_checkpoint'
  ]
  LOOP
...
with session CAS. Review-certificate seal never joins a columnar
...
enum AgentPrecClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentPrecAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentPrecUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentPrecInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentPrecBudget {
  nominateUnits: Long!
  evaluateUnits: Long!
  sealUnits: Long!
  vectorUnits: Long!
  invalidateUnits: Long!
  containmentUnits: Long!
  maxWallTimeMs: Long!
...
type AgentPrecSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentPrecSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentPrecBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentPrecNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentPrecSourceKind!
  placementKind: AgentPrecPlacementKind!
  keyLifecycle: AgentPrecKeyLifecycle!
  clockLifecycle: AgentPrecClockLifecycle!
...
type AgentPrecContainmentSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentPrecEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentPrecContainmentCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentPrecBinding {
  ...
  sourceWindowKind: AgentPrecSourceKind!
  bindingOrdinal: Int!
  status: AgentPrecBindingStatus!
  placementKind: AgentPrecPlacementKind!
  keyLifecycle: AgentPrecKeyLifecycle!
  clockLifecycle: AgentPrecClockLifecycle!
  attestedKeyKind: AgentPrecAttestedKeyKind!
  containmentKind: AgentPrecContainmentKind!
  purposeRelation: AgentPrecPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentPrecEffectObservation {
  effectId: ID!
  status: AgentPrecEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentPrecPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentPrecSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  containmentCurrentBindingCount: Int!
  containmentActiveBindingCount: Int!
  containmentExpiredBindingCount: Int!
  containExpiredPromotionResolutionBindingCount: Int!
  holdContainmentWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectResolutionExpiryAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentPrecEffectObservation!]!
  remainingBudget: AgentPrecBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentPrecNextAction!]!
  blockedReasons: [AgentPrecBlockedReason!]!
  cardHash: SHA256!
}

type AgentPrecMutationResult {
  decision: String!
  session: AgentPrecSession
  certificate: AgentPrecContainmentCertificate
  member: AgentPrecBinding
  receipt: AgentPrecNominationReceipt
  snapshot: AgentPrecContainmentSnapshot
  evaluation: AgentPrecEvaluationReceipt
  perception: AgentPrecPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreatePromotionResolutionExpiryContainmentSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentPrecBudgetInput!
}
...
input RefreshPromotionResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluatePromotionResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input SealContainmentCertificateInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  evaluationId: ID!
...
input InvalidatePromotionResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentPrecInvalidationReason!
  idempotencyKey: String!
}

input PrepareContainmentEffectInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
...
input ContainmentEffectUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  promotion: AgentPrecUncertaintyResolution!
  idempotencyKey: String!
}

input AgentPrecProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentPrecSession(accountId: ID!, sessionId: ID!): AgentPrecSession
  agentPrecContainmentCertificate(accountId: ID!, certificateId: ID!): AgentPrecContainmentCertificate
  agentPrecPerceptionCard(accountId: ID!, sessionId: ID!): AgentPrecPerceptionCard
  agentPrecNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentPrecNominationReceipt
  agentPrecSearchProfiles(input: AgentPrecProfileSearchInput!): [AgentPrecProfile!]!
}

type Mutation {
  createPromotionResolutionExpiryContainmentSession(
    input: CreatePromotionResolutionExpiryContainmentSessionInput!
  ): AgentPrecMutationResult!
  nominateSealedCertificate(
    input: NominateSealedCertificateInput!
  ): AgentPrecMutationResult!
  refreshPromotionResolutionExpiryContainment(
    input: RefreshPromotionResolutionExpiryContainmentInput!
  ): AgentPrecMutationResult!
  evaluatePromotionResolutionExpiryContainment(
    input: EvaluatePromotionResolutionExpiryContainmentInput!
  ): AgentPrecMutationResult!
  sealContainmentCertificate(input: SealContainmentCertificateInput!): AgentPrecMutationResult!
  invalidatePromotionResolutionExpiryContainment(
    input: InvalidatePromotionResolutionExpiryContainmentInput!
  ): AgentPrecMutationResult!
  prepareContainmentEffect(input: PrepareContainmentEffectInput!): AgentPrecMutationResult!
  containmentEffectUncertainty(
    input: ContainmentEffectUncertaintyInput!
  ): AgentPrecMutationResult!
  closePromotionResolutionExpiryContainmentSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentPrecMutationResult!
  approvePromotionResolutionExpiryContainmentProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- Perception cards never embed raw private keys, tool payloads, or redacted
  fact bodies.
- `sealContainmentCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  remaining TTL would invent `TRUSTED_ENROLLED`, with
  `RESOLUTION_EXPIRY_AS_CONFIRMATION_DENIED` when a sealed certificate is treated as promotion confirmation, and with
...
Approved containment profiles are procedural memory:
versioned instructions for how sealed promotion-containment-
review-resolution, containment, resolution, review,
escalation, acknowledgement, SLO, window, TTL, and confirm
certificates become envelope-scoped first-enrollment
steward freezes without inventing a winner and without
rewriting a sealed resolution-expiry certificate as
dual-control success or `TRUSTED_ENROLLED`.
Procedure refs may point to successor-containment playbook
steps. Profiles are immutable after approval; agents
perceive `procedureTags` and `allowedNextActions` on
perception cards, never inventing containment policy from
embeddings.

## 11. Semantic retrieval and HNSW compatibility

Profile embeddings support advisory discovery ("which
containment profile fits incident hop-attenuated first-
...
Semantic retrieval may return containment profiles only. It
never authorizes nominate, refresh, evaluate, seal, or
contain. Vector `topK` is budgeted and clamped.

```sql
CREATE TABLE agent_prec_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
sealed / containment-current / containment-active /
containment-expired / contain-expired-promotion-resolution /
hold-containment-window / hold-unknown /
reject-resolution-expiry-as-confirmation / unknown / skipped /
...
`CONTAINMENT_CURRENT` as a first-seen sealed certificate
whose containment window is still open and still cannot
invent a winner, `CONTAINMENT_ACTIVE` as a bounded steward
vote whose remaining time is below the profile
threshold without confirmation, `CONTAINMENT_EXPIRED` as a
sealed certificate that the window closed without dual-
control trust, `CONTAIN_EXPIRED_PROMOTION_RESOLUTION` as a dual-
control sealed-certificate freeze that does not confirm the key,
`HOLD_CONTAINMENT_WINDOW` as a trusted hold that refuses
silent confirmation, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION` as a trusted
negative for a sealed review-expiry certificate that would have
rewritten silence as success, and `SKIP` as a sealed
refusal — never as a key that "must have been trusted
because nobody contained the expired resolution."

## 13. ACID and consistency

### Row store

Session CAS, nomination receipts, review snapshots,
...
review certificates but are not authoritative for
promotion-current, promotion-active, or reject-
resolution-expiry-as-confirmation outcomes.
...
- Binding/threshold caps on promotions per certificate and
  per session.
- Budget ledgers for NOMINATE/EVALUATE/SEAL/VECTOR/INVALIDATE/CONTAIN.
...
- promotion-resolution-expiry-containment refresh: unique
...
reconstructs session, review snapshot, evaluation, and
...
- Historical silence invented as `TRUSTED_ENROLLED` from a
  ...
  and expired-resolution-as-trust fences.
...
- Original containment authorities serving as the review-steward
  pair: prior-resolution-steward-exclusion fence.
...
### Phase 1: shadow containment

Compile profiles and validate certificate-catalog nomination
without durable certificates.

### Phase 2: reject-resolution-expiry-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_RESOLUTION_EXPIRY_AS_CONFIRMATION` or `SKIP`.
Containment-active stays closed.

### Phase 3: expire-only containment and halt-contain fences

Enable budgeted `CONTAINMENT_EXPIRED` from
...
### Phase 4: resolution-expired containments and notify uncertainty

Enable `CONTAINMENT_CURRENT` / `CONTAINMENT_ACTIVE` under
...
budgets, including promotion-resolution-expired containments only for
first-enrollment keys that cannot rewrite an unanswered
escalation as success and cannot auto-trust from resolution
silence.
...
- Refresh promotion-resolution-expiry-containment is a point
...
- Evaluate binds window set, remaining set, and attenuation
  under budget and prior-resolution-steward exclusion.
- Seal is rejected when a sealed certificate would
  become `TRUSTED_ENROLLED`, when containment expiry is treated
...
Adopt the Promotion-Resolution-Expiry Containment
Plane as the deterministic steward-freeze path for
first-enrollment keys whose sealed Promotion-Containment
Review-Expiry Resolution certificates expired without
dual-control resolution, bound by the
Promotion-Containment Review-Expiry Resolution,
Promotion-Containment-Expiry Review, Promotion-Expiry
Containment, Decision-Memory Promotion, Review-Expiry
Resolution, Containment-Expiry Review, Resolution-Expiry
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-promotion-resolution expiry gap
   without history rewrite, silence-success, first-enrollment
   auto-trust, resolution-expiry-as-confirmation, expired-
   resolution-as-trust, ledger scans, halt leak, purpose
   amplification, invented winners, prior-resolution-steward
   reuse, or vote/page/ack/escalation/containment/
   review/resolution/promotion-ledger walks.
