# mondayDB Agentic Promotion-Containment Review-Expiry Resolution Plane
...
A sealed Promotion-Containment-Expiry Review certificate can
dual-control `REVIEW_EXPIRED_PROMOTION_CONTAINMENT` or seal
`REVIEW_EXPIRED` when the review window closes without two
distinct review stewards. It does not decide **how a
`REVIEW_EXPIRED` first-enrollment key is dual-control
resolved** — without scanning every window, TTL, confirm,
SLO, acknowledgement, escalation, containment, review,
resolution, or promotion ledger, rewriting resolution
silence into `TRUSTED_ENROLLED`, treating review expiry as
confirmation, or restoring a halted fact as a winner.
...
already resolves successor stewards after
`CONTAINMENT_EXPIRED`. None of those planes accept
`REVIEW_EXPIRED` as a dual-control resolution that cannot
rewrite silence into `TRUSTED_ENROLLED`.

Without a promotion-containment review-expiry resolution
plane, operators and agents either:
...
- scan every pending window, TTL, confirm, SLO,
  ...
  sealed promotion-containment review is still missing a
  review-expiry resolution" (neighbor-harmful on boards
  with 1M+ rows), or
- treat resolution silence as retroactive engine truth, so
  a missing steward vote is rewritten as confirmation,
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a
  silent resolution auto-promotes `FIRST_ENROLLMENT` to
...
- Treating every sealed promotion-containment-review
  certificate as an implicit resolution maximizes agent
  fluency and reduces re-planning cost, but creates
  history-rewrite invention, review-expiry-as-confirmation,
  expired-review-as-trust, silence-success, first-
  enrollment auto-trust, unauditable resolution storms, and
  recursive review-ledger walks against neighbors.
- Binding a sealed resolution certificate under an approved
  resolution profile, certificate-catalog point lookups,
  history-rewrite fences, silence-success fences,
  first-enrollment auto-trust fences,
  review-expiry-as-confirmation fences,
  expired-review-as-trust fences, halt-resolve fences,
  purpose-amplification fences, successor-leak fences,
  prior-review-steward-exclusion fences, ledger-scan
  fences, and steward budgets adds one bounded evaluate
  transaction and short-lived resolution storage.
- Semantic similarity may discover resolution profiles, but
  it must never decide whether a sealed
  promotion-containment-review, promotion-containment,
  promotion, review, containment, resolution, escalation,
  acknowledgement, SLO, window, TTL, or confirm certificate
  may be nominated, a resolution snapshot refreshed, a
...
1. An approved resolution profile defines allowed source
   ...
   sealed promotion-containment-review certificate into
   `TRUSTED_ENROLLED`.
2. A resolution session opens under purpose, budget, and
   authorization fences, and only nominates sealed
   promotion-containment-review certificates, sealed
   ...
   from the Promotion-Containment-Expiry Review,
...
3. mondayDB evaluates a resolution whose kind is a pure
   ...
   prior_review_steward_set_hash, resolution_steward_count,
   resolution_window_ms, resolution_kind)`. Silence cannot
   become historical success. A sealed certificate seals
   `REJECT_REVIEW_EXPIRY_AS_CONFIRMATION` or
   `RESOLUTION_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW`
   resolves a first-enrollment key after `REVIEW_EXPIRED`
   without inventing confirmation. `HOLD_RESOLUTION_WINDOW`
   holds further resolution for a bounded window without
   restoring a winner. Halted slots cannot become
   resolution-restored winners. Original review stewards
   are excluded from the resolution-steward pair.
...
protection: recursive "re-resolve every sealed
promotion-containment review forever" or "treat
review-expiry silence as confirmation until it looks like
success" loops are rejectable before they scan boards with
1M+ rows. Perception is restored by sealed resolution
certificates, not by magic resolution orchestration inside
the engine.

### Product outcome

For any promotion-containment-review-expiry-resolution
...
- Which nominated sealed certificates, placement kinds,
  ...
  prior-review-steward exclusions, and resolution kinds
  were bound?
...
The Promotion-Containment Review-Expiry Resolution Plane owns:

1. Immutable approved resolution profiles as procedural
   memory of "how REVIEW_EXPIRED first-enrollment keys are dual-control resolved
...
2. Tenant-scoped resolution sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed promotion-containment-review
   certificates, sealed promotion-containment certificates,
   sealed promotion certificates, sealed
...
- **Containment-Expiry Review:** supplies sealed
  review certificates whose `RESOLUTION_EXPIRED`,
  unanswered, or single-control outcomes this plane may
...
- review-window buckets (`resolution_window_ms`), max
  ...
  steward principals for `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW`,
...
- allowed review kinds (`RESOLUTION_CURRENT`,
  ...
  `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW`, `HOLD_RESOLUTION_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_REVIEW_EXPIRY_AS_CONFIRMATION`,
...
Refresh promotion-containment-review-expiry-resolution is a point lookup
...
10. Silence and unknown windows cannot evaluate to
    `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW` or any kind that implies
...
14. A resolved certificate never writes
    ...
    (review-expiry-as-confirmation and expired-promotion-
    as-trust fences).
15. `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW` requires two distinct
...
certificate. Refresh promotion-containment-review-expiry-resolution is a
...
type TrustedNextAction =
  | "NOMINATE_SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE"
  | "REFRESH_RESOLUTION"
  | "EVALUATE_RESOLUTION"
...
type PromotionContainmentReviewExpiryResolutionBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "REVIEW_EXPIRY_AS_CONFIRMATION_DENIED"
...
type ResolutionKind =
  ...
  | "HOLD_UNKNOWN"
  | "REJECT_REVIEW_EXPIRY_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface PromotionContainmentReviewExpiryResolutionBudget {
  ...
  readonly resolutionUnits: number;
...
interface PromotionContainmentReviewExpiryResolutionProfile {
...
interface PromotionContainmentReviewExpiryResolutionSession {
  readonly accountId: AccountId;
  ...
  readonly budget: PromotionContainmentReviewExpiryResolutionBudget;
  readonly consumed: Omit<
    PromotionContainmentReviewExpiryResolutionBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface PromotionContainmentReviewExpiryResolutionSnapshot {
  readonly accountId: AccountId;
...
interface PromotionContainmentReviewExpiryResolutionEvaluationReceipt {
  readonly accountId: AccountId;
...
interface PromotionContainmentReviewExpiryResolutionBinding {
  readonly accountId: AccountId;
...
interface PromotionContainmentReviewExpiryResolutionCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface PromotionContainmentReviewExpiryResolutionEffectObservation {
...
interface AgentPromotionContainmentReviewExpiryResolutionPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  ...
  readonly resolveExpiredPromotionContainmentReviewBindingCount: number;
  readonly holdResolutionWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectReviewExpiryAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly PromotionContainmentReviewExpiryResolutionEffectObservation[];
  ...
  readonly blockedReasons: readonly PromotionContainmentReviewExpiryResolutionBlockedReason[];
...
```ts
interface CreatePromotionContainmentReviewExpiryResolutionSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshPromotionContainmentReviewExpiryResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluatePromotionContainmentReviewExpiryResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidatePromotionContainmentReviewExpiryResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ClosePromotionContainmentReviewExpiryResolutionSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type PromotionContainmentReviewExpiryResolutionDecision =
  | { readonly decision: "ALLOWED"; readonly session: PromotionContainmentReviewExpiryResolutionSession;
      readonly certificate?: PromotionContainmentReviewExpiryResolutionCertificate;
      readonly member?: PromotionContainmentReviewExpiryResolutionBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: PromotionContainmentReviewExpiryResolutionSnapshot;
      readonly evaluation?: PromotionContainmentReviewExpiryResolutionEvaluationReceipt;
      readonly perception: AgentPromotionContainmentReviewExpiryResolutionPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: PromotionContainmentReviewExpiryResolutionBlockedReason;
...
```sql
CREATE TYPE pcrer_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE pcrer_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_source_kind AS ENUM (
  'SEALED_PROMOTION_CONTAINMENT_REVIEW_CERTIFICATE',
...
CREATE TYPE pcrer_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_resolution_kind AS ENUM (
  'RESOLUTION_CURRENT', 'RESOLUTION_ACTIVE', 'RESOLUTION_EXPIRED',
  'RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW', 'HOLD_RESOLUTION_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_REVIEW_EXPIRY_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pcrer_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_pcrer_profile_authority NOLOGIN;

CREATE TABLE agent_pcrer_authorization_evidence (
...
CREATE TABLE agent_pcrer_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status pcrer_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_pcrer_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_pcrer_profile_resolution_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_pcrer_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_pcrer_certificate_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind pcrer_source_kind NOT NULL,
  placement_kind pcrer_placement_kind NOT NULL,
  key_lifecycle pcrer_key_lifecycle NOT NULL,
  clock_lifecycle pcrer_clock_lifecycle NOT NULL,
  status pcrer_catalog_status NOT NULL,
...
CREATE TABLE agent_pcrer_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status pcrer_session_status NOT NULL,
  ...
  budget_resolution_units BIGINT NOT NULL CHECK (budget_resolution_units >= 0),
  ...
  consumed_resolution_units BIGINT NOT NULL
    CHECK (consumed_resolution_units >= 0),
  ...
  CHECK (consumed_resolution_units <= budget_resolution_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_pcrer_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pcrer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pcrer_certificate_catalog (account_id, source_window_id)
);

CREATE TABLE agent_pcrer_resolution_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_resolution_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_pcrer_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_pcrer_resolution_binding (
  account_id BIGINT NOT NULL,
  ...
  status pcrer_binding_status NOT NULL,
  placement_kind pcrer_placement_kind NOT NULL,
  key_lifecycle pcrer_key_lifecycle NOT NULL,
  clock_lifecycle pcrer_clock_lifecycle NOT NULL,
  attested_key_kind pcrer_attested_key_kind NOT NULL,
  resolution_kind pcrer_resolution_kind NOT NULL,
  purpose_relation pcrer_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_pcrer_resolution_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pcrer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pcrer_certificate_catalog (account_id, source_window_id)
...
CREATE TABLE agent_pcrer_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status pcrer_effect_status NOT NULL,
...
CREATE TABLE agent_pcrer_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_pcrer_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pcrer_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status pcrer_session_status NOT NULL,
...
CREATE TABLE agent_pcrer_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_pcrer_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.pcrer_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.pcrer_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_pcrer_profile_protect
BEFORE INSERT OR UPDATE ON agent_pcrer_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcrer_profile();

CREATE FUNCTION protect_agent_pcrer_profile_resolution_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status pcrer_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_pcrer_profile
...
CREATE TRIGGER agent_pcrer_profile_resolution_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_pcrer_profile_resolution_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcrer_profile_resolution_rule();

CREATE FUNCTION protect_agent_pcrer_resolution_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_pcrer_resolution_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_pcrer_resolution_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcrer_resolution_snapshot();

CREATE FUNCTION protect_agent_pcrer_resolution_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.resolution_kind IN (
       'RESOLUTION_CURRENT', 'RESOLUTION_ACTIVE', 'RESOLUTION_EXPIRED',
       'RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW', 'REJECT_REVIEW_EXPIRY_AS_CONFIRMATION'
     )
  ...
  IF NEW.resolution_kind = 'REJECT_REVIEW_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'review-expiry-as-confirmation fence blocks TRUSTED_ENROLLED from a sealed review-expiry certificate';
  END IF;

  IF NEW.resolution_kind = 'RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW'
  ...
  IF NEW.resolution_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from promotion-containment-review-expiry-resolution';
  END IF;
  ...
  IF NEW.resolution_kind IN (
       'RESOLUTION_CURRENT', 'RESOLUTION_ACTIVE', 'RESOLUTION_EXPIRED', 'RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW'
     )
  ...
  IF NEW.resolution_kind IN (
       'RESOLUTION_CURRENT', 'RESOLUTION_ACTIVE', 'RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-resolve fence blocks review that would restore a halted key';
  END IF;
...
CREATE TRIGGER agent_pcrer_resolution_binding_protect
BEFORE INSERT OR UPDATE ON agent_pcrer_resolution_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcrer_resolution_binding();

CREATE FUNCTION protect_agent_pcrer_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_pcrer_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_pcrer_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_pcrer_effect_intent();

CREATE FUNCTION approve_agent_pcrer_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status pcrer_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_pcrer_profile_resolution_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_pcrer_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_pcrer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pcrer_profile_authority;
ALTER FUNCTION revoke_agent_pcrer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pcrer_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_pcrer_profile_authority;
GRANT SELECT ON
  agent_pcrer_profile,
  agent_pcrer_profile_resolution_rule
TO agent_pcrer_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pcrer_profile TO agent_pcrer_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_pcrer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_pcrer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pcrer_profile FROM PUBLIC;

CREATE INDEX agent_pcrer_session_work_idx ON agent_pcrer_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_pcrer_session_profile_idx ON agent_pcrer_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_pcrer_binding_certificate_idx ON agent_pcrer_resolution_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pcrer_binding_window_idx ON agent_pcrer_resolution_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pcrer_catalog_ref_idx ON agent_pcrer_certificate_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pcrer_catalog_kind_idx ON agent_pcrer_certificate_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pcrer_evaluation_session_idx ON agent_pcrer_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_pcrer_certificate_session_idx ON agent_pcrer_resolution_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_pcrer_snapshot_session_idx ON agent_pcrer_resolution_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_pcrer_effect_work_idx ON agent_pcrer_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_pcrer_audit_time_idx ON agent_pcrer_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_pcrer_perception_status_idx ON agent_pcrer_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_pcrer_command_expiry_idx ON agent_pcrer_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_pcrer_invalidation_certificate_idx ON agent_pcrer_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_pcrer_authorization_evidence',
    'agent_pcrer_profile',
    'agent_pcrer_profile_resolution_rule',
    'agent_pcrer_certificate_catalog',
    'agent_pcrer_session',
    'agent_pcrer_nomination_window',
    'agent_pcrer_resolution_snapshot',
    'agent_pcrer_evaluation_receipt',
    'agent_pcrer_resolution_certificate',
    'agent_pcrer_resolution_binding',
    'agent_pcrer_invalidation',
    'agent_pcrer_effect_intent',
    'agent_pcrer_budget_ledger',
    'agent_pcrer_terminal_record',
    'agent_pcrer_command_result',
    'agent_pcrer_audit_head',
    'agent_pcrer_audit_event',
    'agent_pcrer_audit_anchor',
    'agent_pcrer_perception_snapshot',
    'agent_pcrer_projection_checkpoint'
  ]
  LOOP
...
enum AgentPcrerClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentPcrerAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentPcrerPurposeRelation {
  EQUAL
  NARROWS
  AMPLIFIES
  UNRELATED
...
enum AgentPcrerUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentPcrerInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentPcrerBudget {
  nominateUnits: Long!
  evaluateUnits: Long!
  sealUnits: Long!
  vectorUnits: Long!
  invalidateUnits: Long!
  resolutionUnits: Long!
  maxWallTimeMs: Long!
...
type AgentPcrerSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentPcrerSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentPcrerBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentPcrerNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentPcrerSourceKind!
  placementKind: AgentPcrerPlacementKind!
  keyLifecycle: AgentPcrerKeyLifecycle!
  clockLifecycle: AgentPcrerClockLifecycle!
...
type AgentPcrerResolutionSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentPcrerEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentPcrerResolutionCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentPcrerBinding {
  ...
  sourceWindowKind: AgentPcrerSourceKind!
  bindingOrdinal: Int!
  status: AgentPcrerBindingStatus!
  placementKind: AgentPcrerPlacementKind!
  keyLifecycle: AgentPcrerKeyLifecycle!
  clockLifecycle: AgentPcrerClockLifecycle!
  attestedKeyKind: AgentPcrerAttestedKeyKind!
  resolutionKind: AgentPcrerResolutionKind!
  purposeRelation: AgentPcrerPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentPcrerEffectObservation {
  effectId: ID!
  status: AgentPcrerEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentPcrerPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentPcrerSessionStatus!
  ...
  resolveExpiredPromotionContainmentReviewBindingCount: Int!
  holdResolutionWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectReviewExpiryAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentPcrerEffectObservation!]!
  remainingBudget: AgentPcrerBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentPcrerNextAction!]!
  blockedReasons: [AgentPcrerBlockedReason!]!
  cardHash: SHA256!
}

type AgentPcrerMutationResult {
  decision: String!
  session: AgentPcrerSession
  certificate: AgentPcrerResolutionCertificate
  member: AgentPcrerBinding
  receipt: AgentPcrerNominationReceipt
  snapshot: AgentPcrerResolutionSnapshot
  evaluation: AgentPcrerEvaluationReceipt
  perception: AgentPcrerPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreatePromotionContainmentReviewExpiryResolutionSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentPcrerBudgetInput!
}
...
input RefreshPromotionContainmentReviewExpiryResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluatePromotionContainmentReviewExpiryResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidatePromotionContainmentReviewExpiryResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentPcrerInvalidationReason!
  idempotencyKey: String!
}
...
input ResolutionEffectUncertaintyInput {
  ...
  expectedRevision: Long!
  effectId: ID!
  promotion: AgentPcrerUncertaintyResolution!
  idempotencyKey: String!
}

input AgentPcrerProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentPcrerSession(accountId: ID!, sessionId: ID!): AgentPcrerSession
  agentPcrerResolutionCertificate(accountId: ID!, certificateId: ID!): AgentPcrerResolutionCertificate
  agentPcrerPerceptionCard(accountId: ID!, sessionId: ID!): AgentPcrerPerceptionCard
  agentPcrerNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentPcrerNominationReceipt
  agentPcrerSearchProfiles(input: AgentPcrerProfileSearchInput!): [AgentPcrerProfile!]!
}

type Mutation {
  createPromotionContainmentReviewExpiryResolutionSession(
    input: CreatePromotionContainmentReviewExpiryResolutionSessionInput!
  ): AgentPcrerMutationResult!
  nominateSealedCertificate(
    input: NominateSealedCertificateInput!
  ): AgentPcrerMutationResult!
  refreshPromotionContainmentReviewExpiryResolution(
    input: RefreshPromotionContainmentReviewExpiryResolutionInput!
  ): AgentPcrerMutationResult!
  evaluatePromotionContainmentReviewExpiryResolution(
    input: EvaluatePromotionContainmentReviewExpiryResolutionInput!
  ): AgentPcrerMutationResult!
  sealResolutionCertificate(input: SealResolutionCertificateInput!): AgentPcrerMutationResult!
  invalidatePromotionContainmentReviewExpiryResolution(
    input: InvalidatePromotionContainmentReviewExpiryResolutionInput!
  ): AgentPcrerMutationResult!
  prepareResolutionEffect(input: PrepareResolutionEffectInput!): AgentPcrerMutationResult!
  resolutionEffectUncertainty(
    input: ResolutionEffectUncertaintyInput!
  ): AgentPcrerMutationResult!
  closePromotionContainmentReviewExpiryResolutionSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentPcrerMutationResult!
  approvePromotionContainmentReviewExpiryResolutionProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealResolutionCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `REVIEW_EXPIRY_AS_CONFIRMATION_DENIED` when a sealed certificate is treated as promotion confirmation, and with
...
review, containment, resolution, review, escalation,
acknowledgement, SLO, window, TTL, and confirm certificates
become envelope-scoped first-enrollment steward resolutions
without inventing a winner and without rewriting a sealed
review-expiry certificate as dual-control success or
`TRUSTED_ENROLLED`.
Procedure refs may point to successor-resolution playbook
steps. Profiles are immutable after approval; agents
perceive `procedureTags` and `allowedNextActions` on
perception cards, never inventing resolution policy from
embeddings.
...
```sql
CREATE TABLE agent_pcrer_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
resolution-expired / resolve-expired-promotion-containment-review /
hold-resolution-window / hold-unknown /
reject-review-expiry-as-confirmation / unknown / skipped /
...
`RESOLUTION_CURRENT` as a first-seen sealed certificate
whose resolution window is still open and still cannot
...
control trust, `RESOLVE_EXPIRED_PROMOTION_CONTAINMENT_REVIEW` as a dual-
control sealed-certificate resolution that does not confirm the key,
`HOLD_RESOLUTION_WINDOW` as a trusted hold that refuses
silent confirmation, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_REVIEW_EXPIRY_AS_CONFIRMATION` as a trusted
...
because nobody resolved the expired review."
...
- promotion-containment-review-expiry-resolution refresh: unique
...
`REJECT_REVIEW_EXPIRY_AS_CONFIRMATION` or `SKIP`.
...
### Phase 4: review-expired resolutions and notify uncertainty
...
- Refresh promotion-containment-review-expiry-resolution is a point
...
Adopt the Promotion-Containment Review-Expiry Resolution
Plane as the deterministic steward-resolution path for
first-enrollment keys whose sealed
Promotion-Containment-Expiry Review certificates expired
without dual-control review, bound by the
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-promotion-containment-review expiry gap
   without history rewrite, silence-success, first-enrollment
   auto-trust, review-expiry-as-confirmation, expired-
   review-as-trust, ledger scans, halt leak, purpose
   amplification, invented winners, prior-review-steward
