# mondayDB Agentic Promotion-Expiry Containment Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-09-06.v1`

## 1. Why this plane, before how

A sealed Decision-Memory Promotion certificate can
dual-control `PROMOTE_SEALED_CERTIFICATE` or seal
`PROMOTION_EXPIRED` when the promotion window closes
without two distinct promotion stewards. It does not decide
**how a `PROMOTION_EXPIRED` first-enrollment key is frozen**
— without scanning every window, TTL, confirm, SLO,
acknowledgement, escalation, containment, review,
resolution, or promotion ledger, rewriting containment
silence into `TRUSTED_ENROLLED`, treating promotion expiry
as confirmation, or restoring a halted fact as a winner.
...
placement-aware promotion receipts. None of those planes
accept `PROMOTION_EXPIRED` as a dual-control freeze that
cannot rewrite silence into `TRUSTED_ENROLLED`.

Without a promotion-expiry containment plane, operators
and agents either:

- scan every pending window, TTL, confirm, SLO,
  acknowledgement, escalation, containment, review,
  resolution, page, and vote ledger looking for "which
  sealed certificate is still missing a promotion-expiry freeze" (neighbor-harmful on boards with 1M+ rows), or
- treat containment silence as retroactive engine truth, so
  a missing freeze is rewritten as
...
- Treating every sealed governance certificate as an
  implicit freeze maximizes agent fluency
  and reduces re-planning cost, but creates
  history-rewrite invention, promotion-expiry-as-confirmation,
  expired-promotion-as-trust, silence-success, first-
...
- Binding a sealed containment certificate under an
  ...
  promotion-expiry-as-confirmation fences,
  expired-promotion-as-trust fences, halt-contain
  fences, purpose-amplification fences, successor-leak
  fences, prior-promotion-steward-exclusion fences,
  ledger-scan fences, and steward budgets adds one
  bounded evaluate transaction and short-lived containment
  storage.
- Semantic similarity may discover containment profiles,
  but it must never decide whether a sealed promotion,
...
1. An approved containment profile defines allowed source
   ...
   containment-steward policy, prior-promotion-steward
...
2. A containment session opens under purpose, budget, and
   ...
   promotion certificates, sealed resolution certificates,
   sealed review certificates, sealed containment
   ...
   sealed confirm certificates, or first-enrollment claims
   by point lookup from the Decision-Memory Promotion,
...
3. mondayDB evaluates a containment whose kind is a pure
   ...
   prior_promotion_steward_set_hash,
   ...
   `REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION` or
   `CONTAINMENT_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `CONTAIN_EXPIRED_PROMOTION` freezes a
   first-enrollment key after `PROMOTION_EXPIRED` without
   ...
   Original promotion stewards are excluded from the
...
certificate forever" or "treat containment silence
...
For any promotion-expiry-containment evaluation,
mondayDB can answer:
...
- Which nominated sealed certificates, placement kinds,
  ...
  prior-promotion-steward exclusions, and containment kinds were
  bound?
...
The Promotion-Expiry Containment Plane owns:

1. Immutable approved containment profiles as procedural
   memory of "how PROMOTION_EXPIRED first-enrollment keys are dual-control frozen
   ...
   `TRUSTED_ENROLLED` from containment silence."
2. Tenant-scoped containment sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed promotion
   ...
   page-ledger, ack-ledger, escalation-ledger,
   review-ledger, resolution-ledger, promotion-ledger,
...
- **Decision-Memory Promotion:** supplies sealed
  ...
  a new promotion by walking promotion bindings.
...
- **Unanswered-Ack Escalation:** supplies sealed escalation
  ...
  escalations this plane may bind as containment evidence. This
...
- **Certificate Placement / Envelope Purpose Gate / Decision
  ...
  answer; it only seals the containment binding those
...
- Scanning vote, TTL, page, ack, escalation, review,
  resolution, or containment ledgers to decide who may contain.
...
- allowed source kinds (`SEALED_PROMOTION_CERTIFICATE`,
...
- containment-window buckets (`containment_window_ms`), max
  ...
  steward principals for `CONTAIN_EXPIRED_PROMOTION`,
...
- promotion policy (`HISTORY_NEVER_REWRITTEN`,
  ...
  `PROMOTION_EXPIRY_NEVER_CONFIRMS`, `EXPIRED_PROMOTION_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_CONTAINMENT_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_PROMOTION_STEWARD_NEVER_CONTAINS`);
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed promotion kinds (`CONTAINMENT_CURRENT`,
  `CONTAINMENT_ACTIVE`, `CONTAINMENT_EXPIRED`,
  `CONTAIN_EXPIRED_PROMOTION`, `HOLD_CONTAINMENT_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION`,
  `SKIP`) and notify policy after seal, invalidation, or
  upstream key or clock change;
- optional procedural refs for "how to contain expired sealed
  governance certificates after PROMOTION_EXPIRED without inventing a winner."
...
Refresh promotion-expiry-containment is a point lookup
...
dual-control containment-steward counts, prior-resolution-
...
`contained_fact_hash`. A sealed certificate cannot
...
10. Silence and unknown windows cannot evaluate to
    `CONTAIN_EXPIRED_PROMOTION` or any kind that implies
...
14. A promoted certificate never writes
    ...
    (promotion-expiry-as-confirmation and expired-promotion-
    as-trust fences).
15. `CONTAIN_EXPIRED_PROMOTION` requires two distinct
...
certificate. Refresh promotion-expiry-containment is a
...
type TrustedNextAction =
  | "NOMINATE_SEALED_PROMOTION_CERTIFICATE"
  | "REFRESH_CONTAINMENT"
...
type PromotionExpiryContainmentBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "PROMOTION_EXPIRY_AS_CONFIRMATION_DENIED"
  | "EXPIRED_PROMOTION_AS_TRUST_DENIED"
  | "PRIOR_PROMOTION_STEWARD_EXCLUSION_DENIED"
  | "LEDGER_SCAN_DENIED"
  | "HALT_CONTAIN_DENIED"
...
type ContainmentKind =
  ...
  | "CONTAINMENT_ACTIVE"
  | "CONTAINMENT_EXPIRED"
  | "CONTAIN_EXPIRED_PROMOTION"
  | "HOLD_CONTAINMENT_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface PromotionExpiryContainmentBudget {
...
interface PromotionExpiryContainmentProfile {
...
interface PromotionExpiryContainmentSession {
  readonly accountId: AccountId;
  ...
  readonly budget: PromotionExpiryContainmentBudget;
  readonly consumed: Omit<
    PromotionExpiryContainmentBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface PromotionExpiryContainmentSnapshot {
  readonly accountId: AccountId;
...
interface PromotionExpiryContainmentEvaluationReceipt {
  readonly accountId: AccountId;
...
interface PromotionExpiryContainmentBinding {
  readonly accountId: AccountId;
...
interface PromotionExpiryContainmentCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface PromotionExpiryContainmentEffectObservation {
...
interface AgentPromotionExpiryContainmentPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  ...
  readonly containExpiredPromotionBindingCount: number;
  readonly holdContainmentWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectPromotionExpiryAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly PromotionExpiryContainmentEffectObservation[];
  ...
  readonly blockedReasons: readonly PromotionExpiryContainmentBlockedReason[];
...
```ts
interface CreatePromotionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshPromotionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluatePromotionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidatePromotionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ClosePromotionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type PromotionExpiryContainmentDecision =
  | { readonly decision: "ALLOWED"; readonly session: PromotionExpiryContainmentSession;
      readonly certificate?: PromotionExpiryContainmentCertificate;
      readonly member?: PromotionExpiryContainmentBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: PromotionExpiryContainmentSnapshot;
      readonly evaluation?: PromotionExpiryContainmentEvaluationReceipt;
      readonly perception: AgentPromotionExpiryContainmentPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: PromotionExpiryContainmentBlockedReason;
...
```sql
CREATE TYPE pec_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE pec_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_source_kind AS ENUM (
  'SEALED_PROMOTION_CERTIFICATE', 'SEALED_TTL_CERTIFICATE',
  'SEALED_CONFIRM_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM',
...
CREATE TYPE pec_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE pec_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_containment_kind AS ENUM (
  'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED',
  'CONTAIN_EXPIRED_PROMOTION', 'HOLD_CONTAINMENT_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE pec_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_pec_profile_authority NOLOGIN;

CREATE TABLE agent_pec_authorization_evidence (
...
CREATE TABLE agent_pec_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status pec_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_pec_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_pec_profile_containment_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_pec_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_pec_certificate_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind pec_source_kind NOT NULL,
  placement_kind pec_placement_kind NOT NULL,
  key_lifecycle pec_key_lifecycle NOT NULL,
  clock_lifecycle pec_clock_lifecycle NOT NULL,
  status pec_catalog_status NOT NULL,
...
CREATE TABLE agent_pec_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status pec_session_status NOT NULL,
...
CREATE TABLE agent_pec_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pec_certificate_catalog (account_id, source_window_id)
);

CREATE TABLE agent_pec_containment_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_containment_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_pec_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_pec_containment_binding (
  account_id BIGINT NOT NULL,
  ...
  status pec_binding_status NOT NULL,
  placement_kind pec_placement_kind NOT NULL,
  key_lifecycle pec_key_lifecycle NOT NULL,
  clock_lifecycle pec_clock_lifecycle NOT NULL,
  attested_key_kind pec_attested_key_kind NOT NULL,
  containment_kind pec_containment_kind NOT NULL,
  purpose_relation pec_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_pec_containment_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_pec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_pec_certificate_catalog (account_id, source_window_id)
...
CREATE TABLE agent_pec_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status pec_effect_status NOT NULL,
...
CREATE TABLE agent_pec_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_pec_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_pec_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status pec_session_status NOT NULL,
...
CREATE TABLE agent_pec_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_pec_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.pec_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.pec_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_pec_profile_protect
BEFORE INSERT OR UPDATE ON agent_pec_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_pec_profile();

CREATE FUNCTION protect_agent_pec_profile_containment_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status pec_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_pec_profile
...
CREATE TRIGGER agent_pec_profile_containment_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_pec_profile_containment_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_pec_profile_containment_rule();

CREATE FUNCTION protect_agent_pec_containment_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_pec_containment_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_pec_containment_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_pec_containment_snapshot();

CREATE FUNCTION protect_agent_pec_containment_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED',
       'CONTAIN_EXPIRED_PROMOTION', 'REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION'
     )
  ...
  IF NEW.containment_kind = 'REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'promotion-expiry-as-confirmation fence blocks TRUSTED_ENROLLED from a sealed governance certificate';
  END IF;

  IF NEW.containment_kind = 'CONTAIN_EXPIRED_PROMOTION'
  ...
  IF NEW.containment_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from promotion-expiry-containment';
  END IF;
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED', 'CONTAIN_EXPIRED_PROMOTION'
     )
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAIN_EXPIRED_PROMOTION'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-contain fence blocks promotion that would restore a halted key';
  END IF;
...
CREATE TRIGGER agent_pec_containment_binding_protect
BEFORE INSERT OR UPDATE ON agent_pec_containment_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_pec_containment_binding();

CREATE FUNCTION protect_agent_pec_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_pec_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_pec_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_pec_effect_intent();

CREATE FUNCTION approve_agent_pec_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status pec_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_pec_profile_containment_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_pec_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_pec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pec_profile_authority;
ALTER FUNCTION revoke_agent_pec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_pec_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_pec_profile_authority;
GRANT SELECT ON
  agent_pec_profile,
  agent_pec_profile_containment_rule
TO agent_pec_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pec_profile TO agent_pec_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_pec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_pec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_pec_profile FROM PUBLIC;

CREATE INDEX agent_pec_session_work_idx ON agent_pec_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_pec_session_profile_idx ON agent_pec_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_pec_binding_certificate_idx ON agent_pec_containment_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pec_binding_window_idx ON agent_pec_containment_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_pec_catalog_ref_idx ON agent_pec_certificate_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pec_catalog_kind_idx ON agent_pec_certificate_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_pec_evaluation_session_idx ON agent_pec_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_pec_certificate_session_idx ON agent_pec_containment_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_pec_snapshot_session_idx ON agent_pec_containment_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_pec_effect_work_idx ON agent_pec_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_pec_audit_time_idx ON agent_pec_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_pec_perception_status_idx ON agent_pec_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_pec_command_expiry_idx ON agent_pec_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_pec_invalidation_certificate_idx ON agent_pec_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_pec_authorization_evidence',
    'agent_pec_profile',
    'agent_pec_profile_containment_rule',
    'agent_pec_certificate_catalog',
    'agent_pec_session',
    'agent_pec_nomination_window',
    'agent_pec_containment_snapshot',
    'agent_pec_evaluation_receipt',
    'agent_pec_containment_certificate',
    'agent_pec_containment_binding',
    'agent_pec_invalidation',
    'agent_pec_effect_intent',
    'agent_pec_budget_ledger',
    'agent_pec_terminal_record',
    'agent_pec_command_result',
    'agent_pec_audit_head',
    'agent_pec_audit_event',
    'agent_pec_audit_anchor',
    'agent_pec_perception_snapshot',
    'agent_pec_projection_checkpoint'
  ]
  LOOP
...
enum AgentPecClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentPecAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentPecUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentPecInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentPecSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentPecSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentPecBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentPecNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentPecSourceKind!
  placementKind: AgentPecPlacementKind!
  keyLifecycle: AgentPecKeyLifecycle!
  clockLifecycle: AgentPecClockLifecycle!
...
type AgentPecPromotionExpiryContainmentSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentPecEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentPecContainmentCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentPecBinding {
  ...
  sourceWindowKind: AgentPecSourceKind!
  bindingOrdinal: Int!
  status: AgentPecBindingStatus!
  placementKind: AgentPecPlacementKind!
  keyLifecycle: AgentPecKeyLifecycle!
  clockLifecycle: AgentPecClockLifecycle!
  attestedKeyKind: AgentPecAttestedKeyKind!
  containmentKind: AgentPecContainmentKind!
  purposeRelation: AgentPecPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentPecEffectObservation {
  effectId: ID!
  status: AgentPecEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentPecPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentPecSessionStatus!
  ...
  containExpiredPromotionBindingCount: Int!
  holdContainmentWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectPromotionExpiryAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentPecEffectObservation!]!
  remainingBudget: AgentPecBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentPecNextAction!]!
  blockedReasons: [AgentPecBlockedReason!]!
  cardHash: SHA256!
}

type AgentPecMutationResult {
  decision: String!
  session: AgentPecSession
  certificate: AgentPecContainmentCertificate
  member: AgentPecBinding
  receipt: AgentPecNominationReceipt
  snapshot: AgentPecPromotionExpiryContainmentSnapshot
  evaluation: AgentPecEvaluationReceipt
  perception: AgentPecPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreatePromotionExpiryContainmentSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentPecBudgetInput!
}
...
input RefreshPromotionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluatePromotionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidatePromotionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentPecInvalidationReason!
  idempotencyKey: String!
}
...
input ContainmentEffectUncertaintyInput {
  ...
  expectedRevision: Long!
  effectId: ID!
  promotion: AgentPecUncertaintyResolution!
  idempotencyKey: String!
}

input AgentPecProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentPecSession(accountId: ID!, sessionId: ID!): AgentPecSession
  agentPecContainmentCertificate(accountId: ID!, certificateId: ID!): AgentPecContainmentCertificate
  agentPecPerceptionCard(accountId: ID!, sessionId: ID!): AgentPecPerceptionCard
  agentPecNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentPecNominationReceipt
  agentPecSearchProfiles(input: AgentPecProfileSearchInput!): [AgentPecProfile!]!
}

type Mutation {
  createPromotionExpiryContainmentSession(
    input: CreatePromotionExpiryContainmentSessionInput!
  ): AgentPecMutationResult!
  nominateSealedCertificate(
    input: NominateSealedCertificateInput!
  ): AgentPecMutationResult!
  refreshPromotionExpiryContainment(
    input: RefreshPromotionExpiryContainmentInput!
  ): AgentPecMutationResult!
  evaluatePromotionExpiryContainment(
    input: EvaluatePromotionExpiryContainmentInput!
  ): AgentPecMutationResult!
  sealContainmentCertificate(input: SealContainmentCertificateInput!): AgentPecMutationResult!
  invalidatePromotionExpiryContainment(
    input: InvalidatePromotionExpiryContainmentInput!
  ): AgentPecMutationResult!
  prepareContainmentEffect(input: PrepareContainmentEffectInput!): AgentPecMutationResult!
  containmentEffectUncertainty(
    input: ContainmentEffectUncertaintyInput!
  ): AgentPecMutationResult!
  closePromotionExpiryContainmentSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentPecMutationResult!
  approvePromotionExpiryContainmentProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealContainmentCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `PROMOTION_EXPIRY_AS_CONFIRMATION_DENIED` when a sealed certificate is treated as promotion confirmation, and with
...
```sql
CREATE TABLE agent_pec_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
reject-promotion-expiry-as-confirmation / unknown / skipped /
...
control trust, `CONTAIN_EXPIRED_PROMOTION` as a dual-
...
window, `REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION` as a trusted
...
promotion-expiry-as-confirmation outcomes.
...
- promotion-expiry-containment refresh: unique
...
- Historical silence invented as `TRUSTED_ENROLLED` from a
  ...
  and expired-promotion-as-trust fences.
...
### Phase 2: reject-promotion-expiry-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_PROMOTION_EXPIRY_AS_CONFIRMATION` or `SKIP`.
...
- Refresh promotion-expiry-containment is a point
...
- Seal is rejected when a sealed certificate would
  become `TRUSTED_ENROLLED`, when promotion expiry is treated
...
Adopt the Promotion-Expiry Containment Plane as the
deterministic steward-containment path for first-enrollment
keys whose sealed Decision-Memory Promotion certificates
expired without dual-control promotion, bound by the
Decision-Memory Promotion, Review-Expiry Resolution,
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-promotion expiry gap without
   history rewrite, silence-success, first-enrollment auto-
   trust, promotion-expiry-as-confirmation, expired-
   promotion-as-trust, ledger scans, halt leak, purpose
   amplification, invented winners, prior-promotion-
