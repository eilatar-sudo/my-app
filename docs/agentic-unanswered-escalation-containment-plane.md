# mondayDB Agentic Unanswered-Escalation Containment Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-08-30.v1`

## 1. Why this plane, before how

A sealed unanswered-ack escalation certificate can page a
containment-authority after dual-control
`ESCALATE_UNANSWERED_ACK`. It does not decide **how an
unanswered, expired, or single-control escalation contains a
first-enrollment key** — without scanning every escalation
binding, rewriting unanswered-escalation silence into
`TRUSTED_ENROLLED`, or treating pager silence as confirmation.

Without an unanswered-escalation containment plane, operators
and agents either:

- scan every pending escalation, acknowledgement, page, and
  vote ledger looking for "who still has not dual-control
  contained the unanswered escalation" (neighbor-harmful on
  boards with 1M+ rows), or
- treat unanswered escalation silence as retroactive engine
  truth, so escalation silence is rewritten as confirmation,
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a
  silent containment auto-promotes `FIRST_ENROLLMENT` to
  `TRUSTED_ENROLLED`, a halt-scoped body is "unlocked" by
  later containment, and hop-attenuated purpose is amplified
  back to the donor.

The product trade-off is **containment fluency versus
containment isolation**:

- Treating every unanswered escalation, pager timeout, or
  containment timer as implicit confirmation maximizes agent
  fluency and reduces re-planning cost, but creates history-
  rewrite invention, containment-as-confirmation, unanswered-
  escalation-as-trust, silence-success, first-enrollment
  auto-trust, unauditable containment storms, and recursive
  escalation-ledger walks against neighbors.
- Binding a sealed containment certificate under an approved
  containment profile, escalation-catalog point lookups,
  history-rewrite fences, silence-success fences, first-
  enrollment auto-trust fences, containment-as-confirmation
  fences, unanswered-escalation-as-trust fences, halt-contain
  fences, purpose-amplification fences, successor-leak
  fences, prior-escalator-exclusion fences, ledger-scan
  fences, and steward budgets adds one bounded evaluate
  transaction and short-lived containment storage.
- Semantic similarity may discover containment profiles, but
  it must never decide whether a sealed escalation,
  acknowledgement, SLO, window, TTL, or confirm certificate
  may be nominated, a containment snapshot refreshed, a
  certificate sealed, or a first-enrollment key frozen.
...
1. An approved containment profile defines allowed source
   ...
   containment-authority policy, prior-escalator exclusion,
   and notify policy. Evaluation **never** invents a winning
   fact hash and **never** rewrites an unanswered escalation
   into `TRUSTED_ENROLLED`.
2. A containment session opens under purpose, budget, and
   ...
   from the Unanswered-Ack Escalation, SLO Ack Suppression,
   Remaining-Window SLO, Confirmation-Window Observability,
   Confirmation-TTL Retirement, and First-Enrollment
   Confirmation planes.
3. mondayDB evaluates a containment whose kind is a pure
   ...
   now_attested, ttl_expired, escalation_count,
   distinct_escalator_count, prior_escalator_set_hash,
   containment_authority_count, containment_window_ms,
   containment_kind)`. Silence cannot become historical
   success. An unanswered escalation seals
   `REJECT_CONTAINMENT_AS_CONFIRMATION` or
   `CONTAINMENT_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `CONTAIN_UNANSWERED_ESCALATION` freezes the
   first-enrollment key without inventing confirmation.
   `HOLD_CONTAINMENT_WINDOW` holds further containment for a
   bounded window without restoring a winner. Halted slots
   cannot become contain-restored winners. Original
   escalators are excluded from the containment-authority
   pair.
...
6. Unscoped key-catalog, clock-catalog, confirm-ledger, vote-
   ...
   ledger, working-set, grant-graph, or board scans are
   **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor
protection: recursive "re-contain every unanswered
escalation forever" or "treat unanswered escalation silence
as confirmation until it looks like success" loops are
rejectable before they scan boards with 1M+ rows. Perception
is restored by sealed containment certificates, not by magic
containment orchestration inside the engine.

### Product outcome

For any unanswered-escalation containment evaluation,
mondayDB can answer:
...
- Which nominated escalation certificates, placement kinds,
  hop counts, attenuation hashes, remaining-set hashes,
  prior-escalator exclusions, and containment kinds were
  bound?
- Is the containment certificate still current, invalidated,
  contained, hold-windowed, containment-expired, or awaiting
  notify acknowledgement?
- Did async notify or first-enrollment sync become
  `UNKNOWN_EFFECT`?
- Can the containment history be replayed without invoking an
  LLM?

## 2. Scope and ownership

The Unanswered-Escalation Containment Plane owns:

1. Immutable approved containment profiles as procedural
   memory of "how unanswered escalations freeze a first-
   enrollment key without amplifying purpose, leaking halted
   facts, rewriting attested history, scanning vote, page,
   ack, or escalation ledgers, or inventing
   `TRUSTED_ENROLLED` from unanswered-escalation silence."
2. Tenant-scoped containment sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed escalation
   certificates, sealed acknowledgement certificates, sealed
   SLO certificates, sealed window certificates, sealed TTL
   certificates, confirm certificates, and first-enrollment
   claims by point lookup — never key-catalog, vote-ledger,
   TTL-ledger, page-ledger, ack-ledger, escalation-ledger,
   working-set, or board scans.
4. Deterministic containment snapshots, evaluation receipts,
   sealed containment certificates, and immutable
...
- **Unanswered-Ack Escalation:** supplies sealed escalation
  ...
  escalations this plane may contain. This plane never
  invents a new escalation by walking escalation bindings.
- **SLO Ack Suppression:** supplies sealed acknowledgement
  certificates whose unanswered acknowledgements this plane
  may bind as containment evidence. This plane never invents
...
- **Confirmation-Window Observability:** supplies sealed
  ...
  computes remaining TTL by walking escalation bindings.
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may freeze only through a sealed
...
- **Executive Freeze / Thaw SLA:** halt/restore context that
  still forbids contain-as-restore against a halted body.
  ...
  decides higher-authority paging; this plane decides
  unanswered-escalation containment.
- **Emergency Containment:** the coarse stop/drain path used
  ...
  first-enrollment freeze isolation, not workspace-wide
  containment.
...
- Letting an LLM decide that an unanswered escalation "feels
  confirmable enough" or "feels frozen enough."
...
- Inventing a winning fact hash when an escalation goes
  unanswered after a restored slot.
- Promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because
  an escalation was unanswered, contained, held, or expired.
- Scanning vote, TTL, page, ack, or escalation ledgers to
  decide who may contain.
- Letting the original dual-control escalators also serve as
  the containment-authority pair.
...
- allowed source kinds (`SEALED_ESCALATION_CERTIFICATE`,
  `SEALED_ACK_CERTIFICATE`, `SEALED_SLO_CERTIFICATE`,
  `SEALED_TTL_CERTIFICATE`, `SEALED_CONFIRM_CERTIFICATE`,
  `FIRST_ENROLLMENT_CLAIM`);
- containment-window buckets (`containment_window_ms`), max
  refresh count, evaluate threshold (distinct containment-
  authority principals for `CONTAIN_UNANSWERED_ESCALATION`,
  minimum 2), max bindings per certificate, and max
  nominated windows;
- containment policy (`HISTORY_NEVER_REWRITTEN`,
  `SILENCE_NEVER_SUCCESS`, `FIRST_ENROLLMENT_NEVER_AUTO_TRUSTED`,
  `CONTAINMENT_NEVER_CONFIRMS`, `UNANSWERED_ESCALATION_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_CONTAIN_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_ESCALATOR_NEVER_CONTAINS`);
...
- allowed containment kinds (`CONTAINMENT_CURRENT`,
  ...
  `CONTAIN_UNANSWERED_ESCALATION`, `HOLD_CONTAINMENT_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_CONTAINMENT_AS_CONFIRMATION`,
  `SKIP`) and notify policy after seal, invalidation, or
  upstream key or clock change;
- optional procedural refs for "how to contain unanswered
  escalations without inventing a winner."
...
source-window point lookup against the escalation catalog.
Refresh unanswered-escalation containment is a point lookup
...
containment snapshots, clock lifecycles, retire kinds,
dual-control containment-authority counts, prior-escalator
exclusion, and purpose relation. It never walks catalogs.
Seal binds window-set, clock-set, remaining-set, and
attenuation hashes. The certificate must not store
`resolved_fact_hash`. An unanswered escalation cannot
...
REJECTED** planner rules and contain-unit budgets.
...
6. Window nomination uses point lookup by
   ...
   escalation-ledger, or board scans.
...
9. Semantic retrieval discovers profiles only; it never
   authorizes nominate/refresh/evaluate/seal/contain.
10. Silence and unknown windows cannot evaluate to
    `CONTAIN_UNANSWERED_ESCALATION` or any kind that implies
...
13. Successor containment cannot emit a winning fact hash or
    restore a halted body (successor-leak fence).
14. An unanswered escalation never writes
    `attested_key_kind = TRUSTED_ENROLLED`
    (containment-as-confirmation and unanswered-escalation-
    as-trust fences).
15. `CONTAIN_UNANSWERED_ESCALATION` requires two distinct
    containment-authority principals who are not the
    nominating principal and are not members of the prior-
    escalator set.
16. Containment certificates bind window set, clock set,
...
17. Plans that require unscoped board, session, working-set,
    ...
    vote-ledger, TTL-ledger, page-ledger, ack-ledger,
    escalation-ledger, or citation-ledger scans are
    **FULL SCAN REJECTED**.
...
validates definition hash, requires at least one containment
...
Nominate performs a point lookup on the escalation catalog
and writes an immutable nomination receipt with
`remaining_ttl_ms` copied from the sealed source
certificate. Refresh unanswered-escalation containment is a
point lookup against that same source — never a vote-ledger,
page-ledger, ack-ledger, or escalation-ledger walk. Evaluate
is a pure function of nominated windows, remaining-set
hashes, clock lifecycles, prior-escalator exclusion, and
purpose relation. It never walks the catalog.

### 5.4 Seal containment certificate
...
type TrustedNextAction =
  | "NOMINATE_SEALED_ESCALATION_CERTIFICATE"
  | "REFRESH_UNANSWERED_ESCALATION_CONTAINMENT"
  | "EVALUATE_UNANSWERED_ESCALATION_CONTAINMENT"
  | "SEAL_CONTAINMENT_CERTIFICATE"
  | "INVALIDATE_UNANSWERED_ESCALATION_CONTAINMENT"
  | "PREPARE_CONTAINMENT_EFFECT"
  | "RESOLVE_CONTAINMENT_UNCERTAINTY"
  | "CLOSE_SESSION";

type UnansweredEscalationContainmentBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "CONTAINMENT_AS_CONFIRMATION_DENIED"
  | "LEDGER_SCAN_DENIED"
  | "HALT_CONTAIN_DENIED"
...
type ContainmentKind =
  ...
  | "CONTAINMENT_ACTIVE"
  | "CONTAINMENT_EXPIRED"
  | "CONTAIN_UNANSWERED_ESCALATION"
  | "HOLD_CONTAINMENT_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_CONTAINMENT_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface UnansweredEscalationContainmentBudget {
...
interface UnansweredEscalationContainmentProfile {
...
interface UnansweredEscalationContainmentSession {
  readonly accountId: AccountId;
  ...
  readonly budget: UnansweredEscalationContainmentBudget;
  readonly consumed: Omit<
    UnansweredEscalationContainmentBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface UnansweredEscalationContainmentSnapshot {
  readonly accountId: AccountId;
...
interface UnansweredEscalationContainmentEvaluationReceipt {
  readonly accountId: AccountId;
...
interface UnansweredEscalationContainmentBinding {
  readonly accountId: AccountId;
...
interface UnansweredEscalationContainmentCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface UnansweredEscalationContainmentEffectObservation {
...
interface AgentUnansweredEscalationContainmentPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  ...
  readonly containUnansweredEscalationBindingCount: number;
  readonly holdContainmentWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectContainmentAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly UnansweredEscalationContainmentEffectObservation[];
  ...
  readonly blockedReasons: readonly UnansweredEscalationContainmentBlockedReason[];
...
```ts
interface CreateUnansweredEscalationContainmentSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface NominateSealedEscalationCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
...
interface RefreshUnansweredEscalationContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateUnansweredEscalationContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateUnansweredEscalationContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ResolveContainmentUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface CloseUnansweredEscalationContainmentSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type UnansweredEscalationContainmentDecision =
  | { readonly decision: "ALLOWED"; readonly session: UnansweredEscalationContainmentSession;
      readonly certificate?: UnansweredEscalationContainmentCertificate;
      readonly member?: UnansweredEscalationContainmentBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: UnansweredEscalationContainmentSnapshot;
      readonly evaluation?: UnansweredEscalationContainmentEvaluationReceipt;
      readonly perception: AgentUnansweredEscalationContainmentPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: UnansweredEscalationContainmentBlockedReason;
...
CREATE TABLE agent_uec_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status uec_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_uec_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_uec_profile_containment_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_uec_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_uec_escalation_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind uec_source_kind NOT NULL,
  placement_kind uec_placement_kind NOT NULL,
  key_lifecycle uec_key_lifecycle NOT NULL,
  clock_lifecycle uec_clock_lifecycle NOT NULL,
  status uec_catalog_status NOT NULL,
...
CREATE TABLE agent_uec_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status uec_session_status NOT NULL,
  ...
  budget_contain_units BIGINT NOT NULL CHECK (budget_contain_units >= 0),
  ...
  consumed_contain_units BIGINT NOT NULL
    CHECK (consumed_contain_units >= 0),
  ...
  CHECK (consumed_contain_units <= budget_contain_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_uec_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_uec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_uec_escalation_catalog (account_id, source_window_id)
);

CREATE TABLE agent_uec_containment_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_containment_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_uec_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_uec_containment_binding (
  account_id BIGINT NOT NULL,
  ...
  status uec_binding_status NOT NULL,
  placement_kind uec_placement_kind NOT NULL,
  key_lifecycle uec_key_lifecycle NOT NULL,
  clock_lifecycle uec_clock_lifecycle NOT NULL,
  attested_key_kind uec_attested_key_kind NOT NULL,
  containment_kind uec_containment_kind NOT NULL,
  purpose_relation uec_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_uec_containment_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_uec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_uec_escalation_catalog (account_id, source_window_id)
...
CREATE TABLE agent_uec_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status uec_effect_status NOT NULL,
...
CREATE TABLE agent_uec_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_uec_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uec_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status uec_session_status NOT NULL,
...
CREATE TABLE agent_uec_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_uec_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.uec_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.uec_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_uec_profile_protect
BEFORE INSERT OR UPDATE ON agent_uec_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_uec_profile();

CREATE FUNCTION protect_agent_uec_profile_containment_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status uec_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_uec_profile
  ...
  IF profile_status IS DISTINCT FROM 'DRAFT' THEN
    RAISE EXCEPTION 'sealed profile containment rules are immutable';
  END IF;
  RETURN COALESCE(NEW, OLD);
END
$protect_rule$;

CREATE TRIGGER agent_uec_profile_containment_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_uec_profile_containment_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_uec_profile_containment_rule();

CREATE FUNCTION protect_agent_uec_containment_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'containment snapshots are append-only';
  END IF;
  RETURN NEW;
END
$protect_snapshot$;

CREATE TRIGGER agent_uec_containment_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_uec_containment_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_uec_containment_snapshot();

CREATE FUNCTION protect_agent_uec_containment_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.source_window_id IS DISTINCT FROM OLD.source_window_id
      ...
      RAISE EXCEPTION 'containment binding identity is immutable';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED',
       'CONTAIN_UNANSWERED_ESCALATION', 'REJECT_CONTAINMENT_AS_CONFIRMATION'
     )
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'history-rewrite fence blocks containment from rewriting confirmed trust';
  END IF;

  IF NEW.containment_kind = 'REJECT_CONTAINMENT_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'containment-as-confirmation fence blocks TRUSTED_ENROLLED from an unanswered escalation';
  END IF;

  IF NEW.containment_kind = 'CONTAIN_UNANSWERED_ESCALATION'
     AND NEW.attested_key_kind IN ('SILENCE', 'UNKNOWN_EFFECT') THEN
    RAISE EXCEPTION 'silence-success fence blocks containment of silent or unknown successor';
  END IF;
  ...
  IF NEW.containment_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from unanswered-escalation containment';
  END IF;

  IF NEW.containment_kind = 'CONTAIN_UNANSWERED_ESCALATION'
     AND NEW.clock_lifecycle = 'UNATTESTED' THEN
    RAISE EXCEPTION 'unattested-clock fence blocks containment against an unattested clock';
  END IF;

  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAINMENT_EXPIRED', 'CONTAIN_UNANSWERED_ESCALATION'
     )
     AND NEW.attested_key_kind = 'INVENTED_HISTORY' THEN
    RAISE EXCEPTION 'invented-history fence blocks containment for invented successor history';
  END IF;

  IF NEW.containment_kind IN (
       'CONTAINMENT_CURRENT', 'CONTAINMENT_ACTIVE', 'CONTAIN_UNANSWERED_ESCALATION'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-contain fence blocks containment that would restore a halted key';
  END IF;
  ...
  IF NEW.hop_count > 0
    ...
    RAISE EXCEPTION 'hop-leak fence blocks donor-purpose containment after attenuation hops';
  END IF;
...
CREATE TRIGGER agent_uec_containment_binding_protect
BEFORE INSERT OR UPDATE ON agent_uec_containment_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_uec_containment_binding();

CREATE FUNCTION protect_agent_uec_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_uec_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_uec_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_uec_effect_intent();

CREATE FUNCTION approve_agent_uec_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status uec_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_uec_profile_containment_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_uec_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_uec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_uec_profile_authority;
ALTER FUNCTION revoke_agent_uec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_uec_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_uec_profile_authority;
GRANT SELECT ON
  agent_uec_profile,
  agent_uec_profile_containment_rule
TO agent_uec_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_uec_profile TO agent_uec_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_uec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_uec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_uec_profile FROM PUBLIC;

CREATE INDEX agent_uec_session_work_idx ON agent_uec_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_uec_session_profile_idx ON agent_uec_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_uec_binding_certificate_idx ON agent_uec_containment_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_uec_binding_window_idx ON agent_uec_containment_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_uec_catalog_ref_idx ON agent_uec_escalation_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_uec_catalog_kind_idx ON agent_uec_escalation_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_uec_evaluation_session_idx ON agent_uec_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_uec_certificate_session_idx ON agent_uec_containment_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_uec_snapshot_session_idx ON agent_uec_containment_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_uec_effect_work_idx ON agent_uec_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_uec_audit_time_idx ON agent_uec_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_uec_perception_status_idx ON agent_uec_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_uec_command_expiry_idx ON agent_uec_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_uec_invalidation_certificate_idx ON agent_uec_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_uec_authorization_evidence',
    'agent_uec_profile',
    'agent_uec_profile_containment_rule',
    'agent_uec_escalation_catalog',
    'agent_uec_session',
    'agent_uec_nomination_window',
    'agent_uec_containment_snapshot',
    'agent_uec_evaluation_receipt',
    'agent_uec_containment_certificate',
    'agent_uec_containment_binding',
    'agent_uec_invalidation',
    'agent_uec_effect_intent',
    'agent_uec_budget_ledger',
    'agent_uec_terminal_record',
    'agent_uec_command_result',
    'agent_uec_audit_head',
    'agent_uec_audit_event',
    'agent_uec_audit_anchor',
    'agent_uec_perception_snapshot',
    'agent_uec_projection_checkpoint'
  ]
  LOOP
...
enum AgentUecClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentUecAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentUecUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentUecInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentUecSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentUecSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentUecBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentUecNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentUecSourceKind!
  placementKind: AgentUecPlacementKind!
  keyLifecycle: AgentUecKeyLifecycle!
  clockLifecycle: AgentUecClockLifecycle!
...
type AgentUecUnansweredEscalationContainmentSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentUecEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentUecContainmentCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentUecBinding {
  ...
  sourceWindowKind: AgentUecSourceKind!
  bindingOrdinal: Int!
  status: AgentUecBindingStatus!
  placementKind: AgentUecPlacementKind!
  keyLifecycle: AgentUecKeyLifecycle!
  clockLifecycle: AgentUecClockLifecycle!
  attestedKeyKind: AgentUecAttestedKeyKind!
  containmentKind: AgentUecContainmentKind!
  purposeRelation: AgentUecPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentUecEffectObservation {
  effectId: ID!
  status: AgentUecEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentUecPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentUecSessionStatus!
  ...
  containUnansweredEscalationBindingCount: Int!
  holdContainmentWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectContainmentAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentUecEffectObservation!]!
  remainingBudget: AgentUecBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentUecNextAction!]!
  blockedReasons: [AgentUecBlockedReason!]!
  cardHash: SHA256!
}

type AgentUecMutationResult {
  decision: String!
  session: AgentUecSession
  certificate: AgentUecContainmentCertificate
  member: AgentUecBinding
  receipt: AgentUecNominationReceipt
  snapshot: AgentUecUnansweredEscalationContainmentSnapshot
  evaluation: AgentUecEvaluationReceipt
  perception: AgentUecPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateUnansweredEscalationContainmentSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentUecBudgetInput!
}

input NominateSealedEscalationCertificateInput {
  accountId: ID!
  sessionId: ID!
...
input RefreshUnansweredEscalationContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateUnansweredEscalationContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateUnansweredEscalationContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentUecInvalidationReason!
  idempotencyKey: String!
}
...
input ResolveContainmentUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentUecUncertaintyResolution!
  idempotencyKey: String!
}

input AgentUecProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentUecSession(accountId: ID!, sessionId: ID!): AgentUecSession
  agentUecContainmentCertificate(accountId: ID!, certificateId: ID!): AgentUecContainmentCertificate
  agentUecPerceptionCard(accountId: ID!, sessionId: ID!): AgentUecPerceptionCard
  agentUecNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentUecNominationReceipt
  agentUecSearchProfiles(input: AgentUecProfileSearchInput!): [AgentUecProfile!]!
}

type Mutation {
  createUnansweredEscalationContainmentSession(
    input: CreateUnansweredEscalationContainmentSessionInput!
  ): AgentUecMutationResult!
  nominateSealedEscalationCertificate(
    input: NominateSealedEscalationCertificateInput!
  ): AgentUecMutationResult!
  refreshUnansweredEscalationContainment(
    input: RefreshUnansweredEscalationContainmentInput!
  ): AgentUecMutationResult!
  evaluateUnansweredEscalationContainment(
    input: EvaluateUnansweredEscalationContainmentInput!
  ): AgentUecMutationResult!
  sealContainmentCertificate(input: SealContainmentCertificateInput!): AgentUecMutationResult!
  invalidateUnansweredEscalationContainment(
    input: InvalidateUnansweredEscalationContainmentInput!
  ): AgentUecMutationResult!
  prepareContainmentEffect(input: PrepareContainmentEffectInput!): AgentUecMutationResult!
  resolveContainmentUncertainty(
    input: ResolveContainmentUncertaintyInput!
  ): AgentUecMutationResult!
  closeUnansweredEscalationContainmentSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentUecMutationResult!
  approveUnansweredEscalationContainmentProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealContainmentCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `CONTAINMENT_AS_CONFIRMATION_DENIED` when an unanswered escalation is treated as confirmation, and with
  `LEDGER_SCAN_DENIED` when remaining time would be computed by walking
  a vote, TTL, page, ack, or escalation ledger.

## 10. Procedural memory

Approved containment profiles are procedural memory:
versioned instructions for how sealed escalation,
acknowledgement, SLO, window, TTL, and confirm certificates
become envelope-scoped first-enrollment freezes without
inventing a winner and without rewriting an unanswered
escalation as dual-control success or `TRUSTED_ENROLLED`.
...
enrollment unanswered escalation?"). Embeddings are
...
```sql
CREATE TABLE agent_uec_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
containment-expired / contain-unanswered-escalation /
hold-containment-window / hold-unknown /
reject-containment-as-confirmation / unknown / skipped /
...
`CONTAINMENT_CURRENT` as a first-seen unanswered escalation
whose containment window is still open and still cannot
invent a winner, `CONTAINMENT_ACTIVE` as a bounded first-
enrollment freeze whose remaining time is below the profile
threshold without confirmation, `CONTAINMENT_EXPIRED` as a
sealed containment that the window closed without dual-
control trust, `CONTAIN_UNANSWERED_ESCALATION` as a dual-
control freeze that does not confirm the key,
`HOLD_CONTAINMENT_WINDOW` as a trusted hold that refuses
silent confirmation, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_CONTAINMENT_AS_CONFIRMATION` as a trusted
negative for an unanswered escalation that would have
rewritten silence as success, and `SKIP` as a sealed
refusal — never as a key that "must have been trusted
because nobody contained the unanswered escalation."
...
containment-current, containment-active, or reject-
containment-as-confirmation outcomes.
...
- Planner rejects unscoped key-catalog, clock-catalog,
  ...
  vote-ledger, TTL-ledger, page-ledger, ack-ledger,
  escalation-ledger, or board scans as **FULL SCAN
  REJECTED**.
...
- Evaluation never auto-restores neighbor-visible board
  ...
  acknowledgements, unanswered escalations, or unbound
  windows.
- Prior-escalator exclusion rejects original dual-control
  escalators as the containment-authority pair.

## 15. Performance check for boards with 1M+ rows

### Full-table-scan risks

- Finding remaining unanswered escalations to contain by
...
- unanswered-escalation containment refresh: unique
...
page-ledger/ack-ledger/escalation-ledger/citation scan is
...
- Historical silence invented as `TRUSTED_ENROLLED` from an
  unanswered escalation: history-rewrite, silence-success,
  and unanswered-escalation-as-trust fences.
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED`
  by a containment: first-enrollment-auto-trust and
  containment-as-confirmation fences.
- Remaining time or containment targets computed by walking
  vote, TTL, page, ack, or escalation ledgers: ledger-scan
  fence and **FULL SCAN REJECTED**.
- Halt leak of frozen bodies into contain-as-restore:
  halt-contain fence.
...
- Inventing a winner under restored-slot containment:
  ...
  `resolved_fact_hash`.
- Original escalators serving as the containment-authority
  pair: prior-escalator-exclusion fence.
...
- History-rewrite rejection, silence-success rejection,
  first-enrollment-auto-trust rejection, containment-as-
...
Compile profiles and validate escalation-catalog nomination
without durable certificates.

### Phase 2: reject-containment-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_CONTAINMENT_AS_CONFIRMATION` or `SKIP`.
...
and `AWAITING_CONFIRMATION` only. An unanswered escalation
never writes `TRUSTED_ENROLLED`.

### Phase 4: containment-authority freezes and notify uncertainty
...
budgets, including containment-authority freezes only for
first-enrollment keys that cannot rewrite an unanswered
escalation as success and cannot auto-trust from containment
silence.
...
- Nominate requires sealed escalation-catalog material point
  lookup and hash match.
- Refresh unanswered-escalation containment is a point
...
- Evaluate binds window set, remaining set, and attenuation
  under budget and prior-escalator exclusion.
- Seal is rejected when an unanswered escalation would
...
Adopt the Unanswered-Escalation Containment Plane as the
deterministic first-enrollment freeze path for unanswered,
expired, or single-control escalations bound by the
Unanswered-Ack Escalation, SLO Ack Suppression, Remaining-
Window SLO, Confirmation-Window Observability, Confirmation-
TTL Retirement, and First-Enrollment Confirmation planes.

Ship it because:

1. It preserves ACID and multi-tenant isolation while
   closing the post-escalation containment gap without
   history rewrite, silence-success, first-enrollment auto-
   trust, containment-as-confirmation, unanswered-
   escalation-as-trust, ledger scans, halt leak, purpose
   amplification, invented winners, prior-escalator reuse,
   or vote/page/ack/escalation-ledger walks.
