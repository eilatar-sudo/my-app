# mondayDB Agentic Unanswered-Ack Escalation Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-08-29.v1`

## 1. Why this plane, before how

A sealed SLO acknowledgement certificate can suppress paging
after dual-control `ACK_WINDOW_SLO`. It does not decide **how
an unanswered, expired, or single-control acknowledgement
escalates to a higher authority** — without scanning every
acknowledgement binding, rewriting unanswered escalation
silence into `TRUSTED_ENROLLED`, or treating pager silence as
confirmation.

Without an unanswered-ack escalation plane, operators and
agents either:

- scan every pending acknowledgement, page, and vote ledger
  looking for "who still has not dual-control acknowledged"
  (neighbor-harmful on boards with 1M+ rows), or
- treat unanswered acknowledgement or unanswered escalation as
  retroactive engine truth, so SLO silence is rewritten as
  confirmation, historical `UNKNOWN_EFFECT` is rewritten as
  `ACKED`, a silent escalation auto-promotes
  `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED`, a halt-scoped body
  is "unlocked" by later escalation, and hop-attenuated purpose
  is amplified back to the donor.

The product trade-off is **escalation fluency versus
escalation isolation**:

- Treating every unanswered acknowledgement, pager timeout, or
  escalation timer as implicit confirmation maximizes agent
  fluency and reduces re-planning cost, but creates history-
  rewrite invention, escalation-as-confirmation, unanswered-
  ack-as-trust, silence-success, first-enrollment auto-trust,
  unauditable escalation storms, and recursive ack-ledger
  walks against neighbors.
- Binding a sealed escalation certificate under an approved
  escalation profile, acknowledgement-catalog point lookups,
  history-rewrite fences, silence-success fences, first-
  enrollment auto-trust fences, escalation-as-confirmation
  fences, unanswered-ack-as-trust fences, halt-escalate
  fences, purpose-amplification fences, successor-leak
  fences, prior-acker-exclusion fences, ledger-scan fences,
  and steward budgets adds one bounded evaluate transaction
  and short-lived escalation storage.
- Semantic similarity may discover escalation profiles, but it
  must never decide whether a sealed acknowledgement, SLO,
  window, TTL, or confirm certificate may be nominated, an
  escalation snapshot refreshed, a certificate sealed, or a
  higher-authority page opened.

The recommended model keeps the data plane deterministic:

1. An approved escalation profile defines allowed source
   kinds, escalation-window duration, dual-control higher-
   authority policy, prior-acker exclusion, and notify policy.
   Evaluation **never** invents a winning fact hash and
   **never** rewrites an unanswered acknowledgement or
   unanswered escalation into `TRUSTED_ENROLLED`.
2. An escalation session opens under purpose, budget, and
   authorization fences, and only nominates sealed
   acknowledgement certificates, sealed SLO certificates,
   sealed window certificates, sealed TTL certificates, sealed
   confirm certificates, or first-enrollment claims by point
   lookup from the SLO Ack Suppression, Remaining-Window SLO,
   Confirmation-Window Observability, Confirmation-TTL
   Retirement, and First-Enrollment Confirmation planes.
3. mondayDB evaluates an escalation whose kind is a pure
   ...
   prior_acker_set_hash, higher_authority_count,
   escalation_window_ms, escalation_kind)`. Silence cannot
   become historical success. An unanswered acknowledgement or
   unanswered escalation seals
   `REJECT_ESCALATION_AS_CONFIRMATION` or `ESCALATION_EXPIRED`
   only — never `TRUSTED_ENROLLED`. Dual-control
   `ESCALATE_UNANSWERED_ACK` pages a higher authority without
   inventing confirmation. `HOLD_ESCALATION_WINDOW` holds
   further paging for a bounded window without restoring a
   winner. Halted slots cannot become escalate-restored
   winners. Original ackers are excluded from the higher-
   authority pair.
4. Sealing an escalation certificate binds
...
6. Unscoped key-catalog, clock-catalog, confirm-ledger, vote-
   ledger, TTL-ledger, page-ledger, ack-ledger, working-set,
   grant-graph, or board scans are **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor
protection: recursive "re-escalate every unanswered
acknowledgement forever" or "treat unanswered escalation
silence as confirmation until it looks like success" loops are
rejectable before they scan boards with 1M+ rows. Perception
is restored by sealed escalation certificates, not by magic
escalation orchestration inside the engine.

### Product outcome

For any unanswered-ack escalation evaluation, mondayDB can answer:

- Which profile, principal, and session authorized the
  nomination, escalation refresh, evaluate, seal, invalidate,
  or notify dispatch?
- Which nominated acknowledgement certificates, placement
  kinds, hop counts, attenuation hashes, remaining-set hashes,
  prior-acker exclusions, and escalation kinds were bound?
- Is the escalation certificate still current, invalidated,
  escalated, hold-windowed, escalation-expired, or awaiting
  notify acknowledgement?
- Did async notify or first-enrollment sync become
  `UNKNOWN_EFFECT`?
- Can the escalation history be replayed without invoking an
  LLM?

## 2. Scope and ownership

The Unanswered-Ack Escalation Plane owns:

1. Immutable approved escalation profiles as procedural memory
   of "how unanswered acknowledgements page a higher authority
   without amplifying purpose, leaking halted facts, rewriting
   attested history, scanning vote, page, or ack ledgers, or
   inventing `TRUSTED_ENROLLED` from unanswered
   acknowledgement or unanswered escalation."
2. Tenant-scoped escalation sessions with purpose and budget
   fences.
3. Deterministic nomination of sealed acknowledgement
   ...
   ack-ledger, working-set, or board scans.
4. Deterministic escalation snapshots, evaluation receipts,
   sealed escalation certificates, and immutable escalation
   bindings that never invent a winner.
...
- **SLO Ack Suppression:** supplies sealed acknowledgement
  ...
  acknowledgements this plane may escalate. This plane never
...
- **Remaining-Window SLO:** supplies sealed SLO certificates
  ...
  as escalation evidence. This plane never invents a new page
...
- **Confirmation-Window Observability:** supplies sealed
  ...
  plane may bind as escalation evidence. This plane never
  computes remaining TTL by walking ack bindings.
...
- **First-Enrollment Confirmation:** supplies sealed confirm
  ...
  remaining confirmation window this plane may escalate. This
...
- **Successor-Key Enrollment / Successor-Clock Re-Attestation /
  ...
  rotation claims that this plane may bind an escalation
...
- **Certificate Placement / Envelope Purpose Gate / Decision
  Memory:** may consume sealed escalation certificates as
  perception evidence, not raw pager webhooks.
- **Executive Freeze / Thaw SLA:** halt/restore context that
  still forbids escalate-as-restore against a halted body.
  Thaw SLA decides freeze liveness; TTL retirement decides
  confirmation-vote liveness; observability decides remaining-
  window perception; ack suppression decides acknowledgement
  suppression; this plane decides unanswered-ack escalation.
- **Emergency Containment:** the coarse stop/drain path used
  when a contained key evaluates to `SKIP` or
  `HOLD_ESCALATION_WINDOW`; this plane is purpose-scoped
  escalation isolation, not workspace-wide containment.
- **Query Governor / Budgets:** reserves nominate, evaluate,
  vector, seal, invalidate, and escalate units.

### Non-goals

- Letting an LLM decide that an unanswered acknowledgement or
  unanswered escalation "feels confirmable enough."
- Auto-amplifying a hop-narrowed purpose back to the donor
  purpose.
- Reconstructing authoritative escalation certificates from
  columnar or vector projections.
- Cross-account escalation or global nearest-neighbor
  authorization.
- Storing raw private keys, unrestricted tool payloads, or
  redacted plaintext.
- Claiming distributed atomicity with external pager or time-
  distribution providers.
- Inventing a winning fact hash when an acknowledgement or
  escalation goes unanswered after a restored slot.
- Promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because an
  acknowledgement was unanswered, escalated, held, or expired.
- Scanning vote, TTL, page, or ack ledgers to decide who may
  escalate.
- Letting the original dual-control ackers also serve as the
  higher-authority pair.
...
- allowed source kinds (`SEALED_ACK_CERTIFICATE`,
  `SEALED_SLO_CERTIFICATE`, `SEALED_TTL_CERTIFICATE`,
  `SEALED_CONFIRM_CERTIFICATE`, `FIRST_ENROLLMENT_CLAIM`);
- escalation-window buckets (`escalation_window_ms`), max
  refresh count, evaluate threshold (distinct higher-authority
  principals for `ESCALATE_UNANSWERED_ACK`, minimum 2), max
  bindings per certificate, and max nominated windows;
- escalation policy (`HISTORY_NEVER_REWRITTEN`,
  `SILENCE_NEVER_SUCCESS`, `FIRST_ENROLLMENT_NEVER_AUTO_TRUSTED`,
  `ESCALATION_NEVER_CONFIRMS`, `UNANSWERED_ACK_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_ESCALATE_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_ACKER_NEVER_ESCALATES`);
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed escalation kinds (`ESCALATION_CURRENT`,
  `ESCALATION_ACTIVE`, `ESCALATION_EXPIRED`,
  `ESCALATE_UNANSWERED_ACK`, `HOLD_ESCALATION_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_ESCALATION_AS_CONFIRMATION`, `SKIP`)
  and notify policy after seal, invalidation, or upstream key
  or clock change;
- optional procedural refs for "how to escalate unanswered
  acknowledgements without inventing a winner."
...
source-window point lookup against the acknowledgement catalog.
Refresh unanswered-ack escalation is a point lookup against the
nominated sealed certificate — never a ledger scan.

### 3.3 Evaluate and certificate contract

Evaluate is a pure function of nominated windows, escalation
snapshots, clock lifecycles, retire kinds, dual-control higher-
authority counts, prior-acker exclusion, and purpose relation.
It never walks catalogs. Seal binds window-set, clock-set,
remaining-set, and attenuation hashes. The certificate must not
store `resolved_fact_hash`. An unanswered acknowledgement or
unanswered escalation cannot evaluate to a kind that implies
`TRUSTED_ENROLLED`.
...
planner rules and escalate-unit budgets.
...
6. Window nomination uses point lookup by
   ...
   vote-ledger, TTL-ledger, page-ledger, ack-ledger, or board
   scans.
...
9. Semantic retrieval discovers profiles only; it never
   authorizes nominate/refresh/evaluate/seal/escalate.
10. Silence and unknown windows cannot evaluate to
    `ESCALATE_UNANSWERED_ACK` or any kind that implies
    `TRUSTED_ENROLLED` (history-rewrite and silence-success
    fences).
11. Halted, extended-halt, and omitted windows cannot evaluate
    to an escalation kind that restores a winner (halt-escalate
    fence).
12. Requested purposes that amplify a window attenuation hash
    are rejected (purpose-amplification fence).
13. Successor escalation cannot emit a winning fact hash or
    restore a halted body (successor-leak fence).
14. An unanswered acknowledgement or unanswered escalation
    never writes `attested_key_kind = TRUSTED_ENROLLED`
    (escalation-as-confirmation and unanswered-ack-as-trust
    fences).
15. `ESCALATE_UNANSWERED_ACK` requires two distinct higher-
    authority principals who are not the nominating principal
    and are not members of the prior-acker set.
16. Escalation certificates bind window set, clock set,
    remaining set, and attenuation hashes; they never invent a
    winning fact hash.
17. Plans that require unscoped board, session, working-set,
    grant-graph, key-catalog, clock-catalog, confirm-ledger,
    vote-ledger, TTL-ledger, page-ledger, ack-ledger, or
...
Draft profiles accumulate escalation rules. Approval validates
definition hash, requires at least one escalation rule, requires
`evaluate_threshold >= 2`, requires `escalation_window_ms > 0`,
...
Nominate performs a point lookup on the acknowledgement catalog
and writes an immutable nomination receipt with
`remaining_ttl_ms` copied from the sealed source certificate.
Refresh unanswered-ack escalation is a point lookup against that
same source — never a vote-ledger, page-ledger, or ack-ledger
walk. Evaluate is a pure function of nominated windows,
remaining-set hashes, clock lifecycles, prior-acker exclusion,
and purpose relation. It never walks the catalog.

### 5.4 Seal escalation certificate
...
A steward inserts a `DRAFT` profile and at least one escalation
...
Budgets decrement in the ledger. Escalation snapshots are
append-only.
...
type TrustedNextAction =
  | "NOMINATE_SEALED_ACK_CERTIFICATE"
  | "REFRESH_UNANSWERED_ACK_ESCALATION"
  | "EVALUATE_UNANSWERED_ACK_ESCALATION"
  | "SEAL_ESCALATION_CERTIFICATE"
  | "INVALIDATE_UNANSWERED_ACK_ESCALATION"
  | "PREPARE_ESCALATION_EFFECT"
  | "RESOLVE_ESCALATION_UNCERTAINTY"
  | "CLOSE_SESSION";

type UnansweredAckEscalationBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "ESCALATION_AS_CONFIRMATION_DENIED"
  | "LEDGER_SCAN_DENIED"
  | "HALT_ESCALATE_DENIED"
...
type EscalationKind =
  ...
  | "ESCALATION_ACTIVE"
  | "ESCALATION_EXPIRED"
  | "ESCALATE_UNANSWERED_ACK"
  | "HOLD_ESCALATION_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_ESCALATION_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface UnansweredAckEscalationBudget {
...
interface UnansweredAckEscalationProfile {
...
interface UnansweredAckEscalationSession {
  readonly accountId: AccountId;
  ...
  readonly budget: UnansweredAckEscalationBudget;
  readonly consumed: Omit<
    UnansweredAckEscalationBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface UnansweredAckEscalationSnapshot {
  readonly accountId: AccountId;
...
interface UnansweredAckEscalationEvaluationReceipt {
  readonly accountId: AccountId;
...
interface UnansweredAckEscalationBinding {
  readonly accountId: AccountId;
...
interface UnansweredAckEscalationCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface UnansweredAckEscalationEffectObservation {
...
interface AgentUnansweredAckEscalationPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly escalationCurrentBindingCount: number;
  readonly escalationActiveBindingCount: number;
  readonly escalationExpiredBindingCount: number;
  readonly escalateUnansweredAckBindingCount: number;
  readonly holdEscalationWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectEscalationAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly UnansweredAckEscalationEffectObservation[];
  ...
  readonly blockedReasons: readonly UnansweredAckEscalationBlockedReason[];
...
```ts
interface CreateUnansweredAckEscalationSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface NominateSealedAckCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
...
interface RefreshUnansweredAckEscalationInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateUnansweredAckEscalationInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateUnansweredAckEscalationInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface PrepareEscalationEffectInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface CloseUnansweredAckEscalationSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type UnansweredAckEscalationDecision =
  | { readonly decision: "ALLOWED"; readonly session: UnansweredAckEscalationSession;
      readonly certificate?: UnansweredAckEscalationCertificate;
      readonly member?: UnansweredAckEscalationBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: UnansweredAckEscalationSnapshot;
      readonly evaluation?: UnansweredAckEscalationEvaluationReceipt;
      readonly perception: AgentUnansweredAckEscalationPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: UnansweredAckEscalationBlockedReason;
...
```sql
CREATE TYPE uae_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE uae_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_source_kind AS ENUM (
  'SEALED_TTL_CERTIFICATE', 'SEALED_CONFIRM_CERTIFICATE',
  'FIRST_ENROLLMENT_CLAIM', 'SEALED_SLO_CERTIFICATE',
  'SEALED_ACK_CERTIFICATE'
);
CREATE TYPE uae_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE uae_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_escalation_kind AS ENUM (
  'ESCALATION_CURRENT', 'ESCALATION_ACTIVE', 'ESCALATION_EXPIRED',
  'ESCALATE_UNANSWERED_ACK', 'HOLD_ESCALATION_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_ESCALATION_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE uae_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_uae_profile_authority NOLOGIN;

CREATE TABLE agent_uae_authorization_evidence (
...
CREATE TABLE agent_uae_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status uae_profile_status NOT NULL,
  ...
  escalation_window_ms BIGINT NOT NULL
    CHECK (escalation_window_ms BETWEEN 1000 AND 2592000000),
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_uae_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_uae_profile_escalation_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  escalation_window_ms BIGINT NOT NULL
    CHECK (escalation_window_ms BETWEEN 1000 AND 2592000000),
  require_dual_control_escalation BOOLEAN NOT NULL,
  escalation_instruction JSONB NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_uae_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_uae_ack_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind uae_source_kind NOT NULL,
  placement_kind uae_placement_kind NOT NULL,
  key_lifecycle uae_key_lifecycle NOT NULL,
  clock_lifecycle uae_clock_lifecycle NOT NULL,
  status uae_catalog_status NOT NULL,
...
CREATE TABLE agent_uae_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status uae_session_status NOT NULL,
  ...
  budget_escalate_units BIGINT NOT NULL CHECK (budget_escalate_units >= 0),
  ...
  consumed_escalate_units BIGINT NOT NULL
    CHECK (consumed_escalate_units >= 0),
  ...
  CHECK (consumed_escalate_units <= budget_escalate_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_uae_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_uae_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_uae_ack_catalog (account_id, source_window_id)
);

CREATE TABLE agent_uae_escalation_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_escalation_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_uae_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_uae_escalation_binding (
  account_id BIGINT NOT NULL,
  ...
  status uae_binding_status NOT NULL,
  placement_kind uae_placement_kind NOT NULL,
  key_lifecycle uae_key_lifecycle NOT NULL,
  clock_lifecycle uae_clock_lifecycle NOT NULL,
  attested_key_kind uae_attested_key_kind NOT NULL,
  escalation_kind uae_escalation_kind NOT NULL,
  purpose_relation uae_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_uae_escalation_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_uae_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_uae_ack_catalog (account_id, source_window_id)
...
CREATE TABLE agent_uae_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status uae_effect_status NOT NULL,
...
CREATE TABLE agent_uae_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  entry_sequence BIGINT NOT NULL CHECK (entry_sequence >= 0),
  unit_type TEXT NOT NULL CHECK (
    unit_type IN (
      'NOMINATE', 'EVALUATE', 'SEAL', 'VECTOR', 'INVALIDATE', 'ESCALATE'
    )
  ),
...
CREATE TABLE agent_uae_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_uae_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status uae_session_status NOT NULL,
...
CREATE TABLE agent_uae_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_uae_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM 'DRAFT'
     AND (
       ...
       OR NEW.escalation_window_ms IS DISTINCT FROM OLD.escalation_window_ms
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.uae_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.uae_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_uae_profile_protect
BEFORE INSERT OR UPDATE ON agent_uae_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_uae_profile();

CREATE FUNCTION protect_agent_uae_profile_escalation_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status uae_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_uae_profile
...
CREATE TRIGGER agent_uae_profile_escalation_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_uae_profile_escalation_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_uae_profile_escalation_rule();

CREATE FUNCTION protect_agent_uae_escalation_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'escalation snapshots are append-only';
  END IF;
  RETURN NEW;
END
$protect_snapshot$;

CREATE TRIGGER agent_uae_escalation_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_uae_escalation_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_uae_escalation_snapshot();

CREATE FUNCTION protect_agent_uae_escalation_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.source_window_id IS DISTINCT FROM OLD.source_window_id
       ...
       OR NEW.escalation_kind IS DISTINCT FROM OLD.escalation_kind
      ...
      RAISE EXCEPTION 'escalation binding identity is immutable';
    END IF;
    RETURN NEW;
  END IF;

  IF NEW.escalation_kind IN (
       'ESCALATION_CURRENT', 'ESCALATION_ACTIVE', 'ESCALATION_EXPIRED',
       'ESCALATE_UNANSWERED_ACK', 'REJECT_ESCALATION_AS_CONFIRMATION'
     )
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'history-rewrite fence blocks escalation from rewriting confirmed trust';
  END IF;

  IF NEW.escalation_kind = 'REJECT_ESCALATION_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'escalation-as-confirmation fence blocks TRUSTED_ENROLLED from an unanswered acknowledgement or unanswered escalation';
  END IF;

  IF NEW.escalation_kind = 'ESCALATE_UNANSWERED_ACK'
     AND NEW.attested_key_kind IN ('SILENCE', 'UNKNOWN_EFFECT') THEN
    RAISE EXCEPTION 'silence-success fence blocks escalation of silent or unknown successor';
  END IF;
  ...
  IF NEW.escalation_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from an unanswered acknowledgement or unanswered escalation';
  END IF;

  IF NEW.escalation_kind = 'ESCALATE_UNANSWERED_ACK'
     AND NEW.clock_lifecycle = 'UNATTESTED' THEN
    RAISE EXCEPTION 'unattested-clock fence blocks escalation against an unattested clock';
  END IF;

  IF NEW.escalation_kind IN (
       'ESCALATION_CURRENT', 'ESCALATION_ACTIVE', 'ESCALATION_EXPIRED', 'ESCALATE_UNANSWERED_ACK'
     )
     AND NEW.attested_key_kind = 'INVENTED_HISTORY' THEN
    RAISE EXCEPTION 'invented-history fence blocks escalation for invented successor history';
  END IF;

  IF NEW.escalation_kind IN (
       'ESCALATION_CURRENT', 'ESCALATION_ACTIVE', 'ESCALATE_UNANSWERED_ACK'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-escalate fence blocks escalation that would restore a halted key';
  END IF;
  ...
  IF NEW.hop_count > 0
     AND NEW.escalation_kind IN (
       'ESCALATION_CURRENT', 'ESCALATION_ACTIVE', 'ESCALATE_UNANSWERED_ACK'
     )
     AND NEW.requested_purpose_hash IS NOT DISTINCT FROM NEW.donor_purpose_hash THEN
    RAISE EXCEPTION 'hop-leak fence blocks donor-purpose escalation after attenuation hops';
  END IF;
...
CREATE TRIGGER agent_uae_escalation_binding_protect
BEFORE INSERT OR UPDATE ON agent_uae_escalation_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_uae_escalation_binding();

CREATE FUNCTION protect_agent_uae_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_uae_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_uae_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_uae_effect_intent();

CREATE FUNCTION approve_agent_uae_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status uae_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_uae_profile_escalation_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_uae_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_uae_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_uae_profile_authority;
ALTER FUNCTION revoke_agent_uae_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_uae_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_uae_profile_authority;
GRANT SELECT ON
  agent_uae_profile,
  agent_uae_profile_escalation_rule
TO agent_uae_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_uae_profile TO agent_uae_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_uae_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_uae_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_uae_profile FROM PUBLIC;

CREATE INDEX agent_uae_session_work_idx ON agent_uae_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_uae_session_profile_idx ON agent_uae_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_uae_binding_certificate_idx ON agent_uae_escalation_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_uae_binding_window_idx ON agent_uae_escalation_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_uae_catalog_ref_idx ON agent_uae_ack_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_uae_catalog_kind_idx ON agent_uae_ack_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_uae_evaluation_session_idx ON agent_uae_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_uae_certificate_session_idx ON agent_uae_escalation_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_uae_snapshot_session_idx ON agent_uae_escalation_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_uae_effect_work_idx ON agent_uae_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_uae_audit_time_idx ON agent_uae_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_uae_perception_status_idx ON agent_uae_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_uae_command_expiry_idx ON agent_uae_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_uae_invalidation_certificate_idx ON agent_uae_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_uae_authorization_evidence',
    'agent_uae_profile',
    'agent_uae_profile_escalation_rule',
    'agent_uae_ack_catalog',
    'agent_uae_session',
    'agent_uae_nomination_window',
    'agent_uae_escalation_snapshot',
    'agent_uae_evaluation_receipt',
    'agent_uae_escalation_certificate',
    'agent_uae_escalation_binding',
    'agent_uae_invalidation',
    'agent_uae_effect_intent',
    'agent_uae_budget_ledger',
    'agent_uae_terminal_record',
    'agent_uae_command_result',
    'agent_uae_audit_head',
    'agent_uae_audit_event',
    'agent_uae_audit_anchor',
    'agent_uae_perception_snapshot',
    'agent_uae_projection_checkpoint'
  ]
  LOOP
...
with session CAS. Escalation-certificate seal never joins a columnar
...
enum AgentUaeClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentUaeAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentUaeUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentUaeInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentUaeSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentUaeSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentUaeBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentUaeNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentUaeSourceKind!
  placementKind: AgentUaePlacementKind!
  keyLifecycle: AgentUaeKeyLifecycle!
  clockLifecycle: AgentUaeClockLifecycle!
...
type AgentUaeUnansweredAckEscalationSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentUaeEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentUaeEscalationCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentUaeBinding {
  ...
  sourceWindowKind: AgentUaeSourceKind!
  bindingOrdinal: Int!
  status: AgentUaeBindingStatus!
  placementKind: AgentUaePlacementKind!
  keyLifecycle: AgentUaeKeyLifecycle!
  clockLifecycle: AgentUaeClockLifecycle!
  attestedKeyKind: AgentUaeAttestedKeyKind!
  escalationKind: AgentUaeEscalationKind!
  purposeRelation: AgentUaePurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentUaeEffectObservation {
  effectId: ID!
  status: AgentUaeEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentUaePerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentUaeSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  escalationCurrentBindingCount: Int!
  escalationActiveBindingCount: Int!
  escalationExpiredBindingCount: Int!
  escalateUnansweredAckBindingCount: Int!
  holdEscalationWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectEscalationAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentUaeEffectObservation!]!
  remainingBudget: AgentUaeBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentUaeNextAction!]!
  blockedReasons: [AgentUaeBlockedReason!]!
  cardHash: SHA256!
}

type AgentUaeMutationResult {
  decision: String!
  session: AgentUaeSession
  certificate: AgentUaeEscalationCertificate
  member: AgentUaeBinding
  receipt: AgentUaeNominationReceipt
  snapshot: AgentUaeUnansweredAckEscalationSnapshot
  evaluation: AgentUaeEvaluationReceipt
  perception: AgentUaePerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateUnansweredAckEscalationSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentUaeBudgetInput!
}

input NominateSealedAckCertificateInput {
  accountId: ID!
  sessionId: ID!
...
input RefreshUnansweredAckEscalationInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateUnansweredAckEscalationInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateUnansweredAckEscalationInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentUaeInvalidationReason!
  idempotencyKey: String!
}

input PrepareEscalationEffectInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
...
input ResolveEscalationUncertaintyInput {
  ...
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentUaeUncertaintyResolution!
  idempotencyKey: String!
}

input AgentUaeProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentUaeSession(accountId: ID!, sessionId: ID!): AgentUaeSession
  agentUaeSloCertificate(accountId: ID!, certificateId: ID!): AgentUaeEscalationCertificate
  agentUaePerceptionCard(accountId: ID!, sessionId: ID!): AgentUaePerceptionCard
  agentUaeNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentUaeNominationReceipt
  agentUaeSearchProfiles(input: AgentUaeProfileSearchInput!): [AgentUaeProfile!]!
}

type Mutation {
  createUnansweredAckEscalationSession(
    input: CreateUnansweredAckEscalationSessionInput!
  ): AgentUaeMutationResult!
  nominateSealedAckCertificate(
    input: NominateSealedAckCertificateInput!
  ): AgentUaeMutationResult!
  refreshUnansweredAckEscalation(
    input: RefreshUnansweredAckEscalationInput!
  ): AgentUaeMutationResult!
  evaluateUnansweredAckEscalation(
    input: EvaluateUnansweredAckEscalationInput!
  ): AgentUaeMutationResult!
  sealEscalationCertificate(input: SealEscalationCertificateInput!): AgentUaeMutationResult!
  invalidateUnansweredAckEscalation(
    input: InvalidateUnansweredAckEscalationInput!
  ): AgentUaeMutationResult!
  prepareEscalationEffect(input: PrepareEscalationEffectInput!): AgentUaeMutationResult!
  resolveEscalationUncertainty(
    input: ResolveEscalationUncertaintyInput!
  ): AgentUaeMutationResult!
  closeUnansweredAckEscalationSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentUaeMutationResult!
  approveUnansweredAckEscalationProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- Perception cards never embed raw private keys, tool payloads, or redacted
  fact bodies.
- `sealEscalationCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  remaining TTL would invent `TRUSTED_ENROLLED`, with
  `ESCALATION_AS_CONFIRMATION_DENIED` when an unanswered acknowledgement
  or unanswered escalation is treated as confirmation, and with
  `LEDGER_SCAN_DENIED` when remaining time would be computed by walking
  a vote, TTL, page, or ack ledger.

## 10. Procedural memory

Approved escalation profiles are procedural memory: versioned
instructions for how sealed acknowledgement, SLO, window, TTL,
and confirm certificates become envelope-scoped higher-authority
escalations without inventing a winner and without rewriting an
unanswered acknowledgement or unanswered escalation as dual-
control success or `TRUSTED_ENROLLED`. Procedure refs may point
to successor-containment playbook steps. Profiles are immutable
after approval; agents perceive `procedureTags` and
`allowedNextActions` on perception cards, never inventing
escalation policy from embeddings.

## 11. Semantic retrieval and HNSW compatibility

Profile embeddings support advisory discovery ("which escalation
profile fits incident hop-attenuated first-enrollment unanswered
acknowledgement?"). Embeddings are account-owned and must be
...
Semantic retrieval may return escalation profiles only. It never
authorizes nominate, refresh, evaluate, seal, or escalate. Vector
`topK` is budgeted and clamped.

```sql
CREATE TABLE agent_uae_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
sealed / escalation-current / escalation-active /
escalation-expired / escalate-unanswered-ack /
hold-escalation-window / hold-unknown /
reject-escalation-as-confirmation / unknown / skipped /
...
perception replayable. Agents perceive `ESCALATION_CURRENT` as
a first-seen unanswered acknowledgement whose escalation window
is still open and still cannot invent a winner,
`ESCALATION_ACTIVE` as a bounded higher-authority page that
remaining time is below the profile threshold without
confirmation, `ESCALATION_EXPIRED` as a sealed escalation that
the window closed without dual-control trust,
`ESCALATE_UNANSWERED_ACK` as a dual-control higher-authority
page that does not confirm the key, `HOLD_ESCALATION_WINDOW` as
a trusted hold that refuses silent confirmation, `HOLD_UNKNOWN`
as a bounded uncertainty window,
`REJECT_ESCALATION_AS_CONFIRMATION` as a trusted negative for
an unanswered acknowledgement or unanswered escalation that
would have rewritten silence as success, and `SKIP` as a sealed
refusal — never as a key that "must have been trusted because
nobody escalated the unanswered acknowledgement."

## 13. ACID and consistency

### Row store

Session CAS, nomination receipts, escalation snapshots, evaluation
receipts, escalation-certificate seals, and audit appends are ACID
transactions in the hybrid row store.

### Columnar store

Columnar projections may accelerate analytics over sealed escalation
certificates but are not authoritative for escalation-current,
escalation-active, or reject-escalation-as-confirmation outcomes.
...
- Binding/threshold caps on escalations per certificate and per
  session.
- Budget ledgers for NOMINATE/EVALUATE/SEAL/VECTOR/INVALIDATE/ESCALATE.
...
- Planner rejects unscoped key-catalog, clock-catalog, working-
  ...
  ledger, page-ledger, ack-ledger, or board scans as **FULL SCAN
  REJECTED**.
...
- Evaluation never auto-restores neighbor-visible board
  ...
  acknowledgements, unanswered escalations, or unbound windows.
- Prior-acker exclusion rejects original dual-control ackers as
  the higher-authority pair.

## 15. Performance check for boards with 1M+ rows

### Full-table-scan risks

- Finding remaining unanswered acknowledgements to escalate by
  scanning the confirmation-vote, TTL-retirement, page, or ack
  ledger (rejected; nominate by `(account_id, source_window_id)`).
- Evaluating an escalation by walking all notify intents for an
  account (rejected).
...
- unanswered-ack escalation refresh: unique `(account_id, session_id,
...
ack-ledger/citation scan is **FULL SCAN REJECTED** before execution.
...
reconstructs session, escalation snapshot, evaluation, and certificate
...
- Purpose amplification for consumers: attenuation hash must
  narrow relative to escalation and session purposes.
- Sticky first-escalation success after supersession:
  invalidation + re-evaluate + notify uncertainty + profile
  revocation.
- Historical silence invented as `TRUSTED_ENROLLED` from an
  unanswered acknowledgement or unanswered escalation:
  history-rewrite, silence-success, and unanswered-ack-as-trust
  fences.
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED` by
  an escalation: first-enrollment-auto-trust and
  escalation-as-confirmation fences.
- Remaining time or escalation targets computed by walking
  vote, TTL, page, or ack ledgers: ledger-scan fence and
  **FULL SCAN REJECTED**.
- Halt leak of frozen bodies into escalate-as-restore:
  halt-escalate fence.
- Escalation that restores a halted body or invents a winner:
  successor-leak fence.
- Hop leak of donor purpose after attenuation hops: hop-leak
  fence.
- Inventing a winner under restored-slot escalation:
  certificates bind receipt and remaining sets, never
  `resolved_fact_hash`.
- Original ackers serving as the higher-authority pair:
  prior-acker-exclusion fence.
...
- History-rewrite rejection, silence-success rejection, first-
  enrollment-auto-trust rejection, escalation-as-confirmation
  rejection, ledger-scan rejection, halt-escalate rejection,
...
### Phase 1: shadow escalation

Compile profiles and validate acknowledgement-catalog nomination
without durable certificates.

### Phase 2: reject-escalation-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as `REJECT_ESCALATION_AS_CONFIRMATION`
or `SKIP`. Escalation-active stays closed.

### Phase 3: expire-only escalation and halt-escalate fences

Enable budgeted `ESCALATION_EXPIRED` from
`RESTORED_WITHOUT_WINNER` windows with `ttl_expired = true` and
`AWAITING_CONFIRMATION` only. An unanswered acknowledgement or
unanswered escalation never writes `TRUSTED_ENROLLED`.

### Phase 4: higher-authority pages and notify uncertainty

Enable `ESCALATION_CURRENT` / `ESCALATION_ACTIVE` under
point-lookup refresh and confirmation notify intents with
`UNKNOWN_EFFECT` reconciliation and `HOLD_UNKNOWN` that cannot
invent confirmation trust from an unanswered acknowledgement or
unanswered escalation.

### Phase 5: broad availability

Open approved profiles to autonomous agents under neighbor
budgets, including higher-authority escalations only for first-
enrollment keys that cannot rewrite an unanswered
acknowledgement as success and cannot auto-trust from
escalation silence.
...
- Nominate requires sealed acknowledgement-catalog material
  point lookup and hash match.
- Refresh unanswered-ack escalation is a point lookup and never
  a vote-ledger, page-ledger, or ack-ledger scan.
- Evaluate binds window set, remaining set, and attenuation
  under budget and prior-acker exclusion.
- Seal is rejected when an unanswered acknowledgement or
  unanswered escalation would become `TRUSTED_ENROLLED`, when
  an escalation is treated as confirmation, or when remaining
  time would be computed by scanning ledgers, and never invents
  a winning fact hash.
- Escalation-certificate seal binds immutable bindings under
  window-set, clock-set, remaining-set, and attenuation hashes
  — never a winner hash.
...
- Audit replay reconstructs sealed escalation certificates after
  process restart.

## 21. Product decision

Adopt the Unanswered-Ack Escalation Plane as the deterministic
higher-authority escalation path for unanswered, expired, or
single-control acknowledgements bound by the SLO Ack
Suppression, Remaining-Window SLO, Confirmation-Window
...
1. It preserves ACID and multi-tenant isolation while closing
   the post-acknowledgement escalation gap without history
   rewrite, silence-success, first-enrollment auto-trust,
   escalation-as-confirmation, unanswered-ack-as-trust, ledger
   scans, halt leak, purpose amplification, invented winners,
   prior-acker reuse, or vote/page/ack-ledger walks.
