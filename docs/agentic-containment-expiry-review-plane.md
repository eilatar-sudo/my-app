# mondayDB Agentic Containment-Expiry Review Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-09-03.v1`

## 1. Why this plane, before how

A sealed resolution-expiry-containment certificate can
dual-control `CONTAIN_EXPIRED_RESOLUTION` or seal
`CONTAINMENT_EXPIRED` when the containment window closes
without two distinct stewards. It does not decide **how
an expired, unanswered, or single-control containment is
reviewed** — without scanning every containment binding,
rewriting containment-expiry silence into
`TRUSTED_ENROLLED`, or treating a closed containment
window as confirmation.

Without a containment-expiry review plane, operators
and agents either:

- scan every pending containment, resolution, review,
  escalation, acknowledgement, page, and vote ledger
  looking for "who still has not dual-control reviewed
  the expired containment" (neighbor-harmful on boards
  with 1M+ rows), or
- treat containment-expiry silence as retroactive engine
  truth, so a closed containment window is rewritten as
  confirmation, historical `UNKNOWN_EFFECT` is rewritten
  as `ACKED`, a silent review auto-promotes
  `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED`, a halt-scoped
  body is "unlocked" by later review, and hop-
  attenuated purpose is amplified back to the donor.

The product trade-off is **review fluency versus
review isolation**:

- Treating every expired containment, freeze timeout, or
  containment timer as implicit confirmation maximizes
  agent fluency and reduces re-planning cost, but creates
  history-rewrite invention, containment-expiry-as-confirmation,
  expired-containment-as-trust, silence-success, first-
  enrollment auto-trust, unauditable containment storms,
  and recursive containment-ledger walks against neighbors.
- Binding a sealed review certificate under an
  approved review profile, containment-catalog point
  lookups, history-rewrite fences, silence-success
  fences, first-enrollment auto-trust fences, containment-
  expiry-as-confirmation fences, expired-containment-as-
  trust fences, halt-review fences, purpose-amplification
  fences, successor-leak fences, prior-containment-steward-
  exclusion fences, ledger-scan fences, and steward
  budgets adds one bounded evaluate transaction and
  short-lived review storage.
- Semantic similarity may discover review profiles,
  ...
  review snapshot refreshed, a certificate sealed, or
...
2. A review session opens under purpose, budget, and
   ...
   lookup from the Resolution-Expiry Containment,
...
3. mondayDB evaluates a review whose kind is a pure
   ...
   prior_containment_steward_set_hash,
   review_steward_count, review_window_ms,
   review_kind)`. Silence cannot become historical
   success. An expired containment seals
   `REJECT_EXPIRY_AS_CONFIRMATION` or
   `REVIEW_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `REVIEW_EXPIRED_CONTAINMENT` pages stewards
   for a first-enrollment key after `CONTAINMENT_EXPIRED`
   without inventing confirmation.
   `HOLD_REVIEW_WINDOW` holds further review for
   a bounded window without restoring a winner. Halted
   slots cannot become review-restored winners.
   Original containment stewards are excluded from the
   review-steward pair.
...
protection: recursive "re-review every expired
containment forever" or "treat containment-expiry silence
...
For any containment-expiry-review evaluation,
mondayDB can answer:
...
- Which nominated containment certificates, placement kinds,
  ...
  prior-containment-steward exclusions, and review kinds were
  bound?
...
The Containment-Expiry Review Plane owns:

1. Immutable approved review profiles as procedural
   memory of "how expired containments page stewards for
   a first-enrollment key without amplifying purpose,
   ...
   `TRUSTED_ENROLLED` from containment-expiry silence."
...
- **Resolution-Expiry Containment:** supplies sealed
  containment certificates whose `CONTAINMENT_EXPIRED`,
  unanswered, or single-control outcomes this plane may
  review. This plane never invents a new containment by
...
- **Executive Freeze / Thaw SLA:** halt/restore context that
  ...
  decides higher-authority paging; resolution-expiry
  containment decides first-enrollment freeze; this plane
  decides containment-expiry review.
...
- Letting an LLM decide that an expired containment "feels
...
- Scanning vote, TTL, page, ack, escalation, review, or
  containment ledgers to decide who may contain.
...
- review-window buckets (`review_window_ms`), max
  ...
  steward principals for `REVIEW_EXPIRED_CONTAINMENT`,
...
- allowed review kinds (`REVIEW_CURRENT`,
  ...
  `REVIEW_EXPIRED_CONTAINMENT`, `HOLD_REVIEW_WINDOW`,
...
Refresh containment-expiry-review is a point lookup
...
10. Silence and unknown windows cannot evaluate to
    `REVIEW_EXPIRED_CONTAINMENT` or any kind that implies
...
14. An expired containment never writes
    ...
    (containment-expiry-as-confirmation and unanswered-containment-
    as-trust fences).
15. `REVIEW_EXPIRED_CONTAINMENT` requires two distinct
...
certificate. Refresh containment-expiry-review is a
...
type TrustedNextAction =
  ...
  | "REFRESH_CONTAINMENT_EXPIRY"
  | "EVALUATE_CONTAINMENT_EXPIRY"
  | "SEAL_REVIEW_CERTIFICATE"
  | "INVALIDATE_CONTAINMENT_EXPIRY"
  | "PREPARE_REVIEW_EFFECT"
  | "REVIEW_EFFECT_UNCERTAINTY"
  | "CLOSE_SESSION";

type ContainmentExpiryReviewBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
...
interface ContainmentExpiryReviewBudget {
...
interface ContainmentExpiryReviewProfile {
...
interface ContainmentExpiryReviewSession {
  readonly accountId: AccountId;
  ...
  readonly budget: ContainmentExpiryReviewBudget;
  readonly consumed: Omit<
    ContainmentExpiryReviewBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface ContainmentExpiryReviewSnapshot {
  readonly accountId: AccountId;
...
interface ContainmentExpiryReviewEvaluationReceipt {
  readonly accountId: AccountId;
...
interface ContainmentExpiryReviewBinding {
  readonly accountId: AccountId;
...
interface ContainmentExpiryReviewCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface ContainmentExpiryReviewEffectObservation {
...
interface AgentContainmentExpiryReviewPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  ...
  readonly reviewExpiredContainmentBindingCount: number;
  ...
  readonly uncertainEffectIntents: readonly ContainmentExpiryReviewEffectObservation[];
  ...
  readonly blockedReasons: readonly ContainmentExpiryReviewBlockedReason[];
...
```ts
interface CreateContainmentExpiryReviewSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateContainmentExpiryReviewInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface CloseContainmentExpiryReviewSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type ContainmentExpiryReviewDecision =
  | { readonly decision: "ALLOWED"; readonly session: ContainmentExpiryReviewSession;
      readonly certificate?: ContainmentExpiryReviewCertificate;
      readonly member?: ContainmentExpiryReviewBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: ContainmentExpiryReviewSnapshot;
      readonly evaluation?: ContainmentExpiryReviewEvaluationReceipt;
      readonly perception: AgentContainmentExpiryReviewPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: ContainmentExpiryReviewBlockedReason;
...
```sql
CREATE TYPE cer_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE cer_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE cer_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE cer_source_kind AS ENUM (
...
CREATE TABLE agent_cer_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status cer_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_cer_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_cer_profile_review_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_cer_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_cer_containment_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind cer_source_kind NOT NULL,
  placement_kind cer_placement_kind NOT NULL,
  key_lifecycle cer_key_lifecycle NOT NULL,
  clock_lifecycle cer_clock_lifecycle NOT NULL,
  status cer_catalog_status NOT NULL,
...
CREATE TABLE agent_cer_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status cer_session_status NOT NULL,
...
CREATE TABLE agent_cer_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_cer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_cer_containment_catalog (account_id, source_window_id)
);

CREATE TABLE agent_cer_review_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_review_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_cer_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_cer_review_binding (
  account_id BIGINT NOT NULL,
  ...
  status cer_binding_status NOT NULL,
  placement_kind cer_placement_kind NOT NULL,
  key_lifecycle cer_key_lifecycle NOT NULL,
  clock_lifecycle cer_clock_lifecycle NOT NULL,
  attested_key_kind cer_attested_key_kind NOT NULL,
  review_kind cer_review_kind NOT NULL,
  purpose_relation cer_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_cer_review_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_cer_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_cer_containment_catalog (account_id, source_window_id)
...
CREATE TABLE agent_cer_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status cer_effect_status NOT NULL,
...
CREATE TABLE agent_cer_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_cer_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_cer_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status cer_session_status NOT NULL,
...
CREATE TABLE agent_cer_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_cer_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.cer_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.cer_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_cer_profile_protect
BEFORE INSERT OR UPDATE ON agent_cer_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_cer_profile();

CREATE FUNCTION protect_agent_cer_profile_review_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status cer_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_cer_profile
...
CREATE TRIGGER agent_cer_profile_review_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_cer_profile_review_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_cer_profile_review_rule();

CREATE FUNCTION protect_agent_cer_review_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_cer_review_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_cer_review_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_cer_review_snapshot();

CREATE FUNCTION protect_agent_cer_review_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED',
       'REVIEW_EXPIRED_CONTAINMENT', 'REJECT_EXPIRY_AS_CONFIRMATION'
     )
  ...
  IF NEW.review_kind = 'REJECT_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'containment-expiry-as-confirmation fence blocks TRUSTED_ENROLLED from an expired containment';
  END IF;

  IF NEW.review_kind = 'REVIEW_EXPIRED_CONTAINMENT'
  ...
  IF NEW.review_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from containment-expiry-review';
  END IF;
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED', 'REVIEW_EXPIRED_CONTAINMENT'
     )
  ...
  IF NEW.review_kind IN (
       'REVIEW_CURRENT', 'REVIEW_ACTIVE', 'REVIEW_EXPIRED_CONTAINMENT'
     )
...
CREATE TRIGGER agent_cer_review_binding_protect
BEFORE INSERT OR UPDATE ON agent_cer_review_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_cer_review_binding();

CREATE FUNCTION protect_agent_cer_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_cer_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_cer_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_cer_effect_intent();

CREATE FUNCTION approve_agent_cer_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status cer_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_cer_profile_review_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_cer_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_cer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_cer_profile_authority;
ALTER FUNCTION revoke_agent_cer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_cer_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_cer_profile_authority;
GRANT SELECT ON
  agent_cer_profile,
  agent_cer_profile_review_rule
TO agent_cer_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_cer_profile TO agent_cer_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_cer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_cer_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_cer_profile FROM PUBLIC;

CREATE INDEX agent_cer_session_work_idx ON agent_cer_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_cer_session_profile_idx ON agent_cer_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_cer_binding_certificate_idx ON agent_cer_review_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_cer_binding_window_idx ON agent_cer_review_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_cer_catalog_ref_idx ON agent_cer_containment_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_cer_catalog_kind_idx ON agent_cer_containment_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_cer_evaluation_session_idx ON agent_cer_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_cer_certificate_session_idx ON agent_cer_review_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_cer_snapshot_session_idx ON agent_cer_review_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_cer_effect_work_idx ON agent_cer_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_cer_audit_time_idx ON agent_cer_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_cer_perception_status_idx ON agent_cer_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_cer_command_expiry_idx ON agent_cer_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_cer_invalidation_certificate_idx ON agent_cer_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_cer_authorization_evidence',
    'agent_cer_profile',
    'agent_cer_profile_review_rule',
    'agent_cer_containment_catalog',
    'agent_cer_session',
    'agent_cer_nomination_window',
    'agent_cer_review_snapshot',
    'agent_cer_evaluation_receipt',
    'agent_cer_review_certificate',
    'agent_cer_review_binding',
    'agent_cer_invalidation',
    'agent_cer_effect_intent',
    'agent_cer_budget_ledger',
    'agent_cer_terminal_record',
    'agent_cer_command_result',
    'agent_cer_audit_head',
    'agent_cer_audit_event',
    'agent_cer_audit_anchor',
    'agent_cer_perception_snapshot',
    'agent_cer_projection_checkpoint'
  ]
  LOOP
...
enum AgentCerClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentCerAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentCerUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentCerInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentCerSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentCerSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentCerBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentCerNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentCerSourceKind!
  placementKind: AgentCerPlacementKind!
  keyLifecycle: AgentCerKeyLifecycle!
  clockLifecycle: AgentCerClockLifecycle!
...
type AgentCerContainmentExpiryReviewSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentCerEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentCerReviewCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentCerBinding {
  ...
  sourceWindowKind: AgentCerSourceKind!
  bindingOrdinal: Int!
  status: AgentCerBindingStatus!
  placementKind: AgentCerPlacementKind!
  keyLifecycle: AgentCerKeyLifecycle!
  clockLifecycle: AgentCerClockLifecycle!
  attestedKeyKind: AgentCerAttestedKeyKind!
  reviewKind: AgentCerReviewKind!
  purposeRelation: AgentCerPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentCerEffectObservation {
  effectId: ID!
  status: AgentCerEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentCerPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentCerSessionStatus!
  ...
  reviewExpiredContainmentBindingCount: Int!
  ...
  uncertainEffectIntents: [AgentCerEffectObservation!]!
  remainingBudget: AgentCerBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentCerNextAction!]!
  blockedReasons: [AgentCerBlockedReason!]!
  cardHash: SHA256!
}

type AgentCerMutationResult {
  decision: String!
  session: AgentCerSession
  certificate: AgentCerReviewCertificate
  member: AgentCerBinding
  receipt: AgentCerNominationReceipt
  snapshot: AgentCerContainmentExpiryReviewSnapshot
  evaluation: AgentCerEvaluationReceipt
  perception: AgentCerPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateContainmentExpiryReviewSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentCerBudgetInput!
}
...
input RefreshContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateContainmentExpiryReviewInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentCerInvalidationReason!
  idempotencyKey: String!
}
...
input ReviewEffectUncertaintyInput {
  ...
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentCerUncertaintyResolution!
  idempotencyKey: String!
}

input AgentCerProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentCerSession(accountId: ID!, sessionId: ID!): AgentCerSession
  agentCerReviewCertificate(accountId: ID!, certificateId: ID!): AgentCerReviewCertificate
  agentCerPerceptionCard(accountId: ID!, sessionId: ID!): AgentCerPerceptionCard
  agentCerNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentCerNominationReceipt
  agentCerSearchProfiles(input: AgentCerProfileSearchInput!): [AgentCerProfile!]!
}

type Mutation {
  createContainmentExpiryReviewSession(
    input: CreateContainmentExpiryReviewSessionInput!
  ): AgentCerMutationResult!
  nominateSealedContainmentCertificate(
    input: NominateSealedContainmentCertificateInput!
  ): AgentCerMutationResult!
  refreshContainmentExpiryReview(
    input: RefreshContainmentExpiryReviewInput!
  ): AgentCerMutationResult!
  evaluateContainmentExpiryReview(
    input: EvaluateContainmentExpiryReviewInput!
  ): AgentCerMutationResult!
  sealReviewCertificate(input: SealReviewCertificateInput!): AgentCerMutationResult!
  invalidateContainmentExpiryReview(
    input: InvalidateContainmentExpiryReviewInput!
  ): AgentCerMutationResult!
  prepareReviewEffect(input: PrepareReviewEffectInput!): AgentCerMutationResult!
  reviewEffectUncertainty(
    input: ReviewEffectUncertaintyInput!
  ): AgentCerMutationResult!
  closeContainmentExpiryReviewSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentCerMutationResult!
  approveContainmentExpiryReviewProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
scoped first-enrollment steward pages without inventing a
...
```sql
CREATE TABLE agent_cer_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
review-expired / review-expired-containment /
...
`REVIEW_CURRENT` as a first-seen expired containment
...
control trust, `REVIEW_EXPIRED_CONTAINMENT` as a dual-
...
- Finding remaining expired containments to contain by
...
- containment-expiry-review refresh: unique
...
- Refresh containment-expiry-review is a point
...
Adopt the Containment-Expiry Review Plane as the
deterministic steward-page path for expired, unanswered, or
single-control resolution-expiry-containment outcomes bound by the
Resolution-Expiry Containment, Unanswered-Containment Review,
Unanswered-Escalation Containment, Unanswered-Ack
Escalation, SLO Ack Suppression, Remaining-Window SLO,
Confirmation-Window Observability, Confirmation-TTL
Retirement, and First-Enrollment Confirmation planes.
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-containment expiry gap without
