# mondayDB Agentic Resolution-Expiry Containment Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-09-02.v1`

## 1. Why this plane, before how

A sealed review-resolution certificate can dual-control
`RESOLVE_UNANSWERED_REVIEW` or seal `RESOLUTION_EXPIRED`
when the resolution window closes without two distinct
stewards. It does not decide **how an expired,
unanswered, or single-control resolution is contained**
— without scanning every resolution binding, rewriting
resolution-expiry silence into `TRUSTED_ENROLLED`, or
treating a closed resolution window as confirmation.

Without a resolution-expiry containment plane, operators
and agents either:

- scan every pending resolution, review, containment,
  escalation, acknowledgement, page, and vote ledger
  looking for "who still has not dual-control contained
  the expired resolution" (neighbor-harmful on boards
  with 1M+ rows), or
- treat resolution-expiry silence as retroactive engine
  truth, so a closed resolution window is rewritten as
  confirmation, historical `UNKNOWN_EFFECT` is rewritten
  as `ACKED`, a silent containment auto-promotes
  `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED`, a halt-scoped
  body is "unlocked" by later containment, and hop-
  attenuated purpose is amplified back to the donor.

The product trade-off is **containment fluency versus
containment isolation**:

- Treating every expired resolution, freeze timeout, or
  containment timer as implicit confirmation maximizes
  agent fluency and reduces re-planning cost, but creates
  history-rewrite invention, expiry-as-confirmation,
  expired-resolution-as-trust, silence-success, first-
  enrollment auto-trust, unauditable containment storms,
  and recursive resolution-ledger walks against neighbors.
- Binding a sealed containment certificate under an
  approved containment profile, resolution-catalog point
  lookups, history-rewrite fences, silence-success
  fences, first-enrollment auto-trust fences, expiry-as-
  confirmation fences, expired-resolution-as-trust
  fences, halt-contain fences, purpose-amplification
  fences, successor-leak fences, prior-resolution-steward-
  exclusion fences, ledger-scan fences, and steward
  budgets adds one bounded evaluate transaction and
  short-lived containment storage.
- Semantic similarity may discover containment profiles,
  but it must never decide whether a sealed resolution,
  review, containment, escalation, acknowledgement, SLO,
  window, TTL, or confirm certificate may be nominated, a
  containment snapshot refreshed, a certificate sealed, or
...
1. An approved containment profile defines allowed source
   ...
   expired resolution into `TRUSTED_ENROLLED`.
...
3. mondayDB evaluates a containment whose kind is a pure
   ...
   success. An expired resolution seals
   `REJECT_EXPIRY_AS_CONFIRMATION` or
   `CONTAINMENT_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `CONTAIN_EXPIRED_RESOLUTION` freezes a
   first-enrollment key after `RESOLUTION_EXPIRED`
   without inventing confirmation.
   `HOLD_CONTAINMENT_WINDOW` holds further containment for
   a bounded window without restoring a winner. Halted
   slots cannot become containment-restored winners.
   Original resolution stewards are excluded from the
   containment-steward pair.
4. Sealing a containment certificate binds
   ...
   The certificate **must not** emit a `contained_fact_hash`.
...
protection: recursive "re-contain every expired
resolution forever" or "treat resolution-expiry silence
...
For any resolution-expiry-containment evaluation,
mondayDB can answer:
...
- Which nominated containment certificates, placement kinds,
  hop counts, attenuation hashes, remaining-set hashes,
  prior-containment-steward exclusions, and containment kinds were
  bound?
- Is the containment certificate still current, invalidated,
  resolved, hold-windowed, containment-expired, or awaiting
...
The Resolution-Expiry Containment Plane owns:

1. Immutable approved containment profiles as procedural
   memory of "how expired resolutions freeze a
   first-enrollment key without amplifying purpose,
   leaking halted facts, rewriting attested history,
   scanning vote, page, ack, escalation, review, or
   resolution ledgers, or inventing `TRUSTED_ENROLLED`
   from resolution-expiry silence."
...
3. Deterministic nomination of sealed resolution
   ...
   resolution-ledger, working-set, or board scans.
...
- **Review-Resolution:** supplies sealed resolution
  certificates whose `RESOLUTION_EXPIRED`, unanswered, or
  single-control outcomes this plane may contain. This
  plane never invents a new resolution by walking
  resolution bindings.
...
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may resolve a expired-resolution containment only through a
  sealed containment certificate.
...
- **Executive Freeze / Thaw SLA:** halt/restore context that
  ...
  decides higher-authority paging; review-resolution
  decides steward resolution; this plane decides
  resolution-expiry containment.
- **Emergency Containment:** the coarse stop/drain path used
  ...
  steward containment isolation, not workspace-wide
  containment.
...
- Letting an LLM decide that an expired resolution "feels
...
- Promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because
  a containment was unanswered, resolved, held, or expired.
- Scanning vote, TTL, page, ack, escalation, review, or
  resolution ledgers to decide who may contain.
...
- containment-window buckets (`containment_window_ms`), max
  ...
  steward principals for `CONTAIN_EXPIRED_RESOLUTION`,
...
- containment policy (`HISTORY_NEVER_REWRITTEN`,
  ...
  `EXPIRY_NEVER_CONFIRMS`, `EXPIRED_RESOLUTION_NEVER_TRUSTS`,
...
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed containment kinds (`CONTAIN_CURRENT`,
  `CONTAIN_ACTIVE`, `CONTAINMENT_EXPIRED`,
  `CONTAIN_EXPIRED_RESOLUTION`, `HOLD_CONTAINMENT_WINDOW`,
...
source-window point lookup against the resolution catalog.
Refresh resolution-expiry-containment is a point lookup
...
dual-control containment-steward counts, prior-containment-
...
`contained_fact_hash`. An expired resolution cannot
...
10. Silence and unknown windows cannot evaluate to
    `CONTAIN_EXPIRED_RESOLUTION` or any kind that implies
...
14. An expired resolution never writes
    `attested_key_kind = TRUSTED_ENROLLED`
    (expiry-as-confirmation and unanswered-containment-
    as-trust fences).
15. `CONTAIN_EXPIRED_RESOLUTION` requires two distinct
    containment-steward principals who are not the
...
17. Plans that require unscoped board, session, working-set,
    ...
    vote-ledger, TTL-ledger, page-ledger, ack-ledger,
    escalation-ledger, review-ledger, resolution-ledger, or citation-ledger scans are
    **FULL SCAN REJECTED**.
...
Nominate performs a point lookup on the resolution catalog
and writes an immutable nomination receipt with
`remaining_ttl_ms` copied from the sealed source
certificate. Refresh resolution-expiry-containment is a
...
type TrustedNextAction =
  | "NOMINATE_SEALED_RESOLUTION_CERTIFICATE"
  | "REFRESH_RESOLUTION_EXPIRY"
  | "EVALUATE_RESOLUTION_EXPIRY"
  | "SEAL_CONTAINMENT_CERTIFICATE"
  | "INVALIDATE_RESOLUTION_EXPIRY"
  | "PREPARE_CONTAINMENT_EFFECT"
  | "CONTAIN_EFFECT_UNCERTAINTY"
  | "CLOSE_SESSION";

type ResolutionExpiryContainmentBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
...
interface ResolutionExpiryContainmentBudget {
...
interface ResolutionExpiryContainmentProfile {
...
interface ResolutionExpiryContainmentSession {
  readonly accountId: AccountId;
  ...
  readonly budget: ResolutionExpiryContainmentBudget;
  readonly consumed: Omit<
    ResolutionExpiryContainmentBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface ResolutionExpiryContainmentSnapshot {
  readonly accountId: AccountId;
...
interface ResolutionExpiryContainmentEvaluationReceipt {
  readonly accountId: AccountId;
...
interface ResolutionExpiryContainmentBinding {
  readonly accountId: AccountId;
  ...
  readonly containmentKind: ResolutionKind;
...
interface ResolutionExpiryContainmentCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface ResolutionExpiryContainmentEffectObservation {
...
interface AgentResolutionExpiryContainmentPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly containCurrentBindingCount: number;
  readonly containActiveBindingCount: number;
  readonly containmentExpiredBindingCount: number;
  readonly containExpiredResolutionBindingCount: number;
  ...
  readonly uncertainEffectIntents: readonly ResolutionExpiryContainmentEffectObservation[];
  ...
  readonly blockedReasons: readonly ResolutionExpiryContainmentBlockedReason[];
...
```ts
interface CreateResolutionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface NominateSealedResolutionCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
...
interface RefreshResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateResolutionExpiryContainmentInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ContainEffectUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface CloseResolutionExpiryContainmentSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type ResolutionExpiryContainmentDecision =
  | { readonly decision: "ALLOWED"; readonly session: ResolutionExpiryContainmentSession;
      readonly certificate?: ResolutionExpiryContainmentCertificate;
      readonly member?: ResolutionExpiryContainmentBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: ResolutionExpiryContainmentSnapshot;
      readonly evaluation?: ResolutionExpiryContainmentEvaluationReceipt;
      readonly perception: AgentResolutionExpiryContainmentPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: ResolutionExpiryContainmentBlockedReason;
...
```sql
CREATE TYPE rex_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE rex_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_source_kind AS ENUM (
...
CREATE TYPE rex_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE rex_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_containment_kind AS ENUM (
  'CONTAIN_CURRENT', 'CONTAIN_ACTIVE', 'CONTAINMENT_EXPIRED',
  'CONTAIN_EXPIRED_RESOLUTION', 'HOLD_CONTAINMENT_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_EXPIRY_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE rex_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_rex_profile_authority NOLOGIN;

CREATE TABLE agent_rex_authorization_evidence (
...
CREATE TABLE agent_rex_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status rex_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_rex_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_rex_profile_containment_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_rex_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_rex_resolution_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind rex_source_kind NOT NULL,
  placement_kind rex_placement_kind NOT NULL,
  key_lifecycle rex_key_lifecycle NOT NULL,
  clock_lifecycle rex_clock_lifecycle NOT NULL,
  status rex_catalog_status NOT NULL,
...
CREATE TABLE agent_rex_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status rex_session_status NOT NULL,
...
CREATE TABLE agent_rex_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_rex_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_rex_resolution_catalog (account_id, source_window_id)
);

CREATE TABLE agent_rex_containment_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_containment_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_rex_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_rex_containment_binding (
  account_id BIGINT NOT NULL,
  ...
  status rex_binding_status NOT NULL,
  placement_kind rex_placement_kind NOT NULL,
  key_lifecycle rex_key_lifecycle NOT NULL,
  clock_lifecycle rex_clock_lifecycle NOT NULL,
  attested_key_kind rex_attested_key_kind NOT NULL,
  containment_kind rex_containment_kind NOT NULL,
  purpose_relation rex_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_rex_containment_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_rex_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_rex_resolution_catalog (account_id, source_window_id)
...
CREATE TABLE agent_rex_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status rex_effect_status NOT NULL,
...
CREATE TABLE agent_rex_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
...
CREATE TABLE agent_rex_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_rex_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status rex_session_status NOT NULL,
...
CREATE TABLE agent_rex_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_rex_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.rex_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.rex_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_rex_profile_protect
BEFORE INSERT OR UPDATE ON agent_rex_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_rex_profile();

CREATE FUNCTION protect_agent_rex_profile_containment_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status rex_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_rex_profile
...
CREATE TRIGGER agent_rex_profile_containment_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_rex_profile_containment_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_rex_profile_containment_rule();

CREATE FUNCTION protect_agent_rex_containment_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_rex_containment_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_rex_containment_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_rex_containment_snapshot();

CREATE FUNCTION protect_agent_rex_containment_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.containment_kind IN (
       'CONTAIN_CURRENT', 'CONTAIN_ACTIVE', 'CONTAINMENT_EXPIRED',
       'CONTAIN_EXPIRED_RESOLUTION', 'REJECT_EXPIRY_AS_CONFIRMATION'
     )
  ...
  IF NEW.containment_kind = 'REJECT_EXPIRY_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'expiry-as-confirmation fence blocks TRUSTED_ENROLLED from an expired resolution';
  END IF;

  IF NEW.containment_kind = 'CONTAIN_EXPIRED_RESOLUTION'
  ...
  IF NEW.containment_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from resolution-expiry-containment';
  END IF;
  ...
  IF NEW.containment_kind IN (
       'CONTAIN_CURRENT', 'CONTAIN_ACTIVE', 'CONTAINMENT_EXPIRED', 'CONTAIN_EXPIRED_RESOLUTION'
     )
  ...
  IF NEW.containment_kind IN (
       'CONTAIN_CURRENT', 'CONTAIN_ACTIVE', 'CONTAIN_EXPIRED_RESOLUTION'
     )
...
CREATE TRIGGER agent_rex_containment_binding_protect
BEFORE INSERT OR UPDATE ON agent_rex_containment_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_rex_containment_binding();

CREATE FUNCTION protect_agent_rex_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_rex_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_rex_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_rex_effect_intent();

CREATE FUNCTION approve_agent_rex_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status rex_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_rex_profile_containment_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_rex_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_rex_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_rex_profile_authority;
ALTER FUNCTION revoke_agent_rex_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_rex_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_rex_profile_authority;
GRANT SELECT ON
  agent_rex_profile,
  agent_rex_profile_containment_rule
TO agent_rex_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_rex_profile TO agent_rex_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_rex_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_rex_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_rex_profile FROM PUBLIC;

CREATE INDEX agent_rex_session_work_idx ON agent_rex_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_rex_session_profile_idx ON agent_rex_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_rex_binding_certificate_idx ON agent_rex_containment_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_rex_binding_window_idx ON agent_rex_containment_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_rex_catalog_ref_idx ON agent_rex_resolution_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_rex_catalog_kind_idx ON agent_rex_resolution_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_rex_evaluation_session_idx ON agent_rex_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_rex_certificate_session_idx ON agent_rex_containment_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_rex_snapshot_session_idx ON agent_rex_containment_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_rex_effect_work_idx ON agent_rex_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_rex_audit_time_idx ON agent_rex_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_rex_perception_status_idx ON agent_rex_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_rex_command_expiry_idx ON agent_rex_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_rex_invalidation_certificate_idx ON agent_rex_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_rex_authorization_evidence',
    'agent_rex_profile',
    'agent_rex_profile_containment_rule',
    'agent_rex_resolution_catalog',
    'agent_rex_session',
    'agent_rex_nomination_window',
    'agent_rex_containment_snapshot',
    'agent_rex_evaluation_receipt',
    'agent_rex_containment_certificate',
    'agent_rex_containment_binding',
    'agent_rex_invalidation',
    'agent_rex_effect_intent',
    'agent_rex_budget_ledger',
    'agent_rex_terminal_record',
    'agent_rex_command_result',
    'agent_rex_audit_head',
    'agent_rex_audit_event',
    'agent_rex_audit_anchor',
    'agent_rex_perception_snapshot',
    'agent_rex_projection_checkpoint'
  ]
  LOOP
...
enum AgentRexClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentRexAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentRexUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentRexInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentRexSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentRexSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentRexBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentRexNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentRexSourceKind!
  placementKind: AgentRexPlacementKind!
  keyLifecycle: AgentRexKeyLifecycle!
  clockLifecycle: AgentRexClockLifecycle!
...
type AgentRexResolutionExpiryContainmentSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentRexEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentRexContainmentCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentRexBinding {
  ...
  sourceWindowKind: AgentRexSourceKind!
  bindingOrdinal: Int!
  status: AgentRexBindingStatus!
  placementKind: AgentRexPlacementKind!
  keyLifecycle: AgentRexKeyLifecycle!
  clockLifecycle: AgentRexClockLifecycle!
  attestedKeyKind: AgentRexAttestedKeyKind!
  containmentKind: AgentRexContainmentKind!
  purposeRelation: AgentRexPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentRexEffectObservation {
  effectId: ID!
  status: AgentRexEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentRexPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentRexSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  containCurrentBindingCount: Int!
  containActiveBindingCount: Int!
  containmentExpiredBindingCount: Int!
  containExpiredResolutionBindingCount: Int!
  ...
  uncertainEffectIntents: [AgentRexEffectObservation!]!
  remainingBudget: AgentRexBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentRexNextAction!]!
  blockedReasons: [AgentRexBlockedReason!]!
  cardHash: SHA256!
}

type AgentRexMutationResult {
  decision: String!
  session: AgentRexSession
  certificate: AgentRexContainmentCertificate
  member: AgentRexBinding
  receipt: AgentRexNominationReceipt
  snapshot: AgentRexResolutionExpiryContainmentSnapshot
  evaluation: AgentRexEvaluationReceipt
  perception: AgentRexPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateResolutionExpiryContainmentSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentRexBudgetInput!
}

input NominateSealedResolutionCertificateInput {
  accountId: ID!
  sessionId: ID!
...
input RefreshResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateResolutionExpiryContainmentInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentRexInvalidationReason!
  idempotencyKey: String!
}
...
input ContainEffectUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentRexUncertaintyResolution!
  idempotencyKey: String!
}

input AgentRexProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentRexSession(accountId: ID!, sessionId: ID!): AgentRexSession
  agentRexResolutionCertificate(accountId: ID!, certificateId: ID!): AgentRexContainmentCertificate
  agentRexPerceptionCard(accountId: ID!, sessionId: ID!): AgentRexPerceptionCard
  agentRexNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentRexNominationReceipt
  agentRexSearchProfiles(input: AgentRexProfileSearchInput!): [AgentRexProfile!]!
}

type Mutation {
  createResolutionExpiryContainmentSession(
    input: CreateResolutionExpiryContainmentSessionInput!
  ): AgentRexMutationResult!
  nominateSealedResolutionCertificate(
    input: NominateSealedResolutionCertificateInput!
  ): AgentRexMutationResult!
  refreshResolutionExpiryContainment(
    input: RefreshResolutionExpiryContainmentInput!
  ): AgentRexMutationResult!
  evaluateResolutionExpiryContainment(
    input: EvaluateResolutionExpiryContainmentInput!
  ): AgentRexMutationResult!
  sealContainmentCertificate(input: SealContainmentCertificateInput!): AgentRexMutationResult!
  invalidateResolutionExpiryContainment(
    input: InvalidateResolutionExpiryContainmentInput!
  ): AgentRexMutationResult!
  prepareContainmentEffect(input: PrepareContainmentEffectInput!): AgentRexMutationResult!
  containEffectUncertainty(
    input: ContainEffectUncertaintyInput!
  ): AgentRexMutationResult!
  closeResolutionExpiryContainmentSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentRexMutationResult!
  approveResolutionExpiryContainmentProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealContainmentCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `EXPIRY_AS_CONFIRMATION_DENIED` when an expired resolution is treated as confirmation, and with
...
versioned instructions for how sealed resolution, review,
containment, escalation, acknowledgement, SLO, window, TTL,
and confirm certificates become envelope-scoped first-
enrollment freezes without inventing a winner and without
rewriting an expired resolution as dual-control success or
`TRUSTED_ENROLLED`.
...
enrollment expired resolution?"). Embeddings are
...
```sql
CREATE TABLE agent_rex_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
sealed / contain-current / contain-active /
containment-expired / contain-expired-resolution /
...
`CONTAIN_CURRENT` as a first-seen expired resolution
whose containment window is still open and still cannot
invent a winner, `CONTAIN_ACTIVE` as a bounded steward
...
control trust, `CONTAIN_EXPIRED_RESOLUTION` as a dual-
control expired-resolution containment that does not confirm the key,
...
expiry-as-confirmation outcomes.
...
- Evaluation never auto-restores neighbor-visible board
  ...
  acknowledgements, expired resolutions, or unbound
  windows.
- Prior-escalator exclusion rejects original dual-control
  containment authorities as the containment-steward pair.

## 15. Performance check for boards with 1M+ rows

### Full-table-scan risks

- Finding remaining expired resolutions to contain by
...
- resolution-expiry-containment refresh: unique
...
page-ledger/ack-ledger/escalation-ledger/review-ledger/resolution-ledger/citation scan is
...
- Historical silence invented as `TRUSTED_ENROLLED` from an
  expired resolution: history-rewrite, silence-success,
...
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED`
  ...
  expiry-as-confirmation fences.
- Remaining time or containment targets computed by walking
  vote, TTL, page, ack, escalation, review, or resolution ledgers: ledger-scan
...
- Inventing a winner under restored-slot containment:
  ...
  `contained_fact_hash`.
- Original containment authorities serving as the containment-steward
...
Compile profiles and validate resolution-catalog nomination
without durable certificates.

### Phase 2: reject-expiry-as-confirmation and skip only
...
### Phase 3: expire-only review and halt-contain fences

Enable budgeted `CONTAINMENT_EXPIRED` from
`RESTORED_WITHOUT_WINNER` windows with `ttl_expired = true`
and `AWAITING_CONFIRMATION` only. An expired resolution
never writes `TRUSTED_ENROLLED`.

### Phase 4: containment-expired-resolution containments and notify uncertainty

Enable `CONTAIN_CURRENT` / `CONTAIN_ACTIVE` under
...
budgets, including containment-expired-resolution containments only for
...
- Nominate requires sealed resolution-catalog material point
  lookup and hash match.
- Refresh resolution-expiry-containment is a point
...
- Seal is rejected when an expired resolution would
...
Adopt the Resolution-Expiry Containment Plane as the
deterministic freeze path for expired, unanswered, or
single-control review-resolution outcomes bound by the
Review-Resolution, Unanswered-Containment Review,
Unanswered-Escalation Containment, Unanswered-Ack
Escalation, SLO Ack Suppression, Remaining-Window SLO,
Confirmation-Window Observability, Confirmation-TTL
...
1. It preserves ACID and multi-tenant isolation while
   closing the post-resolution expiry gap without
   history rewrite, silence-success, first-enrollment auto-
   trust, expiry-as-confirmation, expired-resolution-as-
   trust, ledger scans, halt leak, purpose amplification,
   invented winners, prior-resolution-steward reuse, or
   vote/page/ack/escalation/containment/review/resolution-
   ledger walks.
