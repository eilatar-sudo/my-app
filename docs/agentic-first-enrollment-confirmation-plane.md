# mondayDB Agentic First-Enrollment Confirmation Plane

**Status:** Proposed  
**Decision owner:** mondayDB Product and Engineering  
**Contract version:** `2026-08-24.v1`

## 1. Why this plane, before how

A sealed successor-key enrollment certificate can bind a rotated signing
key to an already-attested successor clock. It does not decide
**whether a first-seen key may become `TRUSTED_ENROLLED`** — without
scanning every historical enrollment, rewriting silence into a trusted
first-ACK, or inventing a winner from a self-confirmed, single-control,
or halted body.

Without a first-enrollment confirmation plane, operators and agents either:

- scan every first-seen key looking for "which signatures can ride
  `TRUSTED_ENROLLED`" (neighbor-harmful on boards with 1M+ rows), or
- treat a newly enrolled first-seen key as retroactive engine truth, so
  historical `UNKNOWN_EFFECT` is rewritten as `ACKED`, a single steward
  auto-promotes their own nomination, a halt-scoped body is "unlocked"
  by later confirmation, and hop-attenuated purpose is amplified back
  to the donor.

The product trade-off is **confirmation fluency versus confirmation isolation**:

- Accepting every first-enrollment arrival immediately maximizes agent
  fluency and reduces re-planning cost, but creates history-rewrite
  invention, first-enrollment auto-trust, self-confirm leak, unauditable
  confirmation storms, and recursive key-catalog walks against neighbors.
- Binding a sealed confirmation certificate under an approved confirm
  profile, key point lookups, history-rewrite fences, first-enrollment
  auto-trust fences, self-confirm fences, single-control fences,
  halt-confirm fences, purpose-amplification fences, successor-leak
  fences, and steward budgets adds one bounded evaluate transaction and
  short-lived confirmation storage.
- Semantic similarity may discover confirm profiles, but it must never
  decide whether a key may be nominated, a vote recorded, a
  certificate sealed, or a notify dispatched.

The recommended model keeps the data plane deterministic:

1. An approved confirm profile defines allowed key kinds, dual-control
   policy, and notify policy. Evaluation **never** invents a winning fact
   hash and **never** rewrites a historical key kind.
2. A confirm session opens under purpose, budget, and authorization
   fences, and only nominates sealed enroll certificates, first-enrollment
   claims, successor-key claims, or rotated signing material by point lookup
   from the Successor-Key Enrollment, Successor-Clock Re-Attestation, and
   Provider-Key Rotation planes.
3. mondayDB evaluates a confirmation whose kind is a pure function of
   `(source_key_kind, placement_kind, key_lifecycle,
   clock_lifecycle, attested_key_kind, requested_purpose_hash,
   attenuation_hash, hop_count, confirmation_count,
   distinct_confirmer_count, self_confirm, first_seen_at,
   enroll_sealed_at, clock_attested_at, max_skew_ms)`.
   Silence cannot become historical success. First-enrollment keys
   cannot become trusted without two distinct humans. A nominating
   principal cannot confirm their own key. Halted slots cannot become
   confirmation-trusted.
4. Sealing a confirm certificate binds
   `consumer_ref + purpose_hash + key_set_hash + clock_set_hash +
   confirmer_set_hash + attenuation_hash`. The certificate **must not**
   emit a `resolved_fact_hash`.
5. Upstream invalidation marks certificates stale; notify intents may become
   `UNKNOWN_EFFECT` until acknowledged. Confirmation of `UNKNOWN_EFFECT`
   remains uncertain until a trusted dual-control vote arrives.
6. Unscoped key-catalog, clock-catalog, enroll-ledger, working-set,
   grant-graph, or board scans are **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor protection: recursive
"re-judge every first-seen key forever" or "keep every auto-trusted
signature until silence looks like success" loops are rejectable before they
scan boards with 1M+ rows. Perception is restored by sealed confirm
certificates, not by magic key orchestration inside the engine.

### Product outcome

For any first-enrollment confirmation evaluation, mondayDB can answer:

- Which profile, principal, and session authorized the nomination, vote,
  evaluate, seal, invalidate, or notify dispatch?
- Which nominated keys, placement kinds, hop counts, attenuation
  hashes, confirmer set hashes, and confirm kinds were bound?
- Is the confirm certificate still current, invalidated, or awaiting
  notify acknowledgement?
- Did async notify or first-enrollment sync become `UNKNOWN_EFFECT`?
- Can the confirmation history be replayed without invoking an LLM?

## 2. Scope and ownership

The First-Enrollment Confirmation Plane owns:

1. Immutable approved confirm profiles as procedural memory of "how a
   first-seen signing key is dual-control confirmed before it becomes
   `TRUSTED_ENROLLED` against an already-attested successor clock without
   amplifying purpose, leaking halted facts, rewriting attested history,
   or inventing success from silence or a single steward."
2. Tenant-scoped confirm sessions with purpose and budget fences.
3. Deterministic nomination of sealed keys, enroll certificates, and
   first-enrollment claims by point lookup — never key-catalog, working-set,
   or board scans.
4. Deterministic confirmation votes from two distinct humans, evaluation
   receipts, sealed confirm certificates, and immutable confirm bindings
   that never invent a winner.
5. Invalidation and notify intents with honest uncertainty.
6. Agent perception cards, audit replay, and bounded confirm budgets.

It integrates with, but does not replace:

- **Successor-Key Enrollment:** supplies sealed enroll certificates whose
  `FIRST_ENROLLMENT` keys this plane may confirm. This plane never
  enrolls a key against a clock and never rotates signing material.
- **Successor-Clock Re-Attestation / Watermark-Clock Attestation:**
  supply sealed successor-clock certificates that this plane may bind a
  confirmation against. This plane never invents a first clock sample.
- **Provider-Key Rotation:** supplies rotated signing-material claims that
  still require first-enrollment confirmation before `TRUSTED_ENROLLED`.
- **Key-Compromise Quarantine / Provider-Receipt Attestation:** supply
  historical receipt IDs whose trusted-at timestamps remain historical;
  this plane does not rewrite them as first-enrollment confirmed.
- **Envelope Tool-Effect Saga / Envelope Purpose Gate / Certificate
  Placement:** upstream hop-attenuated context that produced the keys
  now under confirmation review.
- **Executive Freeze / Thaw SLA:** halt/restore context that still forbids
  confirm-first-enrollment against a halted body.
- **Emergency Containment:** the coarse stop/drain path used when a
  contained key evaluates to `SKIP` or `REJECT_SINGLE_CONTROL`; this
  plane is purpose-scoped confirmation isolation, not workspace-wide
  containment.
- **Decision Memory:** may consume sealed confirm certificates as reuse
  evidence, not raw first-enrollment webhooks.
- **Query Governor / Budgets:** reserves nominate, evaluate, vector, seal,
  invalidate, and confirm units.

### Non-goals

- Letting an LLM decide that a silent or single-control key "feels
  confirmable enough."
- Auto-amplifying a hop-narrowed purpose back to the donor purpose.
- Reconstructing authoritative confirm certificates from columnar or
  vector projections.
- Cross-account first-enrollment confirmation or global nearest-neighbor
  authorization.
- Storing raw private keys, unrestricted tool payloads, or redacted
  plaintext.
- Claiming distributed atomicity with external time-distribution providers.
- Inventing a winning fact hash when a first-enrollment key arrives after a
  restored slot.
- Rewriting historical `SILENCE` or `UNKNOWN_EFFECT` keys as
  `CONFIRM_FIRST_ENROLLMENT` because a new vote exists.
- Auto-promoting `FIRST_ENROLLMENT` to `TRUSTED_ENROLLED` because a
  single steward nominated and confirmed the same key.
- Unbounded recursive key-catalog or board walks across boards with
  1M+ rows.

## 3. Product contract

### 3.1 Confirm profile contract

A profile version is immutable after approval. It defines:

- allowed observation kinds (`SEALED_ENROLL_CERTIFICATE`,
  `FIRST_ENROLLMENT_CLAIM`, `SUCCESSOR_KEY_CLAIM`,
  `ROTATED_SIGNING_MATERIAL`);
- evaluate threshold (distinct human principals, minimum 2), max bindings
  per certificate, and max nominated keys;
- confirm policy (`HISTORY_NEVER_REWRITTEN`,
  `FIRST_ENROLLMENT_NEVER_AUTO_TRUSTED`, `SELF_NEVER_CONFIRMS`,
  `SINGLE_CONTROL_NEVER_TRUSTED`, `HALT_DENIES_CONFIRM`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `GRACE_SILENCE_NEVER_SUCCESS`, `SKEW_BOUND_NEVER_TRUSTED_BEYOND`);
- purpose attenuation rules (narrowing only; never amplification);
- allowed confirm kinds (`CONFIRM_FIRST_ENROLLMENT`, `HOLD_UNKNOWN`,
  `REJECT_SINGLE_CONTROL`, `REJECT_SELF_CONFIRM`, `SKIP`) and notify
  policy after seal, invalidation, or upstream key or clock change;
- optional procedural refs for "how to present unknown, single-control, or
  self-confirm truth without a winner."

Only `APPROVED` versions are discoverable or executable. Revocation blocks new
sessions; in-flight sessions follow the captured revocation policy.

### 3.2 Session contract

Opening a session requires
`(account_id, principal_id, profile_id, version, purpose, budgets,
idempotency_key)`. The service validates authorization, captures policy and ACL
revisions, and reserves budgets.

Every mutation supplies `expected_revision` and a command idempotency key.
State advances by compare-and-swap on `state_revision`.

### 3.3 Evaluate and certificate contract

Nominating sealed key material returns a nomination receipt. Recording a
confirmation vote binds a distinct human principal to the session. Evaluating
a confirmation binds each nominated key to a confirm kind that is
compatible with the attested key kind, placement kind, key
lifecycle, clock lifecycle, dual-control threshold, self-confirm
exclusion, skew bound, and purpose relation. Sealing a
certificate binds
`consumer_ref + purpose_hash + key_set_hash + clock_set_hash +
confirmer_set_hash + attenuation_hash`. Certificates **must not** emit a
`resolved_fact_hash` winner. Bindings compiled from silence or unknown
keys are rejected when the requested confirm kind is
`CONFIRM_FIRST_ENROLLMENT` (history-rewrite fence). Bindings compiled from
`FIRST_ENROLLMENT` keys with fewer than two distinct confirmers are
rejected when the requested confirm kind is `CONFIRM_FIRST_ENROLLMENT`
(first-enrollment-auto-trust / single-control fence). Bindings compiled
from a confirmer who nominated the key or already voted are rejected
(self-confirm fence). Bindings compiled from halted, extended-halt, or
omitted keys are rejected when the requested confirm kind is
`CONFIRM_FIRST_ENROLLMENT` (halt-confirm fence). Bindings that would
amplify purpose relative to the key attenuation hash are rejected
(purpose-amplification fence). Successor bindings that would emit a
winner or restore a halted body are rejected (successor-leak fence).

### 3.4 Invalidation and effect contract

Invalidations bind certificates to upstream rotation, quarantine, placement,
clock, or visibility revocation. Notify intents start as `PREPARED`, may
become `UNKNOWN_EFFECT` when the successor provider does not acknowledge,
and never invent success from silence. Confirmation of `UNKNOWN_EFFECT`
remains `UNKNOWN_EFFECT` until a trusted dual-control vote arrives.

### 3.5 Availability contract

Confirm control-plane APIs target 99.99% availability for open, nominate,
vote, evaluate, seal, and perception reads. External notify and successor-clock
side-effects are best-effort and surfaced as uncertainty rather than silent
success. Confirm evaluation must not silently restore neighbor-impacting
board mutations from halted slots, single-control votes, or unbound
keys.

## 4. Deterministic invariants

1. Every row is scoped by `account_id`; resolvers set `app.account_id` before
   query.
2. Profiles start as `DRAFT` and become `APPROVED` only through an authority-
   fenced approval function.
3. Sealed profile definitions and confirm rules are immutable.
4. Binding identity
   (`source_key_id`, `disputed_fact_hash`, `attenuation_hash`,
   `binding_ordinal`, `successor_key_hash`, `confirmer_set_hash`) is
   immutable after seal.
5. Purpose attenuation may only narrow for consumers; amplification is rejected.
6. Key nomination uses point lookup by
   `(account_id, source_key_id)` — never full key-catalog or board
   scans.
7. Notify intents start as `PREPARED` and may become `UNKNOWN_EFFECT`.
8. Audit events form a hash chain per account.
9. Semantic retrieval discovers profiles only; it never authorizes
   nominate/vote/evaluate/seal/confirm.
10. Silence and unknown keys cannot evaluate to `CONFIRM_FIRST_ENROLLMENT`
    (history-rewrite fence).
11. Halted, extended-halt, and omitted keys cannot evaluate to
    `CONFIRM_FIRST_ENROLLMENT` (halt-confirm fence).
12. Requested purposes that amplify a key attenuation hash are rejected
    (purpose-amplification fence).
13. Successor confirmation cannot emit a winning fact hash or restore a
    halted body (successor-leak fence).
14. First-enrollment keys cannot evaluate to `CONFIRM_FIRST_ENROLLMENT`
    with fewer than two distinct confirmers (first-enrollment-auto-trust
    and single-control fences).
15. A nominating principal or a principal who already voted cannot
    evaluate to `CONFIRM_FIRST_ENROLLMENT` (self-confirm fence).
16. Confirm certificates bind key set, clock set, confirmer set,
    and attenuation hashes; they never invent a winning fact hash.
17. Plans that require unscoped board, session, working-set, grant-graph,
    key-catalog, clock-catalog, enroll-ledger, or citation-ledger
    scans are **FULL SCAN REJECTED**.

## 5. Execution model

### 5.1 Profile sealing

Draft profiles accumulate confirm rules. Approval validates definition
hash, requires at least one confirm rule, requires
`evaluate_threshold >= 2`, and fences the status transition.

### 5.2 Open session

Open captures the approved profile version, purpose hash, budget
reservations, and authorization evidence. The session starts `OPEN` with
`state_revision = 0`. Duplicate `(account_id, idempotency_key)` is rejected.

### 5.3 Nominate, vote, and evaluate

Nominate performs a point lookup on the key catalog and writes an
immutable nomination receipt. Vote appends a confirmation from a distinct
human principal. Evaluate is a pure function of nominated keys, confirmer
set hashes, clock lifecycles, dual-control counts, and purpose relation.
It never walks the catalog.

### 5.4 Seal confirm certificate

Seal binds the evaluation hashes to a consumer ref. The certificate stores
key-set, clock-set, confirmer-set, and attenuation hashes. It must
not store `resolved_fact_hash`.

### 5.5 Invalidate and dispatch

Invalidation is source-key keyed. Notify intents start `PREPARED` and may
become `UNKNOWN_EFFECT`. Dispatch never scans neighbor boards.

## 6. Lifecycle

### 6.1 Draft profile

A steward inserts a `DRAFT` profile and at least one confirm rule. Approval
is authority-fenced.

### 6.2 Session open

An authorized principal opens a session under the approved version.

### 6.3 Nominating / voting / evaluating

The session moves `OPEN → NOMINATING → EVALUATING` by CAS. Budgets decrement
in the ledger. Votes are append-only.

### 6.4 Sealed / invalidated

Seal moves the session to `SEALED`. Upstream revocation writes an
invalidation and may move bindings to `INVALIDATED`.

### 6.5 Terminal states

`CLOSED`, `EXPIRED`, `CANCELLED`, `FAILED`, `QUARANTINED`, and
`UNKNOWN_EFFECT` are terminal. Terminal records are append-only.

### 6.6 Retain

Audit events, certificates, and bindings retain for the account's legal
hold. Perception snapshots are derived and may be compacted after Merkle
anchor.

## 7. TypeScript contracts

```ts
type AccountId = number & { readonly brand: "AccountId" };
type ProfileId = string & { readonly brand: "ProfileId" };
type SessionId = string & { readonly brand: "SessionId" };
type SourceKeyId = string & { readonly brand: "SourceKeyId" };
type EvaluationId = string & { readonly brand: "EvaluationId" };
type CertificateId = string & { readonly brand: "CertificateId" };
type BindingId = string & { readonly brand: "BindingId" };
type VoteId = string & { readonly brand: "VoteId" };
type ConsumerRef = string & { readonly brand: "ConsumerRef" };
type Sha256 = string & { readonly brand: "Sha256" };
type Timestamp = string & { readonly brand: "Timestamp" };

type TrustedNextAction =
  | "NOMINATE_FIRST_ENROLLMENT_KEY"
  | "RECORD_CONFIRMATION_VOTE"
  | "EVALUATE_FIRST_ENROLLMENT_CONFIRMATION"
  | "SEAL_CONFIRM_CERTIFICATE"
  | "INVALIDATE_FIRST_ENROLLMENT_CONFIRMATION"
  | "PREPARE_CONFIRM_EFFECT"
  | "RESOLVE_CONFIRM_UNCERTAINTY"
  | "CLOSE_SESSION";

type FirstEnrollmentConfirmationBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  | "KEY_MISSING"
  | "EVALUATE_NOT_READY"
  | "HISTORY_REWRITE_DENIED"
  | "FIRST_ENROLLMENT_AUTO_TRUST_DENIED"
  | "SELF_CONFIRM_DENIED"
  | "SINGLE_CONTROL_DENIED"
  | "HALT_CONFIRM_DENIED"
  | "PURPOSE_AMPLIFICATION_DENIED"
  | "SUCCESSOR_LEAK_DENIED"
  | "HOP_LEAK_DENIED"
  | "UNBOUND_KEY_DENIED"
  | "UNATTESTED_CLOCK_DENIED"
  | "INVENTED_HISTORY_DENIED"
  | "SKEW_BOUND_DENIED"
  | "HASH_MISMATCH"
  | "DECISION_THRESHOLD_NOT_MET"
  | "POLICY_DENIED"
  | "UNKNOWN_EFFECT";

interface UntrustedText {
  readonly value: string;
  readonly provenance: "USER_INPUT" | "BOARD_VALUE" | "PROVIDER_VALUE" | "AGENT_DRAFT";
  readonly trust: "UNTRUSTED_CONTENT";
}

type ProfileStatus = "DRAFT" | "APPROVED" | "REVOKED";
type SessionStatus =
  | "OPEN"
  | "NOMINATING"
  | "EVALUATING"
  | "SEALED"
  | "DISPATCHING"
  | "CLOSED"
  | "EXPIRED"
  | "CANCELLED"
  | "FAILED"
  | "QUARANTINED"
  | "UNKNOWN_EFFECT";

type MemberStatus =
  | "SEALED"
  | "INVALIDATED"
  | "DISPATCHING"
  | "SUPERSEDED_REF"
  | "UNKNOWN_EFFECT";

type SourceKeyKind =
  | "SEALED_ENROLL_CERTIFICATE"
  | "FIRST_ENROLLMENT_CLAIM"
  | "SUCCESSOR_KEY_CLAIM"
  | "ROTATED_SIGNING_MATERIAL";

type PlacementKind =
  | "HALTED"
  | "EXTENDED_HALT"
  | "RESTORED_WITHOUT_WINNER"
  | "OMITTED"
  | "UNKNOWN_EFFECT";

type KeyLifecycle =
  | "FIRST_ENROLLMENT"
  | "CONFIRMED"
  | "MONOTONIC"
  | "REGRESSED"
  | "INVENTED_HISTORY"
  | "UNKNOWN_EFFECT";

type ClockLifecycle =
  | "ATTESTED"
  | "UNATTESTED"
  | "UNKNOWN_EFFECT";

type AttestedKeyKind =
  | "AWAITING_CONFIRMATION"
  | "TRUSTED_ENROLLED"
  | "TRUSTED_WITHIN_SKEW"
  | "REGRESSED"
  | "INVENTED_HISTORY"
  | "SILENCE"
  | "UNKNOWN_EFFECT";

type ConfirmKind =
  | "CONFIRM_FIRST_ENROLLMENT"
  | "HOLD_UNKNOWN"
  | "REJECT_SINGLE_CONTROL"
  | "REJECT_SELF_CONFIRM"
  | "SKIP"
  | "UNKNOWN_EFFECT";

type PurposeRelation =
  | "EQUAL"
  | "NARROWS"
  | "AMPLIFIES"
  | "UNRELATED"
  | "UNKNOWN_EFFECT";

type EffectIntentStatus =
  | "PREPARED"
  | "DISPATCHED"
  | "ACKED"
  | "FAILED"
  | "UNKNOWN_EFFECT";

interface FirstEnrollmentConfirmationBudget {
  readonly nominateUnits: number;
  readonly evaluateUnits: number;
  readonly sealUnits: number;
  readonly vectorUnits: number;
  readonly invalidateUnits: number;
  readonly confirmUnits: number;
  readonly maxWallTimeMs: number;
  readonly evaluateThreshold: number;
  readonly maxBindingsPerCertificate: number;
  readonly maxNominatedKeys: number;
}

interface FirstEnrollmentConfirmationProfile {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly name: string;
  readonly status: ProfileStatus;
  readonly definitionHash: Sha256;
  readonly evaluateThreshold: number;
  readonly maxBindingsPerCertificate: number;
  readonly maxNominatedKeys: number;
  readonly semanticTags: readonly string[];
  readonly procedureRef: string | null;
}

interface FirstEnrollmentConfirmationSession {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly status: SessionStatus;
  readonly stateRevision: bigint;
  readonly purpose: string;
  readonly budget: FirstEnrollmentConfirmationBudget;
  readonly consumed: Omit<
    FirstEnrollmentConfirmationBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
    | "maxBindingsPerCertificate"
    | "maxNominatedKeys"
  >;
  readonly principalId: string;
  readonly deadlineAt: Timestamp;
}

interface KeyNominationReceipt {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly sourceKeyId: SourceKeyId;
  readonly sourceKeyKind: SourceKeyKind;
  readonly placementKind: PlacementKind;
  readonly keyLifecycle: KeyLifecycle;
  readonly clockLifecycle: ClockLifecycle;
  readonly disputedFactHash: Sha256;
  readonly attenuationHash: Sha256;
  readonly donorPurposeHash: Sha256;
  readonly hopCount: number;
  readonly nominationHash: Sha256;
  readonly nominatedAt: Timestamp;
}

interface ConfirmationVote {
  readonly accountId: AccountId;
  readonly voteId: VoteId;
  readonly sessionId: SessionId;
  readonly sourceKeyId: SourceKeyId;
  readonly confirmerPrincipalId: string;
  readonly voteOrdinal: number;
  readonly voteHash: Sha256;
  readonly votedAt: Timestamp;
}

interface FirstEnrollmentConfirmationEvaluationReceipt {
  readonly accountId: AccountId;
  readonly evaluationId: EvaluationId;
  readonly sessionId: SessionId;
  readonly keySetHash: Sha256;
  readonly clockSetHash: Sha256;
  readonly confirmerSetHash: Sha256;
  readonly attenuationHash: Sha256;
  readonly evaluationHash: Sha256;
  readonly evaluatedAt: Timestamp;
}

interface FirstEnrollmentConfirmationBinding {
  readonly accountId: AccountId;
  readonly bindingId: BindingId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
  readonly sourceKeyId: SourceKeyId;
  readonly sourceKeyKind: SourceKeyKind;
  readonly bindingOrdinal: number;
  readonly status: MemberStatus;
  readonly placementKind: PlacementKind;
  readonly keyLifecycle: KeyLifecycle;
  readonly clockLifecycle: ClockLifecycle;
  readonly attestedKeyKind: AttestedKeyKind;
  readonly confirmKind: ConfirmKind;
  readonly purposeRelation: PurposeRelation;
  readonly disputedFactHash: Sha256;
  readonly attenuationHash: Sha256;
  readonly requestedPurposeHash: Sha256;
  readonly successorKeyHash: Sha256;
  readonly clockAttestationHash: Sha256;
  readonly confirmationCount: number;
  readonly distinctConfirmerCount: number;
  readonly selfConfirm: boolean;
  readonly sealedAt: Timestamp;
}

interface FirstEnrollmentConfirmationCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
  readonly consumerRef: ConsumerRef;
  readonly purposeHash: Sha256;
  readonly keySetHash: Sha256;
  readonly clockSetHash: Sha256;
  readonly confirmerSetHash: Sha256;
  readonly attenuationHash: Sha256;
  readonly bindingWatermark: number;
  readonly sealedAt: Timestamp;
}

interface FirstEnrollmentConfirmationEffectObservation {
  readonly effectId: string;
  readonly status: EffectIntentStatus;
  readonly providerIdempotencyKey: string;
  readonly generation: number;
  readonly lastErrorCode: string | null;
}

interface AgentFirstEnrollmentConfirmationPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly confirmFirstEnrollmentBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectSingleControlBindingCount: number;
  readonly rejectSelfConfirmBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly FirstEnrollmentConfirmationEffectObservation[];
  readonly remainingBudget: Omit<
    FirstEnrollmentConfirmationBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
    | "maxBindingsPerCertificate"
    | "maxNominatedKeys"
  >;
  readonly procedureTags: readonly string[];
  readonly allowedNextActions: readonly TrustedNextAction[];
  readonly blockedReasons: readonly FirstEnrollmentConfirmationBlockedReason[];
  readonly cardHash: Sha256;
}
```

Mutation contracts keep transition decisions deterministic and explicit:

```ts
interface CreateFirstEnrollmentConfirmationSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
  readonly idempotencyKey: string;
  readonly budget: FirstEnrollmentConfirmationBudget;
}

interface NominateFirstEnrollmentKeyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceKeyId: SourceKeyId;
  readonly expectedAttenuationHash: Sha256;
  readonly idempotencyKey: string;
}

interface RecordConfirmationVoteInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceKeyId: SourceKeyId;
  readonly confirmerPrincipalId: string;
  readonly idempotencyKey: string;
}

interface EvaluateFirstEnrollmentConfirmationInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly expectedReceiptSetHash: Sha256;
  readonly expectedConfirmerSetHash: Sha256;
  readonly expectedAttenuationHash: Sha256;
  readonly idempotencyKey: string;
}

interface SealConfirmCertificateInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly evaluationId: EvaluationId;
  readonly consumerRef: ConsumerRef;
  readonly expectedPurposeHash: Sha256;
  readonly expectedConfirmerSetHash: Sha256;
  readonly idempotencyKey: string;
}

interface InvalidateFirstEnrollmentConfirmationInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
  readonly sourceKeyId: SourceKeyId;
  readonly reasonCode: "SUPERSEDED" | "RETRACTED" | "QUARANTINED" | "KEY_REVOKED";
  readonly idempotencyKey: string;
}

interface PrepareConfirmEffectInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
  readonly idempotencyKey: string;
}

interface ResolveConfirmUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
  readonly resolution:
    | "RETRY_SAME_KEY"
    | "ACCEPT_CONFIRMATION"
    | "REJECT_ENVELOPE"
    | "REQUIRE_HUMAN";
  readonly idempotencyKey: string;
}

interface CloseFirstEnrollmentConfirmationSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
}

type FirstEnrollmentConfirmationDecision =
  | { readonly decision: "ALLOWED"; readonly session: FirstEnrollmentConfirmationSession;
      readonly certificate?: FirstEnrollmentConfirmationCertificate;
      readonly member?: FirstEnrollmentConfirmationBinding;
      readonly receipt?: KeyNominationReceipt;
      readonly vote?: ConfirmationVote;
      readonly evaluation?: FirstEnrollmentConfirmationEvaluationReceipt;
      readonly perception: AgentFirstEnrollmentConfirmationPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: FirstEnrollmentConfirmationBlockedReason;
      readonly retryable: boolean; readonly reason: string;
      readonly perception?: AgentFirstEnrollmentConfirmationPerceptionCard;
      readonly auditHash: Sha256 };
```

## 8. SQL row-store schema

```sql
CREATE TYPE fec_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE fec_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_source_kind AS ENUM (
  'SEALED_ENROLL_CERTIFICATE', 'FIRST_ENROLLMENT_CLAIM',
  'SUCCESSOR_KEY_CLAIM', 'ROTATED_SIGNING_MATERIAL'
);
CREATE TYPE fec_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE fec_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_confirm_kind AS ENUM (
  'CONFIRM_FIRST_ENROLLMENT', 'HOLD_UNKNOWN', 'REJECT_SINGLE_CONTROL',
  'REJECT_SELF_CONFIRM', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE fec_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_fec_profile_authority NOLOGIN;

CREATE TABLE agent_fec_authorization_evidence (
  account_id BIGINT NOT NULL,
  evidence_id UUID NOT NULL,
  principal_id TEXT NOT NULL,
  policy_revision BIGINT NOT NULL CHECK (policy_revision >= 0),
  resource_acl_revision BIGINT NOT NULL CHECK (resource_acl_revision >= 0),
  redacted_scope_summary JSONB NOT NULL,
  encrypted_evidence_ref TEXT NOT NULL,
  evidence_hash CHAR(64) NOT NULL,
  immutable_archive_ref TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, evidence_id),
  UNIQUE (account_id, evidence_id, evidence_hash),
  CHECK (length(evidence_hash) = 64)
);

CREATE TABLE agent_fec_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status fec_profile_status NOT NULL,
  definition_hash CHAR(64) NOT NULL CHECK (length(definition_hash) = 64),
  canonicalization_version TEXT NOT NULL,
  compiler_version TEXT NOT NULL,
  approval_validation_hash CHAR(64),
  authorization_evidence_id UUID NOT NULL,
  authorization_snapshot_hash CHAR(64) NOT NULL,
  evaluate_threshold SMALLINT NOT NULL
    CHECK (evaluate_threshold BETWEEN 2 AND 8),
  max_bindings_per_certificate SMALLINT NOT NULL
    CHECK (max_bindings_per_certificate BETWEEN 1 AND 256),
  max_nominated_keys SMALLINT NOT NULL
    CHECK (max_nominated_keys BETWEEN 1 AND 256),
  semantic_tags TEXT[] NOT NULL,
  procedure_ref TEXT,
  revocation_policy TEXT NOT NULL CHECK (
    revocation_policy IN (
      'ALLOW_IN_FLIGHT', 'STOP_BEFORE_EVALUATE', 'REQUIRE_CONTAINMENT'
    )
  ),
  created_by TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  approved_by TEXT,
  approved_at TIMESTAMPTZ,
  revoked_by TEXT,
  revoked_at TIMESTAMPTZ,
  PRIMARY KEY (account_id, profile_id, profile_version),
  UNIQUE (account_id, profile_id, profile_version, definition_hash),
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_fec_authorization_evidence (account_id, evidence_id),
  CHECK (
    (status = 'DRAFT' AND approved_at IS NULL
      AND approval_validation_hash IS NULL) OR
    (status IN ('APPROVED', 'REVOKED') AND approved_at IS NOT NULL
      AND approval_validation_hash IS NOT NULL)
  ),
  CHECK (
    (status <> 'REVOKED' AND revoked_by IS NULL AND revoked_at IS NULL) OR
    (status = 'REVOKED' AND revoked_by IS NOT NULL AND revoked_at IS NOT NULL)
  ),
  CHECK (length(authorization_snapshot_hash) = 64)
);

CREATE TABLE agent_fec_profile_confirm_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  rule_id TEXT NOT NULL,
  ordinal SMALLINT NOT NULL CHECK (ordinal BETWEEN 1 AND 64),
  allowed_source_kinds TEXT[] NOT NULL,
  evaluate_threshold SMALLINT NOT NULL CHECK (evaluate_threshold BETWEEN 2 AND 8),
  max_bindings_per_certificate SMALLINT NOT NULL
    CHECK (max_bindings_per_certificate BETWEEN 1 AND 256),
  require_dual_control BOOLEAN NOT NULL,
  confirm_instruction JSONB NOT NULL,
  PRIMARY KEY (account_id, profile_id, profile_version, rule_id),
  UNIQUE (account_id, profile_id, profile_version, ordinal),
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_fec_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_fec_key_catalog (
  account_id BIGINT NOT NULL,
  source_key_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_key_kind fec_source_kind NOT NULL,
  placement_kind fec_placement_kind NOT NULL,
  key_lifecycle fec_key_lifecycle NOT NULL,
  clock_lifecycle fec_clock_lifecycle NOT NULL,
  status fec_catalog_status NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  donor_purpose_hash CHAR(64) NOT NULL CHECK (length(donor_purpose_hash) = 64),
  hop_count SMALLINT NOT NULL CHECK (hop_count BETWEEN 0 AND 8),
  first_seen_at TIMESTAMPTZ NOT NULL,
  enroll_sealed_at TIMESTAMPTZ,
  clock_attested_at TIMESTAMPTZ,
  max_skew_ms BIGINT NOT NULL CHECK (max_skew_ms BETWEEN 0 AND 86400000),
  sealed_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, source_key_id),
  UNIQUE (account_id, receipt_ref, source_key_kind)
);

CREATE TABLE agent_fec_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status fec_session_status NOT NULL,
  state_revision BIGINT NOT NULL CHECK (state_revision >= 0),
  purpose TEXT NOT NULL,
  idempotency_key TEXT NOT NULL,
  budget_nominate_units BIGINT NOT NULL CHECK (budget_nominate_units >= 0),
  budget_evaluate_units BIGINT NOT NULL CHECK (budget_evaluate_units >= 0),
  budget_seal_units BIGINT NOT NULL CHECK (budget_seal_units >= 0),
  budget_vector_units BIGINT NOT NULL CHECK (budget_vector_units >= 0),
  budget_invalidate_units BIGINT NOT NULL CHECK (budget_invalidate_units >= 0),
  budget_confirm_units BIGINT NOT NULL CHECK (budget_confirm_units >= 0),
  consumed_nominate_units BIGINT NOT NULL CHECK (consumed_nominate_units >= 0),
  consumed_evaluate_units BIGINT NOT NULL CHECK (consumed_evaluate_units >= 0),
  consumed_seal_units BIGINT NOT NULL CHECK (consumed_seal_units >= 0),
  consumed_vector_units BIGINT NOT NULL CHECK (consumed_vector_units >= 0),
  consumed_invalidate_units BIGINT NOT NULL
    CHECK (consumed_invalidate_units >= 0),
  consumed_confirm_units BIGINT NOT NULL
    CHECK (consumed_confirm_units >= 0),
  budget_max_wall_time_ms BIGINT NOT NULL
    CHECK (budget_max_wall_time_ms BETWEEN 1000 AND 86400000),
  evaluate_threshold SMALLINT NOT NULL
    CHECK (evaluate_threshold BETWEEN 2 AND 8),
  max_bindings_per_certificate SMALLINT NOT NULL
    CHECK (max_bindings_per_certificate BETWEEN 1 AND 256),
  max_nominated_keys SMALLINT NOT NULL
    CHECK (max_nominated_keys BETWEEN 1 AND 256),
  deadline_at TIMESTAMPTZ NOT NULL,
  started_by TEXT NOT NULL,
  principal_id TEXT NOT NULL,
  authorization_evidence_id UUID NOT NULL,
  delegated_scope_hash CHAR(64) NOT NULL,
  authorization_revision BIGINT NOT NULL CHECK (authorization_revision >= 0),
  resource_scope_hash CHAR(64) NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  terminal_outcome_hash CHAR(64),
  PRIMARY KEY (account_id, session_id),
  UNIQUE (account_id, idempotency_key),
  UNIQUE (account_id, session_id, profile_id, profile_version),
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_fec_profile (account_id, profile_id, profile_version),
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_fec_authorization_evidence (account_id, evidence_id),
  CHECK (consumed_nominate_units <= budget_nominate_units),
  CHECK (consumed_evaluate_units <= budget_evaluate_units),
  CHECK (consumed_seal_units <= budget_seal_units),
  CHECK (consumed_vector_units <= budget_vector_units),
  CHECK (consumed_invalidate_units <= budget_invalidate_units),
  CHECK (consumed_confirm_units <= budget_confirm_units),
  CHECK (deadline_at > created_at),
  CHECK (length(delegated_scope_hash) = 64),
  CHECK (length(resource_scope_hash) = 64)
);

CREATE TABLE agent_fec_nomination_key (
  account_id BIGINT NOT NULL,
  receipt_id UUID NOT NULL,
  session_id UUID NOT NULL,
  source_key_id UUID NOT NULL,
  source_key_kind fec_source_kind NOT NULL,
  placement_kind fec_placement_kind NOT NULL,
  key_lifecycle fec_key_lifecycle NOT NULL,
  clock_lifecycle fec_clock_lifecycle NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  donor_purpose_hash CHAR(64) NOT NULL CHECK (length(donor_purpose_hash) = 64),
  hop_count SMALLINT NOT NULL CHECK (hop_count BETWEEN 0 AND 8),
  nomination_hash CHAR(64) NOT NULL CHECK (length(nomination_hash) = 64),
  nominated_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, receipt_id),
  UNIQUE (account_id, session_id, source_key_id, nomination_hash),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_key_id)
    REFERENCES agent_fec_key_catalog (account_id, source_key_id)
);

CREATE TABLE agent_fec_confirmation_vote (
  account_id BIGINT NOT NULL,
  vote_id UUID NOT NULL,
  session_id UUID NOT NULL,
  source_key_id UUID NOT NULL,
  confirmer_principal_id TEXT NOT NULL,
  vote_ordinal SMALLINT NOT NULL CHECK (vote_ordinal BETWEEN 1 AND 8),
  vote_hash CHAR(64) NOT NULL CHECK (length(vote_hash) = 64),
  voted_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, vote_id),
  UNIQUE (account_id, session_id, source_key_id, confirmer_principal_id),
  UNIQUE (account_id, session_id, source_key_id, vote_ordinal),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_key_id)
    REFERENCES agent_fec_key_catalog (account_id, source_key_id)
);

CREATE TABLE agent_fec_evaluation_receipt (
  account_id BIGINT NOT NULL,
  evaluation_id UUID NOT NULL,
  session_id UUID NOT NULL,
  key_set_hash CHAR(64) NOT NULL CHECK (length(key_set_hash) = 64),
  clock_set_hash CHAR(64) NOT NULL
    CHECK (length(clock_set_hash) = 64),
  confirmer_set_hash CHAR(64) NOT NULL
    CHECK (length(confirmer_set_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  evaluation_hash CHAR(64) NOT NULL CHECK (length(evaluation_hash) = 64),
  evaluated_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, evaluation_id),
  UNIQUE (account_id, session_id, evaluation_hash),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id)
);

CREATE TABLE agent_fec_confirm_certificate (
  account_id BIGINT NOT NULL,
  certificate_id UUID NOT NULL,
  session_id UUID NOT NULL,
  evaluation_id UUID NOT NULL,
  consumer_ref TEXT NOT NULL,
  purpose_hash CHAR(64) NOT NULL CHECK (length(purpose_hash) = 64),
  key_set_hash CHAR(64) NOT NULL CHECK (length(key_set_hash) = 64),
  clock_set_hash CHAR(64) NOT NULL
    CHECK (length(clock_set_hash) = 64),
  confirmer_set_hash CHAR(64) NOT NULL
    CHECK (length(confirmer_set_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  binding_watermark SMALLINT NOT NULL CHECK (binding_watermark BETWEEN 0 AND 256),
  sealed_revision BIGINT NOT NULL CHECK (sealed_revision >= 0),
  sealed_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, certificate_id),
  UNIQUE (account_id, session_id, consumer_ref, sealed_revision),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id),
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_fec_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_fec_confirm_binding (
  account_id BIGINT NOT NULL,
  binding_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  session_id UUID NOT NULL,
  source_key_id UUID NOT NULL,
  source_key_kind fec_source_kind NOT NULL,
  binding_ordinal SMALLINT NOT NULL CHECK (binding_ordinal BETWEEN 0 AND 256),
  status fec_binding_status NOT NULL,
  placement_kind fec_placement_kind NOT NULL,
  key_lifecycle fec_key_lifecycle NOT NULL,
  clock_lifecycle fec_clock_lifecycle NOT NULL,
  attested_key_kind fec_attested_key_kind NOT NULL,
  confirm_kind fec_confirm_kind NOT NULL,
  purpose_relation fec_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  requested_purpose_hash CHAR(64) NOT NULL
    CHECK (length(requested_purpose_hash) = 64),
  donor_purpose_hash CHAR(64) NOT NULL CHECK (length(donor_purpose_hash) = 64),
  hop_count SMALLINT NOT NULL CHECK (hop_count BETWEEN 0 AND 8),
  successor_key_hash CHAR(64) NOT NULL
    CHECK (length(successor_key_hash) = 64),
  clock_attestation_hash CHAR(64) NOT NULL
    CHECK (length(clock_attestation_hash) = 64),
  confirmation_count SMALLINT NOT NULL
    CHECK (confirmation_count BETWEEN 0 AND 8),
  distinct_confirmer_count SMALLINT NOT NULL
    CHECK (distinct_confirmer_count BETWEEN 0 AND 8),
  self_confirm BOOLEAN NOT NULL,
  sealed_revision BIGINT NOT NULL CHECK (sealed_revision >= 0),
  sealed_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, binding_id),
  UNIQUE (account_id, certificate_id, source_key_id, binding_ordinal,
    sealed_revision),
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_fec_confirm_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id),
  FOREIGN KEY (account_id, source_key_id)
    REFERENCES agent_fec_key_catalog (account_id, source_key_id)
);

CREATE TABLE agent_fec_invalidation (
  account_id BIGINT NOT NULL,
  invalidation_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  source_key_id UUID NOT NULL,
  prior_disputed_fact_hash CHAR(64) NOT NULL
    CHECK (length(prior_disputed_fact_hash) = 64),
  next_disputed_fact_hash CHAR(64),
  reason_code TEXT NOT NULL CHECK (
    reason_code IN (
      'SUPERSEDED', 'RETRACTED', 'QUARANTINED', 'KEY_REVOKED'
    )
  ),
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, invalidation_id),
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_fec_confirm_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, source_key_id)
    REFERENCES agent_fec_key_catalog (account_id, source_key_id)
);

CREATE TABLE agent_fec_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status fec_effect_status NOT NULL,
  provider_idempotency_key TEXT NOT NULL,
  generation INTEGER NOT NULL CHECK (generation >= 0),
  canonical_request_hash CHAR(64) NOT NULL
    CHECK (length(canonical_request_hash) = 64),
  last_error_code TEXT,
  created_at TIMESTAMPTZ NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, effect_id),
  UNIQUE (account_id, provider_idempotency_key),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id),
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_fec_confirm_certificate (account_id, certificate_id)
);

CREATE TABLE agent_fec_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  entry_sequence BIGINT NOT NULL CHECK (entry_sequence >= 0),
  unit_type TEXT NOT NULL CHECK (
    unit_type IN (
      'NOMINATE', 'EVALUATE', 'SEAL', 'VECTOR', 'INVALIDATE', 'CONFIRM'
    )
  ),
  units BIGINT NOT NULL CHECK (units > 0),
  reason_code TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, session_id, entry_sequence),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id)
);

CREATE TABLE agent_fec_terminal_record (
  account_id BIGINT NOT NULL,
  resolution_id UUID NOT NULL,
  session_id UUID NOT NULL,
  effect_id UUID,
  conflict_id UUID,
  decision_code TEXT NOT NULL,
  decided_by TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, resolution_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id)
);

CREATE TABLE agent_fec_command_result (
  account_id BIGINT NOT NULL,
  command_id UUID NOT NULL,
  session_id UUID,
  operation_name TEXT NOT NULL,
  principal_id TEXT NOT NULL,
  idempotency_key TEXT NOT NULL,
  decision TEXT NOT NULL,
  result_hash CHAR(64) NOT NULL CHECK (length(result_hash) = 64),
  expires_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, command_id),
  UNIQUE (account_id, operation_name, principal_id, idempotency_key)
);

CREATE TABLE agent_fec_audit_head (
  account_id BIGINT NOT NULL,
  head_sequence BIGINT NOT NULL CHECK (head_sequence >= 0),
  head_hash CHAR(64) NOT NULL CHECK (length(head_hash) = 64),
  updated_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id)
);

CREATE TABLE agent_fec_audit_event (
  account_id BIGINT NOT NULL,
  event_sequence BIGINT NOT NULL CHECK (event_sequence >= 0),
  session_id UUID,
  event_type TEXT NOT NULL,
  payload_hash CHAR(64) NOT NULL CHECK (length(payload_hash) = 64),
  prev_hash CHAR(64) NOT NULL CHECK (length(prev_hash) = 64),
  event_hash CHAR(64) NOT NULL CHECK (length(event_hash) = 64),
  occurred_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, event_sequence)
);

CREATE TABLE agent_fec_audit_anchor (
  account_id BIGINT NOT NULL,
  anchor_id UUID NOT NULL,
  from_sequence BIGINT NOT NULL,
  to_sequence BIGINT NOT NULL,
  merkle_root CHAR(64) NOT NULL CHECK (length(merkle_root) = 64),
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, anchor_id),
  CHECK (to_sequence >= from_sequence)
);

CREATE TABLE agent_fec_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status fec_session_status NOT NULL,
  card_hash CHAR(64) NOT NULL CHECK (length(card_hash) = 64),
  card_body JSONB NOT NULL,
  created_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, snapshot_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_fec_session (account_id, session_id)
);

CREATE TABLE agent_fec_projection_checkpoint (
  account_id BIGINT NOT NULL,
  projector_name TEXT NOT NULL,
  source_watermark TEXT NOT NULL,
  projected_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, projector_name)
);

CREATE FUNCTION protect_agent_fec_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.status IS DISTINCT FROM 'DRAFT' THEN
      RAISE EXCEPTION 'profiles must start as DRAFT';
    END IF;
    RETURN NEW;
  END IF;

  IF OLD.status IS DISTINCT FROM 'DRAFT'
     AND (
       NEW.definition_hash IS DISTINCT FROM OLD.definition_hash
       OR NEW.evaluate_threshold IS DISTINCT FROM OLD.evaluate_threshold
       OR NEW.max_bindings_per_certificate
         IS DISTINCT FROM OLD.max_bindings_per_certificate
       OR NEW.max_nominated_keys
         IS DISTINCT FROM OLD.max_nominated_keys
       OR NEW.semantic_tags IS DISTINCT FROM OLD.semantic_tags
       OR NEW.procedure_ref IS DISTINCT FROM OLD.procedure_ref
       OR NEW.revocation_policy IS DISTINCT FROM OLD.revocation_policy
       OR NEW.authorization_evidence_id
         IS DISTINCT FROM OLD.authorization_evidence_id
       OR NEW.authorization_snapshot_hash
         IS DISTINCT FROM OLD.authorization_snapshot_hash
     ) THEN
    RAISE EXCEPTION 'sealed profile definition is immutable';
  END IF;

  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.fec_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
       ) THEN
      RAISE EXCEPTION 'profile revocation requires authority fence';
    END IF;
    RETURN NEW;
  END IF;

  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.fec_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
       ) THEN
      RAISE EXCEPTION 'profile approval requires authority fence';
    END IF;
    RETURN NEW;
  END IF;

  IF OLD.status IS DISTINCT FROM NEW.status
     AND NOT (
       (OLD.status = 'DRAFT' AND NEW.status = 'APPROVED')
       OR (OLD.status = 'APPROVED' AND NEW.status = 'REVOKED')
     ) THEN
    RAISE EXCEPTION 'illegal profile status transition';
  END IF;

  RETURN NEW;
END
$protect_profile$;

CREATE TRIGGER agent_fec_profile_protect
BEFORE INSERT OR UPDATE ON agent_fec_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_fec_profile();

CREATE FUNCTION protect_agent_fec_profile_confirm_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status fec_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_fec_profile
  WHERE account_id = COALESCE(NEW.account_id, OLD.account_id)
    AND profile_id = COALESCE(NEW.profile_id, OLD.profile_id)
    AND profile_version = COALESCE(NEW.profile_version, OLD.profile_version);

  IF profile_status IS DISTINCT FROM 'DRAFT' THEN
    RAISE EXCEPTION 'sealed profile confirm rules are immutable';
  END IF;
  RETURN COALESCE(NEW, OLD);
END
$protect_rule$;

CREATE TRIGGER agent_fec_profile_confirm_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_fec_profile_confirm_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_fec_profile_confirm_rule();

CREATE FUNCTION protect_agent_fec_confirmation_vote()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_vote$
DECLARE
  opener_principal TEXT;
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'confirmation votes are append-only';
  END IF;

  SELECT principal_id INTO opener_principal
  FROM agent_fec_session
  WHERE account_id = NEW.account_id
    AND session_id = NEW.session_id;

  IF opener_principal IS NOT DISTINCT FROM NEW.confirmer_principal_id THEN
    RAISE EXCEPTION 'self-confirm fence blocks vote by nominating principal';
  END IF;

  RETURN NEW;
END
$protect_vote$;

CREATE TRIGGER agent_fec_confirmation_vote_protect
BEFORE INSERT OR UPDATE ON agent_fec_confirmation_vote
FOR EACH ROW EXECUTE FUNCTION protect_agent_fec_confirmation_vote();

CREATE FUNCTION protect_agent_fec_confirm_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.source_key_id IS DISTINCT FROM OLD.source_key_id
       OR NEW.disputed_fact_hash IS DISTINCT FROM OLD.disputed_fact_hash
       OR NEW.attenuation_hash IS DISTINCT FROM OLD.attenuation_hash
       OR NEW.binding_ordinal IS DISTINCT FROM OLD.binding_ordinal
       OR NEW.source_key_kind IS DISTINCT FROM OLD.source_key_kind
       OR NEW.placement_kind IS DISTINCT FROM OLD.placement_kind
       OR NEW.key_lifecycle IS DISTINCT FROM OLD.key_lifecycle
       OR NEW.clock_lifecycle IS DISTINCT FROM OLD.clock_lifecycle
       OR NEW.attested_key_kind
         IS DISTINCT FROM OLD.attested_key_kind
       OR NEW.confirm_kind IS DISTINCT FROM OLD.confirm_kind
       OR NEW.purpose_relation IS DISTINCT FROM OLD.purpose_relation
       OR NEW.requested_purpose_hash IS DISTINCT FROM OLD.requested_purpose_hash
       OR NEW.successor_key_hash IS DISTINCT FROM OLD.successor_key_hash
       OR NEW.clock_attestation_hash
         IS DISTINCT FROM OLD.clock_attestation_hash
       OR NEW.confirmation_count IS DISTINCT FROM OLD.confirmation_count
       OR NEW.distinct_confirmer_count
         IS DISTINCT FROM OLD.distinct_confirmer_count
       OR NEW.self_confirm IS DISTINCT FROM OLD.self_confirm
       OR NEW.certificate_id IS DISTINCT FROM OLD.certificate_id THEN
      RAISE EXCEPTION 'confirm binding identity is immutable';
    END IF;
    RETURN NEW;
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.attested_key_kind IN ('SILENCE', 'UNKNOWN_EFFECT') THEN
    RAISE EXCEPTION 'history-rewrite fence blocks confirmation from silent or unknown successor';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.confirmation_count < 2 THEN
    RAISE EXCEPTION 'single-control fence blocks first-enrollment confirmation below dual-control threshold';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.distinct_confirmer_count < 2 THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED without two distinct humans';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.self_confirm IS TRUE THEN
    RAISE EXCEPTION 'self-confirm fence blocks first-enrollment confirmation by a prior principal';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.clock_lifecycle = 'UNATTESTED' THEN
    RAISE EXCEPTION 'unattested-clock fence blocks confirmation against an unattested clock';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.attested_key_kind = 'INVENTED_HISTORY' THEN
    RAISE EXCEPTION 'invented-history fence blocks confirm for invented successor history';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED') THEN
    RAISE EXCEPTION 'halt-confirm fence blocks confirmation on halted key';
  END IF;

  IF NEW.purpose_relation = 'AMPLIFIES' THEN
    RAISE EXCEPTION 'purpose-amplification fence blocks broader purpose than receipt attenuation';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED') THEN
    RAISE EXCEPTION 'successor-leak fence blocks restore of halted body';
  END IF;

  IF NEW.hop_count > 0
     AND NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.requested_purpose_hash IS NOT DISTINCT FROM NEW.donor_purpose_hash THEN
    RAISE EXCEPTION 'hop-leak fence blocks donor-purpose confirm after attenuation hops';
  END IF;

  IF NEW.confirm_kind = 'CONFIRM_FIRST_ENROLLMENT'
     AND NEW.attested_key_kind NOT IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'unbound-key fence blocks confirmation without trusted enrolled key';
  END IF;

  RETURN NEW;
END
$protect_binding$;

CREATE TRIGGER agent_fec_confirm_binding_protect
BEFORE INSERT OR UPDATE ON agent_fec_confirm_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_fec_confirm_binding();

CREATE FUNCTION protect_agent_fec_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.intent_status IS DISTINCT FROM 'PREPARED' THEN
      RAISE EXCEPTION 'effect intents must start as PREPARED';
    END IF;
    RETURN NEW;
  END IF;

  IF OLD.canonical_request_hash IS DISTINCT FROM NEW.canonical_request_hash
     OR OLD.provider_idempotency_key
       IS DISTINCT FROM NEW.provider_idempotency_key
     OR OLD.certificate_id IS DISTINCT FROM NEW.certificate_id THEN
    RAISE EXCEPTION 'prepared effect identity is immutable';
  END IF;

  RETURN NEW;
END
$protect_effect$;

CREATE TRIGGER agent_fec_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_fec_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_fec_effect_intent();

CREATE FUNCTION approve_agent_fec_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
  validated_definition_hash TEXT,
  approver_id TEXT
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $approve$
DECLARE
  stored_hash CHAR(64);
  stored_status fec_profile_status;
  rule_count INTEGER;
BEGIN
  SELECT definition_hash, status
    INTO stored_hash, stored_status
  FROM agent_fec_profile
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version
  FOR UPDATE;

  IF length(validated_definition_hash) <> 64
     OR stored_status IS DISTINCT FROM 'DRAFT'
     OR stored_hash IS DISTINCT FROM validated_definition_hash THEN
    RAISE EXCEPTION 'profile approval hash or state mismatch';
  END IF;

  SELECT count(*)::INTEGER INTO rule_count
  FROM agent_fec_profile_confirm_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;

  IF rule_count < 1 THEN
    RAISE EXCEPTION 'profile requires at least one confirm rule';
  END IF;

  PERFORM set_config(
    'app.fec_profile_approval',
    concat(
      recipient_profile_id::TEXT, ':',
      recipient_profile_version::TEXT, ':',
      validated_definition_hash
    ),
    true
  );

  UPDATE agent_fec_profile
  SET status = 'APPROVED',
      approved_by = approver_id,
      approved_at = clock_timestamp(),
      approval_validation_hash = validated_definition_hash
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
END
$approve$;

CREATE FUNCTION revoke_agent_fec_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
  expected_definition_hash TEXT,
  revoker_id TEXT
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $revoke$
DECLARE
  stored_hash CHAR(64);
  stored_status fec_profile_status;
BEGIN
  SELECT definition_hash, status
    INTO stored_hash, stored_status
  FROM agent_fec_profile
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version
  FOR UPDATE;

  IF length(expected_definition_hash) <> 64
     OR stored_status IS DISTINCT FROM 'APPROVED'
     OR stored_hash IS DISTINCT FROM expected_definition_hash THEN
    RAISE EXCEPTION 'profile revocation hash or state mismatch';
  END IF;

  PERFORM set_config(
    'app.fec_profile_revocation',
    concat(
      recipient_profile_id::TEXT, ':',
      recipient_profile_version::TEXT, ':',
      expected_definition_hash
    ),
    true
  );

  UPDATE agent_fec_profile
  SET status = 'REVOKED',
      revoked_by = revoker_id,
      revoked_at = clock_timestamp()
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
END
$revoke$;

ALTER FUNCTION approve_agent_fec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_fec_profile_authority;
ALTER FUNCTION revoke_agent_fec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_fec_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_fec_profile_authority;
GRANT SELECT ON
  agent_fec_profile,
  agent_fec_profile_confirm_rule
TO agent_fec_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_fec_profile TO agent_fec_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_fec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_fec_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_fec_profile FROM PUBLIC;

CREATE INDEX agent_fec_session_work_idx ON agent_fec_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_fec_session_profile_idx ON agent_fec_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_fec_binding_certificate_idx ON agent_fec_confirm_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_fec_binding_receipt_idx ON agent_fec_confirm_binding (
  account_id, source_key_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_fec_catalog_ref_idx ON agent_fec_key_catalog (
  account_id, receipt_ref, sealed_at DESC, source_key_id
);
CREATE INDEX agent_fec_catalog_kind_idx ON agent_fec_key_catalog (
  account_id, source_key_kind, sealed_at DESC, source_key_id
);
CREATE INDEX agent_fec_evaluation_session_idx ON agent_fec_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_fec_certificate_session_idx ON agent_fec_confirm_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_fec_vote_session_idx ON agent_fec_confirmation_vote (
  account_id, session_id, voted_at DESC, vote_id
);
CREATE INDEX agent_fec_effect_work_idx ON agent_fec_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_fec_audit_time_idx ON agent_fec_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_fec_perception_status_idx ON agent_fec_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_fec_command_expiry_idx ON agent_fec_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_fec_invalidation_certificate_idx ON agent_fec_invalidation (
  account_id, certificate_id, created_at DESC, invalidation_id
);

DO $tenant_isolation$
DECLARE
  table_name TEXT;
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_fec_authorization_evidence',
    'agent_fec_profile',
    'agent_fec_profile_confirm_rule',
    'agent_fec_key_catalog',
    'agent_fec_session',
    'agent_fec_nomination_key',
    'agent_fec_confirmation_vote',
    'agent_fec_evaluation_receipt',
    'agent_fec_confirm_certificate',
    'agent_fec_confirm_binding',
    'agent_fec_invalidation',
    'agent_fec_effect_intent',
    'agent_fec_budget_ledger',
    'agent_fec_terminal_record',
    'agent_fec_command_result',
    'agent_fec_audit_head',
    'agent_fec_audit_event',
    'agent_fec_audit_anchor',
    'agent_fec_perception_snapshot',
    'agent_fec_projection_checkpoint'
  ]
  LOOP
    EXECUTE format('ALTER TABLE %I ENABLE ROW LEVEL SECURITY', table_name);
    EXECUTE format('ALTER TABLE %I FORCE ROW LEVEL SECURITY', table_name);
    EXECUTE format(
      'CREATE POLICY tenant_isolation ON %I
       USING (account_id = NULLIF(current_setting(''app.account_id'', true), '''')::BIGINT)
       WITH CHECK (account_id = NULLIF(current_setting(''app.account_id'', true), '''')::BIGINT)',
      table_name
    );
  END LOOP;
END
$tenant_isolation$;
```

### 8.1 Transaction boundaries

Open, nominate, vote, evaluate, seal, invalidate, and effect-prepare each run
in a single ACID row-store transaction with session CAS. Confirm-certificate
seal never joins a columnar rebuild or HNSW mutation.

### 8.2 Tenant isolation

Forced RLS on every table uses `app.account_id`. Composite primary keys and
every access index lead with `account_id`. Missing tenant context yields no
rows, not a cross-tenant scan.

## 9. Open API GraphQL contract

All functionality is available through the monday.com Open API. Long-running
notify work returns durable state, not a synchronous board promise.

```graphql
scalar DateTime
scalar Long
scalar JSON
scalar SHA256

enum AgentFecSessionStatus {
  OPEN
  NOMINATING
  EVALUATING
  SEALED
  DISPATCHING
  CLOSED
  EXPIRED
  CANCELLED
  FAILED
  QUARANTINED
  UNKNOWN_EFFECT
}

enum AgentFecBindingStatus {
  SEALED
  INVALIDATED
  DISPATCHING
  SUPERSEDED_REF
  UNKNOWN_EFFECT
}

enum AgentFecSourceKind {
  SEALED_ENROLL_CERTIFICATE
  FIRST_ENROLLMENT_CLAIM
  SUCCESSOR_KEY_CLAIM
  ROTATED_SIGNING_MATERIAL
}

enum AgentFecPlacementKind {
  HALTED
  EXTENDED_HALT
  RESTORED_WITHOUT_WINNER
  OMITTED
  UNKNOWN_EFFECT
}

enum AgentFecKeyLifecycle {
  FIRST_ENROLLMENT
  CONFIRMED
  MONOTONIC
  REGRESSED
  INVENTED_HISTORY
  UNKNOWN_EFFECT
}

enum AgentFecClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentFecAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
  INVENTED_HISTORY
  SILENCE
  UNKNOWN_EFFECT
}

enum AgentFecConfirmKind {
  CONFIRM_FIRST_ENROLLMENT
  HOLD_UNKNOWN
  REJECT_SINGLE_CONTROL
  REJECT_SELF_CONFIRM
  SKIP
  UNKNOWN_EFFECT
}

enum AgentFecPurposeRelation {
  EQUAL
  NARROWS
  AMPLIFIES
  UNRELATED
  UNKNOWN_EFFECT
}

enum AgentFecEffectStatus {
  PREPARED
  DISPATCHED
  ACKED
  FAILED
  UNKNOWN_EFFECT
}

enum AgentFecNextAction {
  NOMINATE_FIRST_ENROLLMENT_KEY
  RECORD_CONFIRMATION_VOTE
  EVALUATE_FIRST_ENROLLMENT_CONFIRMATION
  SEAL_CONFIRM_CERTIFICATE
  INVALIDATE_FIRST_ENROLLMENT_CONFIRMATION
  PREPARE_CONFIRM_EFFECT
  RESOLVE_CONFIRM_UNCERTAINTY
  CLOSE_SESSION
}

enum AgentFecBlockedReason {
  PROFILE_INACTIVE
  PURPOSE_DENIED
  ATTENUATION_DENIED
  BUDGET_EXHAUSTED
  KEY_MISSING
  EVALUATE_NOT_READY
  HISTORY_REWRITE_DENIED
  FIRST_ENROLLMENT_AUTO_TRUST_DENIED
  SELF_CONFIRM_DENIED
  SINGLE_CONTROL_DENIED
  HALT_CONFIRM_DENIED
  PURPOSE_AMPLIFICATION_DENIED
  SUCCESSOR_LEAK_DENIED
  HOP_LEAK_DENIED
  UNBOUND_KEY_DENIED
  UNATTESTED_CLOCK_DENIED
  INVENTED_HISTORY_DENIED
  SKEW_BOUND_DENIED
  HASH_MISMATCH
  DECISION_THRESHOLD_NOT_MET
  POLICY_DENIED
  UNKNOWN_EFFECT
}

enum AgentContentProvenance {
  USER_INPUT
  BOARD_VALUE
  PROVIDER_VALUE
  AGENT_DRAFT
}

enum AgentFecUncertaintyResolution {
  RETRY_SAME_KEY
  ACCEPT_CONFIRMATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
}

enum AgentFecInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
}

type AgentUntrustedText {
  value: String!
  provenance: AgentContentProvenance!
  trust: String!
}

type AgentFecBudget {
  nominateUnits: Long!
  evaluateUnits: Long!
  sealUnits: Long!
  vectorUnits: Long!
  invalidateUnits: Long!
  confirmUnits: Long!
  maxWallTimeMs: Long!
  evaluateThreshold: Int!
  maxBindingsPerCertificate: Int!
  maxNominatedKeys: Int!
}

type AgentFecProfile {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  name: String!
  status: String!
  definitionHash: SHA256!
  evaluateThreshold: Int!
  maxBindingsPerCertificate: Int!
  maxNominatedKeys: Int!
  semanticTags: [String!]!
  procedureRef: String
}

type AgentFecSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentFecSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentFecBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentFecNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceKeyId: ID!
  sourceKeyKind: AgentFecSourceKind!
  placementKind: AgentFecPlacementKind!
  keyLifecycle: AgentFecKeyLifecycle!
  clockLifecycle: AgentFecClockLifecycle!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
  donorPurposeHash: SHA256!
  hopCount: Int!
  nominationHash: SHA256!
  nominatedAt: DateTime!
}

type AgentFecConfirmationVote {
  accountId: ID!
  voteId: ID!
  sessionId: ID!
  sourceKeyId: ID!
  confirmerPrincipalId: ID!
  voteOrdinal: Int!
  voteHash: SHA256!
  votedAt: DateTime!
}

type AgentFecEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
  keySetHash: SHA256!
  clockSetHash: SHA256!
  confirmerSetHash: SHA256!
  attenuationHash: SHA256!
  evaluationHash: SHA256!
  evaluatedAt: DateTime!
}

type AgentFecCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
  purposeHash: SHA256!
  keySetHash: SHA256!
  clockSetHash: SHA256!
  confirmerSetHash: SHA256!
  attenuationHash: SHA256!
  bindingWatermark: Int!
  sealedAt: DateTime!
}

type AgentFecBinding {
  accountId: ID!
  bindingId: ID!
  certificateId: ID!
  sessionId: ID!
  sourceKeyId: ID!
  sourceKeyKind: AgentFecSourceKind!
  bindingOrdinal: Int!
  status: AgentFecBindingStatus!
  placementKind: AgentFecPlacementKind!
  keyLifecycle: AgentFecKeyLifecycle!
  clockLifecycle: AgentFecClockLifecycle!
  attestedKeyKind: AgentFecAttestedKeyKind!
  confirmKind: AgentFecConfirmKind!
  purposeRelation: AgentFecPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
  requestedPurposeHash: SHA256!
  successorKeyHash: SHA256!
  clockAttestationHash: SHA256!
  confirmationCount: Int!
  distinctConfirmerCount: Int!
  selfConfirm: Boolean!
  sealedAt: DateTime!
}

type AgentFecEffectObservation {
  effectId: ID!
  status: AgentFecEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentFecPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentFecSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  confirmFirstEnrollmentBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectSingleControlBindingCount: Int!
  rejectSelfConfirmBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentFecEffectObservation!]!
  remainingBudget: AgentFecBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentFecNextAction!]!
  blockedReasons: [AgentFecBlockedReason!]!
  cardHash: SHA256!
}

type AgentFecMutationResult {
  decision: String!
  session: AgentFecSession
  certificate: AgentFecCertificate
  member: AgentFecBinding
  receipt: AgentFecNominationReceipt
  vote: AgentFecConfirmationVote
  evaluation: AgentFecEvaluationReceipt
  perception: AgentFecPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
}

input AgentFecBudgetInput {
  nominateUnits: Long!
  evaluateUnits: Long!
  sealUnits: Long!
  vectorUnits: Long!
  invalidateUnits: Long!
  confirmUnits: Long!
  maxWallTimeMs: Long!
  evaluateThreshold: Int!
  maxBindingsPerCertificate: Int!
  maxNominatedKeys: Int!
}

input CreateFirstEnrollmentConfirmationSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentFecBudgetInput!
}

input NominateFirstEnrollmentKeyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceKeyId: ID!
  expectedAttenuationHash: SHA256!
  idempotencyKey: String!
}

input RecordConfirmationVoteInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceKeyId: ID!
  confirmerPrincipalId: ID!
  idempotencyKey: String!
}

input EvaluateFirstEnrollmentConfirmationInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  expectedReceiptSetHash: SHA256!
  expectedConfirmerSetHash: SHA256!
  expectedAttenuationHash: SHA256!
  idempotencyKey: String!
}

input SealConfirmCertificateInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  evaluationId: ID!
  consumerRef: String!
  expectedPurposeHash: SHA256!
  expectedConfirmerSetHash: SHA256!
  idempotencyKey: String!
}

input InvalidateFirstEnrollmentConfirmationInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceKeyId: ID!
  reasonCode: AgentFecInvalidationReason!
  idempotencyKey: String!
}

input PrepareConfirmEffectInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  idempotencyKey: String!
}

input ResolveConfirmUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentFecUncertaintyResolution!
  idempotencyKey: String!
}

input AgentFecProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}

type Query {
  agentFecProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
  ): AgentFecProfile
  agentFecSession(accountId: ID!, sessionId: ID!): AgentFecSession
  agentFecConfirmCertificate(accountId: ID!, certificateId: ID!): AgentFecCertificate
  agentFecPerceptionCard(accountId: ID!, sessionId: ID!): AgentFecPerceptionCard
  agentFecNominatedKey(
    accountId: ID!
    sessionId: ID!
    sourceKeyId: ID!
  ): AgentFecNominationReceipt
  agentFecSearchProfiles(input: AgentFecProfileSearchInput!): [AgentFecProfile!]!
}

type Mutation {
  createFirstEnrollmentConfirmationSession(
    input: CreateFirstEnrollmentConfirmationSessionInput!
  ): AgentFecMutationResult!
  nominateFirstEnrollmentKey(
    input: NominateFirstEnrollmentKeyInput!
  ): AgentFecMutationResult!
  recordConfirmationVote(
    input: RecordConfirmationVoteInput!
  ): AgentFecMutationResult!
  evaluateFirstEnrollmentConfirmation(
    input: EvaluateFirstEnrollmentConfirmationInput!
  ): AgentFecMutationResult!
  sealConfirmCertificate(input: SealConfirmCertificateInput!): AgentFecMutationResult!
  invalidateFirstEnrollmentConfirmation(
    input: InvalidateFirstEnrollmentConfirmationInput!
  ): AgentFecMutationResult!
  prepareConfirmEffect(input: PrepareConfirmEffectInput!): AgentFecMutationResult!
  resolveConfirmUncertainty(
    input: ResolveConfirmUncertaintyInput!
  ): AgentFecMutationResult!
  closeFirstEnrollmentConfirmationSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentFecMutationResult!
  approveFirstEnrollmentConfirmationProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
    authorityPrincipalId: ID!
  ): AgentFecMutationResult!
}
```

### GraphQL limits

- `topK` for profile search is clamped to `[1, 20]`.
- Evaluate mutations reject when binding ordinal exceeds session budget.
- Mutations are idempotent by `(accountId, operation, principal, key)`.
- Perception cards never embed raw private keys, tool payloads, or redacted
  fact bodies.
- `sealConfirmCertificate` is rejected with `HISTORY_REWRITE_DENIED`
  when a nominated silent or unknown successor would evaluate to
  `CONFIRM_FIRST_ENROLLMENT`.
- `sealConfirmCertificate` is rejected with
  `FIRST_ENROLLMENT_AUTO_TRUST_DENIED` or `SINGLE_CONTROL_DENIED` when a
  nominated first-enrollment key has fewer than two distinct confirmers.
- `sealConfirmCertificate` is rejected with `SELF_CONFIRM_DENIED`
  when a nominating principal would confirm their own key.
- `sealConfirmCertificate` is rejected with `HALT_CONFIRM_DENIED`
  when a nominated halted, extended-halt, or omitted key would evaluate
  to `CONFIRM_FIRST_ENROLLMENT`.
- `evaluateFirstEnrollmentConfirmation` is rejected with
  `PURPOSE_AMPLIFICATION_DENIED` when the requested purpose would
  amplify a key attenuation hash.
- Successor confirmation that would restore a halted body or invent a
  winner is rejected with `SUCCESSOR_LEAK_DENIED`.
- `recordConfirmationVote` is rejected with `SELF_CONFIRM_DENIED` when
  the confirmer is the session opener or has already voted on the same
  key.

## 10. Procedural memory

Approved confirm profiles are procedural memory: versioned instructions
for how sealed first-enrollment keys become envelope-scoped confirmation
bindings without inventing a winner and without rewriting historical
silence as dual-control success. Procedure refs may point to
successor-containment playbook steps. Profiles are immutable after
approval; agents perceive `procedureTags` and `allowedNextActions` on
perception cards, never inventing confirm policy from embeddings.

## 11. Semantic retrieval and HNSW compatibility

Profile embeddings support advisory discovery ("which confirm profile
fits incident hop-attenuated first-enrollment confirmation?"). Embeddings
are account-owned and must be queried with `account_id` equality. The
reference schema stores vectors but does **not** create a cross-tenant HNSW
index; production builds account-partitioned HNSW segments.

Semantic retrieval may return confirm profiles only. It never authorizes
nominate, vote, evaluate, seal, or confirm. Vector `topK` is budgeted and
clamped.

```sql
CREATE TABLE agent_fec_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  embedding_model TEXT NOT NULL,
  embedding_dims INTEGER NOT NULL CHECK (embedding_dims > 0),
  embedding vector(1536) NOT NULL,
  definition_hash CHAR(64) NOT NULL CHECK (length(definition_hash) = 64),
  source_watermark TEXT NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (account_id, profile_id, profile_version),
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_fec_profile (account_id, profile_id, profile_version)
);
```

```sql
-- Production guidance: CREATE INDEX ... USING hnsw (embedding vector_cosine_ops)
-- only inside an account-partitioned table/segment. Never build one global
-- HNSW across tenants. Reference validation intentionally omits HNSW DDL.
-- ANN queries must include account_id equality before topK.
```

## 12. Agent perception

Agents receive perception cards summarizing session status, sealed /
confirm-first-enrollment / hold-unknown / reject-single-control /
reject-self-confirm / unknown / skipped / invalidated binding counts,
uncertain notify intents, remaining budgets, procedure tags, allowed next
actions, and blocked reasons. Summary text is `UntrustedText`. Cards never
embed raw private keys or redacted fact bodies. `cardHash` makes
perception replayable. Agents perceive `CONFIRM_FIRST_ENROLLMENT` as a
first-seen key dual-control confirmed against an attested clock that
still cannot invent a winner, `HOLD_UNKNOWN` as a bounded uncertainty
window, `REJECT_SINGLE_CONTROL` as a trusted negative for a key that
lacks a second distinct human, `REJECT_SELF_CONFIRM` as a trusted
refusal of a nominating principal confirming their own key, and `SKIP`
as a sealed refusal — never as a key that "must have been trusted before
the first vote."

## 13. ACID and consistency

### Row store

Session CAS, nomination receipts, confirmation votes, evaluation receipts,
confirm-certificate seals, and audit appends are ACID transactions in the
hybrid row store.

### Columnar store

Columnar projections may accelerate analytics over sealed confirm
certificates but are not authoritative for confirm-first-enrollment,
hold-unknown, or reject-single-control outcomes.

### Vector store

Vector indexes are asynchronously enriched from immutable profile approval
events; staleness is visible via source watermarks.

### External tools

Notify dispatch and first-enrollment side-effects are not silently
ACID-coupled; silence becomes `UNKNOWN_EFFECT`.

## 14. Guardrails and neighbor protection

- Binding/threshold caps on confirmations per certificate and per session.
- Budget ledgers for NOMINATE/EVALUATE/SEAL/VECTOR/INVALIDATE/CONFIRM.
- Purpose attenuation narrowing only for consumers.
- Forced RLS on every table.
- Planner rejects unscoped key-catalog, clock-catalog, working-set,
  grant-graph, citation, enroll-ledger, or board scans as **FULL SCAN
  REJECTED**.
- Emergency containment may quarantine sessions without scanning neighbors.
- Evaluation never auto-restores neighbor-visible board mutations from
  halted slots, single-control votes, or unbound keys.

## 15. Performance check for boards with 1M+ rows

### Full-table-scan risks

- Finding confirmable keys by scanning the receipt or working-set
  ledger (rejected; nominate by `(account_id, source_key_id)`).
- Evaluating a confirmation by walking all notify intents for an account
  (rejected).
- Vector search without `account_id` predicate (rejected).
- Invalidation by scanning all certificates for an account (rejected; use
  receipt-keyed active binding indexes).

### Required access paths

- Key nomination: PK `(account_id, source_key_id)`.
- Vote append: unique `(account_id, session_id, source_key_id,
  confirmer_principal_id)`.
- Evaluate/seal: PK `(account_id, evaluation_id)` /
  `(account_id, certificate_id)` and unique
  `(account_id, session_id, consumer_ref, sealed_revision)`.
- Bindings by certificate/receipt: composite indexes leading with
  `account_id`.
- Notify work: partial indexes on effect intent status.
- Profile ANN: account-partitioned HNSW only.

### Planner enforcement

Any plan lacking an `account_id` equality predicate or requiring an unscoped
board/working-set/grant-graph/key-catalog/clock-catalog/enroll-ledger/citation
scan is **FULL SCAN REJECTED** before execution.

## 16. Auditability and replay

Each command appends a hash-chained audit event:
`event_hash = H(prev_hash || payload_hash || event_type || occurred_at)`.
Anchors Merkle-seal ranges for offline replay. Replay reconstructs session,
vote, evaluation, and certificate state without LLM calls.

## 17. Threat and failure analysis

- Cross-tenant certificate via forged IDs: blocked by forced RLS and PK
  scope.
- Purpose amplification for consumers: attenuation hash must narrow relative
  to observation and session purposes.
- Sticky first-ACK success after supersession: invalidation + re-evaluate +
  notify uncertainty + profile revocation.
- Historical silence invented as confirm-first-enrollment: history-rewrite
  fence.
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED` by one steward:
  first-enrollment-auto-trust and single-control fences.
- Nominating principal confirming their own key: self-confirm fence.
- Halt leak of frozen bodies into confirm-first-enrollment: halt-confirm
  fence.
- Key that restores a halted body or invents a winner:
  successor-leak fence.
- Hop leak of donor purpose after attenuation hops: hop-leak fence.
- Inventing a winner under restored-slot confirmation: certificates bind
  receipt and confirmer sets, never `resolved_fact_hash`.
- Silent notify or first-enrollment success: `UNKNOWN_EFFECT` until ACK.
- Recursive key-catalog or board storms: budget and **FULL SCAN
  REJECTED**.
- LLM-invented profile approval: authority-fenced approve/revoke only.

## 18. Observability and SLOs

- Open/nominate/vote/evaluate/seal/perception p99 latency budgets for 99.99%
  control-plane availability.
- History-rewrite rejection, first-enrollment-auto-trust rejection,
  self-confirm rejection, single-control rejection, halt-confirm rejection,
  purpose-amplification rejection, successor-leak rejection, and
  `UNKNOWN_EFFECT` rate as first-class metrics.
- Threshold-failure rejection and full-scan rejection counters per account.
- Audit chain gap alerts.

## 19. Rollout

### Phase 1: shadow confirmation

Compile profiles and validate key nomination without durable
certificates.

### Phase 2: reject-single-control and skip only

Allow sealed certificates from nominated `HALTED` and `EXTENDED_HALT`
keys as `REJECT_SINGLE_CONTROL` or `SKIP`. Confirm-first-enrollment stays
closed.

### Phase 3: dual-control confirmation and halt-confirm fences

Enable budgeted `CONFIRM_FIRST_ENROLLMENT` from `RESTORED_WITHOUT_WINNER`
keys with two distinct human votes and `TRUSTED_ENROLLED` or
`TRUSTED_WITHIN_SKEW` clocks only.

### Phase 4: notify uncertainty

Enable confirmation notify intents with `UNKNOWN_EFFECT` reconciliation and
`HOLD_UNKNOWN` that cannot invent confirmation trust from silence.

### Phase 5: broad availability

Open approved profiles to autonomous agents under neighbor budgets, including
`CONFIRM_FIRST_ENROLLMENT` only for first-enrollment keys that cannot
rewrite historical silence as success and cannot auto-trust from a single
steward.

## 20. Ship criteria

### Contract validation

- TypeScript strict compile of service interfaces.
- GraphQL schema build with 6 queries and 10 mutations.
- PGlite + pgvector executable DDL with forced RLS.
- Negative invariant tests for approval, immutability, history-rewrite,
  purpose-amplification, and effect start state.

### Behavioral validation

- Nominate requires sealed key material point lookup and hash match.
- Vote requires a distinct human principal who is not the session opener.
- Evaluate binds key set, confirmer set, and attenuation under budget.
- Seal is rejected when silence would become confirm-first-enrollment, when
  a first-enrollment key has fewer than two distinct confirmers, or when a
  nominating principal would confirm their own key, and never invents a
  winning fact hash.
- Confirm-certificate seal binds immutable bindings under key-set,
  clock-set, confirmer-set, and attenuation hashes — never a winner
  hash.
- Notify silence becomes `UNKNOWN_EFFECT`.

### Scale and failure validation

- 1M+ row boards: no nominate/vote/evaluate/seal path performs a full table
  scan.
- Neighbor budget exhaustion rejects before execution.
- Audit replay reconstructs sealed confirm certificates after process
  restart.

## 21. Product decision

Adopt the First-Enrollment Confirmation Plane as the deterministic
dual-control path for first-seen signing keys bound by the Successor-Key
Enrollment, Successor-Clock Re-Attestation, and Provider-Key Rotation
planes.

Ship it because:

1. It preserves ACID and multi-tenant isolation while closing the
   post-enrollment first-trust gap without history rewrite,
   first-enrollment auto-trust, self-confirm leak, halt leak, purpose
   amplification, invented winners, or key-catalog scans.
2. Account-leading indexes, history-rewrite and purpose-amplification
   fences, and **FULL SCAN REJECTED** planner rules protect 99.99% neighbor
   latency on boards with 1M+ rows.
3. Open API GraphQL, procedural memory, account-owned HNSW profile
   discovery, perception cards, and hash-chained audit replay make the
   plane agent-ready without putting probabilistic AI inside the data
   engine.
