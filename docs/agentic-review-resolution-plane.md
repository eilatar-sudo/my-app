# mondayDB Agentic Review-Resolution Plane
...
A sealed unanswered-containment review certificate can
page a steward after dual-control
`REVIEW_UNANSWERED_CONTAINMENT`. It does not decide **how
an unanswered, expired, or single-control steward page is
resolved** — without scanning every review binding,
rewriting unanswered-review silence into
`TRUSTED_ENROLLED`, or treating review-page silence as
confirmation.

Without a review-resolution plane, operators
and agents either:

- scan every pending review, containment, escalation,
  acknowledgement, page, and vote ledger looking for
  "who still has not dual-control resolved the steward
  page" (neighbor-harmful on boards with 1M+ rows), or
- treat unanswered review-page silence as retroactive
  engine truth, so review silence is rewritten as
  confirmation, historical `UNKNOWN_EFFECT` is rewritten
  as `ACKED`, a silent steward resolution auto-promotes
...
- Treating every unanswered review, freeze timeout, or
  resolution timer as implicit confirmation maximizes
  agent fluency and reduces re-planning cost, but creates
  history-rewrite invention, resolution-as-confirmation,
  unanswered-review-as-trust, silence-success, first-
...
- Binding a sealed resolution certificate under an
  ...
  fences, first-enrollment auto-trust fences, resolution-
  as-confirmation fences, unanswered-review-as-trust
  fences, halt-resolve fences, purpose-amplification
...
- Semantic similarity may discover resolution profiles,
  ...
  containment, escalation, acknowledgement, SLO, window,
  TTL, or confirm certificate may be nominated, a
...
1. An approved resolution profile defines allowed source
   ...
   unanswered review into `TRUSTED_ENROLLED`.
2. A resolution session opens under purpose, budget, and
   ...
   lookup from the Unanswered-Containment Review,
...
3. mondayDB evaluates a resolution whose kind is a pure
   ...
   distinct_review_steward_count, prior_review_steward_set_hash,
   resolution_steward_count, resolution_window_ms,
   resolution_kind)`. Silence cannot become historical
   success. An unanswered review seals
   `REJECT_RESOLUTION_AS_CONFIRMATION` or
   `RESOLUTION_EXPIRED` only — never `TRUSTED_ENROLLED`.
   Dual-control `RESOLVE_UNANSWERED_REVIEW` acts on a
   steward page without inventing confirmation.
   `HOLD_RESOLUTION_WINDOW` holds further resolution for a
   bounded window without restoring a winner. Halted slots
   cannot become resolution-restored winners. Original
   review stewards are excluded from the resolution-
   steward pair.
...
6. Unscoped key-catalog, clock-catalog, confirm-ledger, vote-
   ...
   ledger, containment-ledger, review-ledger, working-set,
   grant-graph, or board scans are
   **FULL SCAN REJECTED**.

The trade-off buys enterprise trust and 99.99% neighbor
protection: recursive "re-resolve every unanswered
review forever" or "treat unanswered review silence
...
For any review-resolution evaluation,
mondayDB can answer:
...
- Which nominated resolution certificates, placement kinds,
  hop counts, attenuation hashes, remaining-set hashes,
  prior-resolution-steward exclusions, and resolution kinds were
  bound?
...
The Review-Resolution Plane owns:

1. Immutable approved resolution profiles as procedural
   memory of "how unanswered reviews resolve a steward page without amplifying purpose, leaking halted
   facts, rewriting attested history, scanning vote, page,
   ack, escalation, or review ledgers, or inventing
...
3. Deterministic nomination of sealed containment
   ...
   review-ledger, working-set, or board scans.
...
- **Unanswered-Containment Review:** supplies sealed
  ...
  single-control steward pages this plane may resolve.
  This plane never invents a new review by walking
  review bindings.
...
- **Confirmation-Window Observability:** supplies sealed
  ...
  computes remaining TTL by walking resolution bindings.
- **Confirmation-TTL Retirement:** supplies sealed TTL
  ...
  or extends a key; it may resolve a steward page only through a
  sealed resolution certificate.
...
- Letting an LLM decide that an unanswered review "feels
...
- resolution-window buckets (`resolution_window_ms`), max
  refresh count, evaluate threshold (distinct review-
  steward principals for `RESOLVE_UNANSWERED_REVIEW`,
  minimum 2), max bindings per certificate, and max
  nominated windows;
- resolution policy (`HISTORY_NEVER_REWRITTEN`,
  `SILENCE_NEVER_SUCCESS`, `FIRST_ENROLLMENT_NEVER_AUTO_TRUSTED`,
  `RESOLUTION_NEVER_CONFIRMS`, `UNANSWERED_REVIEW_NEVER_TRUSTS`,
  `LEDGER_NEVER_SCANNED`, `HALT_DENIES_RESOLUTION_RESTORE`,
  `PURPOSE_NARROW_ONLY`, `SUCCESSOR_NEVER_RESTORES_WINNER`,
  `PRIOR_REVIEW_STEWARD_NEVER_RESOLVES`);
- purpose attenuation rules (narrowing only; never
  amplification);
- allowed resolution kinds (`RESOLVE_CURRENT`,
  `RESOLVE_ACTIVE`, `RESOLUTION_EXPIRED`,
  `RESOLVE_UNANSWERED_REVIEW`, `HOLD_RESOLUTION_WINDOW`,
  `HOLD_UNKNOWN`, `REJECT_RESOLUTION_AS_CONFIRMATION`,
...
Refresh review-resolution is a point lookup
...
dual-control resolution-steward counts, prior-containment-
...
`resolved_fact_hash`. An unanswered review cannot
...
3. Sealed profile definitions and resolution rules are
   immutable.
4. Binding identity
...
10. Silence and unknown windows cannot evaluate to
    `RESOLVE_UNANSWERED_REVIEW` or any kind that implies
    `TRUSTED_ENROLLED` (history-rewrite and silence-success
    fences).
11. Halted, extended-halt, and omitted windows cannot
    evaluate to a resolution kind that restores a winner
    (halt-resolve fence).
...
14. An unanswered review never writes
    `attested_key_kind = TRUSTED_ENROLLED`
    (resolution-as-confirmation and unanswered-containment-
    as-trust fences).
15. `RESOLVE_UNANSWERED_REVIEW` requires two distinct
    resolution-steward principals who are not the
...
17. Plans that require unscoped board, session, working-set,
    ...
    vote-ledger, TTL-ledger, page-ledger, ack-ledger,
    escalation-ledger, review-ledger, or citation-ledger scans are
    **FULL SCAN REJECTED**.
...
Draft profiles accumulate resolution rules. Approval
...
certificate. Refresh review-resolution is a
...
resolution rule. Approval is authority-fenced.
...
type TrustedNextAction =
  ...
  | "EVALUATE_REVIEW_RESOLUTION"
  | "SEAL_RESOLUTION_CERTIFICATE"
  | "INVALIDATE_REVIEW_RESOLUTION"
  | "PREPARE_RESOLUTION_EFFECT"
  | "RESOLVE_EFFECT_UNCERTAINTY"
  | "CLOSE_SESSION";

type ReviewResolutionBlockedReason =
  | "PROFILE_INACTIVE"
  | "PURPOSE_DENIED"
  | "ATTENUATION_DENIED"
  | "BUDGET_EXHAUSTED"
  ...
  | "RESOLUTION_AS_CONFIRMATION_DENIED"
  | "UNANSWERED_REVIEW_AS_TRUST_DENIED"
...
type ResolutionKind =
  ...
  | "RESOLVE_ACTIVE"
  | "RESOLUTION_EXPIRED"
  | "RESOLVE_UNANSWERED_REVIEW"
  | "HOLD_RESOLUTION_WINDOW"
  | "HOLD_UNKNOWN"
  | "REJECT_RESOLUTION_AS_CONFIRMATION"
  | "SKIP"
  | "UNKNOWN_EFFECT";
...
interface ReviewResolutionBudget {
...
interface ReviewResolutionProfile {
...
interface ReviewResolutionSession {
  readonly accountId: AccountId;
  ...
  readonly budget: ReviewResolutionBudget;
  readonly consumed: Omit<
    ReviewResolutionBudget,
    | "maxWallTimeMs"
    | "evaluateThreshold"
...
interface ReviewResolutionSnapshot {
  readonly accountId: AccountId;
...
interface ReviewResolutionEvaluationReceipt {
  readonly accountId: AccountId;
...
interface ReviewResolutionBinding {
  readonly accountId: AccountId;
...
interface ReviewResolutionCertificate {
  readonly accountId: AccountId;
  readonly certificateId: CertificateId;
  readonly sessionId: SessionId;
...
interface ReviewResolutionEffectObservation {
...
interface AgentReviewResolutionPerceptionCard {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly status: SessionStatus;
  readonly summary: UntrustedText;
  readonly sealedBindingCount: number;
  readonly resolveCurrentBindingCount: number;
  readonly resolveActiveBindingCount: number;
  readonly resolutionExpiredBindingCount: number;
  readonly resolveUnansweredReviewBindingCount: number;
  readonly holdResolutionWindowBindingCount: number;
  readonly holdUnknownBindingCount: number;
  readonly rejectResolutionAsConfirmationBindingCount: number;
  readonly unknownBindingCount: number;
  readonly skippedBindingCount: number;
  readonly invalidatedBindingCount: number;
  readonly uncertainEffectIntents: readonly ReviewResolutionEffectObservation[];
  ...
  readonly blockedReasons: readonly ReviewResolutionBlockedReason[];
...
```ts
interface CreateReviewResolutionSessionInput {
  readonly accountId: AccountId;
  readonly profileId: ProfileId;
  readonly profileVersion: number;
  readonly purpose: string;
...
interface RefreshReviewResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly sourceWindowId: SourceWindowId;
...
interface EvaluateReviewResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
...
interface InvalidateReviewResolutionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly certificateId: CertificateId;
...
interface ResolveEffectUncertaintyInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly effectId: string;
...
interface CloseReviewResolutionSessionInput {
  readonly accountId: AccountId;
  readonly sessionId: SessionId;
  readonly expectedRevision: bigint;
  readonly idempotencyKey: string;
...
type ReviewResolutionDecision =
  | { readonly decision: "ALLOWED"; readonly session: ReviewResolutionSession;
      readonly certificate?: ReviewResolutionCertificate;
      readonly member?: ReviewResolutionBinding;
      readonly receipt?: WindowNominationReceipt;
      readonly snapshot?: ReviewResolutionSnapshot;
      readonly evaluation?: ReviewResolutionEvaluationReceipt;
      readonly perception: AgentReviewResolutionPerceptionCard;
      readonly auditHash: Sha256 }
  | { readonly decision: "REJECTED"; readonly code: ReviewResolutionBlockedReason;
...
```sql
CREATE TYPE urr_profile_status AS ENUM ('DRAFT', 'APPROVED', 'REVOKED');
CREATE TYPE urr_session_status AS ENUM (
  'OPEN', 'NOMINATING', 'EVALUATING', 'SEALED', 'DISPATCHING',
  'CLOSED', 'EXPIRED', 'CANCELLED', 'FAILED', 'QUARANTINED', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_binding_status AS ENUM (
  'SEALED', 'INVALIDATED', 'DISPATCHING', 'SUPERSEDED_REF', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_source_kind AS ENUM (
...
CREATE TYPE urr_placement_kind AS ENUM (
  'HALTED', 'EXTENDED_HALT', 'RESTORED_WITHOUT_WINNER', 'OMITTED',
  'UNKNOWN_EFFECT'
);
CREATE TYPE urr_key_lifecycle AS ENUM (
  'FIRST_ENROLLMENT', 'CONFIRMED', 'MONOTONIC', 'REGRESSED',
  'INVENTED_HISTORY', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_clock_lifecycle AS ENUM (
  'ATTESTED', 'UNATTESTED', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_attested_key_kind AS ENUM (
  'AWAITING_CONFIRMATION', 'TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW',
  'REGRESSED', 'INVENTED_HISTORY', 'SILENCE', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_resolution_kind AS ENUM (
  'RESOLVE_CURRENT', 'RESOLVE_ACTIVE', 'RESOLUTION_EXPIRED',
  'RESOLVE_UNANSWERED_REVIEW', 'HOLD_RESOLUTION_WINDOW', 'HOLD_UNKNOWN',
  'REJECT_RESOLUTION_AS_CONFIRMATION', 'SKIP', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_purpose_relation AS ENUM (
  'EQUAL', 'NARROWS', 'AMPLIFIES', 'UNRELATED', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_effect_status AS ENUM (
  'PREPARED', 'DISPATCHED', 'ACKED', 'FAILED', 'UNKNOWN_EFFECT'
);
CREATE TYPE urr_catalog_status AS ENUM (
  'SEALED', 'INVALIDATED', 'SUPERSEDED_REF', 'RETRACTED_REF', 'UNKNOWN_EFFECT'
);

CREATE ROLE agent_urr_profile_authority NOLOGIN;

CREATE TABLE agent_urr_authorization_evidence (
...
CREATE TABLE agent_urr_profile (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL CHECK (profile_version > 0),
  name TEXT NOT NULL,
  status urr_profile_status NOT NULL,
  ...
  FOREIGN KEY (account_id, authorization_evidence_id)
    REFERENCES agent_urr_authorization_evidence (account_id, evidence_id),
  CHECK (
...
CREATE TABLE agent_urr_profile_resolution_rule (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  ...
  FOREIGN KEY (account_id, profile_id, profile_version)
    REFERENCES agent_urr_profile (account_id, profile_id, profile_version)
);

CREATE TABLE agent_urr_review_catalog (
  account_id BIGINT NOT NULL,
  source_window_id UUID NOT NULL,
  source_session_id UUID NOT NULL,
  source_certificate_id UUID NOT NULL,
  receipt_ref TEXT NOT NULL,
  source_window_kind urr_source_kind NOT NULL,
  placement_kind urr_placement_kind NOT NULL,
  key_lifecycle urr_key_lifecycle NOT NULL,
  clock_lifecycle urr_clock_lifecycle NOT NULL,
  status urr_catalog_status NOT NULL,
...
CREATE TABLE agent_urr_session (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
  status urr_session_status NOT NULL,
...
CREATE TABLE agent_urr_nomination_window (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_urr_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_urr_review_catalog (account_id, source_window_id)
);

CREATE TABLE agent_urr_resolution_snapshot (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_evaluation_receipt (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_resolution_certificate (
  account_id BIGINT NOT NULL,
  ...
  FOREIGN KEY (account_id, evaluation_id)
    REFERENCES agent_urr_evaluation_receipt (account_id, evaluation_id)
);

CREATE TABLE agent_urr_resolution_binding (
  account_id BIGINT NOT NULL,
  ...
  status urr_binding_status NOT NULL,
  placement_kind urr_placement_kind NOT NULL,
  key_lifecycle urr_key_lifecycle NOT NULL,
  clock_lifecycle urr_clock_lifecycle NOT NULL,
  attested_key_kind urr_attested_key_kind NOT NULL,
  resolution_kind urr_resolution_kind NOT NULL,
  purpose_relation urr_purpose_relation NOT NULL,
  disputed_fact_hash CHAR(64) NOT NULL CHECK (length(disputed_fact_hash) = 64),
  attenuation_hash CHAR(64) NOT NULL CHECK (length(attenuation_hash) = 64),
  ...
  FOREIGN KEY (account_id, certificate_id)
    REFERENCES agent_urr_resolution_certificate (account_id, certificate_id),
  FOREIGN KEY (account_id, session_id)
    REFERENCES agent_urr_session (account_id, session_id),
  FOREIGN KEY (account_id, source_window_id)
    REFERENCES agent_urr_review_catalog (account_id, source_window_id)
...
CREATE TABLE agent_urr_invalidation (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_effect_intent (
  account_id BIGINT NOT NULL,
  effect_id UUID NOT NULL,
  session_id UUID NOT NULL,
  certificate_id UUID NOT NULL,
  intent_status urr_effect_status NOT NULL,
...
CREATE TABLE agent_urr_budget_ledger (
  account_id BIGINT NOT NULL,
  session_id UUID NOT NULL,
  entry_sequence BIGINT NOT NULL CHECK (entry_sequence >= 0),
  unit_type TEXT NOT NULL CHECK (
    unit_type IN (
      'NOMINATE', 'EVALUATE', 'SEAL', 'VECTOR', 'INVALIDATE', 'RESOLVE'
    )
  ),
...
CREATE TABLE agent_urr_terminal_record (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_command_result (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_audit_head (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_audit_event (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_audit_anchor (
  account_id BIGINT NOT NULL,
...
CREATE TABLE agent_urr_perception_snapshot (
  account_id BIGINT NOT NULL,
  snapshot_id UUID NOT NULL,
  session_id UUID NOT NULL,
  status urr_session_status NOT NULL,
...
CREATE TABLE agent_urr_projection_checkpoint (
  account_id BIGINT NOT NULL,
...
CREATE FUNCTION protect_agent_urr_profile()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_profile$
BEGIN
  ...
  IF OLD.status = 'APPROVED' AND NEW.status = 'REVOKED' THEN
    IF current_setting('app.urr_profile_revocation', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
  ...
  IF OLD.status = 'DRAFT' AND NEW.status = 'APPROVED' THEN
    IF current_setting('app.urr_profile_approval', true) IS DISTINCT FROM
       concat(
         OLD.profile_id::TEXT, ':',
         OLD.profile_version::TEXT, ':',
         OLD.definition_hash
...
CREATE TRIGGER agent_urr_profile_protect
BEFORE INSERT OR UPDATE ON agent_urr_profile
FOR EACH ROW EXECUTE FUNCTION protect_agent_urr_profile();

CREATE FUNCTION protect_agent_urr_profile_resolution_rule()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_rule$
DECLARE
  profile_status urr_profile_status;
BEGIN
  SELECT status INTO profile_status
  FROM agent_urr_profile
...
CREATE TRIGGER agent_urr_profile_resolution_rule_protect
BEFORE INSERT OR UPDATE OR DELETE ON agent_urr_profile_resolution_rule
FOR EACH ROW EXECUTE FUNCTION protect_agent_urr_profile_resolution_rule();

CREATE FUNCTION protect_agent_urr_resolution_snapshot()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_snapshot$
BEGIN
...
CREATE TRIGGER agent_urr_resolution_snapshot_protect
BEFORE INSERT OR UPDATE ON agent_urr_resolution_snapshot
FOR EACH ROW EXECUTE FUNCTION protect_agent_urr_resolution_snapshot();

CREATE FUNCTION protect_agent_urr_resolution_binding()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_binding$
BEGIN
  ...
  IF NEW.resolution_kind IN (
       'RESOLVE_CURRENT', 'RESOLVE_ACTIVE', 'RESOLUTION_EXPIRED',
       'RESOLVE_UNANSWERED_REVIEW', 'REJECT_RESOLUTION_AS_CONFIRMATION'
     )
  ...
  IF NEW.resolution_kind = 'REJECT_RESOLUTION_AS_CONFIRMATION'
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'resolution-as-confirmation fence blocks TRUSTED_ENROLLED from an unanswered review';
  END IF;

  IF NEW.resolution_kind = 'RESOLVE_UNANSWERED_REVIEW'
  ...
  IF NEW.resolution_kind IN (
     ...
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'first-enrollment-auto-trust fence blocks TRUSTED_ENROLLED from review-resolution';
  END IF;
  ...
  IF NEW.resolution_kind IN (
       'RESOLVE_CURRENT', 'RESOLVE_ACTIVE', 'RESOLUTION_EXPIRED', 'RESOLVE_UNANSWERED_REVIEW'
     )
  ...
  IF NEW.resolution_kind IN (
       'RESOLVE_CURRENT', 'RESOLVE_ACTIVE', 'RESOLVE_UNANSWERED_REVIEW'
     )
     AND NEW.placement_kind IN ('HALTED', 'EXTENDED_HALT', 'OMITTED')
     AND NEW.attested_key_kind IN ('TRUSTED_ENROLLED', 'TRUSTED_WITHIN_SKEW') THEN
    RAISE EXCEPTION 'halt-resolve fence blocks containment that would restore a halted key';
  END IF;
...
CREATE TRIGGER agent_urr_resolution_binding_protect
BEFORE INSERT OR UPDATE ON agent_urr_resolution_binding
FOR EACH ROW EXECUTE FUNCTION protect_agent_urr_resolution_binding();

CREATE FUNCTION protect_agent_urr_effect_intent()
RETURNS trigger
LANGUAGE plpgsql
AS $protect_effect$
BEGIN
...
CREATE TRIGGER agent_urr_effect_intent_protect
BEFORE INSERT OR UPDATE ON agent_urr_effect_intent
FOR EACH ROW EXECUTE FUNCTION protect_agent_urr_effect_intent();

CREATE FUNCTION approve_agent_urr_profile(
  tenant_id BIGINT,
...
DECLARE
  stored_hash CHAR(64);
  stored_status urr_profile_status;
  rule_count INTEGER;
BEGIN
  ...
  FROM agent_urr_profile_resolution_rule
  WHERE account_id = tenant_id
    AND profile_id = recipient_profile_id
    AND profile_version = recipient_profile_version;
...
CREATE FUNCTION revoke_agent_urr_profile(
  tenant_id BIGINT,
  recipient_profile_id UUID,
  recipient_profile_version INTEGER,
...
ALTER FUNCTION approve_agent_urr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_urr_profile_authority;
ALTER FUNCTION revoke_agent_urr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) OWNER TO agent_urr_profile_authority;

GRANT USAGE ON SCHEMA public TO agent_urr_profile_authority;
GRANT SELECT ON
  agent_urr_profile,
  agent_urr_profile_resolution_rule
TO agent_urr_profile_authority;
GRANT UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_urr_profile TO agent_urr_profile_authority;

REVOKE ALL ON FUNCTION approve_agent_urr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE ALL ON FUNCTION revoke_agent_urr_profile(
  BIGINT, UUID, INTEGER, TEXT, TEXT
) FROM PUBLIC;
REVOKE UPDATE (
  status, approved_by, approved_at, approval_validation_hash,
  revoked_by, revoked_at
) ON agent_urr_profile FROM PUBLIC;

CREATE INDEX agent_urr_session_work_idx ON agent_urr_session (
  account_id, status, updated_at, session_id
);
CREATE INDEX agent_urr_session_profile_idx ON agent_urr_session (
  account_id, profile_id, profile_version, created_at DESC
);
CREATE INDEX agent_urr_binding_certificate_idx ON agent_urr_resolution_binding (
  account_id, certificate_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_urr_binding_window_idx ON agent_urr_resolution_binding (
  account_id, source_window_id, sealed_at DESC, binding_id
);
CREATE INDEX agent_urr_catalog_ref_idx ON agent_urr_review_catalog (
  account_id, receipt_ref, sealed_at DESC, source_window_id
);
CREATE INDEX agent_urr_catalog_kind_idx ON agent_urr_review_catalog (
  account_id, source_window_kind, sealed_at DESC, source_window_id
);
CREATE INDEX agent_urr_evaluation_session_idx ON agent_urr_evaluation_receipt (
  account_id, session_id, evaluated_at DESC, evaluation_id
);
CREATE INDEX agent_urr_certificate_session_idx ON agent_urr_resolution_certificate (
  account_id, session_id, sealed_at DESC, certificate_id
);
CREATE INDEX agent_urr_snapshot_session_idx ON agent_urr_resolution_snapshot (
  account_id, session_id, captured_at DESC, snapshot_id
);
CREATE INDEX agent_urr_effect_work_idx ON agent_urr_effect_intent (
  account_id, intent_status, updated_at, effect_id
) WHERE intent_status IN ('PREPARED', 'DISPATCHED', 'UNKNOWN_EFFECT');
CREATE INDEX agent_urr_audit_time_idx ON agent_urr_audit_event (
  account_id, occurred_at, session_id, event_sequence
);
CREATE INDEX agent_urr_perception_status_idx ON agent_urr_perception_snapshot (
  account_id, status, created_at DESC, session_id
);
CREATE INDEX agent_urr_command_expiry_idx ON agent_urr_command_result (
  account_id, expires_at, operation_name, principal_id
);
CREATE INDEX agent_urr_invalidation_certificate_idx ON agent_urr_invalidation (
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'agent_urr_authorization_evidence',
    'agent_urr_profile',
    'agent_urr_profile_resolution_rule',
    'agent_urr_review_catalog',
    'agent_urr_session',
    'agent_urr_nomination_window',
    'agent_urr_resolution_snapshot',
    'agent_urr_evaluation_receipt',
    'agent_urr_resolution_certificate',
    'agent_urr_resolution_binding',
    'agent_urr_invalidation',
    'agent_urr_effect_intent',
    'agent_urr_budget_ledger',
    'agent_urr_terminal_record',
    'agent_urr_command_result',
    'agent_urr_audit_head',
    'agent_urr_audit_event',
    'agent_urr_audit_anchor',
    'agent_urr_perception_snapshot',
    'agent_urr_projection_checkpoint'
  ]
  LOOP
...
enum AgentUrrClockLifecycle {
  ATTESTED
  UNATTESTED
  UNKNOWN_EFFECT
}

enum AgentUrrAttestedKeyKind {
  AWAITING_CONFIRMATION
  TRUSTED_ENROLLED
  TRUSTED_WITHIN_SKEW
  REGRESSED
...
enum AgentUrrUncertaintyResolution {
  RETRY_SAME_WINDOW
  ACCEPT_OBSERVATION
  REJECT_ENVELOPE
  REQUIRE_HUMAN
...
enum AgentUrrInvalidationReason {
  SUPERSEDED
  RETRACTED
  QUARANTINED
  KEY_REVOKED
...
type AgentUrrSession {
  accountId: ID!
  sessionId: ID!
  profileId: ID!
  profileVersion: Int!
  status: AgentUrrSessionStatus!
  stateRevision: Long!
  purpose: String!
  budget: AgentUrrBudget!
  principalId: ID!
  deadlineAt: DateTime!
}

type AgentUrrNominationReceipt {
  accountId: ID!
  sessionId: ID!
  sourceWindowId: ID!
  sourceWindowKind: AgentUrrSourceKind!
  placementKind: AgentUrrPlacementKind!
  keyLifecycle: AgentUrrKeyLifecycle!
  clockLifecycle: AgentUrrClockLifecycle!
...
type AgentUrrReviewResolutionSnapshot {
  accountId: ID!
  snapshotId: ID!
  sessionId: ID!
  sourceWindowId: ID!
...
type AgentUrrEvaluationReceipt {
  accountId: ID!
  evaluationId: ID!
  sessionId: ID!
...
type AgentUrrResolutionCertificate {
  accountId: ID!
  certificateId: ID!
  sessionId: ID!
  consumerRef: String!
...
type AgentUrrBinding {
  ...
  sourceWindowKind: AgentUrrSourceKind!
  bindingOrdinal: Int!
  status: AgentUrrBindingStatus!
  placementKind: AgentUrrPlacementKind!
  keyLifecycle: AgentUrrKeyLifecycle!
  clockLifecycle: AgentUrrClockLifecycle!
  attestedKeyKind: AgentUrrAttestedKeyKind!
  resolutionKind: AgentUrrResolutionKind!
  purposeRelation: AgentUrrPurposeRelation!
  disputedFactHash: SHA256!
  attenuationHash: SHA256!
...
type AgentUrrEffectObservation {
  effectId: ID!
  status: AgentUrrEffectStatus!
  providerIdempotencyKey: String!
  generation: Int!
  lastErrorCode: String
}

type AgentUrrPerceptionCard {
  accountId: ID!
  sessionId: ID!
  status: AgentUrrSessionStatus!
  summary: AgentUntrustedText!
  sealedBindingCount: Int!
  resolveCurrentBindingCount: Int!
  resolveActiveBindingCount: Int!
  resolutionExpiredBindingCount: Int!
  resolveUnansweredReviewBindingCount: Int!
  holdResolutionWindowBindingCount: Int!
  holdUnknownBindingCount: Int!
  rejectResolutionAsConfirmationBindingCount: Int!
  unknownBindingCount: Int!
  skippedBindingCount: Int!
  invalidatedBindingCount: Int!
  uncertainEffectIntents: [AgentUrrEffectObservation!]!
  remainingBudget: AgentUrrBudget!
  procedureTags: [String!]!
  allowedNextActions: [AgentUrrNextAction!]!
  blockedReasons: [AgentUrrBlockedReason!]!
  cardHash: SHA256!
}

type AgentUrrMutationResult {
  decision: String!
  session: AgentUrrSession
  certificate: AgentUrrResolutionCertificate
  member: AgentUrrBinding
  receipt: AgentUrrNominationReceipt
  snapshot: AgentUrrReviewResolutionSnapshot
  evaluation: AgentUrrEvaluationReceipt
  perception: AgentUrrPerceptionCard
  code: String
  retryable: Boolean!
  reason: String
  auditHash: SHA256
...
input CreateReviewResolutionSessionInput {
  accountId: ID!
  profileId: ID!
  profileVersion: Int!
  purpose: String!
  idempotencyKey: String!
  budget: AgentUrrBudgetInput!
}
...
input RefreshReviewResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  sourceWindowId: ID!
...
input EvaluateReviewResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
...
input InvalidateReviewResolutionInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  certificateId: ID!
  sourceWindowId: ID!
  reasonCode: AgentUrrInvalidationReason!
  idempotencyKey: String!
}
...
input ResolveEffectUncertaintyInput {
  accountId: ID!
  sessionId: ID!
  expectedRevision: Long!
  effectId: ID!
  resolution: AgentUrrUncertaintyResolution!
  idempotencyKey: String!
}

input AgentUrrProfileSearchInput {
  accountId: ID!
  queryText: String!
  topK: Int!
}
...
type Query {
  ...
  agentUrrSession(accountId: ID!, sessionId: ID!): AgentUrrSession
  agentUrrResolutionCertificate(accountId: ID!, certificateId: ID!): AgentUrrResolutionCertificate
  agentUrrPerceptionCard(accountId: ID!, sessionId: ID!): AgentUrrPerceptionCard
  agentUrrNominatedWindow(
    accountId: ID!
    sessionId: ID!
    sourceWindowId: ID!
  ): AgentUrrNominationReceipt
  agentUrrSearchProfiles(input: AgentUrrProfileSearchInput!): [AgentUrrProfile!]!
}

type Mutation {
  createReviewResolutionSession(
    input: CreateReviewResolutionSessionInput!
  ): AgentUrrMutationResult!
  nominateSealedResolutionCertificate(
    input: NominateSealedResolutionCertificateInput!
  ): AgentUrrMutationResult!
  refreshReviewResolution(
    input: RefreshReviewResolutionInput!
  ): AgentUrrMutationResult!
  evaluateReviewResolution(
    input: EvaluateReviewResolutionInput!
  ): AgentUrrMutationResult!
  sealResolutionCertificate(input: SealResolutionCertificateInput!): AgentUrrMutationResult!
  invalidateReviewResolution(
    input: InvalidateReviewResolutionInput!
  ): AgentUrrMutationResult!
  prepareResolutionEffect(input: PrepareResolutionEffectInput!): AgentUrrMutationResult!
  resolveEffectUncertainty(
    input: ResolveEffectUncertaintyInput!
  ): AgentUrrMutationResult!
  closeReviewResolutionSession(
    accountId: ID!
    sessionId: ID!
    expectedRevision: Long!
    idempotencyKey: String!
  ): AgentUrrMutationResult!
  approveReviewResolutionProfile(
    accountId: ID!
    profileId: ID!
    profileVersion: Int!
    definitionHash: SHA256!
...
- `sealResolutionCertificate` is rejected with `HISTORY_REWRITE_DENIED` when
  ...
  `RESOLUTION_AS_CONFIRMATION_DENIED` when an unanswered review is treated as confirmation, and with
...
versioned instructions for how sealed review,
containment, escalation, acknowledgement, SLO, window, TTL,
and confirm certificates become envelope-scoped steward
resolutions without inventing a winner and without rewriting
an unanswered review as dual-control success or
`TRUSTED_ENROLLED`.
...
enrollment unanswered review?"). Embeddings are
...
```sql
CREATE TABLE agent_urr_profile_embedding (
  account_id BIGINT NOT NULL,
  profile_id UUID NOT NULL,
  profile_version INTEGER NOT NULL,
...
reject-resolution-as-confirmation / unknown / skipped /
...
`RESOLVE_CURRENT` as a first-seen unanswered review
whose resolution window is still open and still cannot
invent a winner, `RESOLVE_ACTIVE` as a bounded steward
...
control trust, `RESOLVE_UNANSWERED_REVIEW` as a dual-
...
window, `REJECT_RESOLUTION_AS_CONFIRMATION` as a trusted
negative for an unanswered review that would have
rewritten silence as success, and `SKIP` as a sealed
refusal — never as a key that "must have been trusted
because nobody resolved the unanswered review."

## 13. ACID and consistency

### Row store

Session CAS, nomination receipts, resolution snapshots,
evaluation receipts, resolution-certificate seals, and
audit appends are ACID transactions in the hybrid row store.

### Columnar store

Columnar projections may accelerate analytics over sealed
resolution certificates but are not authoritative for
review-current, review-active, or reject-
resolution-as-confirmation outcomes.
...
- Evaluation never auto-restores neighbor-visible board
  ...
  acknowledgements, unanswered reviews, or unbound
  windows.
- Prior-escalator exclusion rejects original dual-control
  containment authorities as the resolution-steward pair.

## 15. Performance check for boards with 1M+ rows

### Full-table-scan risks

- Finding remaining unanswered reviews to contain by
...
- review-resolution refresh: unique
...
page-ledger/ack-ledger/escalation-ledger/review-ledger/citation scan is
...
reconstructs session, resolution snapshot, evaluation, and
...
- Sticky first-resolution success after supersession:
  invalidation + re-evaluate + notify uncertainty + profile
  revocation.
- Historical silence invented as `TRUSTED_ENROLLED` from an
  unanswered review: history-rewrite, silence-success,
  and unanswered-review-as-trust fences.
- First-enrollment key auto-promoted to `TRUSTED_ENROLLED`
  by a containment: first-enrollment-auto-trust and
  resolution-as-confirmation fences.
- Remaining time or resolution targets computed by walking
  vote, TTL, page, ack, escalation, or review ledgers: ledger-scan
...
- Inventing a winner under restored-slot containment:
  ...
  `resolved_fact_hash`.
- Original containment authorities serving as the resolution-steward
...
### Phase 2: reject-resolution-as-confirmation and skip only

Allow sealed certificates from nominated `HALTED` and
`EXTENDED_HALT` windows as
`REJECT_RESOLUTION_AS_CONFIRMATION` or `SKIP`.
Containment-active stays closed.

### Phase 3: expire-only review and halt-resolve fences

Enable budgeted `RESOLUTION_EXPIRED` from
`RESTORED_WITHOUT_WINNER` windows with `ttl_expired = true`
and `AWAITING_CONFIRMATION` only. An unanswered review
never writes `TRUSTED_ENROLLED`.

### Phase 4: resolution-steward pages and notify uncertainty

Enable `RESOLVE_CURRENT` / `RESOLVE_ACTIVE` under
...
budgets, including resolution-steward pages only for
...
- Refresh review-resolution is a point
...
- Seal is rejected when an unanswered review would
  ...
  hash.
- Resolution-certificate seal binds immutable bindings
...
- Audit replay reconstructs sealed resolution certificates
  after process restart.

## 21. Product decision

Adopt the Review-Resolution Plane as the
deterministic steward resolution path for unanswered,
expired, or single-control steward pages bound by the
Unanswered-Containment Review, Unanswered-Escalation Containment, Unanswered-Ack
Escalation, SLO Ack Suppression, Remaining-Window SLO,
Confirmation-Window Observability, Confirmation-TTL
Retirement, and First-Enrollment Confirmation planes.
